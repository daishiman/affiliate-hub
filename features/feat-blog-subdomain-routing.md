...
resource_scope: ["src/middleware.ts","src/app","src/infrastructure/persistence/d1","drizzle","wrangler.jsonc","system-spec","features/feat-blog-subdomain-routing.context.json"]
...
scope_in: ["slug + SITE_BASE_DOMAIN からの hostname 導出 (site_blueprints へ永続化しない)","基底ドメインを環境ごとの構成値として注入する仕組み (再ビルドせず同一成果物へ注入)","wrangler の routes へ *.<基底ドメイン>/* を 1 本だけ宣言する (ブログ追加で DNS も routes も触らない)","middleware でのホスト名→slug 解決と、既存 /s/<slug> ルートへの内部委譲","未知ホスト・導出 slug の公開 identity 欠落/曖昧・workspace 不一致を fail closed の 404 とする判定","cookie の scope を親ドメインへ広げない設定 (サブドメイン間の信頼境界)","hostname のデータ移行を持たず、既存行も slug + SITE_BASE_DOMAIN から実行時に住所を導出する","slug 変更時に永続 hostname への追随書き込みなしで導出住所を変える規則","ホスト解決が効かない実行 (workers.dev 等) でのパス方式 /s/<slug> 後方互換"]
...
acceptance: ["外部 DNS/route 構成完了後、作成したブログが <slug>.<基底ドメイン> と /s/<slug> で同じ内容を返す","wrangler の routes 宣言が *.<基底ドメイン>/* の 1 本だけで、ブログを増やしても routes を編集しない","存在しないサブドメインへのアクセスが 404 を返し、存在するブログ一覧を推測できる情報を返さない","hostname を永続化せず、slug の一意制約と SITE_BASE_DOMAIN から一意のホスト名を導出する","導出 slug の公開 identity が欠落・曖昧または workspace 不一致なら配信せず 404 を返す","cookie の scope が親ドメインへ広がらず、あるブログの cookie が別ブログから読めない","ホスト名と slug の対応が siteHostname/siteSlugFromHost の 1 モジュールに閉じる","基底ドメインが構成値として注入され、環境ごとの再ビルドが不要である","ホスト解決が効かない環境ではパス方式で到達でき、既存の公開 URL が壊れない","既存ブログも hostname のデータ移行なしに slug + SITE_BASE_DOMAIN から住所が決まり、中身と一致する","slug を変更すると追随書き込みなしに導出住所が変わり、旧住所では到達しないか明示的に転送される","SITE_BASE_DOMAIN・wildcard route・proxied wildcard DNS 証跡が揃うまで構成状態を blocked と判定する","両経路 (サブドメイン / パス) が同じ SiteFrame へ収束することをテストが確認する"]
