# System task overlay: 受入7件の受け入れ判定
...
A1-A7 のすべてについて、検証コマンドまたは実画面の確認結果を根拠に判定し、未達0件であることを確定する。

## 背景

P05実装とP06全量実行の結果を受け、受入7件それぞれを独立に判定する。判定は実装から独立した視点で行い、findingがあればP05へ差し戻す。
...
- Frontend: applicable; 編集面と公開ページの実画面確認結果をA1-A6の根拠とする
- Backend: N/A: usecase固有の追加判定は行わない
- API: N/A: API固有の追加判定は行わない
- Data: N/A: データ固有の追加判定は行わない
- Infrastructure: N/A: デプロイ単位を変更しない
- Security: N/A: 許可リストの網羅性検証は P09 が所有する
- Quality: applicable; 受入判定そのものが本 phase の責務である
- Documentation: applicable; 判定結果文書が本 phase の成果物である
...
- Produced artifacts: docs/spec/feat-article-block-editor/acceptance.md (A1-A7の判定結果と根拠)
- Consumed artifacts: docs/spec/feat-article-block-editor/test-run.md, docs/spec/feat-article-block-editor/test-design.md
...
- Branch: assigned after dev-graph registration by C15 as devgraph/SYS-ARTICLE-BLOCK-EDITOR-P07; system-dev-plannerは事前割当を行わない
...
- 層別方針: フロントエンド: 編集面と公開ページの実画面確認を、behavior (可視ラベル・アクセシブル名・実際の見た目の一致) を根拠にA1-A6の受入判定へ充てる。バックエンド/インフラ: 本 phase は追加のusecase固有判定・デプロイ単位変更を行わないため対象外 (P06/P09で既に検証済み)。
...
- Goal: A1-A7 のすべてについて、検証コマンドまたは実画面の確認結果を根拠に判定し、未達0件であることを確定する。
- Generic execution prompt: feat-article-block-editor の goal (編集面と公開ページが同一の描画部品を通り、節と本文断片の2層がUI上で見分けられ、どの編集操作でも見出しレベルが動かず、19種の断片が`/`から挿入・編集・公開でき、商品と画像が手入力ではなく検索選択と直接アップロードで入る状態になっている) と本 task の 前提条件/成果物/write_scope/スコープ外 を渡し、実装手段は固定せず P07 の目的を満たす成果物を作らせる
