# System task overlay: 受入7件に対応するテスト設計
...
A1-A7 の各項目に対して最低1件の落とせるテストケースを対応づけ、19種の断片それぞれの挿入・編集・公開描画、見出しレベル固定の挿入・移動・削除・貼り付け全操作、商品検索0件/複数件、画像アップロード成功/失敗、許可リスト外タグ混入時の無害化、既存10種の断片で書かれた記事の読み込みと再保存の不変性を境界値として列挙し、既存回帰スイートを名前で指定する。

## 背景

受入 A3 (19種すべての挿入・編集・公開一致) と A7 (既存10種記事の不変性) は組合せ数が大きく、境界値を先に固定しないと P05 実装時にテストが後追いになる。P02 の設計成果物が定める19種カタログとAPI契約を根拠に、テストケースを先に確定する。
...
- Frontend: applicable; 編集面コンポーネントの単体・結合テストケースを設計する
- Backend: applicable; 保存形式往復・画像参照usecase・presigned PUT発行のテストケースを設計する
- API: applicable; presigned PUT発行APIと商品検索連携のテストケースを設計する
- Data: applicable; 画像参照テーブルとマイグレーションのテストケースを設計する
- Infrastructure: N/A: デプロイ単位を変更しない
- Security: applicable; 許可リスト外タグ混入時の無害化テストケースを設計する
...
- Produced artifacts: docs/spec/feat-article-block-editor/test-design.md (A1-A7対応表・境界値ケース・回帰スイート指定)
...
- Branch: assigned after dev-graph registration by C15 as devgraph/SYS-ARTICLE-BLOCK-EDITOR-P04; system-dev-plannerは事前割当を行わない
...
- 層別方針: フロントエンド: 編集面コンポーネントのテストケースを、可視ラベルとアクセシブル名に基づくbehavior検証として設計する。バックエンド: presigned PUT発行APIと商品検索APIのAPI 契約 (リクエスト/レスポンス形状・エラー系網羅) を検証するテストケースと、画像参照usecaseのDB 結合 (D1へのINSERT/SELECT往復) を検証するテストケースを設計する。
...
- Goal: A1-A7 の各項目に対して最低1件の落とせるテストケースを対応づけ、19種の断片それぞれの挿入・編集・公開描画、見出しレベル固定の挿入・移動・削除・貼り付け全操作、商品検索0件/複数件、画像アップロード成功/失敗、許可リスト外タグ混入時の無害化、既存10種の断片で書かれた記事の読み込みと再保存の不変性を境界値として列挙し、既存回帰スイートを名前で指定する。
- Generic execution prompt: feat-article-block-editor の goal (編集面と公開ページが同一の描画部品を通り、節と本文断片の2層がUI上で見分けられ、どの編集操作でも見出しレベルが動かず、19種の断片が`/`から挿入・編集・公開でき、商品と画像が手入力ではなく検索選択と直接アップロードで入る状態になっている) と本 task の 前提条件/成果物/write_scope/スコープ外 を渡し、実装手段は固定せず P04 の目的を満たす成果物を作らせる
