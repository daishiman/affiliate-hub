...
テスト全量を実行し、失敗0件を確認するとともに、P04 が基準線として記録した既存スイートの成功件数が減っていないことを実行ログとともに記録する。

## 背景

prose-node.ts・prose-format.ts・prose-editor.tsx への変更は既存記事のパースに波及しうるため、既存 tests/ 配下の全スイートを基準線として維持しつつ、新規19種と画像アップロード・許可リストレンダラの追加テストを合わせて全量実行する。
...
- Frontend: applicable; 編集面と公開ページのテストを全量実行する
- Backend: applicable; usecase・repository のテストを全量実行する
- API: N/A: API固有の追加検証は行わない
- Data: N/A: マイグレーションの追加検証は行わない
- Infrastructure: N/A: デプロイ単位を変更しない
- Security: N/A: 許可リストの網羅性検証は P09 が所有する
- Quality: applicable; 全量実行と回帰0件の確認そのものが本 phase の責務である
- Documentation: applicable; 実行結果記録が本 phase の成果物である
...
- Produced artifacts: docs/spec/feat-article-block-editor/test-run.md (全量実行結果と既存回帰件数の記録)
- Consumed artifacts: docs/spec/feat-article-block-editor/test-design.md, tests
- Write scope/touches: docs/spec/feat-article-block-editor/test-run.md
...
- Branch: assigned after dev-graph registration by C15 as devgraph/SYS-ARTICLE-BLOCK-EDITOR-P06; system-dev-plannerは事前割当を行わない
...
- 層別方針: フロントエンド: P04で設計したbehavior検証 (可視ラベル・アクセシブル名) テストを編集面・公開ページに対して全量実行する。バックエンド: presigned PUT発行APIと商品検索APIのAPI 契約テストと、画像参照usecaseのDB 結合テストをusecase・repositoryに対して全量実行する。
...
- Goal: テスト全量を実行し、失敗0件を確認するとともに、P04 が基準線として記録した既存スイートの成功件数が減っていないことを実行ログとともに記録する。
- Generic execution prompt: feat-article-block-editor の goal (編集面と公開ページが同一の描画部品を通り、節と本文断片の2層がUI上で見分けられ、どの編集操作でも見出しレベルが動かず、19種の断片が`/`から挿入・編集・公開でき、商品と画像が手入力ではなく検索選択と直接アップロードで入る状態になっている) と本 task の 前提条件/成果物/write_scope/スコープ外 を渡し、実装手段は固定せず P06 の目的を満たす成果物を作らせる
