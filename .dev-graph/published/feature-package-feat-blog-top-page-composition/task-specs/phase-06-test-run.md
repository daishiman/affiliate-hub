...
P05の実装に対してP04が用意した単体・結合・境界値・回帰テスト、axe-coreによるアクセシビリティスキャン、JavaScript無効時のe2e、非模倣静的検査を実行し、結果をreportへ残す。
...
- Produced artifacts: test-run report(単体/結合/境界値/回帰/axe-core/no-js e2e/非模倣静的検査の実行結果とcoverage)
- Consumed artifacts: src/app/s/[site]/page.tsx, src/presentation/site/home-content.tsx, src/presentation/site/blog-top-bands.tsx, tests/blog-top-page/*
...
- Acceptance state: P06: P04の単体・結合・境界値・回帰テスト、axe-coreスキャン、no-js e2e、非模倣静的検査が全てPASSし、新規/変更application codeのcoverageが既定80%以上であることをtest-run reportへ記録して成立させる。
...
- Completion condition: P06: P04の単体・結合・境界値・回帰テスト、axe-coreスキャン、no-js e2e、非模倣静的検査が全てPASSし、新規/変更application codeのcoverageが既定80%以上であることをtest-run reportへ記録して成立させる。
