...
P01の要求ベースラインを、既存のブログトップルート(src/app/s/[site]/page.tsx)とfeat-blog-ui-builderが提供するsticky header/footer部品を消費する側のコンポーネント設計、最新/人気切替のURLパラメータ契約、WebSite+SearchAction+ItemListのJSON-LD生成契約、非模倣静的検査(参照元の文章・画像・ロゴ・固有名・色値の混入検知)のCI組込み設計へ変換する。

## 背景

トップページは読者面の最上位入口であり、管理画面のプレビューにも同じ正本が投影される二層構成の制約(arch-two-layer-platform)を満たす必要がある。sticky ヘッダー・フッターの部品実装そのものはfeat-blog-ui-builderの責務であり、本phaseはその部品をサイト名/検索起動/カテゴリへの移動の3機能構成で配置する側の設計だけを行い、部品自体を再実装しない。カテゴリへの移動を担う入口はヘッダー側に置き、一覧の長さに影響されない位置から届くようにする(qa-uiux-web-top-composition-v6のトレードオフ)。
...
- Security: applicable; 非模倣静的検査(参照元固有名・色値の文字列検知)のCI組込み設計を行う
