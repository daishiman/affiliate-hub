...
| `deploy.yml` | `dev` / `main` へ push | 枝で出し先が決まる（`dev`→開発環境 / `main`→本番） |
| `migrate.yml` | 手動実行 | 指定した環境の D1 にマイグレーションを適用（**本番はこれだけ**） |
...
> **順番の守らせ方が、出し先で違います。**
>
> - **本番（`main`）**: `deploy.yml` はデータの形を変えません。マイグレーションは
>   `migrate.yml` の手動実行です。未適用が残っていると公開が止まります。
>   新しいマイグレーションを含む変更を本番へ出すときは、
>   **公開の前に `migrate.yml` を回してください**。
> - **開発環境（`dev`）**: `deploy.yml` がバックアップを取ったうえで自動適用します。
>   手で回す必要はありません。
>
> 本番だけ手動なのは、**本番データは元に戻せない**からです。
> 2026-08-21 に本番の入口が 500 になったのは、コードだけが進んでデータの形が古かったため
> でした（`0004`〜`0017` が未適用のまま公開された）。
