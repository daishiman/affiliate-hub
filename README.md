...
| `migrate.yml` | 手動実行 | 指定した環境の D1 にマイグレーションを適用 |

**マイグレーションは必ずデプロイの前**に走ります。逆順だと新しいコードが存在しないカラムを
参照する瞬間が生まれるためです。つまり後方互換なマイグレーション（カラム追加は可、
削除は 2 段階）が前提です。

> **`deploy.yml` はデータの形を変えません。** マイグレーションは `migrate.yml` の
> 手動実行です。2 つが分かれている以上、**コードだけが進んでデータの形が古い**（またはその逆）
> という食い違いは構造上いつでも起こり得ます。2026-08-21 に本番の入口が 500 になったのは
> これが原因でした（`0004`〜`0017` が未適用のまま公開された）。
> 新しいマイグレーションを含む変更を出すときは、**公開の前に `migrate.yml` を回してください**。
