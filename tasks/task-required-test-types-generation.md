---
graph_node_id: "task-required-test-types-generation"
artifact_kind: "task"
artifact_subtypes: []
...
target_date: null
iteration: null
title: "必須テスト種別を宣言する（生成基盤と API・イベント 28 件）"
owners: ["daishiman"]
created_at: "2026-08-17T22:00:00Z"
updated_at: "2026-09-08T07:20:20.876163Z"
status: "draft"
...
purpose: null
goal: null
mvp_alignment: {"background":"240 要件のうち必須テスト種別を宣言できているのは 12 件だけで、生成基盤と API・イベントの 28 件は未宣言のまま","mvp_fit":"enabling","purpose":"生成基盤と API・イベントの 28 件について、必要なテスト種別を 1 件ずつ決める","rationale":"種別を決めないとテストは書きやすいところから書かれ、権限のできてはいけない側・禁止された状態遷移・障害注入が永久に残る"}
scope_in: []
scope_out: []
acceptance: []
architecture_refs: []
...
phase_ref: null
file_path: "tasks/task-required-test-types-generation.md"
template_id: "task"
...
classification_candidates: [{"artifact_kind":"task","candidate_path":"tasks/task-required-test-types-generation.md","confidence":0.95}]
issue_linkage: null
tracker_binding: "beads"
beads_linkage: {"bd_issue_id":"ah-29w","github_mirror":null,"linked_at":"2026-09-08T07:14:07Z","sync_state":"linked"}
...
そこで**文脈ごと**に分けた。この課題はそのうちの 1 つで、28 件だけを見ればよい。
...
対象の要件 28 件（**生成基盤と API・イベント**）:

`REQ-API01` `REQ-EV01` `REQ-EV02` `REQ-EV03` `REQ-EV04` `REQ-EV05` `REQ-EV06` `REQ-EV07` `REQ-EV08` `REQ-EV09` `REQ-EV10` `REQ-EV11` `REQ-EV12` `REQ-EV13` `REQ-EV14` `REQ-EV15` `REQ-EV16` `REQ-G01` `REQ-G02` `REQ-G03` `REQ-G04` `REQ-G05` `REQ-G06` `REQ-G07` `REQ-G08` `REQ-G09` `REQ-G10` `REQ-G11`
...
1. 対象 28 件を 1 件ずつ**中身を読んで**、性質を決める
...
- 対象 28 件すべてに、宣言か理由つき除外がある
- `node scripts/required-test-types.mjs` が緑
- `TEST_TYPES_MAX_UNDECLARED` が 28 件ぶん下がっている
