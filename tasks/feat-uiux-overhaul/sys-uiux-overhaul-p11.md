...
artifact_kind: "task"
artifact_subtypes: []
beads_linkage: "ah-c8s"
classification_candidates: [{"artifact_kind":"task","candidate_path":"tasks/feat-uiux-overhaul/sys-uiux-overhaul-p11.md","confidence":1.0}]
classification_confidence: 1.0
classification_reason: "feat-uiux-overhaul の P11 lifecycle 責務への確定写像"
...
depends_on: ["SYS-UIUX-OVERHAUL-P07","SYS-UIUX-OVERHAUL-P09"]
domain: "documentation"
evaluation_status: "pass"
execution_contexts: []
feature_package_id: "feature-package/feat-uiux-overhaul"
file_path: "tasks/feat-uiux-overhaul/sys-uiux-overhaul-p11.md"
github_project_linkages: []
github_publication: {"labels":[],"milestone":null,"mode":"local_only","project_aliases":[]}
graph_node_id: "SYS-UIUX-OVERHAUL-P11"
...
related_nodes: []
resource_scope: ["docs/spec/feat-uiux-overhaul/evidence/"]
source_lineage: {"imported_at":"2026-08-21T12:39:22Z","origin_kind":"system-dev-planner","source_digest":"3c9e340b6675b9d0b51c5a8b14331885611cb6e7f9129f16eb853231a3a7fbf0","source_path":".dev-graph/published/feature-package-feat-uiux-overhaul/task-specs/phase-11-evidence.md","source_plugin":"system-dev-planner","source_version":"0.1.0"}
start_date: null
status: "active"
tags: ["p11","feat-uiux-overhaul"]
target_date: null
template_id: "task"
template_version: "1.0.0"
title: "証跡の集約と検証可能性の確保"
tracker_binding: "beads"
updated_at: "2026-08-21T12:39:22Z"
purpose: "P07 の受入判定と P09 の品質検査の証跡を集約し、第三者が同じ手順で再現・検証できる状態を成立させる。"
goal: "P07 の受入判定と P09 の品質検査の証跡を集約し、第三者が同じ手順で再現・検証できる状態を成立させる。"
scope_in: ["Produced artifacts: docs/spec/feat-uiux-overhaul/evidence/ (受入判定・テスト実行・品質検査の証跡と再現手順)","Consumed artifacts: docs/spec/feat-uiux-overhaul/acceptance-report.md, docs/spec/feat-uiux-overhaul/quality-report.md","Write scope/touches: docs/spec/feat-uiux-overhaul/evidence/"]
...
acceptance: ["Automated commands: `pnpm run preview` (Workers ランタイム, localhost:8787) 上で P11 に対応する検証コマンドを実行する","Automated commands: `python3 .claude/plugins/system-dev-planner/scripts/validate-system-plan.py --repo-root . --feature-package feature-package/feat-uiux-overhaul` (本 package の C12 決定論検証を世代非依存に再実行する)","Required evidence: P11 の 成果物 section に記載した produced artifacts のパス"]
architecture_refs: ["arch-two-layer-platform"]
---

# System task overlay: 証跡の集約と検証可能性の確保
...
- parent_feature: feat-uiux-overhaul
- phase_ref: P11
- classification: confidence=1.0; reason=feat-uiux-overhaul の P11 lifecycle 責務への確定写像; candidate=tasks/feat-uiux-overhaul/sys-uiux-overhaul-p11.md
...
P07 の受入判定と P09 の品質検査の証跡を集約し、第三者が同じ手順で再現・検証できる状態を成立させる。

## 背景

判定の根拠が散在すると、後から退行が起きたときに原因の切り分けができない。証跡を一箇所へ集約する。
...
- Frontend: applicable; 画面の受入証跡を集約する
- Backend: applicable; ユースケース検証の証跡を集約する
- API: applicable; API 契約テストの証跡を集約する
- Data: applicable; データ整合検証の証跡を集約する
- Infrastructure: N/A: インフラ変更を伴わない
- Security: applicable; 権限検査の証跡を集約する
- Quality: applicable; 再現手順の記載を完了条件とする
...
- Produced artifacts: docs/spec/feat-uiux-overhaul/evidence/ (受入判定・テスト実行・品質検査の証跡と再現手順)
- Consumed artifacts: docs/spec/feat-uiux-overhaul/acceptance-report.md, docs/spec/feat-uiux-overhaul/quality-report.md
- Write scope/touches: docs/spec/feat-uiux-overhaul/evidence/
...
- PR body contract: Closes #issue (beads issue) + dev-graph graph_node_id=SYS-UIUX-OVERHAUL-P11; PR の宛先は dev ブランチとする
- Ownership boundary: system-dev-plannerはintentのみを宣言し、dev-graphがtracker mutationとreconciliationを行う

## Branch and worktree execution

- Branch: assigned after dev-graph registration by C15 as devgraph/SYS-UIUX-OVERHAUL-P11; system-dev-plannerは事前割当を行わない
- Worktree lease: claim SYS-UIUX-OVERHAUL-P11 before implementation; heartbeat/release は dev-graph worktree lease 契約に従う
- Parallel safety: depends_on=['P07', 'P09'] の完了 + resource_scope (feat-uiux-overhaul 配下) と active lease が重複しないこと
...
- Goal: P07 の受入判定と P09 の品質検査の証跡を集約し、第三者が同じ手順で再現・検証できる状態を成立させる。
- Generic execution prompt: feat-uiux-overhaul の goal (全画面が単一用途に分割され、管理対象に基本管理機能(一覧・新規作成・編集・削除)とそのAPIが揃い、カード間隔・文章量・サイドバーが最適化され、各サイト・SNSへの投稿状態が画面へ反映され、1商品から複数ブログへコンセプト別文章を作成でき、X/Facebook等の新SNSをプロバイダ追加のみで拡張できる構成になっている) と本 task の 前提条件/成果物/write_scope/スコープ外 を渡し、実装手段は固定せず P11 の目的を満たす成果物を作らせる
...
- Dependencies: SYS-UIUX-OVERHAUL-P07, SYS-UIUX-OVERHAUL-P09

## 実行契約

- source spec: `.dev-graph/published/feature-package-feat-uiux-overhaul/task-specs/phase-11-evidence.md` (昇格済み generation 本文を byte-for-byte 引用)
