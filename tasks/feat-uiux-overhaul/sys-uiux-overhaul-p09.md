...
artifact_kind: "task"
artifact_subtypes: []
beads_linkage: "ah-2y4"
classification_candidates: [{"artifact_kind":"task","candidate_path":"tasks/feat-uiux-overhaul/sys-uiux-overhaul-p09.md","confidence":1.0}]
classification_confidence: 1.0
classification_reason: "feat-uiux-overhaul の P09 lifecycle 責務への確定写像"
...
depends_on: ["SYS-UIUX-OVERHAUL-P08"]
domain: "quality"
evaluation_status: "pass"
execution_contexts: []
feature_package_id: "feature-package/feat-uiux-overhaul"
file_path: "tasks/feat-uiux-overhaul/sys-uiux-overhaul-p09.md"
github_project_linkages: []
github_publication: {"labels":[],"milestone":null,"mode":"local_only","project_aliases":[]}
graph_node_id: "SYS-UIUX-OVERHAUL-P09"
...
purpose: "移行後の全画面に対して、型検査・静的解析・アクセシビリティ・表示崩れ・応答性能の非機能検査を実施し、基準を満たした状態を成立させる。"
goal: "移行後の全画面に対して、型検査・静的解析・アクセシビリティ・表示崩れ・応答性能の非機能検査を実施し、基準を満たした状態を成立させる。"
scope_in: ["Produced artifacts: docs/spec/feat-uiux-overhaul/quality-report.md (型検査・静的解析・アクセシビリティ・表示崩れ・性能の結果)","Consumed artifacts: docs/spec/feat-uiux-overhaul/migration-report.md","Write scope/touches: docs/spec/feat-uiux-overhaul/quality-report.md"]
...
acceptance: ["Automated commands: `pnpm run preview` (Workers ランタイム, localhost:8787) 上で P09 に対応する検証コマンドを実行する","Automated commands: `python3 .claude/plugins/system-dev-planner/scripts/validate-system-plan.py --repo-root . --feature-package feature-package/feat-uiux-overhaul` (本 package の C12 決定論検証を世代非依存に再実行する)","Required evidence: P09 の 成果物 section に記載した produced artifacts のパス"]
...
- parent_feature: feat-uiux-overhaul
- phase_ref: P09
- classification: confidence=1.0; reason=feat-uiux-overhaul の P09 lifecycle 責務への確定写像; candidate=tasks/feat-uiux-overhaul/sys-uiux-overhaul-p09.md
...
移行後の全画面に対して、型検査・静的解析・アクセシビリティ・表示崩れ・応答性能の非機能検査を実施し、基準を満たした状態を成立させる。

## 背景

サイドバー折りたたみのような状態を持つUIは、キーボード操作と支援技術での識別性が退行しやすい。アイコンのみ表示になる折りたたみ時にも項目が識別できることを機械と実測の両方で確認する。
...
- Frontend: applicable; アクセシビリティと表示崩れを検査する
- Backend: applicable; 型検査と静的解析を通す
- API: applicable; API 応答の妥当性と性能を検査する
- Data: applicable; 一覧取得のクエリ効率を検査する
- Infrastructure: N/A: インフラ変更を伴わない
- Security: applicable; 管理操作の権限漏れを検査する
- Quality: applicable; 非機能基準の充足が本 phase の責務である
- Documentation: applicable; 品質報告が本 phase の成果物である
...
- Produced artifacts: docs/spec/feat-uiux-overhaul/quality-report.md (型検査・静的解析・アクセシビリティ・表示崩れ・性能の結果)
- Consumed artifacts: docs/spec/feat-uiux-overhaul/migration-report.md
- Write scope/touches: docs/spec/feat-uiux-overhaul/quality-report.md
...
- PR body contract: Closes #issue (beads issue) + dev-graph graph_node_id=SYS-UIUX-OVERHAUL-P09; PR の宛先は dev ブランチとする
- Ownership boundary: system-dev-plannerはintentのみを宣言し、dev-graphがtracker mutationとreconciliationを行う

## Branch and worktree execution

- Branch: assigned after dev-graph registration by C15 as devgraph/SYS-UIUX-OVERHAUL-P09; system-dev-plannerは事前割当を行わない
- Worktree lease: claim SYS-UIUX-OVERHAUL-P09 before implementation; heartbeat/release は dev-graph worktree lease 契約に従う
- Parallel safety: depends_on=['P08'] の完了 + resource_scope (feat-uiux-overhaul 配下) と active lease が重複しないこと
...
- Goal: 移行後の全画面に対して、型検査・静的解析・アクセシビリティ・表示崩れ・応答性能の非機能検査を実施し、基準を満たした状態を成立させる。
- Generic execution prompt: feat-uiux-overhaul の goal (全画面が単一用途に分割され、管理対象に基本管理機能(一覧・新規作成・編集・削除)とそのAPIが揃い、カード間隔・文章量・サイドバーが最適化され、各サイト・SNSへの投稿状態が画面へ反映され、1商品から複数ブログへコンセプト別文章を作成でき、X/Facebook等の新SNSをプロバイダ追加のみで拡張できる構成になっている) と本 task の 前提条件/成果物/write_scope/スコープ外 を渡し、実装手段は固定せず P09 の目的を満たす成果物を作らせる
...
- Dependencies: SYS-UIUX-OVERHAUL-P08

## 実行契約

- source spec: `.dev-graph/published/feature-package-feat-uiux-overhaul/task-specs/phase-09-quality-assurance.md` (昇格済み generation 本文を byte-for-byte 引用)
