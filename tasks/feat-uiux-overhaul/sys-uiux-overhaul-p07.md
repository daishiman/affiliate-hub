...
artifact_kind: "task"
artifact_subtypes: []
beads_linkage: "ah-wxi"
classification_candidates: [{"artifact_kind":"task","candidate_path":"tasks/feat-uiux-overhaul/sys-uiux-overhaul-p07.md","confidence":1.0}]
classification_confidence: 1.0
classification_reason: "feat-uiux-overhaul の P07 lifecycle 責務への確定写像"
...
depends_on: ["SYS-UIUX-OVERHAUL-P06"]
domain: "quality"
evaluation_status: "pass"
execution_contexts: []
feature_package_id: "feature-package/feat-uiux-overhaul"
file_path: "tasks/feat-uiux-overhaul/sys-uiux-overhaul-p07.md"
github_project_linkages: []
github_publication: {"labels":[],"milestone":null,"mode":"local_only","project_aliases":[]}
graph_node_id: "SYS-UIUX-OVERHAUL-P07"
...
related_nodes: []
resource_scope: ["docs/spec/feat-uiux-overhaul/acceptance-report.md"]
source_lineage: {"imported_at":"2026-08-21T12:39:22Z","origin_kind":"system-dev-planner","source_digest":"3c9e340b6675b9d0b51c5a8b14331885611cb6e7f9129f16eb853231a3a7fbf0","source_path":".dev-graph/published/feature-package-feat-uiux-overhaul/task-specs/phase-07-acceptance.md","source_plugin":"system-dev-planner","source_version":"0.1.0"}
start_date: null
status: "active"
tags: ["p07","feat-uiux-overhaul"]
target_date: null
template_id: "task"
...
purpose: "feat-uiux-overhaul の受入10件それぞれについて、実物の画面と API で満たされていることを判定し記録した状態を成立させる。"
goal: "feat-uiux-overhaul の受入10件それぞれについて、実物の画面と API で満たされていることを判定し記録した状態を成立させる。"
scope_in: ["Produced artifacts: docs/spec/feat-uiux-overhaul/acceptance-report.md (受入10件の判定と根拠)","Consumed artifacts: docs/spec/feat-uiux-overhaul/test-run-report.md, docs/spec/feat-uiux-overhaul/requirements-baseline.md","Write scope/touches: docs/spec/feat-uiux-overhaul/acceptance-report.md"]
...
acceptance: ["Automated commands: `pnpm run preview` (Workers ランタイム, localhost:8787) 上で P07 に対応する検証コマンドを実行する","Automated commands: `python3 .claude/plugins/system-dev-planner/scripts/validate-system-plan.py --repo-root . --feature-package feature-package/feat-uiux-overhaul` (本 package の C12 決定論検証を世代非依存に再実行する)","Required evidence: P07 の 成果物 section に記載した produced artifacts のパス"]
...
- parent_feature: feat-uiux-overhaul
- phase_ref: P07
- classification: confidence=1.0; reason=feat-uiux-overhaul の P07 lifecycle 責務への確定写像; candidate=tasks/feat-uiux-overhaul/sys-uiux-overhaul-p07.md
...
feat-uiux-overhaul の受入10件それぞれについて、実物の画面と API で満たされていることを判定し記録した状態を成立させる。

## 背景

テストの緑化は必要条件であり十分条件ではない。実際の管理画面を操作して、認知負荷が下がり直感的に操作できるかを人の判断で確認する。
...
- Frontend: applicable; 実画面で単一用途・サイドバー開閉・アイコン識別・情報削減を確認する
- Backend: applicable; 管理ユースケースの成立を確認する
- API: applicable; CRUD と投稿状態参照の応答を確認する
- Data: applicable; 複数ブログのコンセプト別文章が保存され参照できることを確認する
- Infrastructure: N/A: インフラ変更を伴わない
- Security: applicable; 権限なしロールでの拒否を確認する
- Quality: applicable; 受入10件の判定そのものが本 phase の責務である
- Documentation: applicable; 受入報告が本 phase の成果物である
...
- Produced artifacts: docs/spec/feat-uiux-overhaul/acceptance-report.md (受入10件の判定と根拠)
- Consumed artifacts: docs/spec/feat-uiux-overhaul/test-run-report.md, docs/spec/feat-uiux-overhaul/requirements-baseline.md
- Write scope/touches: docs/spec/feat-uiux-overhaul/acceptance-report.md
...
- PR body contract: Closes #issue (beads issue) + dev-graph graph_node_id=SYS-UIUX-OVERHAUL-P07; PR の宛先は dev ブランチとする
- Ownership boundary: system-dev-plannerはintentのみを宣言し、dev-graphがtracker mutationとreconciliationを行う

## Branch and worktree execution

- Branch: assigned after dev-graph registration by C15 as devgraph/SYS-UIUX-OVERHAUL-P07; system-dev-plannerは事前割当を行わない
- Worktree lease: claim SYS-UIUX-OVERHAUL-P07 before implementation; heartbeat/release は dev-graph worktree lease 契約に従う
- Parallel safety: depends_on=['P06'] の完了 + resource_scope (feat-uiux-overhaul 配下) と active lease が重複しないこと
...
- Goal: feat-uiux-overhaul の受入10件それぞれについて、実物の画面と API で満たされていることを判定し記録した状態を成立させる。
- Generic execution prompt: feat-uiux-overhaul の goal (全画面が単一用途に分割され、管理対象に基本管理機能(一覧・新規作成・編集・削除)とそのAPIが揃い、カード間隔・文章量・サイドバーが最適化され、各サイト・SNSへの投稿状態が画面へ反映され、1商品から複数ブログへコンセプト別文章を作成でき、X/Facebook等の新SNSをプロバイダ追加のみで拡張できる構成になっている) と本 task の 前提条件/成果物/write_scope/スコープ外 を渡し、実装手段は固定せず P07 の目的を満たす成果物を作らせる
...
- Dependencies: SYS-UIUX-OVERHAUL-P06

## 実行契約

- source spec: `.dev-graph/published/feature-package-feat-uiux-overhaul/task-specs/phase-07-acceptance.md` (昇格済み generation 本文を byte-for-byte 引用)
