...
updated_at: "2026-09-08T07:33:30.351816Z"
status: "draft"
depends_on: []
related_nodes: []
...
beads_linkage: {"bd_issue_id":"ah-3pn","github_mirror":null,"linked_at":"2026-09-08T07:14:07Z","sync_state":"linked"}
...
- **手書きに相当する差分を、検査が捕まえることをテストとして固定していること**
  （下の「なぜここを書き換えたか」を必ず読むこと）

### なぜここを書き換えたか（2026-08-18）

当初の受入条件は「手で 1 文字書いて `pnpm run verify` が赤になることを**実測する**」だった。
これを**より強い形へ替えた**。緩めたのではない。理由は 2 つある。

1. **1 回の手作業は、次に同じことをする人を捕まえない。**
   見たその場では赤いが、明日 5 枚目の生成物が増えたときには何も起きない。
   受入条件が「1 回見た」で満たされてしまうと、**満たした瞬間から効かなくなる**。
2. この作業場所の見張り（dev-graph の `guard-graph-schema.py`。
   `docs/` を守り対象にしている）が、`docs/` への手書きをすべて止める。
   **迂回していない。**別の書き方で通せば、次に無害でないものが同じ道を通る。

替えた先は `tests/architecture/generated-docs.test.ts` の
「実際の 4 枚に、手書きと同じ差分を当てると捕まる」で、
**4 枚それぞれのいまの中身を写して、3 通りの手書き**
（末尾に 1 行足す／中身の 1 文字を変える／指紋の行ごと外す）を当て、
すべて止まることを毎回見る。1 回きりの実測より、**当てる枚数も回数も多い**。

## 検証方法

`pnpm vitest run tests/architecture/generated-docs.test.ts` が緑であること。
このファイルの検査を 1 本でも外したり、生成の道具から突き合わせを外したりすると、
**その場で赤になる**（外した状態で緑になるなら、それは検査ではない）。
