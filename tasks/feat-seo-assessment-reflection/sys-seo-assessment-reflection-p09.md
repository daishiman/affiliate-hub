---
graph_node_id: "SYS-SEO-ASSESSMENT-REFLECTION-P09"
artifact_kind: "task"
artifact_subtypes: []
project_id: "feature-package-feat-seo-assessment-reflection"
domain: "quality"
tags: ["p09","feat-seo-assessment-reflection"]
priority: null
start_date: null
target_date: null
iteration: null
...
status: "closed"
depends_on: ["SYS-SEO-ASSESSMENT-REFLECTION-P08"]
related_nodes: []
resource_scope: ["docs/spec/feat-seo-assessment-reflection/quality-report.md","tests/"]
purpose: "月次全記事診断の実行時間、診断失敗が公開処理を止めないこと、推奨採用が workspace 権限で守られること、および構造化データ検証が外部通信を行わないことを検査し、いずれも基準を満たす状態を成立させる。"
goal: "月次全記事診断の実行時間、診断失敗が公開処理を止めないこと、推奨採用が workspace 権限で守られること、および構造化データ検証が外部通信を行わないことを検査し、いずれも基準を満たす状態を成立させる。"
scope_in: ["Produced artifacts: docs/spec/feat-seo-assessment-reflection/quality-report.md (実行時間・非阻害性・権限・外部通信の検査結果)","Consumed artifacts: features/feat-seo-assessment-reflection.md; features/feat-seo-assessment-reflection.context.json; system-spec/backend.md; system-spec/database.md; system-spec/frontend.md","Write scope/touches: docs/spec/feat-seo-assessment-reflection/quality-report.md, tests/"]
scope_out: ["検索順位そのものの保証や、ベンダー推定の数値目標 (作らない)","公開面を直接書き換える経路 (必ず下書き経由とする)","AI 検索・回答エンジン向けの回答単位と llms.txt (feat-aeo-answer-optimization が所有する)","診断結果の残数表示と提示順序 (feat-blog-scoped-admin-console が所有する)","P09 以外の phase が所有する成果物への変更"]
acceptance: ["Automated commands: `pnpm vitest run` (権限拒否と非阻害性のテストを再実行する)","Automated commands: `pnpm run build` (本番相当ビルドが通ることを確認する)","Required evidence: quality-report.md に月次全記事診断の実行時間見積が記録されていること","Required evidence: P09 の 成果物 section に記載した produced artifacts のパス"]
...
phase_ref: "P09"
file_path: "tasks/feat-seo-assessment-reflection/sys-seo-assessment-reflection-p09.md"
template_id: "task"
...
source_lineage: {"imported_at":"2026-09-03T23:47:29Z","origin_kind":"system-dev-planner","source_digest":"7d8842a138d09aacfd5b87277165649d3c17a1ffa89019dfa4a6aa68b4480f49","source_path":".dev-graph/published/generations/feature-package-feat-seo-assessment-reflection/7d8842a138d09aacfd5b87277165649d3c17a1ffa89019dfa4a6aa68b4480f49/task-specs/phase-09-quality-assurance.md","source_plugin":"system-dev-planner","source_version":"0.1.0"}
classification_confidence: 1.0
classification_reason: "feat-seo-assessment-reflection の P09 lifecycle 責務への確定写像"
classification_candidates: [{"artifact_kind":"task","candidate_path":"tasks/feat-seo-assessment-reflection/sys-seo-assessment-reflection-p09.md","confidence":1.0}]
issue_linkage: null
tracker_binding: "beads"
beads_linkage: {"bd_issue_id":"ah-ji7d.9","github_mirror":null,"linked_at":"2026-09-04T02:08:46Z","sync_state":"linked"}
github_publication: {"labels":[],"milestone":null,"mode":"local_only","project_aliases":[]}
github_project_linkages: []
pull_request_linkages: []
execution_contexts: []
completion_evidence: {"completed_at":"2026-09-04T10:45:42Z","evidence_refs":["beads:ah-ji7d.9"],"policy":"manual","reconciled_at":"2026-09-09T00:00:00Z","source":"reconciliation","status":"done"}
implementation_readiness: {"checked_at":"2026-09-04T00:00:00Z","missing_sections":[],"status":"complete"}
---

# System task overlay: SEO 診断の非機能検査
...
- parent_feature: feat-seo-assessment-reflection
- phase_ref: P09
- classification: confidence=1.0; reason=feat-seo-assessment-reflection の P09 lifecycle 責務への確定写像; candidate=tasks/feat-seo-assessment-reflection/sys-seo-assessment-reflection-p09.md
...
月次全記事診断の実行時間、診断失敗が公開処理を止めないこと、推奨採用が workspace 権限で守られること、および構造化データ検証が外部通信を行わないことを検査し、いずれも基準を満たす状態を成立させる。

## 背景

月次診断は記事数に比例して重くなる。また診断の失敗が公開を止めると、SEO のための仕組みが公開を妨げるという本末転倒が起きる。切り離しを検査で確かめる。
...
- Security: applicable; 推奨採用の権限限定を検査する
...
- Produced artifacts: docs/spec/feat-seo-assessment-reflection/quality-report.md (実行時間・非阻害性・権限・外部通信の検査結果)
...
- Write scope/touches: docs/spec/feat-seo-assessment-reflection/quality-report.md, tests/
...
- PR body contract: Closes #issue (beads issue) + dev-graph graph_node_id=SYS-SEO-ASSESSMENT-REFLECTION-P09; PR の宛先は dev ブランチとする
- Ownership boundary: system-dev-planner は intent のみを宣言し、dev-graph が tracker mutation と reconciliation を行う

## Branch and worktree execution

- Branch: assigned after dev-graph registration by C15 as devgraph/SYS-SEO-ASSESSMENT-REFLECTION-P09; system-dev-planner は事前割当を行わない
- Worktree lease: claim SYS-SEO-ASSESSMENT-REFLECTION-P09 before implementation; heartbeat/release は dev-graph worktree lease 契約に従う
...
- テストレベル選定: 境界値: 大量記事・診断例外発生時の公開継続・権限不足からの採用要求。回帰: 既存の公開処理テストを維持する。 N/A: 単体・結合 の各テストレベルは、本 task の成果物が文書と判定であり実行可能なコードを含まないため適用しない。
...
- Automated commands: `pnpm vitest run` (権限拒否と非阻害性のテストを再実行する)
- Automated commands: `pnpm run build` (本番相当ビルドが通ることを確認する)
- Required evidence: quality-report.md に月次全記事診断の実行時間見積が記録されていること
...
- Goal: 月次全記事診断の実行時間、診断失敗が公開処理を止めないこと、推奨採用が workspace 権限で守られること、および構造化データ検証が外部通信を行わないことを検査し、いずれも基準を満たす状態を成立させる。
- Generic execution prompt: feat-seo-assessment-reflection の goal (公開・更新時と月次で article_seo_assessments が生成され、検証可能な指摘 (索引可能性・構造化データの妥当性・更新日の掲出・内部リンク・見出し構造) だけが提示され、採用した推奨は下書きへ書き戻されて既存の人間承認経路を必ず通る状態になっている) と本 task の 前提条件/成果物/write scope/スコープ外 を渡し、実装手段は固定せず P09 の目的を満たす成果物を作らせる
...
- Dependencies: SYS-SEO-ASSESSMENT-REFLECTION-P08
