---
graph_node_id: "SYS-THUMBNAIL-VISUAL-SYSTEM-P06"
artifact_kind: "task"
artifact_subtypes: []
project_id: "feature-package-feat-thumbnail-visual-system"
domain: "quality"
tags: ["p06","feat-thumbnail-visual-system"]
priority: null
start_date: null
target_date: null
iteration: null
title: "単体・結合・CLS・決定論性・OGPテストの実行と緑化"
owners: ["daishiman"]
created_at: "2026-09-04T04:16:11Z"
updated_at: "2026-09-04T04:16:11Z"
status: "active"
depends_on: ["SYS-THUMBNAIL-VISUAL-SYSTEM-P05"]
related_nodes: ["spec-system-spec-index","arch-system-spec-overview"]
resource_scope: ["docs/spec/feat-thumbnail-visual-system/test-run-report.md","test-results/thumbnail-visual-system/"]
purpose: "P04の全テストを現在の実装byteに対して実行し、失敗を修正phaseへ返し、再現コマンド・環境・結果を保存する。"
goal: "P04の全テストを現在の実装byteに対して実行し、失敗を修正phaseへ返し、再現コマンド・環境・結果を保存する。"
scope_in: ["Produced artifacts: 再現可能なtest run reportとmachine-readable結果","Consumed artifacts: features/feat-thumbnail-visual-system.md, features/feat-thumbnail-visual-system.context.json, system-spec/ui-ux.md, system-spec/frontend.md, system-spec/retrieval-evidence/kajetblog-top-analysis.md, SYS-THUMBNAIL-VISUAL-SYSTEM-P05","Write scope/touches: docs/spec/feat-thumbnail-visual-system/test-run-report.md, test-results/thumbnail-visual-system/"]
...
source_lineage: {"imported_at":"2026-09-04T04:16:11Z","origin_kind":"system-dev-planner","source_digest":"3495330b8d67c78fac526b7468ec270cb6f15e6e8c07d88d6e80dd526d37109d","source_path":".dev-graph/published/feature-package-feat-thumbnail-visual-system/task-specs/phase-06-test-run.md","source_plugin":"system-dev-planner","source_version":"0.1.0"}
classification_confidence: 1.0
classification_reason: "feat-thumbnail-visual-systemのP06 lifecycle責務への確定写像"
classification_candidates: [{"artifact_kind":"task","candidate_path":"tasks/feat-thumbnail-visual-system/sys-thumbnail-visual-system-p06.md","confidence":1.0}]
issue_linkage: null
tracker_binding: "beads"
beads_linkage: {"bd_issue_id":"ah-bo1e","github_mirror":null,"linked_at":"2026-09-04T07:33:24Z","sync_state":"linked"}
...
# System task overlay: 単体・結合・CLS・決定論性・OGPテストの実行と緑化
...
- parent_feature: feat-thumbnail-visual-system
- phase_ref: P06
- classification: confidence=1.0; reason=feat-thumbnail-visual-systemのP06 lifecycle責務への確定写像; candidate=tasks/feat-thumbnail-visual-system/sys-thumbnail-visual-system-p06.md
...
P04の全テストを現在の実装byteに対して実行し、失敗を修正phaseへ返し、再現コマンド・環境・結果を保存する。
...
- Frontend: applicable; 8画面のbehavior E2Eを実行する
- Backend: applicable; 登録優先順位・代替図版生成ロジック単体を実行する
- API: applicable; R2派生APIをcontract testする
- Data: applicable; DB結合・cache一致テストを実行する
- Infrastructure: applicable; IaC静的検証とsmokeを実行する
- Security: applicable; 外部接続0のnegativeテストを実行する
- Quality: applicable; 全suiteを集約して失敗0を確認する
- Documentation: applicable; commandとresultを記録する
- Operations: applicable; 障害時fallback演習結果を記録する
...
- Produced artifacts: 再現可能なtest run reportとmachine-readable結果
...
- Write scope/touches: docs/spec/feat-thumbnail-visual-system/test-run-report.md, test-results/thumbnail-visual-system/
...
- PR body contract: Beads issue参照とdev-graph graph_node_id=SYS-THUMBNAIL-VISUAL-SYSTEM-P06を記載し、target branchはdevとする
- Ownership boundary: system-dev-plannerはintentを宣言し、dev-graphが起票・依存・完了収束を所有する

## Branch and worktree execution

- Branch: dev-graph登録後にC15がdevgraph/SYS-THUMBNAIL-VISUAL-SYSTEM-P06として割り当てる
- Worktree lease: 実装開始前にSYS-THUMBNAIL-VISUAL-SYSTEM-P06をclaimし、heartbeatとreleaseをlease契約どおり行う
...
- Required evidence: docs/spec/feat-thumbnail-visual-system/test-run-report.md, test-results/thumbnail-visual-system/
- Acceptance state: P06: typecheck、content validation、unit、contract、DB integration、E2E、決定論性再現テスト、CLS計測、OGP/Twitter Card検証が失敗0件で、各command・exit・件数・artifact pathがtest-run-reportに記録される。
...
- Rollout: 再現可能なtest run reportとmachine-readable結果をwrite_scope内へ適用し、検証PASS後に依存する次phaseへ渡す
...
- Completion condition: P06: typecheck、content validation、unit、contract、DB integration、E2E、決定論性再現テスト、CLS計測、OGP/Twitter Card検証が失敗0件で、各command・exit・件数・artifact pathがtest-run-reportに記録される。
