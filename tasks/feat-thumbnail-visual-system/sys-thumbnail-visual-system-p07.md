---
graph_node_id: "SYS-THUMBNAIL-VISUAL-SYSTEM-P07"
artifact_kind: "task"
artifact_subtypes: []
project_id: "feature-package-feat-thumbnail-visual-system"
domain: "quality"
tags: ["p07","feat-thumbnail-visual-system"]
priority: null
start_date: null
target_date: null
iteration: null
title: "A1-A7のfeature受入"
owners: ["daishiman"]
created_at: "2026-09-04T04:16:11Z"
updated_at: "2026-09-04T04:16:11Z"
status: "active"
depends_on: ["SYS-THUMBNAIL-VISUAL-SYSTEM-P06"]
related_nodes: ["spec-system-spec-index","arch-system-spec-overview"]
resource_scope: ["docs/spec/feat-thumbnail-visual-system/acceptance-report.md"]
purpose: "feature受入A1-A7を実画面と保存済みdataで検証し、8画面全てでサムネイルが表示されるかを測定する。"
goal: "feature受入A1-A7を実画面と保存済みdataで検証し、8画面全てでサムネイルが表示されるかを測定する。"
scope_in: ["Produced artifacts: A1-A7 acceptance reportと8画面表示確認report","Consumed artifacts: features/feat-thumbnail-visual-system.md, features/feat-thumbnail-visual-system.context.json, system-spec/ui-ux.md, system-spec/frontend.md, system-spec/retrieval-evidence/kajetblog-top-analysis.md, SYS-THUMBNAIL-VISUAL-SYSTEM-P06","Write scope/touches: docs/spec/feat-thumbnail-visual-system/acceptance-report.md"]
...
source_lineage: {"imported_at":"2026-09-04T04:16:11Z","origin_kind":"system-dev-planner","source_digest":"3495330b8d67c78fac526b7468ec270cb6f15e6e8c07d88d6e80dd526d37109d","source_path":".dev-graph/published/feature-package-feat-thumbnail-visual-system/task-specs/phase-07-acceptance.md","source_plugin":"system-dev-planner","source_version":"0.1.0"}
classification_confidence: 1.0
classification_reason: "feat-thumbnail-visual-systemのP07 lifecycle責務への確定写像"
classification_candidates: [{"artifact_kind":"task","candidate_path":"tasks/feat-thumbnail-visual-system/sys-thumbnail-visual-system-p07.md","confidence":1.0}]
issue_linkage: null
tracker_binding: "beads"
beads_linkage: {"bd_issue_id":"ah-cuw6","github_mirror":null,"linked_at":"2026-09-04T07:33:27Z","sync_state":"linked"}
...
# System task overlay: A1-A7のfeature受入
...
- parent_feature: feat-thumbnail-visual-system
- phase_ref: P07
- classification: confidence=1.0; reason=feat-thumbnail-visual-systemのP07 lifecycle責務への確定写像; candidate=tasks/feat-thumbnail-visual-system/sys-thumbnail-visual-system-p07.md
...
feature受入A1-A7を実画面と保存済みdataで検証し、8画面全てでサムネイルが表示されるかを測定する。

## 背景

記事・ブログ・管理画面のどの一覧でも内容を一目で識別できるサムネイルが必ず表示され、画像が無いことによる空白や版面のずれが起きないようにする。記事編集での明示アップロード・アイキャッチ指定・本文先頭画像の優先順位で登録し、いずれも無い場合はタイトル・カテゴリー・ブログ配色トークンから外部接続なしで決定論的に代替図版を生成する。R2へオリジナルと16:9派生(複数幅)を保存し、トップページ・記事一覧・カテゴリー・タグ・検索結果・関連記事・管理画面のブログ一覧/記事一覧/プレビューの全画面へ配信する。サムネイル・代替図版・アイコンに限りCLS 0.1未満を満たし、OGP/Twitter Card(summary_large_image)へ再利用する。本phaseはSYS-THUMBNAIL-VISUAL-SYSTEM-P06の確定成果物を入力にする。

## 前提条件

- Required spec/architecture/phase/task nodes: feat-thumbnail-visual-system, spec-system-spec-index, arch-system-spec-overview, SYS-THUMBNAIL-VISUAL-SYSTEM-P06
...
- Frontend: applicable; 8画面の実画面でサムネイル表示を受入する
- Backend: applicable; 保存結果と代替図版生成結果を照合する
- API: applicable; UI表示とR2派生responseの一致を確認する
- Data: applicable; DB結合後のmetadata整合を確認する
- Infrastructure: N/A: development環境で受入する
- Security: applicable; 代替図版生成中に外部通信が発生しないことをnetwork traceで確認する
- Quality: applicable; A1-A7全件を二値判定する
- Documentation: applicable; reportを正本化する
- Operations: applicable; 失敗時代替表示の実地確認結果を記録する
...
- Produced artifacts: A1-A7 acceptance reportと8画面表示確認report
- Consumed artifacts: features/feat-thumbnail-visual-system.md, features/feat-thumbnail-visual-system.context.json, system-spec/ui-ux.md, system-spec/frontend.md, system-spec/retrieval-evidence/kajetblog-top-analysis.md, SYS-THUMBNAIL-VISUAL-SYSTEM-P06
- Write scope/touches: docs/spec/feat-thumbnail-visual-system/acceptance-report.md
...
- PR body contract: Beads issue参照とdev-graph graph_node_id=SYS-THUMBNAIL-VISUAL-SYSTEM-P07を記載し、target branchはdevとする
- Ownership boundary: system-dev-plannerはintentを宣言し、dev-graphが起票・依存・完了収束を所有する

## Branch and worktree execution

- Branch: dev-graph登録後にC15がdevgraph/SYS-THUMBNAIL-VISUAL-SYSTEM-P07として割り当てる
- Worktree lease: 実装開始前にSYS-THUMBNAIL-VISUAL-SYSTEM-P07をclaimし、heartbeatとreleaseをlease契約どおり行う
...
- Required evidence: docs/spec/feat-thumbnail-visual-system/acceptance-report.md
- Acceptance state: P07: A1-A7の全件が手順・expected・actual・evidenceでPASSし、8画面全てで実際にサムネイルまたは代替図版が表示され、代替図版生成が外部通信0であることをnetwork traceで確認し、CLS実測値が0.1未満であることをacceptance-reportに記録する。管理画面一覧のサムネイル行高が読者向け一覧より低いこと、折りたたみ操作でサムネイル列を畳めることも同じreportに手順・expected・actualで記録する。
...
- Rollout: A1-A7 acceptance reportと8画面表示確認reportをwrite_scope内へ適用し、検証PASS後に依存する次phaseへ渡す
...
- Completion condition: P07: A1-A7の全件が手順・expected・actual・evidenceでPASSし、8画面全てで実際にサムネイルまたは代替図版が表示され、代替図版生成が外部通信0であることをnetwork traceで確認し、CLS実測値が0.1未満であることをacceptance-reportに記録する。管理画面一覧のサムネイル行高が読者向け一覧より低いこと、折りたたみ操作でサムネイル列を畳めることも同じreportに手順・expected・actualで記録する。
...
- Dependencies: SYS-THUMBNAIL-VISUAL-SYSTEM-P06
