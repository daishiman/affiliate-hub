---
graph_node_id: "SYS-BLOG-SCOPED-ADMIN-CONSOLE-P08"
artifact_kind: "task"
artifact_subtypes: []
...
related_nodes: []
resource_scope: ["src/app/admin/sites/[site]/","src/app/admin/blog/","src/app/admin/(cross)/","src/components/admin/","docs/spec/feat-blog-scoped-admin-console/migration-notes.md"]
purpose: "記事単位で作られていた既存 admin 画面のうち、ブログ階層へ移した画面と残す画面を確定し、重複した画面と到達不能になった導線が0件である状態を成立させる。"
goal: "記事単位で作られていた既存 admin 画面のうち、ブログ階層へ移した画面と残す画面を確定し、重複した画面と到達不能になった導線が0件である状態を成立させる。"
scope_in: ["Produced artifacts: docs/spec/feat-blog-scoped-admin-console/migration-notes.md (移した画面・残す画面・転送先の対応表); src/app/admin/ の旧画面整理","Consumed artifacts: features/feat-blog-scoped-admin-console.md; features/feat-blog-scoped-admin-console.context.json; system-spec/ui-ux.md; system-spec/frontend.md; docs/spec/feat-uiux-overhaul/ui-rules.md","Write scope/touches: src/app/admin/sites/[site]/, src/app/admin/blog/, src/app/admin/(cross)/, src/components/admin/, docs/spec/feat-blog-scoped-admin-console/migration-notes.md"]
scope_out: ["指標の集計処理そのもの (feat-blog-metrics-rollup が所有する)","読者行動の計測と記録 (feat-reader-behavior-analytics が所有する)","SEO/AEO の診断ロジックと配信 (各 feature が所有する)","ドメイン取得と証明書の発行処理 (feat-blog-custom-domain が所有する)","P08 以外の phase が所有する成果物への変更"]
acceptance: ["Automated commands: `pnpm vitest run` (整理後も全テストが緑であることを確認する)","Automated commands: `pnpm run build` (到達不能ルートが無いことをビルドで確認する)","Required evidence: P08 の 成果物 section に記載した produced artifacts のパス"]
...
phase_ref: "P08"
file_path: "tasks/feat-blog-scoped-admin-console/sys-blog-scoped-admin-console-p08.md"
template_id: "task"
...
classification_reason: "feat-blog-scoped-admin-console の P08 lifecycle 責務への確定写像"
classification_candidates: [{"artifact_kind":"task","candidate_path":"tasks/feat-blog-scoped-admin-console/sys-blog-scoped-admin-console-p08.md","confidence":1.0}]
issue_linkage: null
tracker_binding: "beads"
beads_linkage: {"bd_issue_id":"ah-wois.8","github_mirror":null,"linked_at":"2026-09-04T02:10:27Z","sync_state":"linked"}
github_publication: {"labels":[],"milestone":null,"mode":"local_only","project_aliases":[]}
github_project_linkages: []
pull_request_linkages: []
execution_contexts: []
completion_evidence: {"completed_at":"2026-09-04T11:18:58Z","evidence_refs":["beads:ah-wois.8"],"policy":"manual","reconciled_at":"2026-09-09T00:00:00Z","source":"reconciliation","status":"done"}
