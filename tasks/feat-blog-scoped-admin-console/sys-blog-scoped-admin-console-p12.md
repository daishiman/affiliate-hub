---
graph_node_id: "SYS-BLOG-SCOPED-ADMIN-CONSOLE-P12"
artifact_kind: "task"
artifact_subtypes: []
project_id: "feature-package-feat-blog-scoped-admin-console"
domain: "documentation"
tags: ["p12","feat-blog-scoped-admin-console"]
priority: null
start_date: null
target_date: null
iteration: null
...
purpose: "ブログ階層のどこで何を見るか、注意喚起が出たとき何をするか、旧画面の導線がどこへ移ったかを、運用手順と利用者向け説明として整えた状態を成立させる。"
goal: "ブログ階層のどこで何を見るか、注意喚起が出たとき何をするか、旧画面の導線がどこへ移ったかを、運用手順と利用者向け説明として整えた状態を成立させる。"
scope_in: ["Produced artifacts: docs/spec/feat-blog-scoped-admin-console/operations-runbook.md (注意喚起への対応と移行の案内手順); docs/ 配下の画面の読み方の説明","Consumed artifacts: features/feat-blog-scoped-admin-console.md; features/feat-blog-scoped-admin-console.context.json; system-spec/ui-ux.md; system-spec/frontend.md; docs/spec/feat-uiux-overhaul/ui-rules.md","Write scope/touches: docs/spec/feat-blog-scoped-admin-console/operations-runbook.md, docs/spec/feat-blog-scoped-admin-console/"]
scope_out: ["指標の集計処理そのもの (feat-blog-metrics-rollup が所有する)","読者行動の計測と記録 (feat-reader-behavior-analytics が所有する)","SEO/AEO の診断ロジックと配信 (各 feature が所有する)","ドメイン取得と証明書の発行処理 (feat-blog-custom-domain が所有する)","P12 以外の phase が所有する成果物への変更"]
acceptance: ["Required evidence: operations-runbook.md が注意喚起対応と旧画面からの移行の両方に手順を持つこと","Automated commands: `pnpm run lint` (文書内のコード片が静的検査を通ることを確認する)","Required evidence: P12 の 成果物 section に記載した produced artifacts のパス"]
...
phase_ref: "P12"
file_path: "tasks/feat-blog-scoped-admin-console/sys-blog-scoped-admin-console-p12.md"
template_id: "task"
...
classification_reason: "feat-blog-scoped-admin-console の P12 lifecycle 責務への確定写像"
classification_candidates: [{"artifact_kind":"task","candidate_path":"tasks/feat-blog-scoped-admin-console/sys-blog-scoped-admin-console-p12.md","confidence":1.0}]
issue_linkage: null
tracker_binding: "beads"
beads_linkage: {"bd_issue_id":"ah-wois.12","github_mirror":null,"linked_at":"2026-09-04T02:10:35Z","sync_state":"linked"}
github_publication: {"labels":[],"milestone":null,"mode":"local_only","project_aliases":[]}
github_project_linkages: []
pull_request_linkages: []
execution_contexts: []
completion_evidence: {"completed_at":"2026-09-04T11:19:06Z","evidence_refs":["beads:ah-wois.12"],"policy":"manual","reconciled_at":"2026-09-09T00:00:00Z","source":"reconciliation","status":"done"}
implementation_readiness: {"checked_at":"2026-09-04T00:00:00Z","missing_sections":[],"status":"complete"}
---

# System task overlay: 管理画面の運用手順と読み方の説明
...
- parent_feature: feat-blog-scoped-admin-console
- phase_ref: P12
- classification: confidence=1.0; reason=feat-blog-scoped-admin-console の P12 lifecycle 責務への確定写像; candidate=tasks/feat-blog-scoped-admin-console/sys-blog-scoped-admin-console-p12.md
...
ブログ階層のどこで何を見るか、注意喚起が出たとき何をするか、旧画面の導線がどこへ移ったかを、運用手順と利用者向け説明として整えた状態を成立させる。

## 背景

画面構成が変わると、慣れていた運営者ほど迷う。移行の説明を運用開始と同時に用意する。
...
- Operations: applicable; 旧画面からの移行手順を本 phase が所有する
...
- Produced artifacts: docs/spec/feat-blog-scoped-admin-console/operations-runbook.md (注意喚起への対応と移行の案内手順); docs/ 配下の画面の読み方の説明
...
- Write scope/touches: docs/spec/feat-blog-scoped-admin-console/operations-runbook.md, docs/spec/feat-blog-scoped-admin-console/
...
- PR body contract: Closes #issue (beads issue) + dev-graph graph_node_id=SYS-BLOG-SCOPED-ADMIN-CONSOLE-P12; PR の宛先は dev ブランチとする
- Ownership boundary: system-dev-planner は intent のみを宣言し、dev-graph が tracker mutation と reconciliation を行う

## Branch and worktree execution

- Branch: assigned after dev-graph registration by C15 as devgraph/SYS-BLOG-SCOPED-ADMIN-CONSOLE-P12; system-dev-planner は事前割当を行わない
- Worktree lease: claim SYS-BLOG-SCOPED-ADMIN-CONSOLE-P12 before implementation; heartbeat/release は dev-graph worktree lease 契約に従う
...
- Required evidence: operations-runbook.md が注意喚起対応と旧画面からの移行の両方に手順を持つこと
...
- Goal: ブログ階層のどこで何を見るか、注意喚起が出たとき何をするか、旧画面の導線がどこへ移ったかを、運用手順と利用者向け説明として整えた状態を成立させる。
- Generic execution prompt: feat-blog-scoped-admin-console の goal (管理画面が記事単位ではなくブログ単位の階層で構成され、ブログごとの収益・PV・転換・住所の生死が一枚で読め、記事ごとの改善指示と横断比較が役割どおりに分かれ、既存 /admin/blog/* からの導線が失われていない状態になっている) と本 task の 前提条件/成果物/write scope/スコープ外 を渡し、実装手段は固定せず P12 の目的を満たす成果物を作らせる
...
- Ready when: 運用手順が注意喚起対応・移行案内・画面の読み方を覆っている
...
- Dependencies: SYS-BLOG-SCOPED-ADMIN-CONSOLE-P10, SYS-BLOG-SCOPED-ADMIN-CONSOLE-P11
