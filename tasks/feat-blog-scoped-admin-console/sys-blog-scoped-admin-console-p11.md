---
graph_node_id: "SYS-BLOG-SCOPED-ADMIN-CONSOLE-P11"
artifact_kind: "task"
artifact_subtypes: []
project_id: "feature-package-feat-blog-scoped-admin-console"
domain: "documentation"
tags: ["p11","feat-blog-scoped-admin-console"]
priority: null
start_date: null
target_date: null
iteration: null
...
scope_in: ["Produced artifacts: docs/spec/feat-blog-scoped-admin-console/evidence-index.md (受入10件と証跡ファイル・コマンドの対応表)","Consumed artifacts: features/feat-blog-scoped-admin-console.md; features/feat-blog-scoped-admin-console.context.json; system-spec/ui-ux.md; system-spec/frontend.md; docs/spec/feat-uiux-overhaul/ui-rules.md","Write scope/touches: docs/spec/feat-blog-scoped-admin-console/evidence-index.md"]
scope_out: ["指標の集計処理そのもの (feat-blog-metrics-rollup が所有する)","読者行動の計測と記録 (feat-reader-behavior-analytics が所有する)","SEO/AEO の診断ロジックと配信 (各 feature が所有する)","ドメイン取得と証明書の発行処理 (feat-blog-custom-domain が所有する)","P11 以外の phase が所有する成果物への変更"]
...
phase_ref: "P11"
file_path: "tasks/feat-blog-scoped-admin-console/sys-blog-scoped-admin-console-p11.md"
template_id: "task"
...
classification_reason: "feat-blog-scoped-admin-console の P11 lifecycle 責務への確定写像"
classification_candidates: [{"artifact_kind":"task","candidate_path":"tasks/feat-blog-scoped-admin-console/sys-blog-scoped-admin-console-p11.md","confidence":1.0}]
issue_linkage: null
tracker_binding: "beads"
beads_linkage: {"bd_issue_id":"ah-wois.11","github_mirror":null,"linked_at":"2026-09-04T02:10:33Z","sync_state":"linked"}
github_publication: {"labels":[],"milestone":null,"mode":"local_only","project_aliases":[]}
github_project_linkages: []
pull_request_linkages: []
execution_contexts: []
completion_evidence: {"completed_at":"2026-09-04T11:19:04Z","evidence_refs":["beads:ah-wois.11"],"policy":"manual","reconciled_at":"2026-09-09T00:00:00Z","source":"reconciliation","status":"done"}
implementation_readiness: {"checked_at":"2026-09-04T00:00:00Z","missing_sections":[],"status":"complete"}
---

# System task overlay: 管理画面の証跡集約
...
- parent_feature: feat-blog-scoped-admin-console
- phase_ref: P11
- classification: confidence=1.0; reason=feat-blog-scoped-admin-console の P11 lifecycle 責務への確定写像; candidate=tasks/feat-blog-scoped-admin-console/sys-blog-scoped-admin-console-p11.md
...
本 feature は6 feature の上に載る。どの証跡がどの上流に由来するかを一枚にまとめないと、後で不具合が出たときに切り分けができない。
...
- Produced artifacts: docs/spec/feat-blog-scoped-admin-console/evidence-index.md (受入10件と証跡ファイル・コマンドの対応表)
...
- Write scope/touches: docs/spec/feat-blog-scoped-admin-console/evidence-index.md
...
- PR body contract: Closes #issue (beads issue) + dev-graph graph_node_id=SYS-BLOG-SCOPED-ADMIN-CONSOLE-P11; PR の宛先は dev ブランチとする
- Ownership boundary: system-dev-planner は intent のみを宣言し、dev-graph が tracker mutation と reconciliation を行う

## Branch and worktree execution

- Branch: assigned after dev-graph registration by C15 as devgraph/SYS-BLOG-SCOPED-ADMIN-CONSOLE-P11; system-dev-planner は事前割当を行わない
- Worktree lease: claim SYS-BLOG-SCOPED-ADMIN-CONSOLE-P11 before implementation; heartbeat/release は dev-graph worktree lease 契約に従う
...
- Goal: 受入判定と非機能検査の証跡を一箇所へ集約し、後から第三者が受入10件の成立を再確認できる状態を成立させる。
- Generic execution prompt: feat-blog-scoped-admin-console の goal (管理画面が記事単位ではなくブログ単位の階層で構成され、ブログごとの収益・PV・転換・住所の生死が一枚で読め、記事ごとの改善指示と横断比較が役割どおりに分かれ、既存 /admin/blog/* からの導線が失われていない状態になっている) と本 task の 前提条件/成果物/write scope/スコープ外 を渡し、実装手段は固定せず P11 の目的を満たす成果物を作らせる
...
- Dependencies: SYS-BLOG-SCOPED-ADMIN-CONSOLE-P07, SYS-BLOG-SCOPED-ADMIN-CONSOLE-P09
