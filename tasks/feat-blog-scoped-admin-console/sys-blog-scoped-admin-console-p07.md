---
graph_node_id: "SYS-BLOG-SCOPED-ADMIN-CONSOLE-P07"
artifact_kind: "task"
artifact_subtypes: []
project_id: "feature-package-feat-blog-scoped-admin-console"
domain: "quality"
tags: ["p07","feat-blog-scoped-admin-console"]
priority: null
start_date: null
target_date: null
iteration: null
...
purpose: "feat-blog-scoped-admin-console の受入10件を1件ずつ証跡付きで判定し、すべて満たされていることを確定した状態を成立させる。"
goal: "feat-blog-scoped-admin-console の受入10件を1件ずつ証跡付きで判定し、すべて満たされていることを確定した状態を成立させる。"
scope_in: ["Produced artifacts: docs/spec/feat-blog-scoped-admin-console/acceptance-report.md (受入10件の判定と証跡参照)","Consumed artifacts: features/feat-blog-scoped-admin-console.md; features/feat-blog-scoped-admin-console.context.json; system-spec/ui-ux.md; system-spec/frontend.md; docs/spec/feat-uiux-overhaul/ui-rules.md","Write scope/touches: docs/spec/feat-blog-scoped-admin-console/acceptance-report.md"]
scope_out: ["指標の集計処理そのもの (feat-blog-metrics-rollup が所有する)","読者行動の計測と記録 (feat-reader-behavior-analytics が所有する)","SEO/AEO の診断ロジックと配信 (各 feature が所有する)","ドメイン取得と証明書の発行処理 (feat-blog-custom-domain が所有する)","P07 以外の phase が所有する成果物への変更"]
...
phase_ref: "P07"
file_path: "tasks/feat-blog-scoped-admin-console/sys-blog-scoped-admin-console-p07.md"
template_id: "task"
...
classification_reason: "feat-blog-scoped-admin-console の P07 lifecycle 責務への確定写像"
classification_candidates: [{"artifact_kind":"task","candidate_path":"tasks/feat-blog-scoped-admin-console/sys-blog-scoped-admin-console-p07.md","confidence":1.0}]
issue_linkage: null
tracker_binding: "beads"
beads_linkage: {"bd_issue_id":"ah-wois.7","github_mirror":null,"linked_at":"2026-09-04T02:10:25Z","sync_state":"linked"}
github_publication: {"labels":[],"milestone":null,"mode":"local_only","project_aliases":[]}
github_project_linkages: []
pull_request_linkages: []
execution_contexts: []
completion_evidence: {"completed_at":"2026-09-04T11:18:56Z","evidence_refs":["beads:ah-wois.7"],"policy":"manual","reconciled_at":"2026-09-09T00:00:00Z","source":"reconciliation","status":"done"}
implementation_readiness: {"checked_at":"2026-09-04T00:00:00Z","missing_sections":[],"status":"complete"}
---

# System task overlay: 管理画面の受入10件の判定
...
- parent_feature: feat-blog-scoped-admin-console
- phase_ref: P07
- classification: confidence=1.0; reason=feat-blog-scoped-admin-console の P07 lifecycle 責務への確定写像; candidate=tasks/feat-blog-scoped-admin-console/sys-blog-scoped-admin-console-p07.md
...
feat-blog-scoped-admin-console の受入10件を1件ずつ証跡付きで判定し、すべて満たされていることを確定した状態を成立させる。

## 背景

「横断画面が記事単位の数値を持たない」は、無いことの判定である。画面の表示項目一覧と責務表を突き合わせて判定する。
...
- Produced artifacts: docs/spec/feat-blog-scoped-admin-console/acceptance-report.md (受入10件の判定と証跡参照)
...
- Write scope/touches: docs/spec/feat-blog-scoped-admin-console/acceptance-report.md
...
- PR body contract: Closes #issue (beads issue) + dev-graph graph_node_id=SYS-BLOG-SCOPED-ADMIN-CONSOLE-P07; PR の宛先は dev ブランチとする
- Ownership boundary: system-dev-planner は intent のみを宣言し、dev-graph が tracker mutation と reconciliation を行う

## Branch and worktree execution

- Branch: assigned after dev-graph registration by C15 as devgraph/SYS-BLOG-SCOPED-ADMIN-CONSOLE-P07; system-dev-planner は事前割当を行わない
- Worktree lease: claim SYS-BLOG-SCOPED-ADMIN-CONSOLE-P07 before implementation; heartbeat/release は dev-graph worktree lease 契約に従う
...
- Goal: feat-blog-scoped-admin-console の受入10件を1件ずつ証跡付きで判定し、すべて満たされていることを確定した状態を成立させる。
- Generic execution prompt: feat-blog-scoped-admin-console の goal (管理画面が記事単位ではなくブログ単位の階層で構成され、ブログごとの収益・PV・転換・住所の生死が一枚で読め、記事ごとの改善指示と横断比較が役割どおりに分かれ、既存 /admin/blog/* からの導線が失われていない状態になっている) と本 task の 前提条件/成果物/write scope/スコープ外 を渡し、実装手段は固定せず P07 の目的を満たす成果物を作らせる
...
- Dependencies: SYS-BLOG-SCOPED-ADMIN-CONSOLE-P06
