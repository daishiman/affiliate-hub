---
graph_node_id: "SYS-BLOG-SCOPED-ADMIN-CONSOLE-P09"
artifact_kind: "task"
artifact_subtypes: []
project_id: "feature-package-feat-blog-scoped-admin-console"
domain: "quality"
tags: ["p09","feat-blog-scoped-admin-console"]
priority: null
start_date: null
target_date: null
iteration: null
...
status: "closed"
depends_on: ["SYS-BLOG-SCOPED-ADMIN-CONSOLE-P08"]
related_nodes: []
resource_scope: ["docs/spec/feat-blog-scoped-admin-console/quality-report.md","tests/"]
purpose: "ブログ数と記事数が増えたときの一覧描画性能、ブログ横断画面での閲覧範囲の限定、および空状態の表示を検査し、いずれも基準を満たす状態を成立させる。"
goal: "ブログ数と記事数が増えたときの一覧描画性能、ブログ横断画面での閲覧範囲の限定、および空状態の表示を検査し、いずれも基準を満たす状態を成立させる。"
scope_in: ["Produced artifacts: docs/spec/feat-blog-scoped-admin-console/quality-report.md (描画性能・閲覧範囲・空状態の検査結果)","Consumed artifacts: features/feat-blog-scoped-admin-console.md; features/feat-blog-scoped-admin-console.context.json; system-spec/ui-ux.md; system-spec/frontend.md; docs/spec/feat-uiux-overhaul/ui-rules.md","Write scope/touches: docs/spec/feat-blog-scoped-admin-console/quality-report.md, tests/"]
scope_out: ["指標の集計処理そのもの (feat-blog-metrics-rollup が所有する)","読者行動の計測と記録 (feat-reader-behavior-analytics が所有する)","SEO/AEO の診断ロジックと配信 (各 feature が所有する)","ドメイン取得と証明書の発行処理 (feat-blog-custom-domain が所有する)","P09 以外の phase が所有する成果物への変更"]
acceptance: ["Automated commands: `pnpm vitest run` (閲覧範囲の限定テストを再実行する)","Automated commands: `pnpm run build` (本番相当ビルドが通ることを確認する)","Required evidence: quality-report.md にブログ多数時の一覧描画性能が記録されていること","Required evidence: P09 の 成果物 section に記載した produced artifacts のパス"]
...
phase_ref: "P09"
file_path: "tasks/feat-blog-scoped-admin-console/sys-blog-scoped-admin-console-p09.md"
template_id: "task"
...
classification_reason: "feat-blog-scoped-admin-console の P09 lifecycle 責務への確定写像"
classification_candidates: [{"artifact_kind":"task","candidate_path":"tasks/feat-blog-scoped-admin-console/sys-blog-scoped-admin-console-p09.md","confidence":1.0}]
issue_linkage: null
tracker_binding: "beads"
beads_linkage: {"bd_issue_id":"ah-wois.9","github_mirror":null,"linked_at":"2026-09-04T02:10:29Z","sync_state":"linked"}
github_publication: {"labels":[],"milestone":null,"mode":"local_only","project_aliases":[]}
github_project_linkages: []
pull_request_linkages: []
execution_contexts: []
completion_evidence: {"completed_at":"2026-09-04T11:19:00Z","evidence_refs":["beads:ah-wois.9"],"policy":"manual","reconciled_at":"2026-09-09T00:00:00Z","source":"reconciliation","status":"done"}
implementation_readiness: {"checked_at":"2026-09-04T00:00:00Z","missing_sections":[],"status":"complete"}
---

# System task overlay: 管理画面の非機能検査
...
- parent_feature: feat-blog-scoped-admin-console
- phase_ref: P09
- classification: confidence=1.0; reason=feat-blog-scoped-admin-console の P09 lifecycle 責務への確定写像; candidate=tasks/feat-blog-scoped-admin-console/sys-blog-scoped-admin-console-p09.md
...
ブログ数と記事数が増えたときの一覧描画性能、ブログ横断画面での閲覧範囲の限定、および空状態の表示を検査し、いずれも基準を満たす状態を成立させる。

## 背景

横断画面は複数ブログの情報を並べるため、権限の限定を誤ると他の workspace の数値が見える。ここを検査してから運用へ入る。
...
- Security: applicable; 横断画面が自 workspace のブログだけを表示することを検査する
...
- Produced artifacts: docs/spec/feat-blog-scoped-admin-console/quality-report.md (描画性能・閲覧範囲・空状態の検査結果)
...
- Write scope/touches: docs/spec/feat-blog-scoped-admin-console/quality-report.md, tests/
...
- PR body contract: Closes #issue (beads issue) + dev-graph graph_node_id=SYS-BLOG-SCOPED-ADMIN-CONSOLE-P09; PR の宛先は dev ブランチとする
- Ownership boundary: system-dev-planner は intent のみを宣言し、dev-graph が tracker mutation と reconciliation を行う

## Branch and worktree execution

- Branch: assigned after dev-graph registration by C15 as devgraph/SYS-BLOG-SCOPED-ADMIN-CONSOLE-P09; system-dev-planner は事前割当を行わない
- Worktree lease: claim SYS-BLOG-SCOPED-ADMIN-CONSOLE-P09 before implementation; heartbeat/release は dev-graph worktree lease 契約に従う
...
- テストレベル選定: 境界値: ブログ0件・多数件・権限外 workspace からの参照・記事0件のブログ。回帰: 既存の admin テストを維持する。 N/A: 単体・結合 の各テストレベルは、本 task の成果物が文書と判定であり実行可能なコードを含まないため適用しない。
...
- Automated commands: `pnpm vitest run` (閲覧範囲の限定テストを再実行する)
- Automated commands: `pnpm run build` (本番相当ビルドが通ることを確認する)
- Required evidence: quality-report.md にブログ多数時の一覧描画性能が記録されていること
...
- Goal: ブログ数と記事数が増えたときの一覧描画性能、ブログ横断画面での閲覧範囲の限定、および空状態の表示を検査し、いずれも基準を満たす状態を成立させる。
- Generic execution prompt: feat-blog-scoped-admin-console の goal (管理画面が記事単位ではなくブログ単位の階層で構成され、ブログごとの収益・PV・転換・住所の生死が一枚で読め、記事ごとの改善指示と横断比較が役割どおりに分かれ、既存 /admin/blog/* からの導線が失われていない状態になっている) と本 task の 前提条件/成果物/write scope/スコープ外 を渡し、実装手段は固定せず P09 の目的を満たす成果物を作らせる
...
- Dependencies: SYS-BLOG-SCOPED-ADMIN-CONSOLE-P08
