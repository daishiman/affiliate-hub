---
graph_node_id: "SYS-BLOG-SCOPED-ADMIN-CONSOLE-P02"
artifact_kind: "task"
artifact_subtypes: []
...
purpose: "ブログ階層のルーティング、各画面が読む集計結果と診断結果の取得経路、注意喚起の並べ替え規則、証拠件数が閾値未満のときの提案抑制の設計を確定した状態を成立させる。"
goal: "ブログ階層のルーティング、各画面が読む集計結果と診断結果の取得経路、注意喚起の並べ替え規則、証拠件数が閾値未満のときの提案抑制の設計を確定した状態を成立させる。"
scope_in: ["Produced artifacts: docs/spec/feat-blog-scoped-admin-console/routing-design.md (ブログ階層のルート構成と既存パスからの移行); docs/spec/feat-blog-scoped-admin-console/read-path-design.md (集計・診断結果の取得経路の集約); docs/spec/feat-blog-scoped-admin-console/alerting-design.md (住所が非稼働または証明書期限が近い行の並べ替え規則と抑制規則)","Consumed artifacts: features/feat-blog-scoped-admin-console.md; features/feat-blog-scoped-admin-console.context.json; system-spec/ui-ux.md; system-spec/frontend.md; docs/spec/feat-uiux-overhaul/ui-rules.md","Write scope/touches: docs/spec/feat-blog-scoped-admin-console/routing-design.md, docs/spec/feat-blog-scoped-admin-console/read-path-design.md, docs/spec/feat-blog-scoped-admin-console/alerting-design.md"]
scope_out: ["指標の集計処理そのもの (feat-blog-metrics-rollup が所有する)","読者行動の計測と記録 (feat-reader-behavior-analytics が所有する)","SEO/AEO の診断ロジックと配信 (各 feature が所有する)","ドメイン取得と証明書の発行処理 (feat-blog-custom-domain が所有する)","P02 以外の phase が所有する成果物への変更"]
...
phase_ref: "P02"
file_path: "tasks/feat-blog-scoped-admin-console/sys-blog-scoped-admin-console-p02.md"
template_id: "task"
...
classification_reason: "feat-blog-scoped-admin-console の P02 lifecycle 責務への確定写像"
classification_candidates: [{"artifact_kind":"task","candidate_path":"tasks/feat-blog-scoped-admin-console/sys-blog-scoped-admin-console-p02.md","confidence":1.0}]
issue_linkage: null
tracker_binding: "beads"
beads_linkage: {"bd_issue_id":"ah-wois.2","github_mirror":null,"linked_at":"2026-09-04T02:10:16Z","sync_state":"linked"}
github_publication: {"labels":[],"milestone":null,"mode":"local_only","project_aliases":[]}
github_project_linkages: []
pull_request_linkages: []
execution_contexts: []
completion_evidence: {"completed_at":"2026-09-04T11:18:46Z","evidence_refs":["beads:ah-wois.2"],"policy":"manual","reconciled_at":"2026-09-09T00:00:00Z","source":"reconciliation","status":"done"}
