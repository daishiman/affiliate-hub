---
graph_node_id: "task-api-unit-tests"
artifact_kind: "task"
artifact_subtypes: []
...
target_date: null
iteration: null
title: "API の単体テストを道具の一覧から生成する"
owners: ["daishiman"]
created_at: "2026-08-17T00:00:00Z"
updated_at: "2026-08-17T02:21:06.456075Z"
status: "done"
depends_on: ["task-test-foundation"]
related_nodes: []
resource_scope: ["tests/presentation"]
purpose: null
goal: null
mvp_alignment: {"background":"道具を足しても、どの入口のテストが要るか人が覚えている状態だった","mvp_fit":"enabling","purpose":"入口 3 種（REST / WebMCP / MCP）の抜けを機械的に落とす","rationale":"入口が 3 つある設計なので、手書きだと必ずどれか 1 つが抜ける"}
scope_in: []
scope_out: []
acceptance: []
architecture_refs: []
...
phase_ref: null
file_path: "tasks/task-api-unit-tests.md"
template_id: "task"
...
classification_candidates: [{"artifact_kind":"task","candidate_path":"tasks/task-api-unit-tests.md","confidence":0.95}]
issue_linkage: null
tracker_binding: "beads"
beads_linkage: {"bd_issue_id":"ah-1wf","github_mirror":null,"linked_at":"2026-08-17T02:20:00Z","sync_state":"linked"}
github_publication: {"labels":[],"milestone":null,"mode":"local_only","project_aliases":[]}
github_project_linkages: []
pull_request_linkages: []
execution_contexts: []
completion_evidence: {"completed_at":"2026-08-17T02:20:00Z","evidence_refs":["tasks/task-api-unit-tests.md","docs/product/traceability.md"],"policy":"manual","reconciled_at":"2026-08-17T02:20:00Z","source":"manual","status":"done"}
implementation_readiness: {"checked_at":"2026-08-17T00:00:00Z","missing_sections":[],"status":"complete"}
---

# 目的

入口 3 種（REST / WebMCP / MCP）の抜けを機械的に落とす

道具の一覧（`buildToolCatalog`）と項目名の辞書から 368 件を生成する。
道具を 1 つ足すと辞書に無い項目名として落ちるため、足し忘れが素通りしない。
この検査で、他社のブログが読めていた不具合が見つかり同時に直した。

## 背景

道具を足しても、どの入口のテストが要るか人が覚えている状態だった

MVP 適合度は `enabling`。入口が 3 つある設計なので、手書きだと必ずどれか 1 つが抜ける

## 入力と前提条件

- 入力: `docs/product/backlog.md` の該当項目、`docs/product/traceability.md` の REQ-TS04
- 前提: 先行タスク `task-test-foundation` が完了していること

## 出力と成果物

- 生成物: `tests/presentation/tool-inputs.ts`
- 生成物: `入口 3 種の総当たりテスト`
...
```bash
pnpm test tests/presentation（368 件）
```

## リスクとロールバック

辞書の更新漏れは落ちる側に倒してある（知らない項目名でエラー）。
