---
graph_node_id: "SYS-BLOG-CUSTOM-DOMAIN-P08"
artifact_kind: "task"
artifact_subtypes: []
project_id: "feature-package-feat-blog-custom-domain"
domain: "backend"
tags: ["p08","feat-blog-custom-domain"]
priority: null
start_date: null
target_date: null
iteration: null
...
purpose: "独自ドメイン解決の追加によって生じた既存 src/middleware.ts のホスト解決ロジックの重複を解消し、既定住所解決と独自ドメイン解決が単一の解決関数へ集約された状態を成立させる。"
goal: "独自ドメイン解決の追加によって生じた既存 src/middleware.ts のホスト解決ロジックの重複を解消し、既定住所解決と独自ドメイン解決が単一の解決関数へ集約された状態を成立させる。"
scope_in: ["Produced artifacts: src/middleware.ts の単一ホスト解決関数への集約; docs/spec/feat-blog-custom-domain/migration-notes.md (集約前後の解決順序と非互換の有無)","Consumed artifacts: features/feat-blog-custom-domain.md; features/feat-blog-custom-domain.context.json; system-spec/infrastructure.md; system-spec/backend.md; system-spec/database.md","Write scope/touches: src/middleware.ts, src/domain/domains/, docs/spec/feat-blog-custom-domain/migration-notes.md"]
scope_out: ["ドメインの購入・レジストラ契約そのもの (利用者が外部で取得する)","自前 ACME による証明書発行 (Cloudflare for SaaS が発行する)","既定住所 (slug.基底ドメイン 形式のサブドメイン) の導出とワイルドカード経路 (feat-blog-subdomain-routing が所有する)","ドメイン状態の定期監視と期限警告の掲出順序 (feat-blog-scoped-admin-console が所有する)","P08 以外の phase が所有する成果物への変更"]
acceptance: ["Automated commands: `pnpm vitest run` (集約後も全テストが緑であることを確認する)","Automated commands: `pnpm run typecheck` (集約に伴う型整合を確認する)","Required evidence: P08 の 成果物 section に記載した produced artifacts のパス"]
...
phase_ref: "P08"
file_path: "tasks/feat-blog-custom-domain/sys-blog-custom-domain-p08.md"
template_id: "task"
...
classification_reason: "feat-blog-custom-domain の P08 lifecycle 責務への確定写像"
classification_candidates: [{"artifact_kind":"task","candidate_path":"tasks/feat-blog-custom-domain/sys-blog-custom-domain-p08.md","confidence":1.0}]
issue_linkage: null
tracker_binding: "beads"
beads_linkage: {"bd_issue_id":"ah-t7vv.8","github_mirror":null,"linked_at":"2026-09-04T02:05:58Z","sync_state":"linked"}
github_publication: {"labels":[],"milestone":null,"mode":"local_only","project_aliases":[]}
github_project_linkages: []
pull_request_linkages: []
execution_contexts: []
completion_evidence: {"completed_at":"2026-09-04T08:12:06Z","evidence_refs":["beads:ah-t7vv.8"],"policy":"manual","reconciled_at":"2026-09-09T00:00:00Z","source":"reconciliation","status":"done"}
implementation_readiness: {"checked_at":"2026-09-04T00:00:00Z","missing_sections":[],"status":"complete"}
---

# System task overlay: 既存ホスト解決経路との重複解消と移行
...
- parent_feature: feat-blog-custom-domain
- phase_ref: P08
- classification: confidence=1.0; reason=feat-blog-custom-domain の P08 lifecycle 責務への確定写像; candidate=tasks/feat-blog-custom-domain/sys-blog-custom-domain-p08.md
...
独自ドメイン解決の追加によって生じた既存 src/middleware.ts のホスト解決ロジックの重複を解消し、既定住所解決と独自ドメイン解決が単一の解決関数へ集約された状態を成立させる。

## 背景

P05 では既存経路を壊さないことを優先して解決処理を前段へ追加した。その結果、ホスト名から site_slug を導く責務が2箇所に分かれている。分かれたままだと、片方だけを変更した際に既定住所と独自ドメインで挙動が食い違う。本 phase で単一の解決関数へ集約する。
...
- Compatibility/migration/backfill: 集約は振る舞いを変えないリファクタリングとして行い、解決順序の変更を伴う場合は migration-notes.md に非互換として記録する

## 成果物

- Produced artifacts: src/middleware.ts の単一ホスト解決関数への集約; docs/spec/feat-blog-custom-domain/migration-notes.md (集約前後の解決順序と非互換の有無)
...
- Write scope/touches: src/middleware.ts, src/domain/domains/, docs/spec/feat-blog-custom-domain/migration-notes.md
...
- PR body contract: Closes #issue (beads issue) + dev-graph graph_node_id=SYS-BLOG-CUSTOM-DOMAIN-P08; PR の宛先は dev ブランチとする
- Ownership boundary: system-dev-planner は intent のみを宣言し、dev-graph が tracker mutation と reconciliation を行う

## Branch and worktree execution

- Branch: assigned after dev-graph registration by C15 as devgraph/SYS-BLOG-CUSTOM-DOMAIN-P08; system-dev-planner は事前割当を行わない
- Worktree lease: claim SYS-BLOG-CUSTOM-DOMAIN-P08 before implementation; heartbeat/release は dev-graph worktree lease 契約に従う
...
- テストレベル選定: 回帰: 集約前に緑だったテストが集約後も全件緑であることを唯一の完了条件とする。境界値: 独自ドメインと既定住所の両方が同時に解決される経路を明示的に検査する。 N/A: 単体・結合 の各テストレベルは、本 task の成果物が文書と判定であり実行可能なコードを含まないため適用しない。
...
- Automated commands: `pnpm vitest run` (集約後も全テストが緑であることを確認する)
- Automated commands: `pnpm run typecheck` (集約に伴う型整合を確認する)
...
- Goal: 独自ドメイン解決の追加によって生じた既存 src/middleware.ts のホスト解決ロジックの重複を解消し、既定住所解決と独自ドメイン解決が単一の解決関数へ集約された状態を成立させる。
- Generic execution prompt: feat-blog-custom-domain の goal (所有権が検証されたドメインだけが active になり、Host ヘッダから site_slug が解決されて当該ブログが配信され、active の間は canonical が独自ドメインを指し、切断しても行は revoked として残り、既定住所は常に生きている状態になっている) と本 task の 前提条件/成果物/write scope/スコープ外 を渡し、実装手段は固定せず P08 の目的を満たす成果物を作らせる
...
- Ready when: ホスト解決が単一経路へ集約され、回帰0件である
...
- Dependencies: SYS-BLOG-CUSTOM-DOMAIN-P05
