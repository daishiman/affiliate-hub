---
graph_node_id: "SYS-BLOG-CUSTOM-DOMAIN-P06"
artifact_kind: "task"
artifact_subtypes: []
project_id: "feature-package-feat-blog-custom-domain"
domain: "quality"
tags: ["p06","feat-blog-custom-domain"]
priority: null
start_date: null
target_date: null
iteration: null
...
scope_in: ["Produced artifacts: docs/spec/feat-blog-custom-domain/test-run-report.md (実行結果とカバレッジ); tests/ 配下の独自ドメイン関連テスト実装","Consumed artifacts: features/feat-blog-custom-domain.md; features/feat-blog-custom-domain.context.json; system-spec/infrastructure.md; system-spec/backend.md; system-spec/database.md","Write scope/touches: tests/, docs/spec/feat-blog-custom-domain/test-run-report.md"]
scope_out: ["ドメインの購入・レジストラ契約そのもの (利用者が外部で取得する)","自前 ACME による証明書発行 (Cloudflare for SaaS が発行する)","既定住所 (slug.基底ドメイン 形式のサブドメイン) の導出とワイルドカード経路 (feat-blog-subdomain-routing が所有する)","ドメイン状態の定期監視と期限警告の掲出順序 (feat-blog-scoped-admin-console が所有する)","P06 以外の phase が所有する成果物への変更"]
...
phase_ref: "P06"
file_path: "tasks/feat-blog-custom-domain/sys-blog-custom-domain-p06.md"
template_id: "task"
...
classification_reason: "feat-blog-custom-domain の P06 lifecycle 責務への確定写像"
classification_candidates: [{"artifact_kind":"task","candidate_path":"tasks/feat-blog-custom-domain/sys-blog-custom-domain-p06.md","confidence":1.0}]
issue_linkage: null
tracker_binding: "beads"
beads_linkage: {"bd_issue_id":"ah-t7vv.6","github_mirror":null,"linked_at":"2026-09-04T02:05:54Z","sync_state":"linked"}
github_publication: {"labels":[],"milestone":null,"mode":"local_only","project_aliases":[]}
github_project_linkages: []
pull_request_linkages: []
execution_contexts: []
completion_evidence: {"completed_at":"2026-09-04T08:12:02Z","evidence_refs":["beads:ah-t7vv.6"],"policy":"manual","reconciled_at":"2026-09-09T00:00:00Z","source":"reconciliation","status":"done"}
implementation_readiness: {"checked_at":"2026-09-04T00:00:00Z","missing_sections":[],"status":"complete"}
---

# System task overlay: 独自ドメイン機能のテスト実行と緑化
...
- parent_feature: feat-blog-custom-domain
- phase_ref: P06
- classification: confidence=1.0; reason=feat-blog-custom-domain の P06 lifecycle 責務への確定写像; candidate=tasks/feat-blog-custom-domain/sys-blog-custom-domain-p06.md
...
実装単体では通るが結合で崩れる箇所 (provider 応答の取り込み、D1 の UNIQUE 制約違反のハンドリング、middleware の解決順序) を実行によって検出する phase である。
...
- Produced artifacts: docs/spec/feat-blog-custom-domain/test-run-report.md (実行結果とカバレッジ); tests/ 配下の独自ドメイン関連テスト実装
...
- Write scope/touches: tests/, docs/spec/feat-blog-custom-domain/test-run-report.md
...
- PR body contract: Closes #issue (beads issue) + dev-graph graph_node_id=SYS-BLOG-CUSTOM-DOMAIN-P06; PR の宛先は dev ブランチとする
- Ownership boundary: system-dev-planner は intent のみを宣言し、dev-graph が tracker mutation と reconciliation を行う

## Branch and worktree execution

- Branch: assigned after dev-graph registration by C15 as devgraph/SYS-BLOG-CUSTOM-DOMAIN-P06; system-dev-planner は事前割当を行わない
- Worktree lease: claim SYS-BLOG-CUSTOM-DOMAIN-P06 before implementation; heartbeat/release は dev-graph worktree lease 契約に従う
...
- Goal: P04 で設計した全テストケースを実行し、失敗0件かつ新規実装コードのカバレッジが80%以上である状態を成立させる。
- Generic execution prompt: feat-blog-custom-domain の goal (所有権が検証されたドメインだけが active になり、Host ヘッダから site_slug が解決されて当該ブログが配信され、active の間は canonical が独自ドメインを指し、切断しても行は revoked として残り、既定住所は常に生きている状態になっている) と本 task の 前提条件/成果物/write scope/スコープ外 を渡し、実装手段は固定せず P06 の目的を満たす成果物を作らせる
