---
graph_node_id: "SYS-BLOG-CUSTOM-DOMAIN-P03"
artifact_kind: "task"
artifact_subtypes: []
project_id: "feature-package-feat-blog-custom-domain"
domain: "quality"
tags: ["p03","feat-blog-custom-domain"]
priority: null
start_date: null
target_date: null
iteration: null
...
purpose: "P02 が確定した設計が、未検証ホストを配信しない・既定住所を壊さない・権限と監査が抜けないという3つの不変条件を満たすことを独立観点で検査し、実装着手の可否を判定した状態を成立させる。"
goal: "P02 が確定した設計が、未検証ホストを配信しない・既定住所を壊さない・権限と監査が抜けないという3つの不変条件を満たすことを独立観点で検査し、実装着手の可否を判定した状態を成立させる。"
scope_in: ["Produced artifacts: docs/spec/feat-blog-custom-domain/design-review-findings.md (設計レビューの指摘と解決状況); docs/spec/feat-blog-custom-domain/invariant-checklist.md (未検証ホスト非配信・既定住所生存・権限監査の3不変条件の検査観点)","Consumed artifacts: features/feat-blog-custom-domain.md; features/feat-blog-custom-domain.context.json; system-spec/infrastructure.md; system-spec/backend.md; system-spec/database.md","Write scope/touches: docs/spec/feat-blog-custom-domain/design-review-findings.md, docs/spec/feat-blog-custom-domain/invariant-checklist.md"]
scope_out: ["ドメインの購入・レジストラ契約そのもの (利用者が外部で取得する)","自前 ACME による証明書発行 (Cloudflare for SaaS が発行する)","既定住所 (slug.基底ドメイン 形式のサブドメイン) の導出とワイルドカード経路 (feat-blog-subdomain-routing が所有する)","ドメイン状態の定期監視と期限警告の掲出順序 (feat-blog-scoped-admin-console が所有する)","P03 以外の phase が所有する成果物への変更"]
...
phase_ref: "P03"
file_path: "tasks/feat-blog-custom-domain/sys-blog-custom-domain-p03.md"
template_id: "task"
...
classification_reason: "feat-blog-custom-domain の P03 lifecycle 責務への確定写像"
classification_candidates: [{"artifact_kind":"task","candidate_path":"tasks/feat-blog-custom-domain/sys-blog-custom-domain-p03.md","confidence":1.0}]
issue_linkage: null
tracker_binding: "beads"
beads_linkage: {"bd_issue_id":"ah-t7vv.3","github_mirror":null,"linked_at":"2026-09-04T02:05:47Z","sync_state":"linked"}
github_publication: {"labels":[],"milestone":null,"mode":"local_only","project_aliases":[]}
github_project_linkages: []
pull_request_linkages: []
execution_contexts: []
completion_evidence: {"completed_at":"2026-09-04T08:11:55Z","evidence_refs":["beads:ah-t7vv.3"],"policy":"manual","reconciled_at":"2026-09-09T00:00:00Z","source":"reconciliation","status":"done"}
implementation_readiness: {"checked_at":"2026-09-04T00:00:00Z","missing_sections":[],"status":"complete"}
---

# System task overlay: 独自ドメイン設計の独立レビューと着手可否判定
...
- parent_feature: feat-blog-custom-domain
- phase_ref: P03
- classification: confidence=1.0; reason=feat-blog-custom-domain の P03 lifecycle 責務への確定写像; candidate=tasks/feat-blog-custom-domain/sys-blog-custom-domain-p03.md
...
P02 が確定した設計が、未検証ホストを配信しない・既定住所を壊さない・権限と監査が抜けないという3つの不変条件を満たすことを独立観点で検査し、実装着手の可否を判定した状態を成立させる。

## 背景

ホスト名解決はリクエストの入口に位置するため、設計上の誤りが全ブログの到達性へ波及する。実装前に、状態機械の禁止遷移・UNIQUE 制約の効き方・失敗時のフォールバック先を独立に検査しておくことで、実装後の手戻り範囲を限定する。
...
- Infrastructure: applicable; provider 連携失敗時のフォールバック設計を検査する
- Security: applicable; 権限判定と監査記録の設計妥当性を独立に検査する
...
- Produced artifacts: docs/spec/feat-blog-custom-domain/design-review-findings.md (設計レビューの指摘と解決状況); docs/spec/feat-blog-custom-domain/invariant-checklist.md (未検証ホスト非配信・既定住所生存・権限監査の3不変条件の検査観点)
...
- Write scope/touches: docs/spec/feat-blog-custom-domain/design-review-findings.md, docs/spec/feat-blog-custom-domain/invariant-checklist.md
...
- PR body contract: Closes #issue (beads issue) + dev-graph graph_node_id=SYS-BLOG-CUSTOM-DOMAIN-P03; PR の宛先は dev ブランチとする
- Ownership boundary: system-dev-planner は intent のみを宣言し、dev-graph が tracker mutation と reconciliation を行う

## Branch and worktree execution

- Branch: assigned after dev-graph registration by C15 as devgraph/SYS-BLOG-CUSTOM-DOMAIN-P03; system-dev-planner は事前割当を行わない
- Worktree lease: claim SYS-BLOG-CUSTOM-DOMAIN-P03 before implementation; heartbeat/release は dev-graph worktree lease 契約に従う
...
- テストレベル選定: レビュー phase のため実行テストは持たず、3不変条件それぞれに検査観点が対応していることを完了条件とする。 N/A: 単体・結合・境界値・回帰 の各テストレベルは、本 task の成果物が文書と判定であり実行可能なコードを含まないため適用しない。
...
- Goal: P02 が確定した設計が、未検証ホストを配信しない・既定住所を壊さない・権限と監査が抜けないという3つの不変条件を満たすことを独立観点で検査し、実装着手の可否を判定した状態を成立させる。
- Generic execution prompt: feat-blog-custom-domain の goal (所有権が検証されたドメインだけが active になり、Host ヘッダから site_slug が解決されて当該ブログが配信され、active の間は canonical が独自ドメインを指し、切断しても行は revoked として残り、既定住所は常に生きている状態になっている) と本 task の 前提条件/成果物/write scope/スコープ外 を渡し、実装手段は固定せず P03 の目的を満たす成果物を作らせる
...
- Dependencies: SYS-BLOG-CUSTOM-DOMAIN-P02
