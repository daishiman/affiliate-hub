---
graph_node_id: "SYS-BLOG-CUSTOM-DOMAIN-P12"
artifact_kind: "task"
artifact_subtypes: []
project_id: "feature-package-feat-blog-custom-domain"
domain: "documentation"
tags: ["p12","feat-blog-custom-domain"]
priority: null
start_date: null
target_date: null
iteration: null
title: "独自ドメイン接続の運用手順と利用者向け説明"
owners: ["daishiman"]
created_at: "2026-09-03T23:50:30Z"
updated_at: "2026-09-09T00:00:00Z"
status: "closed"
depends_on: ["SYS-BLOG-CUSTOM-DOMAIN-P10","SYS-BLOG-CUSTOM-DOMAIN-P11"]
related_nodes: []
resource_scope: ["docs/spec/feat-blog-custom-domain/operations-runbook.md","docs/spec/feat-blog-custom-domain/"]
purpose: "利用者が外部で取得したドメインを接続する手順、検証が進まないときの確認点、切断時の影響を、管理画面から辿れる文書として整えた状態を成立させる。"
goal: "利用者が外部で取得したドメインを接続する手順、検証が進まないときの確認点、切断時の影響を、管理画面から辿れる文書として整えた状態を成立させる。"
scope_in: ["Produced artifacts: docs/spec/feat-blog-custom-domain/operations-runbook.md (接続・検証・切断の運用手順と滞留時の確認点); docs/ 配下の利用者向け接続手順","Consumed artifacts: features/feat-blog-custom-domain.md; features/feat-blog-custom-domain.context.json; system-spec/infrastructure.md; system-spec/backend.md; system-spec/database.md","Write scope/touches: docs/spec/feat-blog-custom-domain/operations-runbook.md, docs/spec/feat-blog-custom-domain/"]
scope_out: ["ドメインの購入・レジストラ契約そのもの (利用者が外部で取得する)","自前 ACME による証明書発行 (Cloudflare for SaaS が発行する)","既定住所 (slug.基底ドメイン 形式のサブドメイン) の導出とワイルドカード経路 (feat-blog-subdomain-routing が所有する)","ドメイン状態の定期監視と期限警告の掲出順序 (feat-blog-scoped-admin-console が所有する)","P12 以外の phase が所有する成果物への変更"]
acceptance: ["Required evidence: operations-runbook.md が接続・検証・切断の3操作すべてに手順と確認点を持つこと","Automated commands: `pnpm run lint` (文書内のコード片が静的検査を通ることを確認する)","Required evidence: P12 の 成果物 section に記載した produced artifacts のパス"]
...
phase_ref: "P12"
file_path: "tasks/feat-blog-custom-domain/sys-blog-custom-domain-p12.md"
template_id: "task"
...
classification_reason: "feat-blog-custom-domain の P12 lifecycle 責務への確定写像"
classification_candidates: [{"artifact_kind":"task","candidate_path":"tasks/feat-blog-custom-domain/sys-blog-custom-domain-p12.md","confidence":1.0}]
issue_linkage: null
tracker_binding: "beads"
beads_linkage: {"bd_issue_id":"ah-t7vv.12","github_mirror":null,"linked_at":"2026-09-04T02:06:07Z","sync_state":"linked"}
github_publication: {"labels":[],"milestone":null,"mode":"local_only","project_aliases":[]}
github_project_linkages: []
pull_request_linkages: []
execution_contexts: []
completion_evidence: {"completed_at":"2026-09-04T08:12:14Z","evidence_refs":["beads:ah-t7vv.12"],"policy":"manual","reconciled_at":"2026-09-09T00:00:00Z","source":"reconciliation","status":"done"}
implementation_readiness: {"checked_at":"2026-09-04T00:00:00Z","missing_sections":[],"status":"complete"}
---

# System task overlay: 独自ドメイン接続の運用手順と利用者向け説明
...
- parent_feature: feat-blog-custom-domain
- phase_ref: P12
- classification: confidence=1.0; reason=feat-blog-custom-domain の P12 lifecycle 責務への確定写像; candidate=tasks/feat-blog-custom-domain/sys-blog-custom-domain-p12.md
...
利用者が外部で取得したドメインを接続する手順、検証が進まないときの確認点、切断時の影響を、管理画面から辿れる文書として整えた状態を成立させる。

## 背景

接続には利用者側での DNS レコード追加が要る。この操作は本システムの外側で行われるため、手順が不明瞭だと検証が進まない状態で滞留する。運用手順として、何をどこに追加し、どれくらいで反映されるかを明示する。
...
- Operations: applicable; 接続滞留時の確認手順を本 phase が所有する
...
- Produced artifacts: docs/spec/feat-blog-custom-domain/operations-runbook.md (接続・検証・切断の運用手順と滞留時の確認点); docs/ 配下の利用者向け接続手順
...
- Write scope/touches: docs/spec/feat-blog-custom-domain/operations-runbook.md, docs/spec/feat-blog-custom-domain/
...
- PR body contract: Closes #issue (beads issue) + dev-graph graph_node_id=SYS-BLOG-CUSTOM-DOMAIN-P12; PR の宛先は dev ブランチとする
- Ownership boundary: system-dev-planner は intent のみを宣言し、dev-graph が tracker mutation と reconciliation を行う

## Branch and worktree execution

- Branch: assigned after dev-graph registration by C15 as devgraph/SYS-BLOG-CUSTOM-DOMAIN-P12; system-dev-planner は事前割当を行わない
- Worktree lease: claim SYS-BLOG-CUSTOM-DOMAIN-P12 before implementation; heartbeat/release は dev-graph worktree lease 契約に従う
...
- テストレベル選定: 文書 phase のため実行テストを持たず、3操作すべてに手順が対応することを完了条件とする。 N/A: 単体・結合・境界値・回帰 の各テストレベルは、本 task の成果物が文書と判定であり実行可能なコードを含まないため適用しない。
...
- Required evidence: operations-runbook.md が接続・検証・切断の3操作すべてに手順と確認点を持つこと
...
- Goal: 利用者が外部で取得したドメインを接続する手順、検証が進まないときの確認点、切断時の影響を、管理画面から辿れる文書として整えた状態を成立させる。
- Generic execution prompt: feat-blog-custom-domain の goal (所有権が検証されたドメインだけが active になり、Host ヘッダから site_slug が解決されて当該ブログが配信され、active の間は canonical が独自ドメインを指し、切断しても行は revoked として残り、既定住所は常に生きている状態になっている) と本 task の 前提条件/成果物/write scope/スコープ外 を渡し、実装手段は固定せず P12 の目的を満たす成果物を作らせる
...
- Dependencies: SYS-BLOG-CUSTOM-DOMAIN-P10, SYS-BLOG-CUSTOM-DOMAIN-P11
