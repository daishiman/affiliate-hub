---
graph_node_id: "SYS-BLOG-CUSTOM-DOMAIN-P05"
artifact_kind: "task"
artifact_subtypes: []
project_id: "feature-package-feat-blog-custom-domain"
domain: "backend"
tags: ["p05","feat-blog-custom-domain"]
priority: null
start_date: null
target_date: null
iteration: null
...
purpose: "site_custom_domains テーブル、connect・verify・disconnect の3ユースケース、Cloudflare for SaaS custom hostname 連携、Host ヘッダからの site_slug 解決、active 期間の canonical 切替、Publisher 以上への権限限定と audit_logs 記録、および /admin/sites/[site]/domain 画面を実装した状態を成立させる。"
goal: "site_custom_domains テーブル、connect・verify・disconnect の3ユースケース、Cloudflare for SaaS custom hostname 連携、Host ヘッダからの site_slug 解決、active 期間の canonical 切替、Publisher 以上への権限限定と audit_logs 記録、および /admin/sites/[site]/domain 画面を実装した状態を成立させる。"
scope_in: ["Produced artifacts: src/db/schema.ts の site_custom_domains 定義と drizzle マイグレーション; src/domain/domains/ の状態機械と hostname 値オブジェクト; src/application/domains/ の connect-custom-domain, verify-custom-domain, disconnect-custom-domain; src/infrastructure/cloudflare/custom-hostname.ts の provider 連携; src/middleware.ts への Host 解決の追加; src/app/admin/sites/[site]/domain/ の接続手順・状態・失敗理由の画面; src/app/api/admin/ 配下の接続・検証・切断エンドポイント","Consumed artifacts: features/feat-blog-custom-domain.md; features/feat-blog-custom-domain.context.json; system-spec/infrastructure.md; system-spec/backend.md; system-spec/database.md","Write scope/touches: src/db/schema.ts, drizzle/, src/domain/domains/, src/application/domains/, src/infrastructure/cloudflare/custom-hostname.ts, src/middleware.ts, src/app/admin/sites/[site]/domain/, src/app/api/admin/"]
scope_out: ["ドメインの購入・レジストラ契約そのもの (利用者が外部で取得する)","自前 ACME による証明書発行 (Cloudflare for SaaS が発行する)","既定住所 (slug.基底ドメイン 形式のサブドメイン) の導出とワイルドカード経路 (feat-blog-subdomain-routing が所有する)","ドメイン状態の定期監視と期限警告の掲出順序 (feat-blog-scoped-admin-console が所有する)","P05 以外の phase が所有する成果物への変更"]
...
phase_ref: "P05"
file_path: "tasks/feat-blog-custom-domain/sys-blog-custom-domain-p05.md"
template_id: "task"
...
classification_reason: "feat-blog-custom-domain の P05 lifecycle 責務への確定写像"
classification_candidates: [{"artifact_kind":"task","candidate_path":"tasks/feat-blog-custom-domain/sys-blog-custom-domain-p05.md","confidence":1.0}]
issue_linkage: null
tracker_binding: "beads"
beads_linkage: {"bd_issue_id":"ah-t7vv.5","github_mirror":null,"linked_at":"2026-09-04T02:05:52Z","sync_state":"linked"}
github_publication: {"labels":[],"milestone":null,"mode":"local_only","project_aliases":[]}
github_project_linkages: []
pull_request_linkages: []
execution_contexts: []
completion_evidence: {"completed_at":"2026-09-04T08:12:00Z","evidence_refs":["beads:ah-t7vv.5"],"policy":"manual","reconciled_at":"2026-09-09T00:00:00Z","source":"reconciliation","status":"done"}
implementation_readiness: {"checked_at":"2026-09-04T00:00:00Z","missing_sections":[],"status":"complete"}
---

# System task overlay: 独自ドメイン接続機能の実装
...
- parent_feature: feat-blog-custom-domain
- phase_ref: P05
- classification: confidence=1.0; reason=feat-blog-custom-domain の P05 lifecycle 責務への確定写像; candidate=tasks/feat-blog-custom-domain/sys-blog-custom-domain-p05.md
...
site_custom_domains テーブル、connect・verify・disconnect の3ユースケース、Cloudflare for SaaS custom hostname 連携、Host ヘッダからの site_slug 解決、active 期間の canonical 切替、Publisher 以上への権限限定と audit_logs 記録、および /admin/sites/[site]/domain 画面を実装した状態を成立させる。

## 背景

P02 の設計と P04 のテスト設計を入力に、実装を行う。Host 解決は既存 src/middleware.ts の既定住所解決より前に独自ドメインを引き当て、該当が無いときだけ既定住所の解決へ委譲する順序とする。この順序により、独自ドメインが active でも既定住所が 200 を返し続けるという受入が構造的に満たされる。
...
- Compatibility/migration/backfill: 既存 src/middleware.ts の既定住所解決は残したまま独自ドメイン解決を前段へ追加し、既存経路の振る舞いを変えない

## 成果物

- Produced artifacts: src/db/schema.ts の site_custom_domains 定義と drizzle マイグレーション; src/domain/domains/ の状態機械と hostname 値オブジェクト; src/application/domains/ の connect-custom-domain, verify-custom-domain, disconnect-custom-domain; src/infrastructure/cloudflare/custom-hostname.ts の provider 連携; src/middleware.ts への Host 解決の追加; src/app/admin/sites/[site]/domain/ の接続手順・状態・失敗理由の画面; src/app/api/admin/ 配下の接続・検証・切断エンドポイント
...
- Write scope/touches: src/db/schema.ts, drizzle/, src/domain/domains/, src/application/domains/, src/infrastructure/cloudflare/custom-hostname.ts, src/middleware.ts, src/app/admin/sites/[site]/domain/, src/app/api/admin/
...
- PR body contract: Closes #issue (beads issue) + dev-graph graph_node_id=SYS-BLOG-CUSTOM-DOMAIN-P05; PR の宛先は dev ブランチとする
- Ownership boundary: system-dev-planner は intent のみを宣言し、dev-graph が tracker mutation と reconciliation を行う

## Branch and worktree execution

- Branch: assigned after dev-graph registration by C15 as devgraph/SYS-BLOG-CUSTOM-DOMAIN-P05; system-dev-planner は事前割当を行わない
- Worktree lease: claim SYS-BLOG-CUSTOM-DOMAIN-P05 before implementation; heartbeat/release は dev-graph worktree lease 契約に従う
...
- テストレベル選定: 単体: 状態遷移と hostname 正規化。結合: 接続から検証、切断までを D1 まで通す。境界値: 未検証ホスト・重複 hostname・権限不足・二度目の接続。回帰: 既存テストを0件失敗のまま維持する。
...
- Goal: site_custom_domains テーブル、connect・verify・disconnect の3ユースケース、Cloudflare for SaaS custom hostname 連携、Host ヘッダからの site_slug 解決、active 期間の canonical 切替、Publisher 以上への権限限定と audit_logs 記録、および /admin/sites/[site]/domain 画面を実装した状態を成立させる。
- Generic execution prompt: feat-blog-custom-domain の goal (所有権が検証されたドメインだけが active になり、Host ヘッダから site_slug が解決されて当該ブログが配信され、active の間は canonical が独自ドメインを指し、切断しても行は revoked として残り、既定住所は常に生きている状態になっている) と本 task の 前提条件/成果物/write scope/スコープ外 を渡し、実装手段は固定せず P05 の目的を満たす成果物を作らせる
...
- Dependencies: SYS-BLOG-CUSTOM-DOMAIN-P04
