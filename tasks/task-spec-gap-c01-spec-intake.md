---
graph_node_id: "task-spec-gap-c01-spec-intake"
artifact_kind: "task"
artifact_subtypes: []
...
resource_scope: ["docs","system-spec"]
purpose: null
goal: null
mvp_alignment: {"background":"完全性評価は docs/spec が 7 枚だった日の判定で、いま在る 16 枚のうち 9 枚は 1 度も参照されていない","mvp_fit":"enabling","purpose":"9 枚を書面入力 target として取り込み、影響カテゴリの確定セルを R4-reopen で再確定する","rationale":"参照されていない仕様は、満たしているかどうかを誰も見ていない。9 件のうち唯一の high であり、他の C03 側はこれの後でしか意味を持たない"}
scope_in: []
scope_out: []
acceptance: []
architecture_refs: []
...
phase_ref: null
file_path: "tasks/task-spec-gap-c01-spec-intake.md"
template_id: "task"
...
9 枚を書面入力 target として取り込み、影響カテゴリの確定セルを R4-reopen で再確定する

## 背景

完全性評価は docs/spec が 7 枚だった日の判定で、いま在る 16 枚のうち 9 枚は 1 度も参照されていない

## 評価器の指摘（原文）

> [C01 run-system-spec-elicit へ差し戻し / high] docs/spec/04〜12 の 9 枚 (2,705 行) を書面入力 target として取り込み、影響カテゴリの確定セルを R4-reopen で再確定する。取り込み後に C03 再生成 → C05 再評価。scripts/spec-freshness.mjs --write は C05 が再び PASS した後にのみ実行する (先に焼くと『いまの仕様書に対する PASS』という虚偽が 1 コマンドで作れる)。

出典は `system-spec/completeness-report.json` の `gaps[0]`。
**この文は完全性評価器（`assign-system-spec-completeness-evaluator`、`context: fork`）が
書いたもので、outer session は 1 文字も書いていない。**

## なぜこれを落とすのか

参照されていない仕様は、満たしているかどうかを誰も見ていない。9 件のうち唯一の high であり、他の C03 側はこれの後でしか意味を持たない

## 入力と前提条件

`system-spec/completeness-report.json` の `gaps[0]`。
先行する課題は無い。
...
`docs/`、`system-spec/`。ほかの子とは触る場所が分かれている。
...
`system-spec/completeness-report.json` `gaps[0]`、`docs/product/backlog.md` 項目 90・91
