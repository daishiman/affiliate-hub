---
graph_node_id: "task-required-test-types-cicd"
artifact_kind: "task"
artifact_subtypes: []
...
target_date: null
iteration: null
title: "必須テスト種別を宣言する（禁止依存・テストの網羅・自動チェック 29 件）"
owners: ["daishiman"]
created_at: "2026-08-17T22:00:00Z"
updated_at: "2026-09-08T07:21:38.831138Z"
status: "draft"
...
purpose: null
goal: null
mvp_alignment: {"background":"240 要件のうち必須テスト種別を宣言できているのは 12 件だけで、禁止依存・テストの網羅・自動チェックの 29 件は未宣言のまま","mvp_fit":"enabling","purpose":"禁止依存・テストの網羅・自動チェックの 29 件について、必要なテスト種別を 1 件ずつ決める","rationale":"種別を決めないとテストは書きやすいところから書かれ、権限のできてはいけない側・禁止された状態遷移・障害注入が永久に残る"}
scope_in: []
scope_out: []
acceptance: []
architecture_refs: []
...
phase_ref: null
file_path: "tasks/task-required-test-types-cicd.md"
template_id: "task"
...
classification_candidates: [{"artifact_kind":"task","candidate_path":"tasks/task-required-test-types-cicd.md","confidence":0.95}]
issue_linkage: null
tracker_binding: "beads"
beads_linkage: {"bd_issue_id":"ah-rwn","github_mirror":null,"linked_at":"2026-09-08T07:14:07Z","sync_state":"linked"}
...
**禁止依存・テストの網羅・自動チェック**の要件 29 件について、
...
そこで**文脈ごと**に分けた。この課題はそのうちの 1 つで、29 件だけを見ればよい。
...
対象の要件 29 件（**禁止依存・テストの網羅・自動チェック**）:

`REQ-CI01` `REQ-CI02` `REQ-CI03` `REQ-CI04` `REQ-CI05` `REQ-CI06` `REQ-CI07` `REQ-CI08` `REQ-CI09` `REQ-CI10` `REQ-CI11` `REQ-CI12` `REQ-CI13` `REQ-FD01` `REQ-FD02` `REQ-FD03` `REQ-FD04` `REQ-FD05` `REQ-FD06` `REQ-TS01` `REQ-TS02` `REQ-TS03` `REQ-TS04` `REQ-TS05` `REQ-TS06` `REQ-TS07` `REQ-TS08` `REQ-TS09` `REQ-TS10`
...
1. 対象 29 件を 1 件ずつ**中身を読んで**、性質を決める
...
- 対象 29 件すべてに、宣言か理由つき除外がある
- `node scripts/required-test-types.mjs` が緑
- `TEST_TYPES_MAX_UNDECLARED` が 29 件ぶん下がっている
