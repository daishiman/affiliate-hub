---
graph_node_id: "SYS-FB-CAPTURE-EXCLUSION-P05"
artifact_kind: "task"
artifact_subtypes: []
project_id: "feature-package-feat-feedback-capture-self-exclusion"
domain: "frontend"
tags: ["p05","feat-feedback-capture-self-exclusion"]
priority: null
start_date: null
target_date: null
iteration: null
title: "撮影中だけ送信 UI を写しの対象から外す実装"
owners: ["daishiman"]
created_at: "2026-08-30T12:31:04Z"
updated_at: "2026-08-30T13:32:00.645294Z"
status: "closed"
closed_at: "2026-08-30T09:09:32Z"
depends_on: ["SYS-FB-CAPTURE-EXCLUSION-P04"]
related_nodes: []
resource_scope: ["src/presentation/ui/patterns"]
purpose: "撮影の要求と送信 UI の可視化を別の時点へ分け、撮影中は起動ボタンと送信モーダルを写しの対象から外し、写しが撮れない場合は待たずに送信 UI を開く実装を成立させる。"
goal: "撮影の要求と送信 UI の可視化を別の時点へ分け、撮影中は起動ボタンと送信モーダルを写しの対象から外し、写しが撮れない場合は待たずに送信 UI を開く実装を成立させる。"
scope_in: ["Produced artifacts: src/presentation/ui/patterns/feedback-button.tsx (撮影と可視化の時点分離・撮影中の退避・復元・退避経路); src/presentation/ui/patterns (本文の上に浮く要素への data-floating-overlay 付与)","Consumed artifacts: features/feat-feedback-capture-self-exclusion.md, features/feat-feedback-capture-self-exclusion.context.json, system-spec/frontend.md, docs/spec/12-改善要望フィードバック仕様.md","Write scope/touches: src/presentation/ui/patterns"]
...
acceptance: ["Automated commands: `pnpm run typecheck`","Automated commands: `pnpm run lint`","Automated commands: `python3 .claude/plugins/system-dev-planner/scripts/validate-system-plan.py --repo-root . --feature-package feature-package/feat-feedback-capture-self-exclusion`","Required evidence: P05 の write_scope 配下の変更と、typecheck および lint の実行結果"]
...
phase_ref: "P05"
file_path: "tasks/feat-feedback-capture-self-exclusion/sys-fb-capture-exclusion-p05.md"
template_id: "task"
...
source_lineage: {"imported_at":"2026-08-30T12:31:04Z","origin_kind":"system-dev-planner","source_digest":"892cd5611c443a5f90b97047a9542489c44be963ff4d775c30b640e7aca40b7d","source_path":".dev-graph/published/generations/feature-package-feat-feedback-capture-self-exclusion/892cd5611c443a5f90b97047a9542489c44be963ff4d775c30b640e7aca40b7d/task-specs/phase-05-implementation.md","source_plugin":"system-dev-planner","source_version":"0.1.0"}
classification_confidence: 1.0
classification_reason: "feat-feedback-capture-self-exclusion の P05 lifecycle 責務への確定写像"
classification_candidates: [{"artifact_kind":"task","candidate_path":"tasks/feat-feedback-capture-self-exclusion/sys-fb-capture-exclusion-p05.md","confidence":1.0}]
issue_linkage: null
tracker_binding: "beads"
beads_linkage: {"bd_issue_id":"ah-0d2q.5","github_mirror":null,"linked_at":"2026-08-30T04:17:19Z","sync_state":"linked"}
github_publication: {"labels":[],"milestone":null,"mode":"local_only","project_aliases":[]}
github_project_linkages: []
pull_request_linkages: []
execution_contexts: []
completion_evidence: {"completed_at":"2026-08-30T09:09:32Z","evidence_refs":["beads:ah-0d2q.5"],"policy":"manual","reconciled_at":"2026-08-30T12:05:00Z","source":"reconciliation","status":"done"}
implementation_readiness: {"checked_at":"2026-08-30T03:45:47Z","missing_sections":[],"status":"complete"}
---

# System task overlay: 撮影中だけ送信 UI を写しの対象から外す実装
...
- parent_feature: feat-feedback-capture-self-exclusion
- phase_ref: P05
- classification: confidence=1.0; reason=feat-feedback-capture-self-exclusion の P05 lifecycle 責務への確定写像; candidate=tasks/feat-feedback-capture-self-exclusion/sys-fb-capture-exclusion-p05.md
...
撮影の要求と送信 UI の可視化を別の時点へ分け、撮影中は起動ボタンと送信モーダルを写しの対象から外し、写しが撮れない場合は待たずに送信 UI を開く実装を成立させる。

## 背景

現行 src/presentation/ui/patterns/feedback-button.tsx の押下処理は、撮影の開始と送信 UI の可視化を同じ出来事で行う。P02 の設計に従い、撮影の要求は押した瞬間に出し、送信 UI の描画は写しが確定した後へ移す。撮り直しの経路にも同じ規則を効かせ、本文の上に浮く要素へ data-floating-overlay を付けて写し除外と重なり監査の共通の手掛かりにする。
...
- Produced artifacts: src/presentation/ui/patterns/feedback-button.tsx (撮影と可視化の時点分離・撮影中の退避・復元・退避経路); src/presentation/ui/patterns (本文の上に浮く要素への data-floating-overlay 付与)
...
- Write scope/touches: src/presentation/ui/patterns
...
- PR body contract: Closes #issue (beads issue) + dev-graph graph_node_id=SYS-FB-CAPTURE-EXCLUSION-P05; PR の宛先は dev ブランチとする
- Ownership boundary: system-dev-planner は intent のみを宣言し、dev-graph が tracker mutation と reconciliation を行う

## Branch and worktree execution

- Branch: assigned after dev-graph registration by C15 as devgraph/SYS-FB-CAPTURE-EXCLUSION-P05; system-dev-planner は事前割当を行わない
- Worktree lease: claim SYS-FB-CAPTURE-EXCLUSION-P05 before implementation; heartbeat/release は dev-graph worktree lease 契約に従う
...
- Required evidence: P05 の write_scope 配下の変更と、typecheck および lint の実行結果

## Inner goal-seek execution loop

- Methodology contract: `system-task-goal-seek/v1`
- Goal: 撮影の要求と送信 UI の可視化を別の時点へ分け、撮影中は起動ボタンと送信モーダルを写しの対象から外し、写しが撮れない場合は待たずに送信 UI を開く実装を成立させる。
- Generic execution prompt: feat-feedback-capture-self-exclusion の goal (改善ボタンを押して撮られた写しに送信 UI が 1 画素も写らず、撮り直しでも同じ規則が効き、写しが撮れない環境でも送信 UI が待たずに開く) と本 task の 前提条件/成果物/write_scope/スコープ外 を渡し、実装手段は固定せず P05 の目的を満たす成果物を作らせる
...
- Dependencies: SYS-FB-CAPTURE-EXCLUSION-P04
