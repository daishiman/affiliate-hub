---
graph_node_id: "SYS-FB-CAPTURE-EXCLUSION-P02"
artifact_kind: "task"
artifact_subtypes: []
project_id: "feature-package-feat-feedback-capture-self-exclusion"
domain: "frontend"
tags: ["p02","feat-feedback-capture-self-exclusion"]
priority: null
start_date: null
target_date: null
iteration: null
title: "撮影と送信 UI 可視化を別の時点へ分ける設計の確定"
owners: ["daishiman"]
created_at: "2026-08-30T12:31:04Z"
updated_at: "2026-08-30T13:31:09.761092Z"
status: "closed"
closed_at: "2026-08-30T09:09:26Z"
depends_on: ["SYS-FB-CAPTURE-EXCLUSION-P01"]
related_nodes: []
resource_scope: ["docs/spec/feat-feedback-capture-self-exclusion/capture-sequence-design.md","docs/spec/feat-feedback-capture-self-exclusion/floating-overlay-contract.md"]
purpose: "撮影の開始と送信 UI の描画を別の時点へ分ける設計を、押した勢い (transient activation) を失わない形で確定し、写しが撮れない場合の退避経路と、隠した要素を元へ戻す責務の所在を決めた状態を成立させる。"
goal: "撮影の開始と送信 UI の描画を別の時点へ分ける設計を、押した勢い (transient activation) を失わない形で確定し、写しが撮れない場合の退避経路と、隠した要素を元へ戻す責務の所在を決めた状態を成立させる。"
scope_in: ["Produced artifacts: docs/spec/feat-feedback-capture-self-exclusion/capture-sequence-design.md (撮影要求・許可・写し確定・UI 可視化の順序と状態遷移); docs/spec/feat-feedback-capture-self-exclusion/floating-overlay-contract.md (data-floating-overlay の意味・付与規則・写し除外と重なり監査での用法)","Consumed artifacts: features/feat-feedback-capture-self-exclusion.md, features/feat-feedback-capture-self-exclusion.context.json, system-spec/frontend.md, docs/spec/12-改善要望フィードバック仕様.md","Write scope/touches: docs/spec/feat-feedback-capture-self-exclusion/capture-sequence-design.md, docs/spec/feat-feedback-capture-self-exclusion/floating-overlay-contract.md"]
...
acceptance: ["Automated commands: `pnpm run typecheck`","Automated commands: `python3 .claude/plugins/system-dev-planner/scripts/validate-system-plan.py --repo-root . --feature-package feature-package/feat-feedback-capture-self-exclusion`","Required evidence: P02 の 成果物 section に記した produced artifacts のパス"]
...
phase_ref: "P02"
file_path: "tasks/feat-feedback-capture-self-exclusion/sys-fb-capture-exclusion-p02.md"
template_id: "task"
...
source_lineage: {"imported_at":"2026-08-30T12:31:04Z","origin_kind":"system-dev-planner","source_digest":"892cd5611c443a5f90b97047a9542489c44be963ff4d775c30b640e7aca40b7d","source_path":".dev-graph/published/generations/feature-package-feat-feedback-capture-self-exclusion/892cd5611c443a5f90b97047a9542489c44be963ff4d775c30b640e7aca40b7d/task-specs/phase-02-architecture.md","source_plugin":"system-dev-planner","source_version":"0.1.0"}
classification_confidence: 1.0
classification_reason: "feat-feedback-capture-self-exclusion の P02 lifecycle 責務への確定写像"
classification_candidates: [{"artifact_kind":"task","candidate_path":"tasks/feat-feedback-capture-self-exclusion/sys-fb-capture-exclusion-p02.md","confidence":1.0}]
issue_linkage: null
tracker_binding: "beads"
beads_linkage: {"bd_issue_id":"ah-0d2q.2","github_mirror":null,"linked_at":"2026-08-30T04:17:14Z","sync_state":"linked"}
github_publication: {"labels":[],"milestone":null,"mode":"local_only","project_aliases":[]}
github_project_linkages: []
pull_request_linkages: []
execution_contexts: []
completion_evidence: {"completed_at":"2026-08-30T09:09:26Z","evidence_refs":["beads:ah-0d2q.2"],"policy":"manual","reconciled_at":"2026-08-30T12:05:00Z","source":"reconciliation","status":"done"}
implementation_readiness: {"checked_at":"2026-08-30T03:45:47Z","missing_sections":[],"status":"complete"}
---

# System task overlay: 撮影と送信 UI 可視化を別の時点へ分ける設計の確定
...
- parent_feature: feat-feedback-capture-self-exclusion
- phase_ref: P02
- classification: confidence=1.0; reason=feat-feedback-capture-self-exclusion の P02 lifecycle 責務への確定写像; candidate=tasks/feat-feedback-capture-self-exclusion/sys-fb-capture-exclusion-p02.md
...
撮影の開始と送信 UI の描画を別の時点へ分ける設計を、押した勢い (transient activation) を失わない形で確定し、写しが撮れない場合の退避経路と、隠した要素を元へ戻す責務の所在を決めた状態を成立させる。

## 背景

getDisplayMedia の呼出だけが押した勢いを要し、モーダルの描画は要さない。この非対称性を使うと、撮影の要求を先に出し、写しが確定してから送信 UI を描く順序へ組み替えられる。同時に、許可が拒否された場合や非対応環境では待ちが無限に伸びないよう、送信 UI を即時に開く退避経路が要る。data-floating-overlay を写し除外と重なり監査の共通の手掛かりにする方針もここで確定する。
...
- Produced artifacts: docs/spec/feat-feedback-capture-self-exclusion/capture-sequence-design.md (撮影要求・許可・写し確定・UI 可視化の順序と状態遷移); docs/spec/feat-feedback-capture-self-exclusion/floating-overlay-contract.md (data-floating-overlay の意味・付与規則・写し除外と重なり監査での用法)
...
- Write scope/touches: docs/spec/feat-feedback-capture-self-exclusion/capture-sequence-design.md, docs/spec/feat-feedback-capture-self-exclusion/floating-overlay-contract.md
...
- PR body contract: Closes #issue (beads issue) + dev-graph graph_node_id=SYS-FB-CAPTURE-EXCLUSION-P02; PR の宛先は dev ブランチとする
- Ownership boundary: system-dev-planner は intent のみを宣言し、dev-graph が tracker mutation と reconciliation を行う

## Branch and worktree execution

- Branch: assigned after dev-graph registration by C15 as devgraph/SYS-FB-CAPTURE-EXCLUSION-P02; system-dev-planner は事前割当を行わない
- Worktree lease: claim SYS-FB-CAPTURE-EXCLUSION-P02 before implementation; heartbeat/release は dev-graph worktree lease 契約に従う
...
- Required evidence: P02 の 成果物 section に記した produced artifacts のパス

## Inner goal-seek execution loop

- Methodology contract: `system-task-goal-seek/v1`
- Goal: 撮影の開始と送信 UI の描画を別の時点へ分ける設計を、押した勢い (transient activation) を失わない形で確定し、写しが撮れない場合の退避経路と、隠した要素を元へ戻す責務の所在を決めた状態を成立させる。
- Generic execution prompt: feat-feedback-capture-self-exclusion の goal (改善ボタンを押して撮られた写しに送信 UI が 1 画素も写らず、撮り直しでも同じ規則が効き、写しが撮れない環境でも送信 UI が待たずに開く) と本 task の 前提条件/成果物/write_scope/スコープ外 を渡し、実装手段は固定せず P02 の目的を満たす成果物を作らせる
...
- Dependencies: SYS-FB-CAPTURE-EXCLUSION-P01
