---
graph_node_id: "SYS-FB-CAPTURE-EXCLUSION-P10"
artifact_kind: "task"
artifact_subtypes: []
project_id: "feature-package-feat-feedback-capture-self-exclusion"
domain: "quality"
tags: ["p10","feat-feedback-capture-self-exclusion"]
priority: null
start_date: null
target_date: null
iteration: null
...
related_nodes: []
resource_scope: ["docs/spec/feat-feedback-capture-self-exclusion/final-review.md"]
purpose: "P01 から P09 までの成果を通しで読み、設計の意図・実装・検査・文書の間に食い違いが無いことを独立の観点で判定した状態を成立させる。"
goal: "P01 から P09 までの成果を通しで読み、設計の意図・実装・検査・文書の間に食い違いが無いことを独立の観点で判定した状態を成立させる。"
scope_in: ["Produced artifacts: docs/spec/feat-feedback-capture-self-exclusion/final-review.md (設計意図と現物の突合・P03 指摘の反映状況・残課題の帰属)","Consumed artifacts: features/feat-feedback-capture-self-exclusion.md, features/feat-feedback-capture-self-exclusion.context.json, system-spec/frontend.md, docs/spec/12-改善要望フィードバック仕様.md","Write scope/touches: docs/spec/feat-feedback-capture-self-exclusion/final-review.md"]
...
acceptance: ["Automated commands: `pnpm run typecheck`","Automated commands: `python3 .claude/plugins/system-dev-planner/scripts/validate-system-plan.py --repo-root . --feature-package feature-package/feat-feedback-capture-self-exclusion`","Required evidence: P10 の 成果物 section に記した produced artifacts のパス"]
...
phase_ref: "P10"
file_path: "tasks/feat-feedback-capture-self-exclusion/sys-fb-capture-exclusion-p10.md"
template_id: "task"
...
source_lineage: {"imported_at":"2026-08-30T12:31:04Z","origin_kind":"system-dev-planner","source_digest":"892cd5611c443a5f90b97047a9542489c44be963ff4d775c30b640e7aca40b7d","source_path":".dev-graph/published/generations/feature-package-feat-feedback-capture-self-exclusion/892cd5611c443a5f90b97047a9542489c44be963ff4d775c30b640e7aca40b7d/task-specs/phase-10-final-review.md","source_plugin":"system-dev-planner","source_version":"0.1.0"}
classification_confidence: 1.0
classification_reason: "feat-feedback-capture-self-exclusion の P10 lifecycle 責務への確定写像"
classification_candidates: [{"artifact_kind":"task","candidate_path":"tasks/feat-feedback-capture-self-exclusion/sys-fb-capture-exclusion-p10.md","confidence":1.0}]
issue_linkage: null
tracker_binding: "beads"
beads_linkage: {"bd_issue_id":"ah-0d2q.10","github_mirror":null,"linked_at":"2026-08-30T04:17:27Z","sync_state":"linked"}
...
# System task overlay: 最終レビューと反映漏れの判定
...
- parent_feature: feat-feedback-capture-self-exclusion
- phase_ref: P10
- classification: confidence=1.0; reason=feat-feedback-capture-self-exclusion の P10 lifecycle 責務への確定写像; candidate=tasks/feat-feedback-capture-self-exclusion/sys-fb-capture-exclusion-p10.md
...
P01 から P09 までの成果を通しで読み、設計の意図・実装・検査・文書の間に食い違いが無いことを独立の観点で判定した状態を成立させる。

## 背景

時点分離という設計の意図は、実装の細部や検査の書き方でいくらでも骨抜きにできる。最終段で、意図と現物の乖離と、P03 の指摘の反映漏れを確かめる。
...
- Produced artifacts: docs/spec/feat-feedback-capture-self-exclusion/final-review.md (設計意図と現物の突合・P03 指摘の反映状況・残課題の帰属)
...
- Write scope/touches: docs/spec/feat-feedback-capture-self-exclusion/final-review.md
...
- PR body contract: Closes #issue (beads issue) + dev-graph graph_node_id=SYS-FB-CAPTURE-EXCLUSION-P10; PR の宛先は dev ブランチとする
- Ownership boundary: system-dev-planner は intent のみを宣言し、dev-graph が tracker mutation と reconciliation を行う

## Branch and worktree execution

- Branch: assigned after dev-graph registration by C15 as devgraph/SYS-FB-CAPTURE-EXCLUSION-P10; system-dev-planner は事前割当を行わない
- Worktree lease: claim SYS-FB-CAPTURE-EXCLUSION-P10 before implementation; heartbeat/release は dev-graph worktree lease 契約に従う
...
- Required evidence: P10 の 成果物 section に記した produced artifacts のパス

## Inner goal-seek execution loop

- Methodology contract: `system-task-goal-seek/v1`
- Goal: P01 から P09 までの成果を通しで読み、設計の意図・実装・検査・文書の間に食い違いが無いことを独立の観点で判定した状態を成立させる。
- Generic execution prompt: feat-feedback-capture-self-exclusion の goal (改善ボタンを押して撮られた写しに送信 UI が 1 画素も写らず、撮り直しでも同じ規則が効き、写しが撮れない環境でも送信 UI が待たずに開く) と本 task の 前提条件/成果物/write_scope/スコープ外 を渡し、実装手段は固定せず P10 の目的を満たす成果物を作らせる
...
- Dependencies: SYS-FB-CAPTURE-EXCLUSION-P09
