---
graph_node_id: "SYS-FB-CAPTURE-EXCLUSION-P12"
artifact_kind: "task"
artifact_subtypes: []
project_id: "feature-package-feat-feedback-capture-self-exclusion"
domain: "documentation"
tags: ["p12","feat-feedback-capture-self-exclusion"]
priority: null
start_date: null
target_date: null
iteration: null
...
related_nodes: []
resource_scope: ["docs/spec/12-改善要望フィードバック仕様.md","docs/spec/feat-feedback-capture-self-exclusion/operations.md"]
purpose: "写しが自分自身を含む場合の条件を既存の改善要望フィードバック仕様へ追補し、新しい浮遊要素を足すときに属性を付ける手順を運用として残した状態を成立させる。"
goal: "写しが自分自身を含む場合の条件を既存の改善要望フィードバック仕様へ追補し、新しい浮遊要素を足すときに属性を付ける手順を運用として残した状態を成立させる。"
scope_in: ["Produced artifacts: docs/spec/12-改善要望フィードバック仕様.md (写しの自己写り込みに関する受入条件の追補); docs/spec/feat-feedback-capture-self-exclusion/operations.md (浮遊要素を追加するときの属性付与手順と検査の効かせ方)","Consumed artifacts: features/feat-feedback-capture-self-exclusion.md, features/feat-feedback-capture-self-exclusion.context.json, system-spec/frontend.md, docs/spec/12-改善要望フィードバック仕様.md","Write scope/touches: docs/spec/12-改善要望フィードバック仕様.md, docs/spec/feat-feedback-capture-self-exclusion/operations.md"]
...
acceptance: ["Automated commands: `pnpm run typecheck`","Automated commands: `python3 .claude/plugins/system-dev-planner/scripts/validate-system-plan.py --repo-root . --feature-package feature-package/feat-feedback-capture-self-exclusion`","Required evidence: P12 の 成果物 section に記した produced artifacts のパス"]
...
phase_ref: "P12"
file_path: "tasks/feat-feedback-capture-self-exclusion/sys-fb-capture-exclusion-p12.md"
template_id: "task"
...
source_lineage: {"imported_at":"2026-08-30T12:31:04Z","origin_kind":"system-dev-planner","source_digest":"892cd5611c443a5f90b97047a9542489c44be963ff4d775c30b640e7aca40b7d","source_path":".dev-graph/published/generations/feature-package-feat-feedback-capture-self-exclusion/892cd5611c443a5f90b97047a9542489c44be963ff4d775c30b640e7aca40b7d/task-specs/phase-12-documentation-operations.md","source_plugin":"system-dev-planner","source_version":"0.1.0"}
classification_confidence: 1.0
classification_reason: "feat-feedback-capture-self-exclusion の P12 lifecycle 責務への確定写像"
classification_candidates: [{"artifact_kind":"task","candidate_path":"tasks/feat-feedback-capture-self-exclusion/sys-fb-capture-exclusion-p12.md","confidence":1.0}]
issue_linkage: null
tracker_binding: "beads"
beads_linkage: {"bd_issue_id":"ah-0d2q.12","github_mirror":null,"linked_at":"2026-08-30T04:17:30Z","sync_state":"linked"}
github_publication: {"labels":[],"milestone":null,"mode":"local_only","project_aliases":[]}
github_project_linkages: []
pull_request_linkages: []
execution_contexts: []
completion_evidence: {"completed_at":"2026-09-05T14:29:28Z","evidence_refs":["beads:ah-0d2q.12","dev-graph:SYS-FB-CAPTURE-EXCLUSION-P11"],"policy":"manual","reconciled_at":"2026-09-09T00:00:00Z","source":"reconciliation","status":"done"}
implementation_readiness: {"checked_at":"2026-08-30T03:45:47Z","missing_sections":[],"status":"complete"}
---

# System task overlay: 仕様書と運用手順の追補
...
- parent_feature: feat-feedback-capture-self-exclusion
- phase_ref: P12
- classification: confidence=1.0; reason=feat-feedback-capture-self-exclusion の P12 lifecycle 責務への確定写像; candidate=tasks/feat-feedback-capture-self-exclusion/sys-fb-capture-exclusion-p12.md
...
写しが自分自身を含む場合の条件を既存の改善要望フィードバック仕様へ追補し、新しい浮遊要素を足すときに属性を付ける手順を運用として残した状態を成立させる。

## 背景

docs/spec/12-改善要望フィードバック仕様.md の FB-AC 群には写しの自己写り込みの条件が無い。本 feature が定めた条件をそこへ追補しないと、次の変更で同じ欠けが再発する。浮遊要素の属性付与も、手順として残さないと付け忘れが起きる。
...
- Produced artifacts: docs/spec/12-改善要望フィードバック仕様.md (写しの自己写り込みに関する受入条件の追補); docs/spec/feat-feedback-capture-self-exclusion/operations.md (浮遊要素を追加するときの属性付与手順と検査の効かせ方)
...
- Write scope/touches: docs/spec/12-改善要望フィードバック仕様.md, docs/spec/feat-feedback-capture-self-exclusion/operations.md
...
- PR body contract: Closes #issue (beads issue) + dev-graph graph_node_id=SYS-FB-CAPTURE-EXCLUSION-P12; PR の宛先は dev ブランチとする
- Ownership boundary: system-dev-planner は intent のみを宣言し、dev-graph が tracker mutation と reconciliation を行う

## Branch and worktree execution

- Branch: assigned after dev-graph registration by C15 as devgraph/SYS-FB-CAPTURE-EXCLUSION-P12; system-dev-planner は事前割当を行わない
- Worktree lease: claim SYS-FB-CAPTURE-EXCLUSION-P12 before implementation; heartbeat/release は dev-graph worktree lease 契約に従う
...
- Required evidence: P12 の 成果物 section に記した produced artifacts のパス

## Inner goal-seek execution loop

- Methodology contract: `system-task-goal-seek/v1`
- Goal: 写しが自分自身を含む場合の条件を既存の改善要望フィードバック仕様へ追補し、新しい浮遊要素を足すときに属性を付ける手順を運用として残した状態を成立させる。
- Generic execution prompt: feat-feedback-capture-self-exclusion の goal (改善ボタンを押して撮られた写しに送信 UI が 1 画素も写らず、撮り直しでも同じ規則が効き、写しが撮れない環境でも送信 UI が待たずに開く) と本 task の 前提条件/成果物/write_scope/スコープ外 を渡し、実装手段は固定せず P12 の目的を満たす成果物を作らせる
...
- Dependencies: SYS-FB-CAPTURE-EXCLUSION-P11
