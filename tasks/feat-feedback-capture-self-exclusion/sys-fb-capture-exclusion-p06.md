---
graph_node_id: "SYS-FB-CAPTURE-EXCLUSION-P06"
artifact_kind: "task"
artifact_subtypes: []
project_id: "feature-package-feat-feedback-capture-self-exclusion"
domain: "quality"
tags: ["p06","feat-feedback-capture-self-exclusion"]
priority: null
start_date: null
target_date: null
iteration: null
title: "写り込み検査と回帰テストの実行と緑化"
owners: ["daishiman"]
created_at: "2026-08-30T12:31:04Z"
updated_at: "2026-08-30T13:41:28.779123Z"
status: "closed"
closed_at: "2026-08-30T09:26:47Z"
depends_on: ["SYS-FB-CAPTURE-EXCLUSION-P05"]
related_nodes: []
resource_scope: ["tests/ui","tests/e2e"]
purpose: "P04 で設計した検査を実装として書き下ろして実行し、写り込み・撮り直し・退避経路・復元の各条件が自動で緑になり、既存テストの失敗が 0 件である状態を成立させる。"
goal: "P04 で設計した検査を実装として書き下ろして実行し、写り込み・撮り直し・退避経路・復元の各条件が自動で緑になり、既存テストの失敗が 0 件である状態を成立させる。"
scope_in: ["Produced artifacts: tests/ui 配下の単体テスト (撮影と可視化の順序・退避経路・復元の振る舞い); tests/e2e 配下の E2E テスト (改善ボタンからの写しに送信 UI が含まれないこと・撮り直し・属性付与漏れの検出)","Consumed artifacts: features/feat-feedback-capture-self-exclusion.md, features/feat-feedback-capture-self-exclusion.context.json, system-spec/frontend.md, docs/spec/12-改善要望フィードバック仕様.md","Write scope/touches: tests/ui, tests/e2e"]
...
acceptance: ["Automated commands: `pnpm run test`","Automated commands: `pnpm run test:e2e`","Automated commands: `python3 .claude/plugins/system-dev-planner/scripts/validate-system-plan.py --repo-root . --feature-package feature-package/feat-feedback-capture-self-exclusion`","Required evidence: P06 のテスト実行結果と、失敗 0 件を示す実行ログ"]
...
phase_ref: "P06"
file_path: "tasks/feat-feedback-capture-self-exclusion/sys-fb-capture-exclusion-p06.md"
template_id: "task"
...
source_lineage: {"imported_at":"2026-08-30T12:31:04Z","origin_kind":"system-dev-planner","source_digest":"892cd5611c443a5f90b97047a9542489c44be963ff4d775c30b640e7aca40b7d","source_path":".dev-graph/published/generations/feature-package-feat-feedback-capture-self-exclusion/892cd5611c443a5f90b97047a9542489c44be963ff4d775c30b640e7aca40b7d/task-specs/phase-06-test-run.md","source_plugin":"system-dev-planner","source_version":"0.1.0"}
classification_confidence: 1.0
classification_reason: "feat-feedback-capture-self-exclusion の P06 lifecycle 責務への確定写像"
classification_candidates: [{"artifact_kind":"task","candidate_path":"tasks/feat-feedback-capture-self-exclusion/sys-fb-capture-exclusion-p06.md","confidence":1.0}]
issue_linkage: null
tracker_binding: "beads"
beads_linkage: {"bd_issue_id":"ah-0d2q.6","github_mirror":null,"linked_at":"2026-08-30T04:17:20Z","sync_state":"linked"}
github_publication: {"labels":[],"milestone":null,"mode":"local_only","project_aliases":[]}
github_project_linkages: []
pull_request_linkages: []
execution_contexts: []
completion_evidence: {"completed_at":"2026-08-30T09:26:47Z","evidence_refs":["beads:ah-0d2q.6"],"policy":"manual","reconciled_at":"2026-08-30T12:05:00Z","source":"reconciliation","status":"done"}
implementation_readiness: {"checked_at":"2026-08-30T03:45:47Z","missing_sections":[],"status":"complete"}
---

# System task overlay: 写り込み検査と回帰テストの実行と緑化
...
- parent_feature: feat-feedback-capture-self-exclusion
- phase_ref: P06
- classification: confidence=1.0; reason=feat-feedback-capture-self-exclusion の P06 lifecycle 責務への確定写像; candidate=tasks/feat-feedback-capture-self-exclusion/sys-fb-capture-exclusion-p06.md
...
P04 で設計した検査を実装として書き下ろして実行し、写り込み・撮り直し・退避経路・復元の各条件が自動で緑になり、既存テストの失敗が 0 件である状態を成立させる。

## 背景

受入の中心は「写しに送信 UI が含まれないこと」であり、これは人の目視でなく自動の検査で守る必要がある。P04 の設計に従って単体と E2E の検査を書き、既存の重なり監査 tests/e2e/app-routes.spec.ts と同じ data-floating-overlay の手掛かりを共有させる。
...
- Produced artifacts: tests/ui 配下の単体テスト (撮影と可視化の順序・退避経路・復元の振る舞い); tests/e2e 配下の E2E テスト (改善ボタンからの写しに送信 UI が含まれないこと・撮り直し・属性付与漏れの検出)
...
- Write scope/touches: tests/ui, tests/e2e
...
- PR body contract: Closes #issue (beads issue) + dev-graph graph_node_id=SYS-FB-CAPTURE-EXCLUSION-P06; PR の宛先は dev ブランチとする
- Ownership boundary: system-dev-planner は intent のみを宣言し、dev-graph が tracker mutation と reconciliation を行う

## Branch and worktree execution

- Branch: assigned after dev-graph registration by C15 as devgraph/SYS-FB-CAPTURE-EXCLUSION-P06; system-dev-planner は事前割当を行わない
- Worktree lease: claim SYS-FB-CAPTURE-EXCLUSION-P06 before implementation; heartbeat/release は dev-graph worktree lease 契約に従う
...
- Required evidence: P06 のテスト実行結果と、失敗 0 件を示す実行ログ

## Inner goal-seek execution loop

- Methodology contract: `system-task-goal-seek/v1`
- Goal: P04 で設計した検査を実装として書き下ろして実行し、写り込み・撮り直し・退避経路・復元の各条件が自動で緑になり、既存テストの失敗が 0 件である状態を成立させる。
- Generic execution prompt: feat-feedback-capture-self-exclusion の goal (改善ボタンを押して撮られた写しに送信 UI が 1 画素も写らず、撮り直しでも同じ規則が効き、写しが撮れない環境でも送信 UI が待たずに開く) と本 task の 前提条件/成果物/write_scope/スコープ外 を渡し、実装手段は固定せず P06 の目的を満たす成果物を作らせる
...
- Dependencies: SYS-FB-CAPTURE-EXCLUSION-P05
