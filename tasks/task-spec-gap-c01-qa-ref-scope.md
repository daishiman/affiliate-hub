---
graph_node_id: "task-spec-gap-c01-qa-ref-scope"
artifact_kind: "task"
artifact_subtypes: []
...
target_date: null
iteration: null
title: "qa_ref の差し替えで、確定の裏付け範囲が狭まったままになっている"
owners: ["daishiman"]
created_at: "2026-08-19T08:40:00Z"
updated_at: "2026-09-08T07:39:38.317672Z"
status: "draft"
depends_on: []
...
purpose: null
goal: null
mvp_alignment: {"background":"database / backend / frontend / infrastructure × web の qa_ref を差し替えた結果、確定を支える回答の範囲が狭くなった","mvp_fit":"enabling","purpose":"複数 qa_ref の参照、または回答本文の統合で裏付け範囲を復元する","rationale":"裏付けが狭いことは、確定が間違っていることを意味しない。だが狭いまま確定が積み上がると、どこまでが聞いた内容かが読めなくなる"}
scope_in: []
scope_out: []
acceptance: []
architecture_refs: []
...
phase_ref: null
file_path: "tasks/task-spec-gap-c01-qa-ref-scope.md"
template_id: "task"
...
classification_candidates: [{"artifact_kind":"task","candidate_path":"tasks/task-spec-gap-c01-qa-ref-scope.md","confidence":0.9}]
issue_linkage: null
tracker_binding: "beads"
beads_linkage: {"bd_issue_id":"ah-nq1","github_mirror":null,"linked_at":"2026-09-08T07:14:07Z","sync_state":"linked"}
...
複数 qa_ref の参照、または回答本文の統合で裏付け範囲を復元する

## 背景

database / backend / frontend / infrastructure × web の qa_ref を差し替えた結果、確定を支える回答の範囲が狭くなった

## 評価器の指摘（原文）

> [C01 run-system-spec-elicit へ差し戻し / low] database / backend / frontend / infrastructure × web の qa_ref 差し替えで狭まった確定の裏付け範囲を、複数 qa_ref 参照または回答本文の統合で復元する。

出典は `system-spec/completeness-report.json` の `gaps[7]`。
**この文は完全性評価器（`assign-system-spec-completeness-evaluator`、`context: fork`）が
書いたもので、outer session は 1 文字も書いていない。**

## なぜこれを落とすのか

裏付けが狭いことは、確定が間違っていることを意味しない。だが狭いまま確定が積み上がると、どこまでが聞いた内容かが読めなくなる

## 入力と前提条件

`system-spec/completeness-report.json` の `gaps[7]`。
先行する課題は無い。
...
`system-spec/completeness-report.json` `gaps[7]`、`docs/product/backlog.md` 項目 90・91
