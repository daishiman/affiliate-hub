---
graph_node_id: "SYS-SITE-SCOPED-AUTHORING-IA-P08"
artifact_kind: "task"
artifact_subtypes: []
project_id: "feature-package-feat-site-scoped-authoring-ia"
domain: "frontend"
tags: ["p08","feat-site-scoped-authoring-ia"]
priority: null
start_date: null
target_date: null
iteration: null
title: "旧入口の重複実装解消と転送シェルへの一本化"
owners: ["daishiman"]
created_at: "2026-09-08T04:15:58Z"
updated_at: "2026-09-09T00:00:00Z"
status: "active"
depends_on: ["SYS-SITE-SCOPED-AUTHORING-IA-P05"]
related_nodes: []
resource_scope: ["src/app/admin/personas/","src/app/admin/writing/","src/app/admin/content/","docs/spec/feat-site-scoped-authoring-ia/migration-report.md"]
purpose: "P05で移設した読者像・書き方の決め事の旧実装 (/admin/personas・/admin/writing 配下の非転送コード) を撤去して転送シェルへ一本化し、/admin/content 配下の重複実装が0件であることを機械検査で確認した状態を成立させる。"
goal: "P05で移設した読者像・書き方の決め事の旧実装 (/admin/personas・/admin/writing 配下の非転送コード) を撤去して転送シェルへ一本化し、/admin/content 配下の重複実装が0件であることを機械検査で確認した状態を成立させる。"
scope_in: ["Produced artifacts: src/app/admin/personas/ 配下 (転送シェルのみへの一本化); src/app/admin/writing/ 配下 (転送シェルのみへの一本化); src/app/admin/content/ 配下 (転送シェルのみへの一本化); docs/spec/feat-site-scoped-authoring-ia/migration-report.md","Consumed artifacts: docs/spec/feat-site-scoped-authoring-ia/acceptance-report.md","Write scope/touches: src/app/admin/personas/, src/app/admin/writing/, src/app/admin/content/, docs/spec/feat-site-scoped-authoring-ia/migration-report.md"]
scope_out: ["新しい指標や集計を作ること (指標の正本は feat-blog-metrics-rollup)","/admin/sites/[site]/ 配下の各画面そのものの新設 (feat-blog-scoped-admin-console)","記事画面そのものの新設・配置 (feat-blog-scoped-admin-console が正本。本 feature が記事について担うのは旧 /admin/content/* からの転送だけ)","ブログの作成・削除そのもの (feat-blog-ops-crud)","権限モデルの新設 (既存 workspace 権限を使う)","管理画面の UX 規則そのものの策定 (1 画面 1 目的・日本語の動詞ラベル・進行開示・直接編集・危険操作の分離・次にすべきことの明示) — feat-reference-blog-admin-ux が正本","既存指標の提示順序そのものの規則の策定 (feat-blog-scoped-admin-console が正本)","write_scope外のパスへの変更"]
acceptance: ["Automated commands: `pnpm run typecheck`","Automated commands: `pnpm run lint`","Automated commands: `python3 .claude/plugins/system-dev-planner/scripts/validate-system-plan.py --repo-root . --feature-package feature-package/feat-site-scoped-authoring-ia` (本 package の C12 決定論検証を世代非依存に再実行する)","Required evidence: P08 の 成果物 section に記載した produced artifacts のパス"]
...
phase_ref: "P08"
file_path: "tasks/feat-site-scoped-authoring-ia/sys-site-scoped-authoring-ia-p08.md"
template_id: "task"
...
classification_reason: "feat-site-scoped-authoring-ia の P08 lifecycle 責務への確定写像"
classification_candidates: [{"artifact_kind":"task","candidate_path":"tasks/feat-site-scoped-authoring-ia/sys-site-scoped-authoring-ia-p08.md","confidence":1.0}]
issue_linkage: null
tracker_binding: "beads"
beads_linkage: {"bd_issue_id":"ah-ro3w.8","github_mirror":null,"linked_at":"2026-09-08T05:32:41Z","sync_state":"linked"}
github_publication: {"labels":[],"milestone":null,"mode":"local_only","project_aliases":[]}
github_project_linkages: []
pull_request_linkages: []
execution_contexts: []
completion_evidence: {"completed_at":"2026-09-08T11:31:23Z","evidence_refs":["beads:ah-ro3w.8"],"policy":"manual","reconciled_at":"2026-09-09T00:00:00Z","source":"reconciliation","status":"done"}
implementation_readiness: {"checked_at":"2026-09-08T00:00:00Z","missing_sections":[],"status":"complete"}
---

# System task overlay: 旧入口の重複実装解消と転送シェルへの一本化
...
- parent_feature: feat-site-scoped-authoring-ia
- phase_ref: P08
- classification: confidence=1.0; reason=feat-site-scoped-authoring-ia の P08 lifecycle 責務への確定写像; candidate=tasks/feat-site-scoped-authoring-ia/sys-site-scoped-authoring-ia-p08.md
...
P05で移設した読者像・書き方の決め事の旧実装 (/admin/personas・/admin/writing 配下の非転送コード) を撤去して転送シェルへ一本化し、/admin/content 配下の重複実装が0件であることを機械検査で確認した状態を成立させる。
...
- P08 upstream entry gate: SYS-SITE-SCOPED-AUTHORING-IA-P05 の implementation_readiness=complete
...
- Frontend: applicable; 旧実装の撤去と転送シェルへの一本化を行う
...
- Security: N/A: 権限チェックの変更を伴わない
- Quality: applicable; 重複実装0件の機械検査を完了条件とする
- Documentation: applicable; 移行報告そのものが本 phase の成果物である
...
- Produced artifacts: src/app/admin/personas/ 配下 (転送シェルのみへの一本化); src/app/admin/writing/ 配下 (転送シェルのみへの一本化); src/app/admin/content/ 配下 (転送シェルのみへの一本化); docs/spec/feat-site-scoped-authoring-ia/migration-report.md
- Consumed artifacts: docs/spec/feat-site-scoped-authoring-ia/acceptance-report.md
- Write scope/touches: src/app/admin/personas/, src/app/admin/writing/, src/app/admin/content/, docs/spec/feat-site-scoped-authoring-ia/migration-report.md
...
- PR body contract: Closes #issue (beads issue) + dev-graph graph_node_id=SYS-SITE-SCOPED-AUTHORING-IA-P08; PR の宛先は dev ブランチとする
- Ownership boundary: system-dev-plannerはintentのみを宣言し、dev-graphがtracker mutationとreconciliationを行う

## Branch and worktree execution

- Branch: assigned after dev-graph registration by C15 as devgraph/SYS-SITE-SCOPED-AUTHORING-IA-P08; system-dev-plannerは事前割当を行わない
- Worktree lease: claim SYS-SITE-SCOPED-AUTHORING-IA-P08 before implementation; heartbeat/release は dev-graph worktree lease 契約に従う
...
- Goal: P05で移設した読者像・書き方の決め事の旧実装 (/admin/personas・/admin/writing 配下の非転送コード) を撤去して転送シェルへ一本化し、/admin/content 配下の重複実装が0件であることを機械検査で確認した状態を成立させる。
- Generic execution prompt: feat-site-scoped-authoring-ia の goal (読者像と書き方の決め事が /admin/sites/[site]/ 配下に属し、93 本ある管理ルートが作業の対象物 (ブログ・記事・読者・商品・配信) の数まで畳まれ、各入口の名前から何ができるかが分かり、/admin/content/* と /admin/personas/* と /admin/writing/* が転送で受けられている状態になっている) と本 task の 前提条件/成果物/write_scope/スコープ外 を渡し、実装手段は固定せず P08 の目的を満たす成果物を作らせる
...
- Dependencies: SYS-SITE-SCOPED-AUTHORING-IA-P05
