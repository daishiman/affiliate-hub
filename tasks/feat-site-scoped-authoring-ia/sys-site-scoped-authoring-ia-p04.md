---
graph_node_id: "SYS-SITE-SCOPED-AUTHORING-IA-P04"
artifact_kind: "task"
artifact_subtypes: []
project_id: "feature-package-feat-site-scoped-authoring-ia"
domain: "quality"
tags: ["p04","feat-site-scoped-authoring-ia"]
priority: null
start_date: null
target_date: null
iteration: null
title: "受入10件に対応するテスト設計"
owners: ["daishiman"]
created_at: "2026-09-08T04:15:58Z"
updated_at: "2026-09-09T00:00:00Z"
status: "active"
depends_on: ["SYS-SITE-SCOPED-AUTHORING-IA-P03"]
related_nodes: []
resource_scope: ["docs/spec/feat-site-scoped-authoring-ia/test-design.md","tests/ui/","tests/application/","tests/acceptance/"]
purpose: "受入10件 (A1-A10) それぞれに対応するテストを振る舞いベースで設計し、実装前に失敗する状態でtests/ 配下に配置した状態を成立させる。"
goal: "受入10件 (A1-A10) それぞれに対応するテストを振る舞いベースで設計し、実装前に失敗する状態でtests/ 配下に配置した状態を成立させる。"
scope_in: ["Produced artifacts: docs/spec/feat-site-scoped-authoring-ia/test-design.md; tests/ui/; tests/application/; tests/acceptance/","Consumed artifacts: docs/spec/feat-site-scoped-authoring-ia/redirect-contract.md, docs/spec/feat-site-scoped-authoring-ia/site-scoped-route-contract.md, docs/spec/feat-site-scoped-authoring-ia/entry-consolidation-contract.md, docs/spec/feat-site-scoped-authoring-ia/shortcut-contract.md, docs/spec/feat-site-scoped-authoring-ia/template-clone-contract.md, docs/spec/feat-site-scoped-authoring-ia/design-review.md","Write scope/touches: docs/spec/feat-site-scoped-authoring-ia/test-design.md, tests/ui/, tests/application/, tests/acceptance/"]
scope_out: ["新しい指標や集計を作ること (指標の正本は feat-blog-metrics-rollup)","/admin/sites/[site]/ 配下の各画面そのものの新設 (feat-blog-scoped-admin-console)","記事画面そのものの新設・配置 (feat-blog-scoped-admin-console が正本。本 feature が記事について担うのは旧 /admin/content/* からの転送だけ)","ブログの作成・削除そのもの (feat-blog-ops-crud)","権限モデルの新設 (既存 workspace 権限を使う)","管理画面の UX 規則そのものの策定 (1 画面 1 目的・日本語の動詞ラベル・進行開示・直接編集・危険操作の分離・次にすべきことの明示) — feat-reference-blog-admin-ux が正本","既存指標の提示順序そのものの規則の策定 (feat-blog-scoped-admin-console が正本)"]
acceptance: ["Automated commands: `pnpm run typecheck`","Automated commands: `python3 .claude/plugins/system-dev-planner/scripts/validate-system-plan.py --repo-root . --feature-package feature-package/feat-site-scoped-authoring-ia` (本 package の C12 決定論検証を世代非依存に再実行する)","Required evidence: P04 の 成果物 section に記載した produced artifacts のパス"]
...
phase_ref: "P04"
file_path: "tasks/feat-site-scoped-authoring-ia/sys-site-scoped-authoring-ia-p04.md"
template_id: "task"
...
classification_reason: "feat-site-scoped-authoring-ia の P04 lifecycle 責務への確定写像"
classification_candidates: [{"artifact_kind":"task","candidate_path":"tasks/feat-site-scoped-authoring-ia/sys-site-scoped-authoring-ia-p04.md","confidence":1.0}]
issue_linkage: null
tracker_binding: "beads"
beads_linkage: {"bd_issue_id":"ah-ro3w.4","github_mirror":null,"linked_at":"2026-09-08T05:32:32Z","sync_state":"linked"}
github_publication: {"labels":[],"milestone":null,"mode":"local_only","project_aliases":[]}
github_project_linkages: []
pull_request_linkages: []
execution_contexts: []
completion_evidence: {"completed_at":"2026-09-08T09:39:45Z","evidence_refs":["beads:ah-ro3w.4"],"policy":"manual","reconciled_at":"2026-09-09T00:00:00Z","source":"reconciliation","status":"done"}
...
- parent_feature: feat-site-scoped-authoring-ia
- phase_ref: P04
- classification: confidence=1.0; reason=feat-site-scoped-authoring-ia の P04 lifecycle 責務への確定写像; candidate=tasks/feat-site-scoped-authoring-ia/sys-site-scoped-authoring-ia-p04.md
...
受入10件 (A1-A10) それぞれに対応するテストを振る舞いベースで設計し、実装前に失敗する状態でtests/ 配下に配置した状態を成立させる。
...
- P04 upstream entry gate: SYS-SITE-SCOPED-AUTHORING-IA-P03 の implementation_readiness=complete
...
- Frontend: applicable; 転送遷移・notFound遷移・入口ラベル表示・近道導線・雛形複製導線のテストを設計する
- Backend: N/A: 本 feature に新規バックエンドロジックはない
- API: N/A: 本 feature は新規APIを持たない
- Data: applicable; 既存 site_daily_metrics / article_daily_metrics 読み出しが新規集計を追加しないことを検証するテストを設計する
- Infrastructure: N/A: 既存 cloudflare-workers-opennext-app デプロイ単位を変更しない
- Security: N/A: 権限テストは既存 admin RBAC のテスト資産を再利用し新規設計を要しない
- Quality: applicable; 受入10件それぞれに対応するテストの実装前失敗状態を完了条件とする
...
- Produced artifacts: docs/spec/feat-site-scoped-authoring-ia/test-design.md; tests/ui/; tests/application/; tests/acceptance/
- Consumed artifacts: docs/spec/feat-site-scoped-authoring-ia/redirect-contract.md, docs/spec/feat-site-scoped-authoring-ia/site-scoped-route-contract.md, docs/spec/feat-site-scoped-authoring-ia/entry-consolidation-contract.md, docs/spec/feat-site-scoped-authoring-ia/shortcut-contract.md, docs/spec/feat-site-scoped-authoring-ia/template-clone-contract.md, docs/spec/feat-site-scoped-authoring-ia/design-review.md
- Write scope/touches: docs/spec/feat-site-scoped-authoring-ia/test-design.md, tests/ui/, tests/application/, tests/acceptance/
...
- PR body contract: Closes #issue (beads issue) + dev-graph graph_node_id=SYS-SITE-SCOPED-AUTHORING-IA-P04; PR の宛先は dev ブランチとする
- Ownership boundary: system-dev-plannerはintentのみを宣言し、dev-graphがtracker mutationとreconciliationを行う

## Branch and worktree execution

- Branch: assigned after dev-graph registration by C15 as devgraph/SYS-SITE-SCOPED-AUTHORING-IA-P04; system-dev-plannerは事前割当を行わない
- Worktree lease: claim SYS-SITE-SCOPED-AUTHORING-IA-P04 before implementation; heartbeat/release は dev-graph worktree lease 契約に従う
...
- Goal: 受入10件 (A1-A10) それぞれに対応するテストを振る舞いベースで設計し、実装前に失敗する状態でtests/ 配下に配置した状態を成立させる。
- Generic execution prompt: feat-site-scoped-authoring-ia の goal (読者像と書き方の決め事が /admin/sites/[site]/ 配下に属し、93 本ある管理ルートが作業の対象物 (ブログ・記事・読者・商品・配信) の数まで畳まれ、各入口の名前から何ができるかが分かり、/admin/content/* と /admin/personas/* と /admin/writing/* が転送で受けられている状態になっている) と本 task の 前提条件/成果物/write_scope/スコープ外 を渡し、実装手段は固定せず P04 の目的を満たす成果物を作らせる
...
- Dependencies: SYS-SITE-SCOPED-AUTHORING-IA-P03
