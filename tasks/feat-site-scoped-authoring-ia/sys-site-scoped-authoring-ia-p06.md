---
graph_node_id: "SYS-SITE-SCOPED-AUTHORING-IA-P06"
artifact_kind: "task"
artifact_subtypes: []
project_id: "feature-package-feat-site-scoped-authoring-ia"
domain: "quality"
tags: ["p06","feat-site-scoped-authoring-ia"]
priority: null
start_date: null
target_date: null
iteration: null
...
related_nodes: []
resource_scope: ["docs/spec/feat-site-scoped-authoring-ia/test-run-report.md"]
purpose: "`pnpm test` と `pnpm run test:coverage` の全量実行を完了し、失敗0件・既存テストの回帰0件・新規実装コードのカバレッジ80%以上を確認した状態を成立させる。"
goal: "`pnpm test` と `pnpm run test:coverage` の全量実行を完了し、失敗0件・既存テストの回帰0件・新規実装コードのカバレッジ80%以上を確認した状態を成立させる。"
scope_in: ["Produced artifacts: docs/spec/feat-site-scoped-authoring-ia/test-run-report.md","Consumed artifacts: docs/spec/feat-site-scoped-authoring-ia/test-design.md","Write scope/touches: docs/spec/feat-site-scoped-authoring-ia/test-run-report.md"]
...
acceptance: ["Automated commands: `pnpm test`","Automated commands: `pnpm run test:coverage`","Automated commands: `python3 .claude/plugins/system-dev-planner/scripts/validate-system-plan.py --repo-root . --feature-package feature-package/feat-site-scoped-authoring-ia` (本 package の C12 決定論検証を世代非依存に再実行する)","Required evidence: P06 の 成果物 section に記載した produced artifacts のパス"]
...
phase_ref: "P06"
file_path: "tasks/feat-site-scoped-authoring-ia/sys-site-scoped-authoring-ia-p06.md"
template_id: "task"
...
classification_reason: "feat-site-scoped-authoring-ia の P06 lifecycle 責務への確定写像"
classification_candidates: [{"artifact_kind":"task","candidate_path":"tasks/feat-site-scoped-authoring-ia/sys-site-scoped-authoring-ia-p06.md","confidence":1.0}]
issue_linkage: null
tracker_binding: "beads"
beads_linkage: {"bd_issue_id":"ah-ro3w.6","github_mirror":null,"linked_at":"2026-09-08T05:32:37Z","sync_state":"linked"}
github_publication: {"labels":[],"milestone":null,"mode":"local_only","project_aliases":[]}
github_project_linkages: []
pull_request_linkages: []
execution_contexts: []
completion_evidence: {"completed_at":"2026-09-08T11:19:25Z","evidence_refs":["beads:ah-ro3w.6"],"policy":"manual","reconciled_at":"2026-09-09T00:00:00Z","source":"reconciliation","status":"done"}
...
- parent_feature: feat-site-scoped-authoring-ia
- phase_ref: P06
- classification: confidence=1.0; reason=feat-site-scoped-authoring-ia の P06 lifecycle 責務への確定写像; candidate=tasks/feat-site-scoped-authoring-ia/sys-site-scoped-authoring-ia-p06.md
...
`pnpm test` と `pnpm run test:coverage` の全量実行を完了し、失敗0件・既存テストの回帰0件・新規実装コードのカバレッジ80%以上を確認した状態を成立させる。
...
- P06 upstream entry gate: SYS-SITE-SCOPED-AUTHORING-IA-P05 の implementation_readiness=complete
...
- Frontend: applicable; P05実装の全テストスイート実行結果を確認する
...
- Security: N/A: 権限テストは既存 admin RBAC のテスト資産を再利用する
- Quality: applicable; 全量実行の失敗0件・回帰0件・カバレッジ80%以上を完了条件とする
- Documentation: applicable; テスト実行結果報告そのものが本 phase の成果物である
...
- Produced artifacts: docs/spec/feat-site-scoped-authoring-ia/test-run-report.md
- Consumed artifacts: docs/spec/feat-site-scoped-authoring-ia/test-design.md
- Write scope/touches: docs/spec/feat-site-scoped-authoring-ia/test-run-report.md
...
- PR body contract: Closes #issue (beads issue) + dev-graph graph_node_id=SYS-SITE-SCOPED-AUTHORING-IA-P06; PR の宛先は dev ブランチとする
- Ownership boundary: system-dev-plannerはintentのみを宣言し、dev-graphがtracker mutationとreconciliationを行う

## Branch and worktree execution

- Branch: assigned after dev-graph registration by C15 as devgraph/SYS-SITE-SCOPED-AUTHORING-IA-P06; system-dev-plannerは事前割当を行わない
- Worktree lease: claim SYS-SITE-SCOPED-AUTHORING-IA-P06 before implementation; heartbeat/release は dev-graph worktree lease 契約に従う
...
- Automated commands: `pnpm run test:coverage`
...
- Goal: `pnpm test` と `pnpm run test:coverage` の全量実行を完了し、失敗0件・既存テストの回帰0件・新規実装コードのカバレッジ80%以上を確認した状態を成立させる。
- Generic execution prompt: feat-site-scoped-authoring-ia の goal (読者像と書き方の決め事が /admin/sites/[site]/ 配下に属し、93 本ある管理ルートが作業の対象物 (ブログ・記事・読者・商品・配信) の数まで畳まれ、各入口の名前から何ができるかが分かり、/admin/content/* と /admin/personas/* と /admin/writing/* が転送で受けられている状態になっている) と本 task の 前提条件/成果物/write_scope/スコープ外 を渡し、実装手段は固定せず P06 の目的を満たす成果物を作らせる
