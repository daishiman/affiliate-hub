---
graph_node_id: "SYS-AEO-ANSWER-OPTIMIZATION-P04"
artifact_kind: "task"
artifact_subtypes: []
project_id: "feature-package-feat-aeo-answer-optimization"
domain: "quality"
tags: ["p04","feat-aeo-answer-optimization"]
priority: null
start_date: null
target_date: null
iteration: null
...
status: "closed"
depends_on: ["SYS-AEO-ANSWER-OPTIMIZATION-P03"]
related_nodes: []
resource_scope: ["docs/spec/feat-aeo-answer-optimization/test-plan.md","docs/spec/feat-aeo-answer-optimization/test-cases.md"]
purpose: "受入9件を自動テストへ写像し、回答単位の欠落が機械的に検出されること、FAQPage が妥当性検証を通ること、llms.txt が方針を反映すること、既定でクローラが許可され拒否設定時だけ拒否になること、著者プロフィールが Person 構造化データへ紐付くこと、引用観測台帳が記録契機ごとに行を残すこと、新設配信路が増えないことを、実行可能なケース表として確定した状態を成立させる。"
goal: "受入9件を自動テストへ写像し、回答単位の欠落が機械的に検出されること、FAQPage が妥当性検証を通ること、llms.txt が方針を反映すること、既定でクローラが許可され拒否設定時だけ拒否になること、著者プロフィールが Person 構造化データへ紐付くこと、引用観測台帳が記録契機ごとに行を残すこと、新設配信路が増えないことを、実行可能なケース表として確定した状態を成立させる。"
scope_in: ["Produced artifacts: docs/spec/feat-aeo-answer-optimization/test-plan.md (受入9件とテストケースの一意対応表・著者 Person 紐付けと引用観測台帳を含む); docs/spec/feat-aeo-answer-optimization/test-cases.md (単体・結合・境界値・回帰のケース一覧)","Consumed artifacts: features/feat-aeo-answer-optimization.md; features/feat-aeo-answer-optimization.context.json; system-spec/frontend.md; system-spec/database.md; docs/spec/feat-seo-assessment-reflection/validation-design.md","Write scope/touches: docs/spec/feat-aeo-answer-optimization/test-plan.md, docs/spec/feat-aeo-answer-optimization/test-cases.md"]
scope_out: ["特定の AI 検索サービスでの露出保証や順位保証 (作らない)","索引可能性・sitemap・一般的な構造化データ検査 (feat-seo-assessment-reflection が所有する)","回答単位の本文を自動生成して無承認で公開すること (行わない)","残数・優先度の提示順序 (feat-blog-scoped-admin-console が所有する)","P04 以外の phase が所有する成果物への変更"]
...
phase_ref: "P04"
file_path: "tasks/feat-aeo-answer-optimization/sys-aeo-answer-optimization-p04.md"
template_id: "task"
...
classification_reason: "feat-aeo-answer-optimization の P04 lifecycle 責務への確定写像"
classification_candidates: [{"artifact_kind":"task","candidate_path":"tasks/feat-aeo-answer-optimization/sys-aeo-answer-optimization-p04.md","confidence":1.0}]
issue_linkage: null
tracker_binding: "beads"
beads_linkage: {"bd_issue_id":"ah-4d0q.4","github_mirror":null,"linked_at":"2026-09-04T02:09:26Z","sync_state":"linked"}
github_publication: {"labels":[],"milestone":null,"mode":"local_only","project_aliases":[]}
github_project_linkages: []
pull_request_linkages: []
execution_contexts: []
completion_evidence: {"completed_at":"2026-09-04T11:01:18Z","evidence_refs":["beads:ah-4d0q.4"],"policy":"manual","reconciled_at":"2026-09-09T00:00:00Z","source":"reconciliation","status":"done"}
implementation_readiness: {"checked_at":"2026-09-04T00:00:00Z","missing_sections":[],"status":"complete"}
---

# System task overlay: AEO の受入テスト設計
...
- parent_feature: feat-aeo-answer-optimization
- phase_ref: P04
- classification: confidence=1.0; reason=feat-aeo-answer-optimization の P04 lifecycle 責務への確定写像; candidate=tasks/feat-aeo-answer-optimization/sys-aeo-answer-optimization-p04.md
...
受入9件を自動テストへ写像し、回答単位の欠落が機械的に検出されること、FAQPage が妥当性検証を通ること、llms.txt が方針を反映すること、既定でクローラが許可され拒否設定時だけ拒否になること、著者プロフィールが Person 構造化データへ紐付くこと、引用観測台帳が記録契機ごとに行を残すこと、新設配信路が増えないことを、実行可能なケース表として確定した状態を成立させる。

## 背景

既定が許可であることは、設定を何もしていない状態のテストとして書かないと確かめられない。既定値のテストは書き忘れやすいため、テスト設計で明示的にケース化する。
...
- Produced artifacts: docs/spec/feat-aeo-answer-optimization/test-plan.md (受入9件とテストケースの一意対応表・著者 Person 紐付けと引用観測台帳を含む); docs/spec/feat-aeo-answer-optimization/test-cases.md (単体・結合・境界値・回帰のケース一覧)
...
- Write scope/touches: docs/spec/feat-aeo-answer-optimization/test-plan.md, docs/spec/feat-aeo-answer-optimization/test-cases.md
...
- PR body contract: Closes #issue (beads issue) + dev-graph graph_node_id=SYS-AEO-ANSWER-OPTIMIZATION-P04; PR の宛先は dev ブランチとする
- Ownership boundary: system-dev-planner は intent のみを宣言し、dev-graph が tracker mutation と reconciliation を行う

## Branch and worktree execution

- Branch: assigned after dev-graph registration by C15 as devgraph/SYS-AEO-ANSWER-OPTIMIZATION-P04; system-dev-planner は事前割当を行わない
- Worktree lease: claim SYS-AEO-ANSWER-OPTIMIZATION-P04 before implementation; heartbeat/release は dev-graph worktree lease 契約に従う
...
- テストレベル選定: 単体: 回答単位の欠落検出と構造化データ生成。結合: 記事から公開面・llms.txt・robots.txt までの配信。境界値: 回答単位が一部だけ存在・FAQ 不在・拒否設定あり/なし・著者未設定 (Person を出力せず欠落として検出する)・引用観測が0件。回帰: 既存 tests 配下を0件失敗のまま維持する。
...
- Goal: 受入9件を自動テストへ写像し、回答単位の欠落が機械的に検出されること、FAQPage が妥当性検証を通ること、llms.txt が方針を反映すること、既定でクローラが許可され拒否設定時だけ拒否になること、著者プロフィールが Person 構造化データへ紐付くこと、引用観測台帳が記録契機ごとに行を残すこと、新設配信路が増えないことを、実行可能なケース表として確定した状態を成立させる。
- Generic execution prompt: feat-aeo-answer-optimization の goal (記事が結論・要点・比較表・FAQ・出典・最終更新日という回答単位を持ち、article_answer_units と site_aeo_profiles として管理され、llms.txt とブログごとのクローラ方針が配信され、AI 検索での引用状況を定点で記録できる状態になっている) と本 task の 前提条件/成果物/write scope/スコープ外 を渡し、実装手段は固定せず P04 の目的を満たす成果物を作らせる
...
- Dependencies: SYS-AEO-ANSWER-OPTIMIZATION-P03
