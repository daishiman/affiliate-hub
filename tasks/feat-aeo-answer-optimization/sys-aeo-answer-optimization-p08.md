---
graph_node_id: "SYS-AEO-ANSWER-OPTIMIZATION-P08"
artifact_kind: "task"
artifact_subtypes: []
project_id: "feature-package-feat-aeo-answer-optimization"
domain: "backend"
tags: ["p08","feat-aeo-answer-optimization"]
priority: null
start_date: null
target_date: null
iteration: null
...
purpose: "構造化データの生成と妥当性検証が SEO feature と本 feature で重複している箇所を洗い出し、生成と検証の責務が一箇所ずつに定まった状態を成立させる。"
goal: "構造化データの生成と妥当性検証が SEO feature と本 feature で重複している箇所を洗い出し、生成と検証の責務が一箇所ずつに定まった状態を成立させる。"
scope_in: ["Produced artifacts: docs/spec/feat-aeo-answer-optimization/migration-notes.md (生成と検証の責務境界と重複解消の内容); src/domain/aeo/ と src/domain/seo/ の境界整理","Consumed artifacts: features/feat-aeo-answer-optimization.md; features/feat-aeo-answer-optimization.context.json; system-spec/frontend.md; system-spec/database.md; docs/spec/feat-seo-assessment-reflection/validation-design.md","Write scope/touches: src/domain/aeo/, src/application/aeo/, docs/spec/feat-aeo-answer-optimization/migration-notes.md"]
scope_out: ["特定の AI 検索サービスでの露出保証や順位保証 (作らない)","索引可能性・sitemap・一般的な構造化データ検査 (feat-seo-assessment-reflection が所有する)","回答単位の本文を自動生成して無承認で公開すること (行わない)","残数・優先度の提示順序 (feat-blog-scoped-admin-console が所有する)","P08 以外の phase が所有する成果物への変更"]
...
phase_ref: "P08"
file_path: "tasks/feat-aeo-answer-optimization/sys-aeo-answer-optimization-p08.md"
template_id: "task"
...
classification_reason: "feat-aeo-answer-optimization の P08 lifecycle 責務への確定写像"
classification_candidates: [{"artifact_kind":"task","candidate_path":"tasks/feat-aeo-answer-optimization/sys-aeo-answer-optimization-p08.md","confidence":1.0}]
issue_linkage: null
tracker_binding: "beads"
beads_linkage: {"bd_issue_id":"ah-4d0q.8","github_mirror":null,"linked_at":"2026-09-04T02:09:35Z","sync_state":"linked"}
github_publication: {"labels":[],"milestone":null,"mode":"local_only","project_aliases":[]}
github_project_linkages: []
pull_request_linkages: []
execution_contexts: []
completion_evidence: {"completed_at":"2026-09-04T11:01:27Z","evidence_refs":["beads:ah-4d0q.8"],"policy":"manual","reconciled_at":"2026-09-09T00:00:00Z","source":"reconciliation","status":"done"}
implementation_readiness: {"checked_at":"2026-09-04T00:00:00Z","missing_sections":[],"status":"complete"}
---

# System task overlay: SEO 側構造化データ経路との重複解消と移行
...
- parent_feature: feat-aeo-answer-optimization
- phase_ref: P08
- classification: confidence=1.0; reason=feat-aeo-answer-optimization の P08 lifecycle 責務への確定写像; candidate=tasks/feat-aeo-answer-optimization/sys-aeo-answer-optimization-p08.md
...
構造化データの生成と妥当性検証が SEO feature と本 feature で重複している箇所を洗い出し、生成と検証の責務が一箇所ずつに定まった状態を成立させる。

## 背景

構造化データは両 feature が触る唯一の共有面である。検証関数を二つ持つと判定が食い違い、片方の画面だけで指摘が出るという状態になる。
...
- Compatibility/migration/backfill: 妥当性検証の実装は SEO 側を正本とし、本 feature からは参照のみとする

## 成果物

- Produced artifacts: docs/spec/feat-aeo-answer-optimization/migration-notes.md (生成と検証の責務境界と重複解消の内容); src/domain/aeo/ と src/domain/seo/ の境界整理
...
- Write scope/touches: src/domain/aeo/, src/application/aeo/, docs/spec/feat-aeo-answer-optimization/migration-notes.md
...
- PR body contract: Closes #issue (beads issue) + dev-graph graph_node_id=SYS-AEO-ANSWER-OPTIMIZATION-P08; PR の宛先は dev ブランチとする
- Ownership boundary: system-dev-planner は intent のみを宣言し、dev-graph が tracker mutation と reconciliation を行う

## Branch and worktree execution

- Branch: assigned after dev-graph registration by C15 as devgraph/SYS-AEO-ANSWER-OPTIMIZATION-P08; system-dev-planner は事前割当を行わない
- Worktree lease: claim SYS-AEO-ANSWER-OPTIMIZATION-P08 before implementation; heartbeat/release は dev-graph worktree lease 契約に従う
...
- Goal: 構造化データの生成と妥当性検証が SEO feature と本 feature で重複している箇所を洗い出し、生成と検証の責務が一箇所ずつに定まった状態を成立させる。
- Generic execution prompt: feat-aeo-answer-optimization の goal (記事が結論・要点・比較表・FAQ・出典・最終更新日という回答単位を持ち、article_answer_units と site_aeo_profiles として管理され、llms.txt とブログごとのクローラ方針が配信され、AI 検索での引用状況を定点で記録できる状態になっている) と本 task の 前提条件/成果物/write scope/スコープ外 を渡し、実装手段は固定せず P08 の目的を満たす成果物を作らせる
...
- Ready when: 生成と検証の責務境界が一意に定まり、回帰0件である
...
- Dependencies: SYS-AEO-ANSWER-OPTIMIZATION-P05
