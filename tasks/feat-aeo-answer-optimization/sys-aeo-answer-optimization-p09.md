---
graph_node_id: "SYS-AEO-ANSWER-OPTIMIZATION-P09"
artifact_kind: "task"
artifact_subtypes: []
project_id: "feature-package-feat-aeo-answer-optimization"
domain: "quality"
tags: ["p09","feat-aeo-answer-optimization"]
priority: null
start_date: null
target_date: null
iteration: null
...
purpose: "回答単位ブロックの追加が読者面の描画性能へ与える影響、llms.txt と robots.txt の配信応答、方針設定の権限限定、および引用観測台帳の蓄積量を検査し、いずれも基準を満たす状態を成立させる。"
goal: "回答単位ブロックの追加が読者面の描画性能へ与える影響、llms.txt と robots.txt の配信応答、方針設定の権限限定、および引用観測台帳の蓄積量を検査し、いずれも基準を満たす状態を成立させる。"
scope_in: ["Produced artifacts: docs/spec/feat-aeo-answer-optimization/quality-report.md (描画性能・配信応答・権限・蓄積量の検査結果)","Consumed artifacts: features/feat-aeo-answer-optimization.md; features/feat-aeo-answer-optimization.context.json; system-spec/frontend.md; system-spec/database.md; docs/spec/feat-seo-assessment-reflection/validation-design.md","Write scope/touches: docs/spec/feat-aeo-answer-optimization/quality-report.md, tests/"]
scope_out: ["特定の AI 検索サービスでの露出保証や順位保証 (作らない)","索引可能性・sitemap・一般的な構造化データ検査 (feat-seo-assessment-reflection が所有する)","回答単位の本文を自動生成して無承認で公開すること (行わない)","残数・優先度の提示順序 (feat-blog-scoped-admin-console が所有する)","P09 以外の phase が所有する成果物への変更"]
acceptance: ["Automated commands: `pnpm vitest run` (権限拒否と配信のテストを再実行する)","Automated commands: `pnpm run build` (本番相当ビルドが通ることを確認する)","Required evidence: quality-report.md に回答単位有無での描画性能差が記録されていること","Required evidence: P09 の 成果物 section に記載した produced artifacts のパス"]
...
phase_ref: "P09"
file_path: "tasks/feat-aeo-answer-optimization/sys-aeo-answer-optimization-p09.md"
template_id: "task"
...
classification_reason: "feat-aeo-answer-optimization の P09 lifecycle 責務への確定写像"
classification_candidates: [{"artifact_kind":"task","candidate_path":"tasks/feat-aeo-answer-optimization/sys-aeo-answer-optimization-p09.md","confidence":1.0}]
issue_linkage: null
tracker_binding: "beads"
beads_linkage: {"bd_issue_id":"ah-4d0q.9","github_mirror":null,"linked_at":"2026-09-04T02:09:37Z","sync_state":"linked"}
github_publication: {"labels":[],"milestone":null,"mode":"local_only","project_aliases":[]}
github_project_linkages: []
pull_request_linkages: []
execution_contexts: []
completion_evidence: {"completed_at":"2026-09-04T11:01:29Z","evidence_refs":["beads:ah-4d0q.9"],"policy":"manual","reconciled_at":"2026-09-09T00:00:00Z","source":"reconciliation","status":"done"}
implementation_readiness: {"checked_at":"2026-09-04T00:00:00Z","missing_sections":[],"status":"complete"}
---

# System task overlay: AEO の非機能検査
...
- parent_feature: feat-aeo-answer-optimization
- phase_ref: P09
- classification: confidence=1.0; reason=feat-aeo-answer-optimization の P09 lifecycle 責務への確定写像; candidate=tasks/feat-aeo-answer-optimization/sys-aeo-answer-optimization-p09.md
...
回答単位ブロックの追加が読者面の描画性能へ与える影響、llms.txt と robots.txt の配信応答、方針設定の権限限定、および引用観測台帳の蓄積量を検査し、いずれも基準を満たす状態を成立させる。

## 背景

回答単位は記事本文に追加される要素であるため、描画量が増える。読者面の描画性能への影響を検査してから運用へ入る。
...
- Security: applicable; ブログ方針の変更が workspace 権限で守られることを検査する
...
- Produced artifacts: docs/spec/feat-aeo-answer-optimization/quality-report.md (描画性能・配信応答・権限・蓄積量の検査結果)
...
- Write scope/touches: docs/spec/feat-aeo-answer-optimization/quality-report.md, tests/
...
- PR body contract: Closes #issue (beads issue) + dev-graph graph_node_id=SYS-AEO-ANSWER-OPTIMIZATION-P09; PR の宛先は dev ブランチとする
- Ownership boundary: system-dev-planner は intent のみを宣言し、dev-graph が tracker mutation と reconciliation を行う

## Branch and worktree execution

- Branch: assigned after dev-graph registration by C15 as devgraph/SYS-AEO-ANSWER-OPTIMIZATION-P09; system-dev-planner は事前割当を行わない
- Worktree lease: claim SYS-AEO-ANSWER-OPTIMIZATION-P09 before implementation; heartbeat/release は dev-graph worktree lease 契約に従う
...
- テストレベル選定: 境界値: 回答単位が多い記事・拒否設定・権限不足からの方針変更要求・観測台帳の長期蓄積。回帰: 既存の読者面テストを維持する。 N/A: 単体・結合 の各テストレベルは、本 task の成果物が文書と判定であり実行可能なコードを含まないため適用しない。
...
- Automated commands: `pnpm vitest run` (権限拒否と配信のテストを再実行する)
- Automated commands: `pnpm run build` (本番相当ビルドが通ることを確認する)
- Required evidence: quality-report.md に回答単位有無での描画性能差が記録されていること
...
- Goal: 回答単位ブロックの追加が読者面の描画性能へ与える影響、llms.txt と robots.txt の配信応答、方針設定の権限限定、および引用観測台帳の蓄積量を検査し、いずれも基準を満たす状態を成立させる。
- Generic execution prompt: feat-aeo-answer-optimization の goal (記事が結論・要点・比較表・FAQ・出典・最終更新日という回答単位を持ち、article_answer_units と site_aeo_profiles として管理され、llms.txt とブログごとのクローラ方針が配信され、AI 検索での引用状況を定点で記録できる状態になっている) と本 task の 前提条件/成果物/write scope/スコープ外 を渡し、実装手段は固定せず P09 の目的を満たす成果物を作らせる
...
- Dependencies: SYS-AEO-ANSWER-OPTIMIZATION-P08
