---
graph_node_id: "SYS-SEO-AEO-MEASUREMENT-LOOP-P08"
artifact_kind: "task"
artifact_subtypes: []
project_id: "feature-package-feat-seo-aeo-measurement-loop"
domain: "data"
tags: ["p08","feat-seo-aeo-measurement-loop","seo-aeo"]
priority: null
start_date: null
target_date: null
iteration: null
title: "永続化スキーマの移行 (所見・反映ログ・系統別最終収集時刻)"
owners: ["daishiman"]
created_at: "2026-09-04T05:19:32Z"
updated_at: "2026-09-10T09:00:00Z"
status: "active"
depends_on: ["SYS-SEO-AEO-MEASUREMENT-LOOP-P05"]
related_nodes: ["arch-system-spec-overview","arch-two-layer-platform"]
resource_scope: ["docs/spec/feat-seo-aeo-measurement-loop/refactoring-migration.md","drizzle/"]
purpose: "3データ源の永続化スキーマに対する移行が必要な場合はdry-runとrollback証跡を伴って実施し、不要な場合はN/A理由を記録する。"
goal: "3データ源の永続化スキーマに対する移行が必要な場合はdry-runとrollback証跡を伴って実施し、不要な場合はN/A理由を記録する。"
scope_in: ["Produced artifacts: docs/spec/feat-seo-aeo-measurement-loop/refactoring-migration.md, drizzle/","Consumed artifacts: features/feat-seo-aeo-measurement-loop.md, features/feat-seo-aeo-measurement-loop.context.json, system-spec/backend.md, system-spec/maintenance-ops.md, system-spec/database.md, SYS-SEO-AEO-MEASUREMENT-LOOP-P05","Write scope/touches: docs/spec/feat-seo-aeo-measurement-loop/refactoring-migration.md, drizzle/"]
...
acceptance: ["Automated commands: `python3 \"/Users/dm/dev/dev/個人開発/harness/marketplaces/local/plugins/system-dev-planner/scripts/validate-system-plan.py\" --repo-root . --staging .dev-graph/staging/feature-package-feat-seo-aeo-measurement-loop`","Automated commands: `pnpm db:drift`","Required evidence:"]
...
phase_ref: "P08"
file_path: "tasks/feat-seo-aeo-measurement-loop/sys-seo-aeo-measurement-loop-p08.md"
template_id: "task"
...
source_lineage: {"imported_at":"2026-09-04T05:19:32Z","origin_kind":"system-dev-planner","source_digest":"46209e547af534e49404d69ee60347cc4e61a4351d2310c497f51b36ee745eda","source_path":".dev-graph/published/feature-package-feat-seo-aeo-measurement-loop/task-specs/phase-08-refactoring-migration.md","source_plugin":"system-dev-planner","source_version":"0.1.0"}
classification_confidence: 1.0
classification_reason: "feat-seo-aeo-measurement-loopのP08 lifecycle責務への確定写像"
classification_candidates: [{"artifact_kind":"task","candidate_path":"tasks/feat-seo-aeo-measurement-loop/sys-seo-aeo-measurement-loop-p08.md","confidence":1.0}]
issue_linkage: null
tracker_binding: "beads"
beads_linkage: {"bd_issue_id":"ah-ee8o","github_mirror":null,"linked_at":"2026-09-04T07:34:29Z","sync_state":"linked"}
...
# System task overlay: 永続化スキーマの移行 (所見・反映ログ・系統別最終収集時刻)
...
- parent_feature: feat-seo-aeo-measurement-loop
- phase_ref: P08
- classification: confidence=1.0; reason=feat-seo-aeo-measurement-loopのP08 lifecycle責務への確定写像; candidate=tasks/feat-seo-aeo-measurement-loop/sys-seo-aeo-measurement-loop-p08.md
...
3データ源の永続化スキーマに対する移行が必要な場合はdry-runとrollback証跡を伴って実施し、不要な場合はN/A理由を記録する。

## 背景

3データ源の永続化スキーマ変更が既存データへ影響する場合、安全な移行経路を確保する。

## 前提条件

- Required spec/architecture/phase/task nodes: feat-seo-aeo-measurement-loop, arch-system-spec-overview, arch-two-layer-platform, SYS-SEO-AEO-MEASUREMENT-LOOP-P05
...
- Frontend: N/A: reason=画面移行は発生しない
- Backend: N/A: reason=P05実装のロジック移行はP08時点で完了済みのため対象外
- API: N/A: reason=API契約の破壊的変更を行わない
- Data: applicable; migration=所見・反映ログ・系統別最終収集時刻テーブルのdry-run移行とrollback証跡を扱う。不要な場合はN/A理由を記録する
- Infrastructure: N/A: reason=Infrastructure構成の移行は発生しない
- Security: N/A: reason=セキュリティ境界の変更は発生しない
- Quality: applicable; tests/gates=移行前後でdb:driftが0であることを確認する
- Documentation: applicable; docs=docs/spec/feat-seo-aeo-measurement-loop/refactoring-migration.md を作成する
- Operations: N/A: reason=運用手順の変更はP12で扱う
...
- Produced artifacts: docs/spec/feat-seo-aeo-measurement-loop/refactoring-migration.md, drizzle/
- Consumed artifacts: features/feat-seo-aeo-measurement-loop.md, features/feat-seo-aeo-measurement-loop.context.json, system-spec/backend.md, system-spec/maintenance-ops.md, system-spec/database.md, SYS-SEO-AEO-MEASUREMENT-LOOP-P05
- Write scope/touches: docs/spec/feat-seo-aeo-measurement-loop/refactoring-migration.md, drizzle/
...
- PR body contract: Beads issue参照とdev-graph graph_node_id=SYS-SEO-AEO-MEASUREMENT-LOOP-P08を記載し、target branchはdevとする
- Ownership boundary: system-dev-plannerはintentを宣言し、dev-graphが起票・依存・完了収束を所有する

## Branch and worktree execution

- Branch: dev-graph登録後にC15がdevgraph/SYS-SEO-AEO-MEASUREMENT-LOOP-P08として割り当てる
- Worktree lease: 実装開始前にSYS-SEO-AEO-MEASUREMENT-LOOP-P08をclaimし、heartbeatとreleaseをlease契約どおり行う
...
- Automated commands: `pnpm db:drift`
- Required evidence:
  - docs/spec/feat-seo-aeo-measurement-loop/refactoring-migration.md
- Acceptance state: P08: 3データ源の永続化スキーマ(所見・反映ログ・差分・系統別最終収集時刻)に対する移行が必要な場合はdry-runとrollback証跡を伴って実施し、不要な場合はN/A理由をevidenceとして記録する。
...
- Rollout: SYS-SEO-AEO-MEASUREMENT-LOOP-P08の成果物をwrite_scope内へ適用し、検証PASS後に依存する次phaseへ渡す
...
- Completion condition: P08: 3データ源の永続化スキーマ(所見・反映ログ・差分・系統別最終収集時刻)に対する移行が必要な場合はdry-runとrollback証跡を伴って実施し、不要な場合はN/A理由をevidenceとして記録する。
...
- Phase doc: references/system-plan-phase-names.md#P08
- Dependencies: SYS-SEO-AEO-MEASUREMENT-LOOP-P05
