---
graph_node_id: "SYS-SEO-AEO-MEASUREMENT-LOOP-P02"
artifact_kind: "task"
artifact_subtypes: []
project_id: "feature-package-feat-seo-aeo-measurement-loop"
domain: "backend"
tags: ["p02","feat-seo-aeo-measurement-loop","seo-aeo"]
priority: null
start_date: null
target_date: null
iteration: null
title: "3データ源収集・突合・自動反映アーキテクチャ設計"
owners: ["daishiman"]
created_at: "2026-09-04T05:19:32Z"
updated_at: "2026-09-10T09:00:00Z"
status: "active"
depends_on: ["SYS-SEO-AEO-MEASUREMENT-LOOP-P01"]
related_nodes: ["arch-system-spec-overview","arch-two-layer-platform"]
resource_scope: ["docs/spec/feat-seo-aeo-measurement-loop/architecture.md"]
purpose: "3データ源の収集・突合・所見生成・自動反映・通知・停止制御・停止検出の各コンポーネント境界とデータモデルを設計し、NFR1-9が設計図上で機械的に追跡できる状態にする。"
goal: "3データ源の収集・突合・所見生成・自動反映・通知・停止制御・停止検出の各コンポーネント境界とデータモデルを設計し、NFR1-9が設計図上で機械的に追跡できる状態にする。"
scope_in: ["Produced artifacts: docs/spec/feat-seo-aeo-measurement-loop/architecture.md","Consumed artifacts: features/feat-seo-aeo-measurement-loop.md, features/feat-seo-aeo-measurement-loop.context.json, system-spec/backend.md, system-spec/maintenance-ops.md, system-spec/database.md, SYS-SEO-AEO-MEASUREMENT-LOOP-P01","Write scope/touches: docs/spec/feat-seo-aeo-measurement-loop/architecture.md"]
...
phase_ref: "P02"
file_path: "tasks/feat-seo-aeo-measurement-loop/sys-seo-aeo-measurement-loop-p02.md"
template_id: "task"
...
source_lineage: {"imported_at":"2026-09-04T05:19:32Z","origin_kind":"system-dev-planner","source_digest":"46209e547af534e49404d69ee60347cc4e61a4351d2310c497f51b36ee745eda","source_path":".dev-graph/published/feature-package-feat-seo-aeo-measurement-loop/task-specs/phase-02-architecture.md","source_plugin":"system-dev-planner","source_version":"0.1.0"}
classification_confidence: 1.0
classification_reason: "feat-seo-aeo-measurement-loopのP02 lifecycle責務への確定写像"
classification_candidates: [{"artifact_kind":"task","candidate_path":"tasks/feat-seo-aeo-measurement-loop/sys-seo-aeo-measurement-loop-p02.md","confidence":1.0}]
issue_linkage: null
tracker_binding: "beads"
beads_linkage: {"bd_issue_id":"ah-xs5l","github_mirror":null,"linked_at":"2026-09-04T07:34:15Z","sync_state":"linked"}
...
# System task overlay: 3データ源収集・突合・自動反映アーキテクチャ設計
...
- parent_feature: feat-seo-aeo-measurement-loop
- phase_ref: P02
- classification: confidence=1.0; reason=feat-seo-aeo-measurement-loopのP02 lifecycle責務への確定写像; candidate=tasks/feat-seo-aeo-measurement-loop/sys-seo-aeo-measurement-loop-p02.md
...
3データ源の収集・突合・所見生成・自動反映・通知・停止制御・停止検出の各コンポーネント境界とデータモデルを設計し、NFR1-9が設計図上で機械的に追跡できる状態にする。

## 背景

P01で確定した要件を、収集・突合・所見生成・自動反映・通知・停止制御・停止検出のコンポーネント境界とデータモデルへ落とす。

## 前提条件

- Required spec/architecture/phase/task nodes: feat-seo-aeo-measurement-loop, arch-system-spec-overview, arch-two-layer-platform, SYS-SEO-AEO-MEASUREMENT-LOOP-P01
...
- Frontend: applicable; change=管理画面ダッシュボード(所見一覧・反映履歴・停止トグル)の画面構成を設計する
- Backend: applicable; change=収集器・突合エンジン・自動反映エンジン・停止検出のコンポーネント境界を設計する
- API: applicable; contract=Search Console/AI検索問い合わせと管理画面向け内部APIの契約を設計する
- Data: applicable; migration=所見・反映ログ・差分・系統別最終収集時刻のテーブル設計を行う
- Infrastructure: applicable; IaC/deploy=Cron Triggerの契機(記事保存時/日次/公開時+週次)を設計する
- Security: applicable; control=資格情報がコードへ現れない境界とNFR5の型的限定を設計する
- Quality: applicable; tests/gates=設計がP04で検証可能な粒度になるよう境界を明示する
- Documentation: applicable; docs=docs/spec/feat-seo-aeo-measurement-loop/architecture.md を作成する
- Operations: applicable; runbook/monitoring=停止/再開と系統別停止検出の運用フローを設計する
...
- Produced artifacts: docs/spec/feat-seo-aeo-measurement-loop/architecture.md
- Consumed artifacts: features/feat-seo-aeo-measurement-loop.md, features/feat-seo-aeo-measurement-loop.context.json, system-spec/backend.md, system-spec/maintenance-ops.md, system-spec/database.md, SYS-SEO-AEO-MEASUREMENT-LOOP-P01
- Write scope/touches: docs/spec/feat-seo-aeo-measurement-loop/architecture.md
...
- PR body contract: Beads issue参照とdev-graph graph_node_id=SYS-SEO-AEO-MEASUREMENT-LOOP-P02を記載し、target branchはdevとする
- Ownership boundary: system-dev-plannerはintentを宣言し、dev-graphが起票・依存・完了収束を所有する

## Branch and worktree execution

- Branch: dev-graph登録後にC15がdevgraph/SYS-SEO-AEO-MEASUREMENT-LOOP-P02として割り当てる
- Worktree lease: 実装開始前にSYS-SEO-AEO-MEASUREMENT-LOOP-P02をclaimし、heartbeatとreleaseをlease契約どおり行う
...
- Required evidence:
  - docs/spec/feat-seo-aeo-measurement-loop/architecture.md
- Acceptance state: P02: 3データ源収集・突合・所見生成・自動反映・通知・停止制御・停止検出の各コンポーネント境界とデータモデル(記事識別子・所見・反映ログ・差分・保留状態・系統別最終収集時刻)を設計し、NFR1(反映前スナップショット必須)・NFR2(記事作成時刻による範囲判定)・NFR5(反映根拠を系統①へ限定する型)・NFR8(資格情報が実装コードへ現れない境界)が設計図上で機械的に追跡できる状態にする。
...
- Rollout: SYS-SEO-AEO-MEASUREMENT-LOOP-P02の成果物をwrite_scope内へ適用し、検証PASS後に依存する次phaseへ渡す
...
- Completion condition: P02: 3データ源収集・突合・所見生成・自動反映・通知・停止制御・停止検出の各コンポーネント境界とデータモデル(記事識別子・所見・反映ログ・差分・保留状態・系統別最終収集時刻)を設計し、NFR1(反映前スナップショット必須)・NFR2(記事作成時刻による範囲判定)・NFR5(反映根拠を系統①へ限定する型)・NFR8(資格情報が実装コードへ現れない境界)が設計図上で機械的に追跡できる状態にする。
...
- Phase doc: references/system-plan-phase-names.md#P02
- Dependencies: SYS-SEO-AEO-MEASUREMENT-LOOP-P01
