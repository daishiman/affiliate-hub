---
graph_node_id: "SYS-SEO-AEO-MEASUREMENT-LOOP-P04"
artifact_kind: "task"
artifact_subtypes: []
project_id: "feature-package-feat-seo-aeo-measurement-loop"
domain: "quality"
tags: ["p04","feat-seo-aeo-measurement-loop","seo-aeo"]
priority: null
start_date: null
target_date: null
iteration: null
title: "A1-A8とNFR1-9のテスト設計"
owners: ["daishiman"]
created_at: "2026-09-04T05:19:32Z"
updated_at: "2026-09-04T05:19:32Z"
status: "active"
depends_on: ["SYS-SEO-AEO-MEASUREMENT-LOOP-P03"]
related_nodes: ["arch-system-spec-overview","arch-two-layer-platform"]
resource_scope: ["docs/spec/feat-seo-aeo-measurement-loop/test-design.md","tests/fixtures/feat-seo-aeo-measurement-loop/"]
purpose: "A1-A8とNFR1-9を単体・contract・integration・E2E・security・performanceの実行可能なテストへ先に写像する。"
goal: "A1-A8とNFR1-9を単体・contract・integration・E2E・security・performanceの実行可能なテストへ先に写像する。"
scope_in: ["Produced artifacts: docs/spec/feat-seo-aeo-measurement-loop/test-design.md, tests/fixtures/feat-seo-aeo-measurement-loop/","Consumed artifacts: features/feat-seo-aeo-measurement-loop.md, features/feat-seo-aeo-measurement-loop.context.json, system-spec/backend.md, system-spec/maintenance-ops.md, system-spec/database.md, SYS-SEO-AEO-MEASUREMENT-LOOP-P03","Write scope/touches: docs/spec/feat-seo-aeo-measurement-loop/test-design.md, tests/fixtures/feat-seo-aeo-measurement-loop/"]
...
phase_ref: "P04"
file_path: "tasks/feat-seo-aeo-measurement-loop/sys-seo-aeo-measurement-loop-p04.md"
template_id: "task"
...
source_lineage: {"imported_at":"2026-09-04T05:19:32Z","origin_kind":"system-dev-planner","source_digest":"46209e547af534e49404d69ee60347cc4e61a4351d2310c497f51b36ee745eda","source_path":".dev-graph/published/feature-package-feat-seo-aeo-measurement-loop/task-specs/phase-04-test-design.md","source_plugin":"system-dev-planner","source_version":"0.1.0"}
classification_confidence: 1.0
classification_reason: "feat-seo-aeo-measurement-loopのP04 lifecycle責務への確定写像"
classification_candidates: [{"artifact_kind":"task","candidate_path":"tasks/feat-seo-aeo-measurement-loop/sys-seo-aeo-measurement-loop-p04.md","confidence":1.0}]
issue_linkage: null
tracker_binding: "beads"
beads_linkage: {"bd_issue_id":"ah-5sps","github_mirror":null,"linked_at":"2026-09-04T07:34:20Z","sync_state":"linked"}
...
# System task overlay: A1-A8とNFR1-9のテスト設計
...
- parent_feature: feat-seo-aeo-measurement-loop
- phase_ref: P04
- classification: confidence=1.0; reason=feat-seo-aeo-measurement-loopのP04 lifecycle責務への確定写像; candidate=tasks/feat-seo-aeo-measurement-loop/sys-seo-aeo-measurement-loop-p04.md
...
A1-A8とNFR1-9を単体・contract・integration・E2E・security・performanceの実行可能なテストへ先に写像する。

## 背景

実装より先にテストを固定し、可逆性・時間範囲限定・根拠系統限定・停止可能性・停止検出・効果判定遅延・秘密情報境界・費用上限の全非機能要件を検証可能な形にする。

## 前提条件

- Required spec/architecture/phase/task nodes: feat-seo-aeo-measurement-loop, arch-system-spec-overview, arch-two-layer-platform, SYS-SEO-AEO-MEASUREMENT-LOOP-P03
...
- Frontend: applicable; change=停止トグル・停止中表示・反映履歴・1操作revertのE2Eテストを設計する
- Backend: applicable; change=収集・突合・自動反映・効果判定保留のunit/integrationテストを設計する
- API: applicable; contract=Search Console/AI検索問い合わせのcontractテストを設計する
- Data: applicable; migration=所見・反映ログ・系統別最終収集時刻の結合テストを設計する
- Infrastructure: applicable; IaC/deploy=Cron Trigger契機ごとの起動条件テストを設計する
- Security: applicable; control=資格情報非露出とNFR5根拠系統限定の否定テストを設計する
- Quality: applicable; tests/gates=A1-A8とNFR1-9のtraceをテストへ写像する
- Documentation: applicable; docs=実行条件と判定式を記録する
- Operations: applicable; runbook/monitoring=停止検出と費用上限到達のオペレーション試験を設計する
...
- Produced artifacts: docs/spec/feat-seo-aeo-measurement-loop/test-design.md, tests/fixtures/feat-seo-aeo-measurement-loop/
- Consumed artifacts: features/feat-seo-aeo-measurement-loop.md, features/feat-seo-aeo-measurement-loop.context.json, system-spec/backend.md, system-spec/maintenance-ops.md, system-spec/database.md, SYS-SEO-AEO-MEASUREMENT-LOOP-P03
- Write scope/touches: docs/spec/feat-seo-aeo-measurement-loop/test-design.md, tests/fixtures/feat-seo-aeo-measurement-loop/
...
- PR body contract: Beads issue参照とdev-graph graph_node_id=SYS-SEO-AEO-MEASUREMENT-LOOP-P04を記載し、target branchはdevとする
- Ownership boundary: system-dev-plannerはintentを宣言し、dev-graphが起票・依存・完了収束を所有する

## Branch and worktree execution

- Branch: dev-graph登録後にC15がdevgraph/SYS-SEO-AEO-MEASUREMENT-LOOP-P04として割り当てる
- Worktree lease: 実装開始前にSYS-SEO-AEO-MEASUREMENT-LOOP-P04をclaimし、heartbeatとreleaseをlease契約どおり行う
...
- Required evidence:
  - docs/spec/feat-seo-aeo-measurement-loop/test-design.md
  - tests/fixtures/feat-seo-aeo-measurement-loop/
- Acceptance state: P04: A1〜A8とNFR1〜NFR9を単体・contract・integration・E2E・security・performanceの実行可能なテストへ写像し、可逆性(記録失敗時は書き換え不実行)・時間範囲境界(作成時刻の境界値)・根拠系統限定(系統②③を反映根拠に使うとfailする否定テスト)・停止中でも収集継続・停止検出の系統別閾値・効果判定の保留状態・費用上限到達時の停止・資格情報非露出の機械検査ケースを含むテストIDが一意に定義される。
...
- Rollout: SYS-SEO-AEO-MEASUREMENT-LOOP-P04の成果物をwrite_scope内へ適用し、検証PASS後に依存する次phaseへ渡す
...
- Completion condition: P04: A1〜A8とNFR1〜NFR9を単体・contract・integration・E2E・security・performanceの実行可能なテストへ写像し、可逆性(記録失敗時は書き換え不実行)・時間範囲境界(作成時刻の境界値)・根拠系統限定(系統②③を反映根拠に使うとfailする否定テスト)・停止中でも収集継続・停止検出の系統別閾値・効果判定の保留状態・費用上限到達時の停止・資格情報非露出の機械検査ケースを含むテストIDが一意に定義される。
...
- Phase doc: references/system-plan-phase-names.md#P04
- Dependencies: SYS-SEO-AEO-MEASUREMENT-LOOP-P03
