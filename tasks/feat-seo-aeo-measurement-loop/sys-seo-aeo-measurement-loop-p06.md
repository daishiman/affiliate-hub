---
graph_node_id: "SYS-SEO-AEO-MEASUREMENT-LOOP-P06"
artifact_kind: "task"
artifact_subtypes: []
project_id: "feature-package-feat-seo-aeo-measurement-loop"
domain: "quality"
tags: ["p06","feat-seo-aeo-measurement-loop","seo-aeo"]
priority: null
start_date: null
target_date: null
iteration: null
...
resource_scope: ["tests/feat-seo-aeo-measurement-loop/","docs/spec/feat-seo-aeo-measurement-loop/test-run.md"]
purpose: "P04で定義した全テストをP05実装に対して実行し、再現可能な証跡として記録する。"
goal: "P04で定義した全テストをP05実装に対して実行し、再現可能な証跡として記録する。"
scope_in: ["Produced artifacts: tests/feat-seo-aeo-measurement-loop/, docs/spec/feat-seo-aeo-measurement-loop/test-run.md","Consumed artifacts: features/feat-seo-aeo-measurement-loop.md, features/feat-seo-aeo-measurement-loop.context.json, system-spec/backend.md, system-spec/maintenance-ops.md, system-spec/database.md, SYS-SEO-AEO-MEASUREMENT-LOOP-P05","Write scope/touches: tests/feat-seo-aeo-measurement-loop/, docs/spec/feat-seo-aeo-measurement-loop/test-run.md"]
...
acceptance: ["Automated commands: `python3 \"/Users/dm/dev/dev/個人開発/harness/marketplaces/local/plugins/system-dev-planner/scripts/validate-system-plan.py\" --repo-root . --staging .dev-graph/staging/feature-package-feat-seo-aeo-measurement-loop`","Automated commands: `pnpm test:coverage`","Required evidence:"]
...
phase_ref: "P06"
file_path: "tasks/feat-seo-aeo-measurement-loop/sys-seo-aeo-measurement-loop-p06.md"
template_id: "task"
...
source_lineage: {"imported_at":"2026-09-04T05:19:32Z","origin_kind":"system-dev-planner","source_digest":"46209e547af534e49404d69ee60347cc4e61a4351d2310c497f51b36ee745eda","source_path":".dev-graph/published/feature-package-feat-seo-aeo-measurement-loop/task-specs/phase-06-test-run.md","source_plugin":"system-dev-planner","source_version":"0.1.0"}
classification_confidence: 1.0
classification_reason: "feat-seo-aeo-measurement-loopのP06 lifecycle責務への確定写像"
classification_candidates: [{"artifact_kind":"task","candidate_path":"tasks/feat-seo-aeo-measurement-loop/sys-seo-aeo-measurement-loop-p06.md","confidence":1.0}]
issue_linkage: null
tracker_binding: "beads"
beads_linkage: {"bd_issue_id":"ah-rduz","github_mirror":null,"linked_at":"2026-09-04T07:34:25Z","sync_state":"linked"}
...
# System task overlay: テスト実行と証跡記録
...
- parent_feature: feat-seo-aeo-measurement-loop
- phase_ref: P06
- classification: confidence=1.0; reason=feat-seo-aeo-measurement-loopのP06 lifecycle責務への確定写像; candidate=tasks/feat-seo-aeo-measurement-loop/sys-seo-aeo-measurement-loop-p06.md
...
P04で定義した全テストをP05実装に対して実行し、再現可能な証跡として記録する。

## 背景

P05実装に対しP04で設計した全テストを実行し、既定カバレッジと0 failureを再現可能な証跡として残す。
...
- Frontend: applicable; change=E2Eテストを実行し結果を記録する
- Backend: applicable; change=unit/integrationテストを実行し結果を記録する
- API: applicable; contract=contractテストを実行し結果を記録する
- Data: applicable; migration=データ結合テストを実行し結果を記録する
- Infrastructure: applicable; IaC/deploy=Cron Trigger起動条件テストを実行し結果を記録する
- Security: applicable; control=資格情報非露出テストを実行し結果を記録する
- Quality: applicable; tests/gates=既定80%カバレッジ達成を確認する
- Documentation: applicable; docs=docs/spec/feat-seo-aeo-measurement-loop/test-run.md へ結果を記録する
- Operations: applicable; runbook/monitoring=停止検出・費用上限テストの結果を記録する
...
- Produced artifacts: tests/feat-seo-aeo-measurement-loop/, docs/spec/feat-seo-aeo-measurement-loop/test-run.md
...
- Write scope/touches: tests/feat-seo-aeo-measurement-loop/, docs/spec/feat-seo-aeo-measurement-loop/test-run.md
...
- PR body contract: Beads issue参照とdev-graph graph_node_id=SYS-SEO-AEO-MEASUREMENT-LOOP-P06を記載し、target branchはdevとする
- Ownership boundary: system-dev-plannerはintentを宣言し、dev-graphが起票・依存・完了収束を所有する

## Branch and worktree execution

- Branch: dev-graph登録後にC15がdevgraph/SYS-SEO-AEO-MEASUREMENT-LOOP-P06として割り当てる
- Worktree lease: 実装開始前にSYS-SEO-AEO-MEASUREMENT-LOOP-P06をclaimし、heartbeatとreleaseをlease契約どおり行う
...
- Automated commands: `pnpm test:coverage`
- Required evidence:
  - docs/spec/feat-seo-aeo-measurement-loop/test-run.md
  - tests/feat-seo-aeo-measurement-loop/
- Acceptance state: P06: P04で定義した全テストをP05実装に対して実行し、既定80%を下回らないカバレッジとNFR1〜NFR9対応ケース0 failureの実行結果が再現可能な証跡として記録される。
...
- Rollout: SYS-SEO-AEO-MEASUREMENT-LOOP-P06の成果物をwrite_scope内へ適用し、検証PASS後に依存する次phaseへ渡す
...
- Completion condition: P06: P04で定義した全テストをP05実装に対して実行し、既定80%を下回らないカバレッジとNFR1〜NFR9対応ケース0 failureの実行結果が再現可能な証跡として記録される。
...
- Phase doc: references/system-plan-phase-names.md#P06
