---
graph_node_id: "SYS-SEO-AEO-MEASUREMENT-LOOP-P11"
artifact_kind: "task"
artifact_subtypes: []
project_id: "feature-package-feat-seo-aeo-measurement-loop"
domain: "quality"
tags: ["p11","feat-seo-aeo-measurement-loop","seo-aeo"]
priority: null
start_date: null
target_date: null
iteration: null
...
status: "active"
depends_on: ["SYS-SEO-AEO-MEASUREMENT-LOOP-P07","SYS-SEO-AEO-MEASUREMENT-LOOP-P09","SYS-SEO-AEO-MEASUREMENT-LOOP-P10"]
related_nodes: ["arch-system-spec-overview","arch-two-layer-platform"]
resource_scope: ["docs/spec/feat-seo-aeo-measurement-loop/evidence/"]
purpose: "P04-P09の全テスト結果・QA結果・レビュー記録を再現可能な証跡バンドルへまとめる。"
goal: "P04-P09の全テスト結果・QA結果・レビュー記録を再現可能な証跡バンドルへまとめる。"
scope_in: ["Produced artifacts: docs/spec/feat-seo-aeo-measurement-loop/evidence/","Consumed artifacts: features/feat-seo-aeo-measurement-loop.md, features/feat-seo-aeo-measurement-loop.context.json, system-spec/backend.md, system-spec/maintenance-ops.md, system-spec/database.md, SYS-SEO-AEO-MEASUREMENT-LOOP-P07, SYS-SEO-AEO-MEASUREMENT-LOOP-P09, SYS-SEO-AEO-MEASUREMENT-LOOP-P10","Write scope/touches: docs/spec/feat-seo-aeo-measurement-loop/evidence/"]
...
phase_ref: "P11"
file_path: "tasks/feat-seo-aeo-measurement-loop/sys-seo-aeo-measurement-loop-p11.md"
template_id: "task"
...
source_lineage: {"imported_at":"2026-09-04T05:19:32Z","origin_kind":"system-dev-planner","source_digest":"46209e547af534e49404d69ee60347cc4e61a4351d2310c497f51b36ee745eda","source_path":".dev-graph/published/feature-package-feat-seo-aeo-measurement-loop/task-specs/phase-11-evidence.md","source_plugin":"system-dev-planner","source_version":"0.1.0"}
classification_confidence: 1.0
classification_reason: "feat-seo-aeo-measurement-loopのP11 lifecycle責務への確定写像"
classification_candidates: [{"artifact_kind":"task","candidate_path":"tasks/feat-seo-aeo-measurement-loop/sys-seo-aeo-measurement-loop-p11.md","confidence":1.0}]
issue_linkage: null
tracker_binding: "beads"
beads_linkage: {"bd_issue_id":"ah-npjo","github_mirror":null,"linked_at":"2026-09-04T07:34:36Z","sync_state":"linked"}
...
- parent_feature: feat-seo-aeo-measurement-loop
- phase_ref: P11
- classification: confidence=1.0; reason=feat-seo-aeo-measurement-loopのP11 lifecycle責務への確定写像; candidate=tasks/feat-seo-aeo-measurement-loop/sys-seo-aeo-measurement-loop-p11.md
...
P04-P09の全テスト結果・QA結果・レビュー記録を再現可能な証跡バンドルへまとめる。

## 背景

featureの完了根拠となる証跡を、再現可能な形で1か所へ集約する。

## 前提条件

- Required spec/architecture/phase/task nodes: feat-seo-aeo-measurement-loop, arch-system-spec-overview, arch-two-layer-platform, SYS-SEO-AEO-MEASUREMENT-LOOP-P07, SYS-SEO-AEO-MEASUREMENT-LOOP-P09, SYS-SEO-AEO-MEASUREMENT-LOOP-P10
...
- Frontend: N/A: reason=証跡集約でありUI変更を行わない
- Backend: applicable; change=P05-P09の実行結果を再現可能な証跡として集約する
- API: N/A: reason=API契約の変更を行わない
- Data: applicable; migration=P08証跡を証跡バンドルへ含める
- Infrastructure: N/A: reason=Infrastructure構成の変更を行わない
- Security: applicable; control=NFR8機械検査結果を証跡バンドルへ含める
- Quality: applicable; tests/gates=再実行コマンドで同一結果が再現できることを確認する
- Documentation: applicable; docs=docs/spec/feat-seo-aeo-measurement-loop/evidence/ を作成する
- Operations: applicable; runbook/monitoring=NFR6/NFR9証跡を含める
...
- Produced artifacts: docs/spec/feat-seo-aeo-measurement-loop/evidence/
- Consumed artifacts: features/feat-seo-aeo-measurement-loop.md, features/feat-seo-aeo-measurement-loop.context.json, system-spec/backend.md, system-spec/maintenance-ops.md, system-spec/database.md, SYS-SEO-AEO-MEASUREMENT-LOOP-P07, SYS-SEO-AEO-MEASUREMENT-LOOP-P09, SYS-SEO-AEO-MEASUREMENT-LOOP-P10
- Write scope/touches: docs/spec/feat-seo-aeo-measurement-loop/evidence/
...
- PR body contract: Beads issue参照とdev-graph graph_node_id=SYS-SEO-AEO-MEASUREMENT-LOOP-P11を記載し、target branchはdevとする
- Ownership boundary: system-dev-plannerはintentを宣言し、dev-graphが起票・依存・完了収束を所有する

## Branch and worktree execution

- Branch: dev-graph登録後にC15がdevgraph/SYS-SEO-AEO-MEASUREMENT-LOOP-P11として割り当てる
- Worktree lease: 実装開始前にSYS-SEO-AEO-MEASUREMENT-LOOP-P11をclaimし、heartbeatとreleaseをlease契約どおり行う
...
- Required evidence:
  - docs/spec/feat-seo-aeo-measurement-loop/evidence/
- Acceptance state: P11: P04〜P09で得た全テスト結果・QA結果・レビュー記録を再現可能な証跡バンドルとしてwrite_scope配下へまとめ、再実行コマンドで同じ結果が再現できることを確認する。
...
- Rollout: SYS-SEO-AEO-MEASUREMENT-LOOP-P11の成果物をwrite_scope内へ適用し、検証PASS後に依存する次phaseへ渡す
...
- Completion condition: P11: P04〜P09で得た全テスト結果・QA結果・レビュー記録を再現可能な証跡バンドルとしてwrite_scope配下へまとめ、再実行コマンドで同じ結果が再現できることを確認する。
...
- Phase doc: references/system-plan-phase-names.md#P11
- Dependencies: SYS-SEO-AEO-MEASUREMENT-LOOP-P07, SYS-SEO-AEO-MEASUREMENT-LOOP-P09, SYS-SEO-AEO-MEASUREMENT-LOOP-P10
