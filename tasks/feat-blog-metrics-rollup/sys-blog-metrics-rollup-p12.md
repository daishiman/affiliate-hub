---
graph_node_id: "SYS-BLOG-METRICS-ROLLUP-P12"
artifact_kind: "task"
artifact_subtypes: []
project_id: "feature-package-feat-blog-metrics-rollup"
domain: "documentation"
tags: ["p12","feat-blog-metrics-rollup"]
priority: null
start_date: null
target_date: null
iteration: null
...
purpose: "定期実行が失敗した日をどう埋め直すか、各指標が何を意味するか、標本数が少ない行をどう扱うかを、運用手順と利用者向け説明として整えた状態を成立させる。"
goal: "定期実行が失敗した日をどう埋め直すか、各指標が何を意味するか、標本数が少ない行をどう扱うかを、運用手順と利用者向け説明として整えた状態を成立させる。"
scope_in: ["Produced artifacts: docs/spec/feat-blog-metrics-rollup/operations-runbook.md (失敗検知・再実行・欠測日の埋め直し手順); docs/ 配下の指標定義の説明","Consumed artifacts: features/feat-blog-metrics-rollup.md; features/feat-blog-metrics-rollup.context.json; system-spec/backend.md; system-spec/database.md; system-spec/maintenance-ops.md","Write scope/touches: docs/spec/feat-blog-metrics-rollup/operations-runbook.md, docs/spec/feat-blog-metrics-rollup/"]
scope_out: ["生の行動イベントの収集そのもの (feat-reader-behavior-analytics が所有する)","集計結果の画面表示と示唆生成 (feat-blog-scoped-admin-console が所有する)","アフィリエイト成果の外部取り込み経路 (既存 feature が所有する)","日次より細かい粒度でのリアルタイム集計 (作らない)","P12 以外の phase が所有する成果物への変更"]
acceptance: ["Required evidence: operations-runbook.md が失敗検知と再実行の両方に手順を持つこと","Automated commands: `pnpm run lint` (文書内のコード片が静的検査を通ることを確認する)","Required evidence: P12 の 成果物 section に記載した produced artifacts のパス"]
...
phase_ref: "P12"
file_path: "tasks/feat-blog-metrics-rollup/sys-blog-metrics-rollup-p12.md"
template_id: "task"
...
source_lineage: {"imported_at":"2026-09-03T23:49:24Z","origin_kind":"system-dev-planner","source_digest":"f38e68c7d023f98268db47f1238dbd88ddbfe496d94d6eab5996b57f4775c2c0","source_path":".dev-graph/published/generations/feature-package-feat-blog-metrics-rollup/f38e68c7d023f98268db47f1238dbd88ddbfe496d94d6eab5996b57f4775c2c0/task-specs/phase-12-documentation-operations.md","source_plugin":"system-dev-planner","source_version":"0.1.0"}
classification_confidence: 1.0
classification_reason: "feat-blog-metrics-rollup の P12 lifecycle 責務への確定写像"
classification_candidates: [{"artifact_kind":"task","candidate_path":"tasks/feat-blog-metrics-rollup/sys-blog-metrics-rollup-p12.md","confidence":1.0}]
issue_linkage: null
tracker_binding: "beads"
beads_linkage: {"bd_issue_id":"ah-q4dt.12","github_mirror":null,"linked_at":"2026-09-04T02:08:01Z","sync_state":"linked"}
github_publication: {"labels":[],"milestone":null,"mode":"local_only","project_aliases":[]}
github_project_linkages: []
pull_request_linkages: []
execution_contexts: []
completion_evidence: {"completed_at":"2026-09-04T10:14:35Z","evidence_refs":["beads:ah-q4dt.12"],"policy":"manual","reconciled_at":"2026-09-09T00:00:00Z","source":"reconciliation","status":"done"}
implementation_readiness: {"checked_at":"2026-09-04T00:00:00Z","missing_sections":[],"status":"complete"}
---

# System task overlay: 日次ロールアップの運用手順と数値定義の説明
...
- parent_feature: feat-blog-metrics-rollup
- phase_ref: P12
- classification: confidence=1.0; reason=feat-blog-metrics-rollup の P12 lifecycle 責務への確定写像; candidate=tasks/feat-blog-metrics-rollup/sys-blog-metrics-rollup-p12.md
...
定期実行が失敗した日をどう埋め直すか、各指標が何を意味するか、標本数が少ない行をどう扱うかを、運用手順と利用者向け説明として整えた状態を成立させる。

## 背景

集計の再実行は運用者が判断して行う。判断に必要な情報 (どの日が欠けているか、再実行が安全か) を手順として明示する。
...
- Operations: applicable; 定期実行の運用手順を本 phase が所有する
...
- Produced artifacts: docs/spec/feat-blog-metrics-rollup/operations-runbook.md (失敗検知・再実行・欠測日の埋め直し手順); docs/ 配下の指標定義の説明
...
- Write scope/touches: docs/spec/feat-blog-metrics-rollup/operations-runbook.md, docs/spec/feat-blog-metrics-rollup/
...
- PR body contract: Closes #issue (beads issue) + dev-graph graph_node_id=SYS-BLOG-METRICS-ROLLUP-P12; PR の宛先は dev ブランチとする
- Ownership boundary: system-dev-planner は intent のみを宣言し、dev-graph が tracker mutation と reconciliation を行う

## Branch and worktree execution

- Branch: assigned after dev-graph registration by C15 as devgraph/SYS-BLOG-METRICS-ROLLUP-P12; system-dev-planner は事前割当を行わない
- Worktree lease: claim SYS-BLOG-METRICS-ROLLUP-P12 before implementation; heartbeat/release は dev-graph worktree lease 契約に従う
...
- Required evidence: operations-runbook.md が失敗検知と再実行の両方に手順を持つこと
...
- Goal: 定期実行が失敗した日をどう埋め直すか、各指標が何を意味するか、標本数が少ない行をどう扱うかを、運用手順と利用者向け説明として整えた状態を成立させる。
- Generic execution prompt: feat-blog-metrics-rollup の goal (記事ごとの売上と PV、滞在・スクロール到達・クリック率が同じ日次行に載り、ブログ単位へ集計され、同じ日を何度処理しても結果が置き換わるだけで二重計上せず、標本数が少ない行は推測に足りないと判る状態になっている) と本 task の 前提条件/成果物/write scope/スコープ外 を渡し、実装手段は固定せず P12 の目的を満たす成果物を作らせる
...
- Ready when: 運用手順が失敗検知・再実行・指標説明を覆っている
...
- Dependencies: SYS-BLOG-METRICS-ROLLUP-P10, SYS-BLOG-METRICS-ROLLUP-P11
