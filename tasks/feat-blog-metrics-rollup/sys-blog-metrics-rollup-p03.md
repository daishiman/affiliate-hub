---
graph_node_id: "SYS-BLOG-METRICS-ROLLUP-P03"
artifact_kind: "task"
artifact_subtypes: []
project_id: "feature-package-feat-blog-metrics-rollup"
domain: "quality"
tags: ["p03","feat-blog-metrics-rollup"]
priority: null
start_date: null
target_date: null
iteration: null
...
purpose: "冪等戦略が実際に二重計上を防ぐこと、指標定義が実装可能であること、入力欠測時に誤った0が記録されないことを独立観点で検査し、実装着手の可否を判定した状態を成立させる。"
goal: "冪等戦略が実際に二重計上を防ぐこと、指標定義が実装可能であること、入力欠測時に誤った0が記録されないことを独立観点で検査し、実装着手の可否を判定した状態を成立させる。"
scope_in: ["Produced artifacts: docs/spec/feat-blog-metrics-rollup/design-review-findings.md (設計レビューの指摘と解決状況); docs/spec/feat-blog-metrics-rollup/missing-data-policy.md (欠測と0の区別の方針)","Consumed artifacts: features/feat-blog-metrics-rollup.md; features/feat-blog-metrics-rollup.context.json; system-spec/backend.md; system-spec/database.md; system-spec/maintenance-ops.md","Write scope/touches: docs/spec/feat-blog-metrics-rollup/design-review-findings.md, docs/spec/feat-blog-metrics-rollup/missing-data-policy.md"]
scope_out: ["生の行動イベントの収集そのもの (feat-reader-behavior-analytics が所有する)","集計結果の画面表示と示唆生成 (feat-blog-scoped-admin-console が所有する)","アフィリエイト成果の外部取り込み経路 (既存 feature が所有する)","日次より細かい粒度でのリアルタイム集計 (作らない)","P03 以外の phase が所有する成果物への変更"]
...
phase_ref: "P03"
file_path: "tasks/feat-blog-metrics-rollup/sys-blog-metrics-rollup-p03.md"
template_id: "task"
...
source_lineage: {"imported_at":"2026-09-03T23:49:24Z","origin_kind":"system-dev-planner","source_digest":"f38e68c7d023f98268db47f1238dbd88ddbfe496d94d6eab5996b57f4775c2c0","source_path":".dev-graph/published/generations/feature-package-feat-blog-metrics-rollup/f38e68c7d023f98268db47f1238dbd88ddbfe496d94d6eab5996b57f4775c2c0/task-specs/phase-03-design-review.md","source_plugin":"system-dev-planner","source_version":"0.1.0"}
classification_confidence: 1.0
classification_reason: "feat-blog-metrics-rollup の P03 lifecycle 責務への確定写像"
classification_candidates: [{"artifact_kind":"task","candidate_path":"tasks/feat-blog-metrics-rollup/sys-blog-metrics-rollup-p03.md","confidence":1.0}]
issue_linkage: null
tracker_binding: "beads"
beads_linkage: {"bd_issue_id":"ah-q4dt.3","github_mirror":null,"linked_at":"2026-09-04T02:07:42Z","sync_state":"linked"}
github_publication: {"labels":[],"milestone":null,"mode":"local_only","project_aliases":[]}
github_project_linkages: []
pull_request_linkages: []
execution_contexts: []
completion_evidence: {"completed_at":"2026-09-04T10:14:15Z","evidence_refs":["beads:ah-q4dt.3"],"policy":"manual","reconciled_at":"2026-09-09T00:00:00Z","source":"reconciliation","status":"done"}
implementation_readiness: {"checked_at":"2026-09-04T00:00:00Z","missing_sections":[],"status":"complete"}
---

# System task overlay: 集計設計の独立レビューと着手可否判定
...
- parent_feature: feat-blog-metrics-rollup
- phase_ref: P03
- classification: confidence=1.0; reason=feat-blog-metrics-rollup の P03 lifecycle 責務への確定写像; candidate=tasks/feat-blog-metrics-rollup/sys-blog-metrics-rollup-p03.md
...
冪等戦略が実際に二重計上を防ぐこと、指標定義が実装可能であること、入力欠測時に誤った0が記録されないことを独立観点で検査し、実装着手の可否を判定した状態を成立させる。

## 背景

欠測を0として記録すると、データが無いことと値が0であることが区別できなくなる。この区別は後から復元できないため、実装前に検査する。
...
- Produced artifacts: docs/spec/feat-blog-metrics-rollup/design-review-findings.md (設計レビューの指摘と解決状況); docs/spec/feat-blog-metrics-rollup/missing-data-policy.md (欠測と0の区別の方針)
...
- Write scope/touches: docs/spec/feat-blog-metrics-rollup/design-review-findings.md, docs/spec/feat-blog-metrics-rollup/missing-data-policy.md
...
- PR body contract: Closes #issue (beads issue) + dev-graph graph_node_id=SYS-BLOG-METRICS-ROLLUP-P03; PR の宛先は dev ブランチとする
- Ownership boundary: system-dev-planner は intent のみを宣言し、dev-graph が tracker mutation と reconciliation を行う

## Branch and worktree execution

- Branch: assigned after dev-graph registration by C15 as devgraph/SYS-BLOG-METRICS-ROLLUP-P03; system-dev-planner は事前割当を行わない
- Worktree lease: claim SYS-BLOG-METRICS-ROLLUP-P03 before implementation; heartbeat/release は dev-graph worktree lease 契約に従う
...
- テストレベル選定: レビュー phase のため実行テストを持たず、欠測方針が全指標を覆うことを完了条件とする。 N/A: 単体・結合・境界値・回帰 の各テストレベルは、本 task の成果物が文書と判定であり実行可能なコードを含まないため適用しない。
...
- Goal: 冪等戦略が実際に二重計上を防ぐこと、指標定義が実装可能であること、入力欠測時に誤った0が記録されないことを独立観点で検査し、実装着手の可否を判定した状態を成立させる。
- Generic execution prompt: feat-blog-metrics-rollup の goal (記事ごとの売上と PV、滞在・スクロール到達・クリック率が同じ日次行に載り、ブログ単位へ集計され、同じ日を何度処理しても結果が置き換わるだけで二重計上せず、標本数が少ない行は推測に足りないと判る状態になっている) と本 task の 前提条件/成果物/write scope/スコープ外 を渡し、実装手段は固定せず P03 の目的を満たす成果物を作らせる
...
- Dependencies: SYS-BLOG-METRICS-ROLLUP-P02
