---
graph_node_id: "SYS-BLOG-METRICS-ROLLUP-P11"
artifact_kind: "task"
artifact_subtypes: []
project_id: "feature-package-feat-blog-metrics-rollup"
domain: "documentation"
tags: ["p11","feat-blog-metrics-rollup"]
priority: null
start_date: null
target_date: null
iteration: null
...
purpose: "受入判定と非機能検査の証跡を一箇所へ集約し、後から第三者が受入9件の成立と指標の意味を再確認できる状態を成立させる。"
goal: "受入判定と非機能検査の証跡を一箇所へ集約し、後から第三者が受入9件の成立と指標の意味を再確認できる状態を成立させる。"
scope_in: ["Produced artifacts: docs/spec/feat-blog-metrics-rollup/evidence-index.md (受入9件と証跡ファイル・コマンドの対応表)","Consumed artifacts: features/feat-blog-metrics-rollup.md; features/feat-blog-metrics-rollup.context.json; system-spec/backend.md; system-spec/database.md; system-spec/maintenance-ops.md","Write scope/touches: docs/spec/feat-blog-metrics-rollup/evidence-index.md"]
scope_out: ["生の行動イベントの収集そのもの (feat-reader-behavior-analytics が所有する)","集計結果の画面表示と示唆生成 (feat-blog-scoped-admin-console が所有する)","アフィリエイト成果の外部取り込み経路 (既存 feature が所有する)","日次より細かい粒度でのリアルタイム集計 (作らない)","P11 以外の phase が所有する成果物への変更"]
...
phase_ref: "P11"
file_path: "tasks/feat-blog-metrics-rollup/sys-blog-metrics-rollup-p11.md"
template_id: "task"
...
source_lineage: {"imported_at":"2026-09-03T23:49:24Z","origin_kind":"system-dev-planner","source_digest":"f38e68c7d023f98268db47f1238dbd88ddbfe496d94d6eab5996b57f4775c2c0","source_path":".dev-graph/published/generations/feature-package-feat-blog-metrics-rollup/f38e68c7d023f98268db47f1238dbd88ddbfe496d94d6eab5996b57f4775c2c0/task-specs/phase-11-evidence.md","source_plugin":"system-dev-planner","source_version":"0.1.0"}
classification_confidence: 1.0
classification_reason: "feat-blog-metrics-rollup の P11 lifecycle 責務への確定写像"
classification_candidates: [{"artifact_kind":"task","candidate_path":"tasks/feat-blog-metrics-rollup/sys-blog-metrics-rollup-p11.md","confidence":1.0}]
issue_linkage: null
tracker_binding: "beads"
beads_linkage: {"bd_issue_id":"ah-q4dt.11","github_mirror":null,"linked_at":"2026-09-04T02:07:59Z","sync_state":"linked"}
github_publication: {"labels":[],"milestone":null,"mode":"local_only","project_aliases":[]}
github_project_linkages: []
pull_request_linkages: []
execution_contexts: []
completion_evidence: {"completed_at":"2026-09-04T10:14:33Z","evidence_refs":["beads:ah-q4dt.11"],"policy":"manual","reconciled_at":"2026-09-09T00:00:00Z","source":"reconciliation","status":"done"}
implementation_readiness: {"checked_at":"2026-09-04T00:00:00Z","missing_sections":[],"status":"complete"}
---

# System task overlay: 日次ロールアップの証跡集約
...
- parent_feature: feat-blog-metrics-rollup
- phase_ref: P11
- classification: confidence=1.0; reason=feat-blog-metrics-rollup の P11 lifecycle 責務への確定写像; candidate=tasks/feat-blog-metrics-rollup/sys-blog-metrics-rollup-p11.md
...
受入判定と非機能検査の証跡を一箇所へ集約し、後から第三者が受入9件の成立と指標の意味を再確認できる状態を成立させる。

## 背景

数字の根拠は運用中に問われる。証跡と指標定義を一枚から辿れるようにしておく。
...
- Produced artifacts: docs/spec/feat-blog-metrics-rollup/evidence-index.md (受入9件と証跡ファイル・コマンドの対応表)
...
- Write scope/touches: docs/spec/feat-blog-metrics-rollup/evidence-index.md
...
- PR body contract: Closes #issue (beads issue) + dev-graph graph_node_id=SYS-BLOG-METRICS-ROLLUP-P11; PR の宛先は dev ブランチとする
- Ownership boundary: system-dev-planner は intent のみを宣言し、dev-graph が tracker mutation と reconciliation を行う

## Branch and worktree execution

- Branch: assigned after dev-graph registration by C15 as devgraph/SYS-BLOG-METRICS-ROLLUP-P11; system-dev-planner は事前割当を行わない
- Worktree lease: claim SYS-BLOG-METRICS-ROLLUP-P11 before implementation; heartbeat/release は dev-graph worktree lease 契約に従う
...
- Goal: 受入判定と非機能検査の証跡を一箇所へ集約し、後から第三者が受入9件の成立と指標の意味を再確認できる状態を成立させる。
- Generic execution prompt: feat-blog-metrics-rollup の goal (記事ごとの売上と PV、滞在・スクロール到達・クリック率が同じ日次行に載り、ブログ単位へ集計され、同じ日を何度処理しても結果が置き換わるだけで二重計上せず、標本数が少ない行は推測に足りないと判る状態になっている) と本 task の 前提条件/成果物/write scope/スコープ外 を渡し、実装手段は固定せず P11 の目的を満たす成果物を作らせる
...
- Dependencies: SYS-BLOG-METRICS-ROLLUP-P07, SYS-BLOG-METRICS-ROLLUP-P09
