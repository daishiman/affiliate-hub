---
graph_node_id: "SYS-REFERENCE-BLOG-ADMIN-UX-P06"
artifact_kind: "task"
artifact_subtypes: []
project_id: "feature-package-feat-reference-blog-admin-ux"
domain: "quality"
tags: ["p06","feat-reference-blog-admin-ux"]
priority: null
start_date: null
target_date: null
iteration: null
title: "単体・結合・E2E・securityテストの実行と緑化"
owners: ["daishiman"]
created_at: "2026-08-29T14:56:20Z"
updated_at: "2026-09-08T08:15:56.232583Z"
status: "active"
depends_on: ["SYS-REFERENCE-BLOG-ADMIN-UX-P05"]
related_nodes: ["spec-system-spec-index","arch-system-spec-overview"]
resource_scope: ["docs/spec/feat-reference-blog-admin-ux/test-run-report.md","test-results/reference-blog-admin-ux/"]
...
scope_in: ["Produced artifacts: 再現可能なtest run reportとmachine-readable結果","Consumed artifacts: features/feat-reference-blog-admin-ux.md, features/feat-reference-blog-admin-ux.context.json, system-spec/ui-ux.md, system-spec/frontend.md, SYS-REFERENCE-BLOG-ADMIN-UX-P05","Write scope/touches: docs/spec/feat-reference-blog-admin-ux/test-run-report.md, test-results/reference-blog-admin-ux/"]
...
acceptance: ["Automated commands: `python3 .claude/plugins/system-dev-planner/scripts/validate-system-plan.py --repo-root . --feature-package feature-package/feat-reference-blog-admin-ux`","Automated commands: `pnpm typecheck`、`pnpm content:validate`、対象test suite。実装を持たないphaseは適用外理由をreportへ残す","Required evidence: docs/spec/feat-reference-blog-admin-ux/test-run-report.md, test-results/reference-blog-admin-ux/","Acceptance state: P06: typecheck、content validation、unit、contract、DB integration、E2E、a11y、security、performance、non-copying gateが失敗0件で、各command・exit・件数・artifact pathがtest-run-reportに記録される。"]
architecture_refs: ["arch-system-spec-overview","arch-two-layer-platform"]
parent_feature: "feat-reference-blog-admin-ux"
feature_package_id: "feature-package/feat-reference-blog-admin-ux"
phase_ref: "P06"
file_path: "tasks/feat-reference-blog-admin-ux/sys-reference-blog-admin-ux-p06.md"
template_id: "task"
...
classification_reason: "feat-reference-blog-admin-ux の P06 lifecycle責務への確定写像"
classification_candidates: [{"artifact_kind":"task","candidate_path":"tasks/feat-reference-blog-admin-ux/sys-reference-blog-admin-ux-p06.md","confidence":1}]
issue_linkage: null
tracker_binding: "beads"
beads_linkage: {"bd_issue_id":"ah-z8x6.6","github_mirror":null,"linked_at":"2026-09-08T08:14:07Z","sync_state":"linked"}
...
# System task overlay: 単体・結合・E2E・securityテストの実行と緑化
...
- parent_feature: feat-reference-blog-admin-ux
- phase_ref: P06
- classification: confidence=1.0; reason=feat-reference-blog-admin-uxのP06 lifecycle責務への確定写像; candidate=tasks/feat-reference-blog-admin-ux/sys-reference-blog-admin-ux-p06.md
...
- Frontend: applicable; public/admin behavior E2Eを実行する
- Backend: applicable; API 契約・ロジック単体を実行する
- API: applicable; preview error taxonomyをcontract testする
- Data: applicable; DB 結合・migrationを実行する
- Infrastructure: applicable; IaC静的検証とsmokeを実行する
- Security: applicable; SSRF・認可negativeを実行する
- Quality: applicable; 全suiteを集約して失敗0を確認する
- Documentation: applicable; commandとresultを記録する
- Operations: applicable; retryとrollback演習結果を記録する
...
- Write scope/touches: docs/spec/feat-reference-blog-admin-ux/test-run-report.md, test-results/reference-blog-admin-ux/
...
- PR body contract: Beads issue参照とdev-graph graph_node_id=SYS-REFERENCE-BLOG-ADMIN-UX-P06を記載し、target branchはdevとする
- Ownership boundary: system-dev-plannerはintentを宣言し、dev-graphが起票・依存・完了収束を所有する

## Branch and worktree execution

- Branch: dev-graph登録後にC15がdevgraph/SYS-REFERENCE-BLOG-ADMIN-UX-P06として割り当てる
- Worktree lease: 実装開始前にSYS-REFERENCE-BLOG-ADMIN-UX-P06をclaimし、heartbeatとreleaseをlease契約どおり行う
...
- Required evidence: docs/spec/feat-reference-blog-admin-ux/test-run-report.md, test-results/reference-blog-admin-ux/
- Acceptance state: P06: typecheck、content validation、unit、contract、DB integration、E2E、a11y、security、performance、non-copying gateが失敗0件で、各command・exit・件数・artifact pathがtest-run-reportに記録される。
...
- Completion condition: P06: typecheck、content validation、unit、contract、DB integration、E2E、a11y、security、performance、non-copying gateが失敗0件で、各command・exit・件数・artifact pathがtest-run-reportに記録される。
