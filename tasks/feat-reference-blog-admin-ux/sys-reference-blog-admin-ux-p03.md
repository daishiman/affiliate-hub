---
graph_node_id: "SYS-REFERENCE-BLOG-ADMIN-UX-P03"
artifact_kind: "task"
artifact_subtypes: []
project_id: "feature-package-feat-reference-blog-admin-ux"
domain: "quality"
tags: ["p03","feat-reference-blog-admin-ux"]
priority: null
start_date: null
target_date: null
iteration: null
title: "設計の独立レビューと非模倣・安全性ゲート"
owners: ["daishiman"]
created_at: "2026-08-29T14:56:20Z"
updated_at: "2026-09-08T08:14:34.926674Z"
status: "active"
depends_on: ["SYS-REFERENCE-BLOG-ADMIN-UX-P02"]
related_nodes: ["spec-system-spec-index","arch-system-spec-overview"]
resource_scope: ["docs/spec/feat-reference-blog-admin-ux/design-review.md"]
purpose: "P02を生成者と分けてレビューし、認知負荷、情報階層、著作物非転用、affiliate previewのSSRF/権利、既存feature境界、A1–A12被覆の設計欠陥を実装前に止める。"
goal: "P02を生成者と分けてレビューし、認知負荷、情報階層、著作物非転用、affiliate previewのSSRF/権利、既存feature境界、A1–A12被覆の設計欠陥を実装前に止める。"
scope_in: ["Produced artifacts: 独立design-review記録と修正済み設計digest","Consumed artifacts: features/feat-reference-blog-admin-ux.md, features/feat-reference-blog-admin-ux.context.json, system-spec/ui-ux.md, system-spec/frontend.md, SYS-REFERENCE-BLOG-ADMIN-UX-P02","Write scope/touches: docs/spec/feat-reference-blog-admin-ux/design-review.md"]
...
acceptance: ["Automated commands: `python3 .claude/plugins/system-dev-planner/scripts/validate-system-plan.py --repo-root . --feature-package feature-package/feat-reference-blog-admin-ux`","Automated commands: `pnpm typecheck`、`pnpm content:validate`、対象test suite。実装を持たないphaseは適用外理由をreportへ残す","Required evidence: docs/spec/feat-reference-blog-admin-ux/design-review.md","Acceptance state: P03: C1矛盾なし・C2漏れなし・C3整合性あり・C4依存整合に加え、独自token、文章/写真/ロゴ/固有名/色値の転用0、公開URL境界、SSRF防御、single primary action、保存状態の見分け、affiliate reverse lookupの全チェックを証拠付きPASSにする。"]
architecture_refs: ["arch-system-spec-overview","arch-two-layer-platform"]
parent_feature: "feat-reference-blog-admin-ux"
feature_package_id: "feature-package/feat-reference-blog-admin-ux"
phase_ref: "P03"
file_path: "tasks/feat-reference-blog-admin-ux/sys-reference-blog-admin-ux-p03.md"
template_id: "task"
...
classification_reason: "feat-reference-blog-admin-ux の P03 lifecycle責務への確定写像"
classification_candidates: [{"artifact_kind":"task","candidate_path":"tasks/feat-reference-blog-admin-ux/sys-reference-blog-admin-ux-p03.md","confidence":1}]
issue_linkage: null
tracker_binding: "beads"
beads_linkage: {"bd_issue_id":"ah-z8x6.3","github_mirror":null,"linked_at":"2026-09-08T08:14:07Z","sync_state":"linked"}
...
# System task overlay: 設計の独立レビューと非模倣・安全性ゲート
...
- parent_feature: feat-reference-blog-admin-ux
- phase_ref: P03
- classification: confidence=1.0; reason=feat-reference-blog-admin-uxのP03 lifecycle責務への確定写像; candidate=tasks/feat-reference-blog-admin-ux/sys-reference-blog-admin-ux-p03.md
...
P02を生成者と分けてレビューし、認知負荷、情報階層、著作物非転用、affiliate previewのSSRF/権利、既存feature境界、A1–A12被覆の設計欠陥を実装前に止める。

## 背景

参照ブログの14サブサイトマップと公開URL 1,072件のベースライン、画面型別の共通構成、現行管理画面の実装済み機能とgapを、features/feat-reference-blog-admin-ux.mdのA1–A12へtraceする。情報階層と操作原則だけを抽象化し、第三者の文章・写真・ロゴ・固有名・色値を転用しない。本phaseはSYS-REFERENCE-BLOG-ADMIN-UX-P02の確定成果物を入力にする。

## 前提条件

- Required spec/architecture/phase/task nodes: feat-reference-blog-admin-ux, spec-system-spec-index, arch-system-spec-overview, SYS-REFERENCE-BLOG-ADMIN-UX-P02
...
- Frontend: applicable; 画面階層とfocus orderを意味レビューする
- Backend: applicable; 状態遷移と失敗復旧をレビューする
- API: applicable; preview APIの入出力とerror taxonomyをレビューする
- Data: applicable; snapshotとrevision整合をレビューする
- Infrastructure: N/A: 配備変更なし
- Security: applicable; SSRF・権利・アクセス境界を独立レビューする
- Quality: applicable; C1–C4とA1–A12被覆を採点する
- Documentation: applicable; verdictとfindingを記録する
- Operations: applicable; 再試行・手動補完・監査をレビューする
...
- Consumed artifacts: features/feat-reference-blog-admin-ux.md, features/feat-reference-blog-admin-ux.context.json, system-spec/ui-ux.md, system-spec/frontend.md, SYS-REFERENCE-BLOG-ADMIN-UX-P02
- Write scope/touches: docs/spec/feat-reference-blog-admin-ux/design-review.md
...
- PR body contract: Beads issue参照とdev-graph graph_node_id=SYS-REFERENCE-BLOG-ADMIN-UX-P03を記載し、target branchはdevとする
- Ownership boundary: system-dev-plannerはintentを宣言し、dev-graphが起票・依存・完了収束を所有する

## Branch and worktree execution

- Branch: dev-graph登録後にC15がdevgraph/SYS-REFERENCE-BLOG-ADMIN-UX-P03として割り当てる
- Worktree lease: 実装開始前にSYS-REFERENCE-BLOG-ADMIN-UX-P03をclaimし、heartbeatとreleaseをlease契約どおり行う
...
- Required evidence: docs/spec/feat-reference-blog-admin-ux/design-review.md
- Acceptance state: P03: C1矛盾なし・C2漏れなし・C3整合性あり・C4依存整合に加え、独自token、文章/写真/ロゴ/固有名/色値の転用0、公開URL境界、SSRF防御、single primary action、保存状態の見分け、affiliate reverse lookupの全チェックを証拠付きPASSにする。
...
- Completion condition: P03: C1矛盾なし・C2漏れなし・C3整合性あり・C4依存整合に加え、独自token、文章/写真/ロゴ/固有名/色値の転用0、公開URL境界、SSRF防御、single primary action、保存状態の見分け、affiliate reverse lookupの全チェックを証拠付きPASSにする。
...
- Dependencies: SYS-REFERENCE-BLOG-ADMIN-UX-P02
