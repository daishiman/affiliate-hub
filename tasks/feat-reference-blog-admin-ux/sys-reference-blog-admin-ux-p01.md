...
purpose: "14サブサイトマップの全公開canonical URLを重複除外した台帳、画面型分類、型ごとの代表詳細解析、現行の公開面・管理面gap、A1–A12対応表を同じ画面IDと要件IDで確定する。"
goal: "14サブサイトマップの全公開canonical URLを重複除外した台帳、画面型分類、型ごとの代表詳細解析、現行の公開面・管理面gap、A1–A12対応表を同じ画面IDと要件IDで確定する。"
scope_in: ["Produced artifacts: 要求ベースライン、全URL台帳、sitemap snapshot、画面型解析、詳細画面一覧、現行画面gap ledger、A1–A12 traceability","Consumed artifacts: features/feat-reference-blog-admin-ux.md, features/feat-reference-blog-admin-ux.context.json, system-spec/ui-ux.md, system-spec/frontend.md","Write scope/touches: docs/spec/feat-reference-blog-admin-ux/requirements-baseline.md, docs/spec/feat-reference-blog-admin-ux/reference-url-inventory.json, docs/spec/feat-reference-blog-admin-ux/sitemap-snapshot.json, docs/spec/feat-reference-blog-admin-ux/page-archetype-analysis.md, docs/spec/feat-reference-blog-admin-ux/screen-inventory.md, docs/spec/feat-reference-blog-admin-ux/current-screen-gap-ledger.md, docs/spec/feat-reference-blog-admin-ux/acceptance-traceability.json"]
...
acceptance: ["Automated commands: `python3 .claude/plugins/system-dev-planner/scripts/validate-system-plan.py --repo-root . --feature-package feature-package/feat-reference-blog-admin-ux`","Automated commands: `pnpm typecheck`、`pnpm content:validate`、対象test suite。実装を持たないphaseは適用外理由をreportへ残す","Required evidence: docs/spec/feat-reference-blog-admin-ux/requirements-baseline.md, docs/spec/feat-reference-blog-admin-ux/reference-url-inventory.json, docs/spec/feat-reference-blog-admin-ux/sitemap-snapshot.json, docs/spec/feat-reference-blog-admin-ux/page-archetype-analysis.md, docs/spec/feat-reference-blog-admin-ux/screen-inventory.md, docs/spec/feat-reference-blog-admin-ux/current-screen-gap-ledger.md, docs/spec/feat-reference-blog-admin-ux/acceptance-traceability.json","Acceptance state: P01: 14サブサイトマップと1,072件ベースラインを再取得可能なsitemap-snapshotとURL台帳で検算でき、全URLの画面型未分類0件、各画面型のdesktop/mobile代表解析あり、A1–A12と画面・データ・証跡の未対応0件、現行画面gapの根拠URLまたはローカルroute欠落0件の状態を成立させる。"]
...
- Automated commands: `pnpm run check:reference-reuse`（= `node scripts/check-reference-site-reuse.mjs .`）。参考サイトの実ホスト・実URL・他所のCMS由来の語形・生の色値が本phaseの成果物へ戻っていないことと、走査対象そのものが痩せていないこと（母集団に対する逆向きの被覆検査）を見る。`quality-gates.config.mjs` の tier1 / blocking にも同じ検査を登録済みで、手元と機械で同じ1つの定義を使う
- Automated commands: `python3 -m pytest scripts/reference-site-analysis/ -q` と `python3 scripts/reference-site-analysis/verify_evidence_index.py`。URL台帳の分類と証跡indexのdigestを検算する
