...
target_date: null
iteration: null
title: "どの性質からも指されていないテスト種別 5 つを性質へ結ぶ（ssrf / decision-table / audit-log / db-migration / property）"
owners: ["daishiman"]
...
purpose: null
goal: null
mvp_alignment: {"background":"ah-0ip で secrets は片付いた（has-secret を語彙へ足し、REQ-SEC10 を宣言し、リポジトリに秘密が載っていないかを毎回読む検査を書いた）。同じ食い違いが 5 種別に残っている","mvp_fit":"enabling","purpose":"種別の一覧にあるだけで一度も要求されない種別を無くす","rationale":"どの性質からも指されない種別は、書かなくても検査が緑になる。いま印が付いているのは書いた人の善意によるもので、次に書かれる保証が無い"}
scope_in: []
scope_out: []
acceptance: []
architecture_refs: []
...
classification_reason: "ah-0ip の手順 6「指されない種別が他にも残るなら、その一覧と理由を §4 に書く」で洗い出した 5 件を切り出したもの"
...
残っているのは 5 つ（`ssrf` / `decision-table` / `audit-log` / `db-migration` / `property`）。
...
そのとき効いた手順が「**足す前に数える**」で、これを 5 件でも守る。
...
- 5 種別のそれぞれについて、**性質へ結んだ**か、**結ばない理由を書いた**かのどちらか
...
`db-migration` → `audit-log` → `property`（対象が狭い順。`property` は最後に、
そもそも性質へ結べるかの判断から入る）。
