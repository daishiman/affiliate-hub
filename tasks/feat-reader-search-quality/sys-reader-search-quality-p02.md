---
graph_node_id: "SYS-READER-SEARCH-QUALITY-P02"
artifact_kind: "task"
artifact_subtypes: []
project_id: "feature-package-feat-reader-search-quality"
domain: "documentation"
tags: ["p02","feat-reader-search-quality"]
priority: null
start_date: null
target_date: null
iteration: null
title: "FTS5 trigram索引・順位付け・URL契約・画面/WebMCP同一ユースケースの設計"
owners: ["daishiman"]
created_at: "2026-09-04T04:37:54Z"
updated_at: "2026-09-04T04:37:54Z"
status: "active"
depends_on: ["SYS-READER-SEARCH-QUALITY-P01"]
related_nodes: ["spec-system-spec-index","arch-system-spec-overview"]
resource_scope: ["docs/spec/feat-reader-search-quality/data-model.md","docs/spec/feat-reader-search-quality/api-contract.md","docs/spec/feat-reader-search-quality/ranking-contract.md","docs/spec/feat-reader-search-quality/url-contract.md","docs/spec/feat-reader-search-quality/progressive-enhancement-contract.md","docs/spec/feat-reader-search-quality/webmcp-parity-contract.md"]
purpose: "FTS5 trigram索引・順位付け・検索結果面・URL契約・画面とWebMCP道具searchArticlesの同一ユースケース化を設計する。"
goal: "FTS5 trigram索引・順位付け・検索結果面・URL契約・画面とWebMCP道具searchArticlesの同一ユースケース化を設計する。"
scope_in: ["Produced artifacts: FTS5 trigram索引契約、順位付け契約、検索結果面API契約、URL契約、progressive enhancement契約、WebMCP同一ユースケース契約","Consumed artifacts: features/feat-reader-search-quality.md, features/feat-reader-search-quality.context.json, system-spec/backend.md, system-spec/database.md, system-spec/retrieval-evidence/kajetblog-top-analysis.md, SYS-READER-SEARCH-QUALITY-P01","Write scope/touches: docs/spec/feat-reader-search-quality/data-model.md, docs/spec/feat-reader-search-quality/api-contract.md, docs/spec/feat-reader-search-quality/ranking-contract.md, docs/spec/feat-reader-search-quality/url-contract.md, docs/spec/feat-reader-search-quality/progressive-enhancement-contract.md, docs/spec/feat-reader-search-quality/webmcp-parity-contract.md"]
...
phase_ref: "P02"
file_path: "tasks/feat-reader-search-quality/sys-reader-search-quality-p02.md"
template_id: "task"
...
source_lineage: {"imported_at":"2026-09-04T04:37:54Z","origin_kind":"system-dev-planner","source_digest":"b55948e2e68707fa2a8b09dbfe1319c1b799fb956a2f6eaa4f667865b9534d5a","source_path":".dev-graph/published/feature-package-feat-reader-search-quality/task-specs/phase-02-architecture.md","source_plugin":"system-dev-planner","source_version":"0.1.0"}
classification_confidence: 1.0
classification_reason: "feat-reader-search-qualityのP02 lifecycle責務への確定写像"
classification_candidates: [{"artifact_kind":"task","candidate_path":"tasks/feat-reader-search-quality/sys-reader-search-quality-p02.md","confidence":1.0}]
issue_linkage: null
tracker_binding: "beads"
beads_linkage: {"bd_issue_id":"ah-qbzy","github_mirror":null,"linked_at":"2026-09-04T07:33:45Z","sync_state":"linked"}
...
# System task overlay: FTS5 trigram索引・順位付け・URL契約・画面/WebMCP同一ユースケースの設計
...
- parent_feature: feat-reader-search-quality
- phase_ref: P02
- classification: confidence=1.0; reason=feat-reader-search-qualityのP02 lifecycle責務への確定写像; candidate=tasks/feat-reader-search-quality/sys-reader-search-quality-p02.md
...
FTS5 trigram索引・順位付け・検索結果面・URL契約・画面とWebMCP道具searchArticlesの同一ユースケース化を設計する。

## 背景

読者が言葉から記事へ到達できるようにし、検索窓はあるのに index が無く実質使えない状態を解消する。現状の searchArticles ユースケースは published_articles の題名・要約に対する LIKE 部分一致に留まり、本文は検索されず関連度・新しさによる順位付けも無い。D1 上の FTS5 trigram 索引を公開記事から自動で保ち、日本語の部分一致でも妥当な順で結果が返り、検索結果面がサムネイル付きで表示され、JavaScript が無効でも検索が完了し、画面の検索と WebMCP 道具 searchArticles が同一ユースケースを通り、検索の入口が構造化データと llms.txt にも表れている状態にする。本phaseはSYS-READER-SEARCH-QUALITY-P01の確定成果物を入力にする。

## 前提条件

- Required spec/architecture/phase/task nodes: feat-reader-search-quality, spec-system-spec-index, arch-system-spec-overview, SYS-READER-SEARCH-QUALITY-P01
...
- Frontend: applicable; 検索結果面(抜粋・サムネイル・カテゴリー・公開日・0件時代替提案)のUI契約を設計する
- Backend: applicable; FTS5索引migration・順位付け・2文字以下フォールバックのusecase契約を設計する
- API: applicable; 検索ページURL契約(query parameter名・応答形)とWebMCP道具searchArticlesの契約を設計する
- Data: applicable; D1 FTS5 trigram仮想表のmigration・同一トランザクション追随契約を設計する
- Infrastructure: N/A: 配備変更を伴わない設計phase
- Security: applicable; 検索語のliteral化(FTS構文非解釈)と負荷上限・ページング契約を設計する
- Quality: applicable; A1-A8をテストへ写像する
- Documentation: applicable; 実行条件と判定式を記録する
- Operations: applicable; 索引再構築手順の設計を残す
...
- Produced artifacts: FTS5 trigram索引契約、順位付け契約、検索結果面API契約、URL契約、progressive enhancement契約、WebMCP同一ユースケース契約
- Consumed artifacts: features/feat-reader-search-quality.md, features/feat-reader-search-quality.context.json, system-spec/backend.md, system-spec/database.md, system-spec/retrieval-evidence/kajetblog-top-analysis.md, SYS-READER-SEARCH-QUALITY-P01
...
- PR body contract: Beads issue参照とdev-graph graph_node_id=SYS-READER-SEARCH-QUALITY-P02を記載し、target branchはdevとする
- Ownership boundary: system-dev-plannerはintentを宣言し、dev-graphが起票・依存・完了収束を所有する

## Branch and worktree execution

- Branch: dev-graph登録後にC15がdevgraph/SYS-READER-SEARCH-QUALITY-P02として割り当てる
- Worktree lease: 実装開始前にSYS-READER-SEARCH-QUALITY-P02をclaimし、heartbeatとreleaseをlease契約どおり行う
...
- Acceptance state: P02: FTS5 trigram索引(題名・要約・本文の3列、ブログ識別子・公開状態を列として持つ)のD1 migration契約、公開・更新・削除・非公開化の同一トランザクション追随契約、2文字以下の問い合わせを題名前方一致とカテゴリー誘導へ切り替える境界契約、関連度(FTS5既定rank)→公開日の新しさ→カテゴリー一致を合成する順位契約、検索結果面(件数・一致箇所抜粋・サムネイル・カテゴリー・公開日・0件時代替提案)のAPI契約、検索ページのURL契約(query parameter名・応答形)とWebSite.potentialAction(SearchAction)・llms.txtとの整合契約、画面のsearchArticlesユースケースとWebMCP道具searchArticlesが同一usecaseを通る契約、検索語のliteral化(FTS構文非解釈)と負荷上限・ページング契約をA1-A8欠落なく設計する。
...
- Rollout: FTS5 trigram索引契約、順位付け契約、検索結果面API契約、URL契約、progressive enhancement契約、WebMCP同一ユースケース契約をwrite_scope内へ適用し、検証PASS後に依存する次phaseへ渡す
...
- Completion condition: P02: FTS5 trigram索引(題名・要約・本文の3列、ブログ識別子・公開状態を列として持つ)のD1 migration契約、公開・更新・削除・非公開化の同一トランザクション追随契約、2文字以下の問い合わせを題名前方一致とカテゴリー誘導へ切り替える境界契約、関連度(FTS5既定rank)→公開日の新しさ→カテゴリー一致を合成する順位契約、検索結果面(件数・一致箇所抜粋・サムネイル・カテゴリー・公開日・0件時代替提案)のAPI契約、検索ページのURL契約(query parameter名・応答形)とWebSite.potentialAction(SearchAction)・llms.txtとの整合契約、画面のsearchArticlesユースケースとWebMCP道具searchArticlesが同一usecaseを通る契約、検索語のliteral化(FTS構文非解釈)と負荷上限・ページング契約をA1-A8欠落なく設計する。
...
- Dependencies: SYS-READER-SEARCH-QUALITY-P01
