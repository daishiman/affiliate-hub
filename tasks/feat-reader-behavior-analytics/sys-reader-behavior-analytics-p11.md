---
graph_node_id: "SYS-READER-BEHAVIOR-ANALYTICS-P11"
artifact_kind: "task"
artifact_subtypes: []
project_id: "feature-package-feat-reader-behavior-analytics"
domain: "documentation"
tags: ["p11","feat-reader-behavior-analytics"]
priority: null
start_date: null
target_date: null
iteration: null
...
scope_in: ["Produced artifacts: docs/spec/feat-reader-behavior-analytics/evidence-index.md (受入10件と証跡ファイル・コマンドの対応表)","Consumed artifacts: features/feat-reader-behavior-analytics.md; features/feat-reader-behavior-analytics.context.json; system-spec/frontend.md; system-spec/database.md; system-spec/security.md","Write scope/touches: docs/spec/feat-reader-behavior-analytics/evidence-index.md"]
scope_out: ["1 読者の行動を時系列で再生する機能 (作らない)","日次ロールアップと収益・PV との突合 (feat-blog-metrics-rollup が所有する)","行動指標を使った示唆生成と提示順序 (feat-blog-scoped-admin-console が所有する)","既存の汎用イベント/KPI 基盤そのもの (feat-analytics-insight が所有する)","P11 以外の phase が所有する成果物への変更"]
...
phase_ref: "P11"
file_path: "tasks/feat-reader-behavior-analytics/sys-reader-behavior-analytics-p11.md"
template_id: "task"
...
classification_reason: "feat-reader-behavior-analytics の P11 lifecycle 責務への確定写像"
classification_candidates: [{"artifact_kind":"task","candidate_path":"tasks/feat-reader-behavior-analytics/sys-reader-behavior-analytics-p11.md","confidence":1.0}]
issue_linkage: null
tracker_binding: "beads"
beads_linkage: {"bd_issue_id":"ah-f4do.11","github_mirror":null,"linked_at":"2026-09-04T02:07:04Z","sync_state":"linked"}
...
# System task overlay: 読者行動計測の証跡集約
...
- parent_feature: feat-reader-behavior-analytics
- phase_ref: P11
- classification: confidence=1.0; reason=feat-reader-behavior-analytics の P11 lifecycle 責務への確定写像; candidate=tasks/feat-reader-behavior-analytics/sys-reader-behavior-analytics-p11.md
...
非追跡の担保は後から問われやすい。証跡を集約しておくことで、問われた時点で再実行可能な形の答えを提示できる。
...
- Produced artifacts: docs/spec/feat-reader-behavior-analytics/evidence-index.md (受入10件と証跡ファイル・コマンドの対応表)
...
- Write scope/touches: docs/spec/feat-reader-behavior-analytics/evidence-index.md
...
- PR body contract: Closes #issue (beads issue) + dev-graph graph_node_id=SYS-READER-BEHAVIOR-ANALYTICS-P11; PR の宛先は dev ブランチとする
- Ownership boundary: system-dev-planner は intent のみを宣言し、dev-graph が tracker mutation と reconciliation を行う

## Branch and worktree execution

- Branch: assigned after dev-graph registration by C15 as devgraph/SYS-READER-BEHAVIOR-ANALYTICS-P11; system-dev-planner は事前割当を行わない
- Worktree lease: claim SYS-READER-BEHAVIOR-ANALYTICS-P11 before implementation; heartbeat/release は dev-graph worktree lease 契約に従う
...
- Goal: 受入判定と非機能検査の証跡を一箇所へ集約し、後から第三者が受入10件の成立を再確認できる状態を成立させる。
- Generic execution prompt: feat-reader-behavior-analytics の goal (読者面が滞在・スクロール到達・要素クリック・ポインタ標本を要素相対比率で束ねて送り、reader_interaction_events へ追記され、管理画面が viewport_bucket ごとの集計分布としてヒートマップを描き、同意が無い読者は reader_key を持たず 90 日で生データが消える状態になっている) と本 task の 前提条件/成果物/write scope/スコープ外 を渡し、実装手段は固定せず P11 の目的を満たす成果物を作らせる
...
- Dependencies: SYS-READER-BEHAVIOR-ANALYTICS-P07, SYS-READER-BEHAVIOR-ANALYTICS-P09
