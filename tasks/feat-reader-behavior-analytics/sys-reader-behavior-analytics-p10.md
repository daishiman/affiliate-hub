---
graph_node_id: "SYS-READER-BEHAVIOR-ANALYTICS-P10"
artifact_kind: "task"
artifact_subtypes: []
project_id: "feature-package-feat-reader-behavior-analytics"
domain: "quality"
tags: ["p10","feat-reader-behavior-analytics"]
priority: null
start_date: null
target_date: null
iteration: null
...
status: "closed"
depends_on: ["SYS-READER-BEHAVIOR-ANALYTICS-P09"]
related_nodes: []
resource_scope: ["docs/spec/feat-reader-behavior-analytics/final-review.md"]
purpose: "P01 から P09 までの成果物を通しでレビューし、要求で宣言した非追跡の制約が実装とクエリの両方で守られていることを確認して残課題を確定した状態を成立させる。"
goal: "P01 から P09 までの成果物を通しでレビューし、要求で宣言した非追跡の制約が実装とクエリの両方で守られていることを確認して残課題を確定した状態を成立させる。"
scope_in: ["Produced artifacts: docs/spec/feat-reader-behavior-analytics/final-review.md (通しレビューの結果と残課題)","Consumed artifacts: features/feat-reader-behavior-analytics.md; features/feat-reader-behavior-analytics.context.json; system-spec/frontend.md; system-spec/database.md; system-spec/security.md","Write scope/touches: docs/spec/feat-reader-behavior-analytics/final-review.md"]
scope_out: ["1 読者の行動を時系列で再生する機能 (作らない)","日次ロールアップと収益・PV との突合 (feat-blog-metrics-rollup が所有する)","行動指標を使った示唆生成と提示順序 (feat-blog-scoped-admin-console が所有する)","既存の汎用イベント/KPI 基盤そのもの (feat-analytics-insight が所有する)","P10 以外の phase が所有する成果物への変更"]
...
phase_ref: "P10"
file_path: "tasks/feat-reader-behavior-analytics/sys-reader-behavior-analytics-p10.md"
template_id: "task"
...
classification_reason: "feat-reader-behavior-analytics の P10 lifecycle 責務への確定写像"
classification_candidates: [{"artifact_kind":"task","candidate_path":"tasks/feat-reader-behavior-analytics/sys-reader-behavior-analytics-p10.md","confidence":1.0}]
issue_linkage: null
tracker_binding: "beads"
beads_linkage: {"bd_issue_id":"ah-f4do.10","github_mirror":null,"linked_at":"2026-09-04T02:07:01Z","sync_state":"linked"}
github_publication: {"labels":[],"milestone":null,"mode":"local_only","project_aliases":[]}
github_project_linkages: []
pull_request_linkages: []
execution_contexts: []
completion_evidence: {"completed_at":"2026-09-04T10:31:17Z","evidence_refs":["beads:ah-f4do.10"],"policy":"manual","reconciled_at":"2026-09-09T00:00:00Z","source":"reconciliation","status":"done"}
implementation_readiness: {"checked_at":"2026-09-04T00:00:00Z","missing_sections":[],"status":"complete"}
---

# System task overlay: 読者行動計測の最終レビュー
...
- parent_feature: feat-reader-behavior-analytics
- phase_ref: P10
- classification: confidence=1.0; reason=feat-reader-behavior-analytics の P10 lifecycle 責務への確定写像; candidate=tasks/feat-reader-behavior-analytics/sys-reader-behavior-analytics-p10.md
...
P01 から P09 までの成果物を通しでレビューし、要求で宣言した非追跡の制約が実装とクエリの両方で守られていることを確認して残課題を確定した状態を成立させる。

## 背景

非追跡の制約は複数の phase に分散して実現される。最終レビューで、制約一覧の各項目が実際にどの実装で担保されているかを突き合わせる。
...
- Produced artifacts: docs/spec/feat-reader-behavior-analytics/final-review.md (通しレビューの結果と残課題)
...
- Write scope/touches: docs/spec/feat-reader-behavior-analytics/final-review.md
...
- PR body contract: Closes #issue (beads issue) + dev-graph graph_node_id=SYS-READER-BEHAVIOR-ANALYTICS-P10; PR の宛先は dev ブランチとする
- Ownership boundary: system-dev-planner は intent のみを宣言し、dev-graph が tracker mutation と reconciliation を行う

## Branch and worktree execution

- Branch: assigned after dev-graph registration by C15 as devgraph/SYS-READER-BEHAVIOR-ANALYTICS-P10; system-dev-planner は事前割当を行わない
- Worktree lease: claim SYS-READER-BEHAVIOR-ANALYTICS-P10 before implementation; heartbeat/release は dev-graph worktree lease 契約に従う
...
- Goal: P01 から P09 までの成果物を通しでレビューし、要求で宣言した非追跡の制約が実装とクエリの両方で守られていることを確認して残課題を確定した状態を成立させる。
- Generic execution prompt: feat-reader-behavior-analytics の goal (読者面が滞在・スクロール到達・要素クリック・ポインタ標本を要素相対比率で束ねて送り、reader_interaction_events へ追記され、管理画面が viewport_bucket ごとの集計分布としてヒートマップを描き、同意が無い読者は reader_key を持たず 90 日で生データが消える状態になっている) と本 task の 前提条件/成果物/write scope/スコープ外 を渡し、実装手段は固定せず P10 の目的を満たす成果物を作らせる
...
- Dependencies: SYS-READER-BEHAVIOR-ANALYTICS-P09
