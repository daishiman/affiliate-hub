---
graph_node_id: "SYS-READER-BEHAVIOR-ANALYTICS-P12"
artifact_kind: "task"
artifact_subtypes: []
project_id: "feature-package-feat-reader-behavior-analytics"
domain: "documentation"
tags: ["p12","feat-reader-behavior-analytics"]
priority: null
start_date: null
target_date: null
iteration: null
...
purpose: "何を計測し何を計測しないか、保持期間はどれだけか、reader_key 指定の抽出・削除をどう実行するかを、運用手順と利用者向け説明として整えた状態を成立させる。"
goal: "何を計測し何を計測しないか、保持期間はどれだけか、reader_key 指定の抽出・削除をどう実行するかを、運用手順と利用者向け説明として整えた状態を成立させる。"
scope_in: ["Produced artifacts: docs/spec/feat-reader-behavior-analytics/operations-runbook.md (保持期間削除と抽出・削除要求への対応手順・audit_logs での実行記録の確認方法); docs/ 配下の計測項目と保持期間の説明","Consumed artifacts: features/feat-reader-behavior-analytics.md; features/feat-reader-behavior-analytics.context.json; system-spec/frontend.md; system-spec/database.md; system-spec/security.md","Write scope/touches: docs/spec/feat-reader-behavior-analytics/operations-runbook.md, docs/spec/feat-reader-behavior-analytics/"]
scope_out: ["1 読者の行動を時系列で再生する機能 (作らない)","日次ロールアップと収益・PV との突合 (feat-blog-metrics-rollup が所有する)","行動指標を使った示唆生成と提示順序 (feat-blog-scoped-admin-console が所有する)","既存の汎用イベント/KPI 基盤そのもの (feat-analytics-insight が所有する)","P12 以外の phase が所有する成果物への変更"]
acceptance: ["Required evidence: operations-runbook.md が保持期間削除と抽出・削除要求の両方に手順を持つこと","Automated commands: `pnpm run lint` (文書内のコード片が静的検査を通ることを確認する)","Required evidence: P12 の 成果物 section に記載した produced artifacts のパス"]
...
phase_ref: "P12"
file_path: "tasks/feat-reader-behavior-analytics/sys-reader-behavior-analytics-p12.md"
template_id: "task"
...
classification_reason: "feat-reader-behavior-analytics の P12 lifecycle 責務への確定写像"
classification_candidates: [{"artifact_kind":"task","candidate_path":"tasks/feat-reader-behavior-analytics/sys-reader-behavior-analytics-p12.md","confidence":1.0}]
issue_linkage: null
tracker_binding: "beads"
beads_linkage: {"bd_issue_id":"ah-f4do.12","github_mirror":null,"linked_at":"2026-09-04T02:07:06Z","sync_state":"linked"}
github_publication: {"labels":[],"milestone":null,"mode":"local_only","project_aliases":[]}
github_project_linkages: []
pull_request_linkages: []
execution_contexts: []
completion_evidence: {"completed_at":"2026-09-04T10:31:21Z","evidence_refs":["beads:ah-f4do.12"],"policy":"manual","reconciled_at":"2026-09-09T00:00:00Z","source":"reconciliation","status":"done"}
implementation_readiness: {"checked_at":"2026-09-04T00:00:00Z","missing_sections":[],"status":"complete"}
---

# System task overlay: 読者行動計測の運用手順と説明
...
- parent_feature: feat-reader-behavior-analytics
- phase_ref: P12
- classification: confidence=1.0; reason=feat-reader-behavior-analytics の P12 lifecycle 責務への確定写像; candidate=tasks/feat-reader-behavior-analytics/sys-reader-behavior-analytics-p12.md
...
何を計測し何を計測しないか、保持期間はどれだけか、reader_key 指定の抽出・削除をどう実行するかを、運用手順と利用者向け説明として整えた状態を成立させる。

## 背景

計測の説明責任は運用時に発生する。読者から問われた際に参照できる説明と、Owner が抽出・削除を実行する手順を、実装と同じ時期に整えておく。
...
- Operations: applicable; 保持期間削除の定期実行手順を本 phase が所有する
...
- Produced artifacts: docs/spec/feat-reader-behavior-analytics/operations-runbook.md (保持期間削除と抽出・削除要求への対応手順・audit_logs での実行記録の確認方法); docs/ 配下の計測項目と保持期間の説明
...
- Write scope/touches: docs/spec/feat-reader-behavior-analytics/operations-runbook.md, docs/spec/feat-reader-behavior-analytics/
...
- PR body contract: Closes #issue (beads issue) + dev-graph graph_node_id=SYS-READER-BEHAVIOR-ANALYTICS-P12; PR の宛先は dev ブランチとする
- Ownership boundary: system-dev-planner は intent のみを宣言し、dev-graph が tracker mutation と reconciliation を行う

## Branch and worktree execution

- Branch: assigned after dev-graph registration by C15 as devgraph/SYS-READER-BEHAVIOR-ANALYTICS-P12; system-dev-planner は事前割当を行わない
- Worktree lease: claim SYS-READER-BEHAVIOR-ANALYTICS-P12 before implementation; heartbeat/release は dev-graph worktree lease 契約に従う
...
- Required evidence: operations-runbook.md が保持期間削除と抽出・削除要求の両方に手順を持つこと
...
- Goal: 何を計測し何を計測しないか、保持期間はどれだけか、reader_key 指定の抽出・削除をどう実行するかを、運用手順と利用者向け説明として整えた状態を成立させる。
- Generic execution prompt: feat-reader-behavior-analytics の goal (読者面が滞在・スクロール到達・要素クリック・ポインタ標本を要素相対比率で束ねて送り、reader_interaction_events へ追記され、管理画面が viewport_bucket ごとの集計分布としてヒートマップを描き、同意が無い読者は reader_key を持たず 90 日で生データが消える状態になっている) と本 task の 前提条件/成果物/write scope/スコープ外 を渡し、実装手段は固定せず P12 の目的を満たす成果物を作らせる
...
- Ready when: 運用手順が保持期間削除と抽出・削除要求を覆っている
...
- Dependencies: SYS-READER-BEHAVIOR-ANALYTICS-P10, SYS-READER-BEHAVIOR-ANALYTICS-P11
