---
graph_node_id: "SYS-READER-BEHAVIOR-ANALYTICS-P08"
artifact_kind: "task"
artifact_subtypes: []
project_id: "feature-package-feat-reader-behavior-analytics"
domain: "backend"
tags: ["p08","feat-reader-behavior-analytics"]
priority: null
start_date: null
target_date: null
iteration: null
...
purpose: "feat-analytics-insight の汎用イベント基盤と本 feature の計測が同じ観測を二重に取っている箇所を洗い出し、観測の責務が一箇所に定まった状態を成立させる。"
goal: "feat-analytics-insight の汎用イベント基盤と本 feature の計測が同じ観測を二重に取っている箇所を洗い出し、観測の責務が一箇所に定まった状態を成立させる。"
scope_in: ["Produced artifacts: docs/spec/feat-reader-behavior-analytics/migration-notes.md (観測項目ごとの正本と重複解消の内容); src/application/analytics/ の重複観測の整理","Consumed artifacts: features/feat-reader-behavior-analytics.md; features/feat-reader-behavior-analytics.context.json; system-spec/frontend.md; system-spec/database.md; system-spec/security.md","Write scope/touches: src/application/analytics/ingest-reader-interactions.ts, src/application/analytics/reader-subject-request.ts, src/components/reader/behavior-probe.tsx, docs/spec/feat-reader-behavior-analytics/migration-notes.md"]
scope_out: ["1 読者の行動を時系列で再生する機能 (作らない)","日次ロールアップと収益・PV との突合 (feat-blog-metrics-rollup が所有する)","行動指標を使った示唆生成と提示順序 (feat-blog-scoped-admin-console が所有する)","既存の汎用イベント/KPI 基盤そのもの (feat-analytics-insight が所有する)","P08 以外の phase が所有する成果物への変更"]
...
phase_ref: "P08"
file_path: "tasks/feat-reader-behavior-analytics/sys-reader-behavior-analytics-p08.md"
template_id: "task"
...
classification_reason: "feat-reader-behavior-analytics の P08 lifecycle 責務への確定写像"
classification_candidates: [{"artifact_kind":"task","candidate_path":"tasks/feat-reader-behavior-analytics/sys-reader-behavior-analytics-p08.md","confidence":1.0}]
issue_linkage: null
tracker_binding: "beads"
beads_linkage: {"bd_issue_id":"ah-f4do.8","github_mirror":null,"linked_at":"2026-09-04T02:06:57Z","sync_state":"linked"}
github_publication: {"labels":[],"milestone":null,"mode":"local_only","project_aliases":[]}
github_project_linkages: []
pull_request_linkages: []
execution_contexts: []
completion_evidence: {"completed_at":"2026-09-04T10:31:13Z","evidence_refs":["beads:ah-f4do.8"],"policy":"manual","reconciled_at":"2026-09-09T00:00:00Z","source":"reconciliation","status":"done"}
implementation_readiness: {"checked_at":"2026-09-04T00:00:00Z","missing_sections":[],"status":"complete"}
---

# System task overlay: 既存イベント基盤との重複解消と移行
...
- parent_feature: feat-reader-behavior-analytics
- phase_ref: P08
- classification: confidence=1.0; reason=feat-reader-behavior-analytics の P08 lifecycle 責務への確定写像; candidate=tasks/feat-reader-behavior-analytics/sys-reader-behavior-analytics-p08.md
...
feat-analytics-insight の汎用イベント基盤と本 feature の計測が同じ観測を二重に取っている箇所を洗い出し、観測の責務が一箇所に定まった状態を成立させる。

## 背景

PV やクリックといった観測は既存基盤にも存在しうる。二重に取ると、同じ現象について2つの数字が並び、どちらが正かを判断できなくなる。本 phase で観測ごとに正本を1つへ定める。
...
- Compatibility/migration/backfill: 既存基盤の観測を削除する場合は migration-notes.md に影響範囲を記録してから行う

## 成果物

- Produced artifacts: docs/spec/feat-reader-behavior-analytics/migration-notes.md (観測項目ごとの正本と重複解消の内容); src/application/analytics/ の重複観測の整理
...
- Write scope/touches: src/application/analytics/ingest-reader-interactions.ts, src/application/analytics/reader-subject-request.ts, src/components/reader/behavior-probe.tsx, docs/spec/feat-reader-behavior-analytics/migration-notes.md
...
- PR body contract: Closes #issue (beads issue) + dev-graph graph_node_id=SYS-READER-BEHAVIOR-ANALYTICS-P08; PR の宛先は dev ブランチとする
- Ownership boundary: system-dev-planner は intent のみを宣言し、dev-graph が tracker mutation と reconciliation を行う

## Branch and worktree execution

- Branch: assigned after dev-graph registration by C15 as devgraph/SYS-READER-BEHAVIOR-ANALYTICS-P08; system-dev-planner は事前割当を行わない
- Worktree lease: claim SYS-READER-BEHAVIOR-ANALYTICS-P08 before implementation; heartbeat/release は dev-graph worktree lease 契約に従う
...
- Goal: feat-analytics-insight の汎用イベント基盤と本 feature の計測が同じ観測を二重に取っている箇所を洗い出し、観測の責務が一箇所に定まった状態を成立させる。
- Generic execution prompt: feat-reader-behavior-analytics の goal (読者面が滞在・スクロール到達・要素クリック・ポインタ標本を要素相対比率で束ねて送り、reader_interaction_events へ追記され、管理画面が viewport_bucket ごとの集計分布としてヒートマップを描き、同意が無い読者は reader_key を持たず 90 日で生データが消える状態になっている) と本 task の 前提条件/成果物/write scope/スコープ外 を渡し、実装手段は固定せず P08 の目的を満たす成果物を作らせる
...
- Ready when: 観測項目ごとの正本が一意に定まり、回帰0件である
...
- Dependencies: SYS-READER-BEHAVIOR-ANALYTICS-P05
