---
graph_node_id: "SYS-READER-BEHAVIOR-ANALYTICS-P06"
artifact_kind: "task"
artifact_subtypes: []
project_id: "feature-package-feat-reader-behavior-analytics"
domain: "quality"
tags: ["p06","feat-reader-behavior-analytics"]
priority: null
start_date: null
target_date: null
iteration: null
title: "読者行動計測のテスト実行と緑化"
owners: ["daishiman"]
...
related_nodes: []
resource_scope: ["tests/","docs/spec/feat-reader-behavior-analytics/test-run-report.md"]
...
scope_in: ["Produced artifacts: docs/spec/feat-reader-behavior-analytics/test-run-report.md (実行結果とカバレッジ); tests/ 配下の読者行動計測関連テスト実装","Consumed artifacts: features/feat-reader-behavior-analytics.md; features/feat-reader-behavior-analytics.context.json; system-spec/frontend.md; system-spec/database.md; system-spec/security.md","Write scope/touches: tests/, docs/spec/feat-reader-behavior-analytics/test-run-report.md"]
scope_out: ["1 読者の行動を時系列で再生する機能 (作らない)","日次ロールアップと収益・PV との突合 (feat-blog-metrics-rollup が所有する)","行動指標を使った示唆生成と提示順序 (feat-blog-scoped-admin-console が所有する)","既存の汎用イベント/KPI 基盤そのもの (feat-analytics-insight が所有する)","P06 以外の phase が所有する成果物への変更"]
...
phase_ref: "P06"
file_path: "tasks/feat-reader-behavior-analytics/sys-reader-behavior-analytics-p06.md"
template_id: "task"
...
classification_reason: "feat-reader-behavior-analytics の P06 lifecycle 責務への確定写像"
classification_candidates: [{"artifact_kind":"task","candidate_path":"tasks/feat-reader-behavior-analytics/sys-reader-behavior-analytics-p06.md","confidence":1.0}]
issue_linkage: null
tracker_binding: "beads"
beads_linkage: {"bd_issue_id":"ah-f4do.6","github_mirror":null,"linked_at":"2026-09-04T02:06:53Z","sync_state":"linked"}
github_publication: {"labels":[],"milestone":null,"mode":"local_only","project_aliases":[]}
github_project_linkages: []
pull_request_linkages: []
execution_contexts: []
completion_evidence: {"completed_at":"2026-09-04T10:31:09Z","evidence_refs":["beads:ah-f4do.6"],"policy":"manual","reconciled_at":"2026-09-09T00:00:00Z","source":"reconciliation","status":"done"}
implementation_readiness: {"checked_at":"2026-09-04T00:00:00Z","missing_sections":[],"status":"complete"}
---

# System task overlay: 読者行動計測のテスト実行と緑化
...
- parent_feature: feat-reader-behavior-analytics
- phase_ref: P06
- classification: confidence=1.0; reason=feat-reader-behavior-analytics の P06 lifecycle 責務への確定写像; candidate=tasks/feat-reader-behavior-analytics/sys-reader-behavior-analytics-p06.md
...
計測プローブはブラウザ API に依存するため、単体では通っても結合で挙動が変わりやすい。実行によって、離脱時の送出欠落や二重受入の件数ずれを検出する。
...
- Produced artifacts: docs/spec/feat-reader-behavior-analytics/test-run-report.md (実行結果とカバレッジ); tests/ 配下の読者行動計測関連テスト実装
...
- Write scope/touches: tests/, docs/spec/feat-reader-behavior-analytics/test-run-report.md
...
- PR body contract: Closes #issue (beads issue) + dev-graph graph_node_id=SYS-READER-BEHAVIOR-ANALYTICS-P06; PR の宛先は dev ブランチとする
- Ownership boundary: system-dev-planner は intent のみを宣言し、dev-graph が tracker mutation と reconciliation を行う

## Branch and worktree execution

- Branch: assigned after dev-graph registration by C15 as devgraph/SYS-READER-BEHAVIOR-ANALYTICS-P06; system-dev-planner は事前割当を行わない
- Worktree lease: claim SYS-READER-BEHAVIOR-ANALYTICS-P06 before implementation; heartbeat/release は dev-graph worktree lease 契約に従う
...
- Goal: P04 で設計した全テストケースを実行し、失敗0件かつ新規実装コードのカバレッジが80%以上である状態を成立させる。
- Generic execution prompt: feat-reader-behavior-analytics の goal (読者面が滞在・スクロール到達・要素クリック・ポインタ標本を要素相対比率で束ねて送り、reader_interaction_events へ追記され、管理画面が viewport_bucket ごとの集計分布としてヒートマップを描き、同意が無い読者は reader_key を持たず 90 日で生データが消える状態になっている) と本 task の 前提条件/成果物/write scope/スコープ外 を渡し、実装手段は固定せず P06 の目的を満たす成果物を作らせる
