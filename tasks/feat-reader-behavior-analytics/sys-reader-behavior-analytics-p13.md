---
graph_node_id: "SYS-READER-BEHAVIOR-ANALYTICS-P13"
artifact_kind: "task"
artifact_subtypes: []
project_id: "feature-package-feat-reader-behavior-analytics"
domain: "operations"
tags: ["p13","feat-reader-behavior-analytics"]
priority: null
start_date: null
target_date: null
iteration: null
...
purpose: "読者行動計測を dev 経由で本番へ反映し、確定した観測項目と保持期間を system-spec の該当章へ書き戻した状態を成立させる。"
goal: "読者行動計測を dev 経由で本番へ反映し、確定した観測項目と保持期間を system-spec の該当章へ書き戻した状態を成立させる。"
scope_in: ["Produced artifacts: system-spec/frontend.md への計測プローブの書き戻し; system-spec/database.md への reader_interaction_events の書き戻し; docs/spec/feat-reader-behavior-analytics/release-notes.md (反映手順と切り戻し手順)","Consumed artifacts: features/feat-reader-behavior-analytics.md; features/feat-reader-behavior-analytics.context.json; system-spec/frontend.md; system-spec/database.md; system-spec/security.md","Write scope/touches: system-spec/frontend.md, system-spec/database.md, docs/spec/feat-reader-behavior-analytics/release-notes.md"]
scope_out: ["1 読者の行動を時系列で再生する機能 (作らない)","日次ロールアップと収益・PV との突合 (feat-blog-metrics-rollup が所有する)","行動指標を使った示唆生成と提示順序 (feat-blog-scoped-admin-console が所有する)","既存の汎用イベント/KPI 基盤そのもの (feat-analytics-insight が所有する)","P13 以外の phase が所有する成果物への変更"]
...
phase_ref: "P13"
file_path: "tasks/feat-reader-behavior-analytics/sys-reader-behavior-analytics-p13.md"
template_id: "task"
...
classification_reason: "feat-reader-behavior-analytics の P13 lifecycle 責務への確定写像"
classification_candidates: [{"artifact_kind":"task","candidate_path":"tasks/feat-reader-behavior-analytics/sys-reader-behavior-analytics-p13.md","confidence":1.0}]
issue_linkage: null
tracker_binding: "beads"
beads_linkage: {"bd_issue_id":"ah-f4do.13","github_mirror":null,"linked_at":"2026-09-04T02:07:08Z","sync_state":"linked"}
github_publication: {"labels":[],"milestone":null,"mode":"local_only","project_aliases":[]}
github_project_linkages: []
pull_request_linkages: []
execution_contexts: []
completion_evidence: {"completed_at":"2026-09-04T10:31:23Z","evidence_refs":["beads:ah-f4do.13"],"policy":"manual","reconciled_at":"2026-09-09T00:00:00Z","source":"reconciliation","status":"done"}
implementation_readiness: {"checked_at":"2026-09-04T00:00:00Z","missing_sections":[],"status":"complete"}
---

# System task overlay: 読者行動計測のリリースと仕様書への書き戻し
...
- parent_feature: feat-reader-behavior-analytics
- phase_ref: P13
- classification: confidence=1.0; reason=feat-reader-behavior-analytics の P13 lifecycle 責務への確定写像; candidate=tasks/feat-reader-behavior-analytics/sys-reader-behavior-analytics-p13.md
...
読者行動計測を dev 経由で本番へ反映し、確定した観測項目と保持期間を system-spec の該当章へ書き戻した状態を成立させる。

## 背景

計測は読者面へ入るため、反映は dev 環境での描画確認を経てから本番へ進める。観測項目と保持期間は後続 feature の入力になるため仕様書へ書き戻す。
...
- Produced artifacts: system-spec/frontend.md への計測プローブの書き戻し; system-spec/database.md への reader_interaction_events の書き戻し; docs/spec/feat-reader-behavior-analytics/release-notes.md (反映手順と切り戻し手順)
...
- Write scope/touches: system-spec/frontend.md, system-spec/database.md, docs/spec/feat-reader-behavior-analytics/release-notes.md
...
- PR body contract: Closes #issue (beads issue) + dev-graph graph_node_id=SYS-READER-BEHAVIOR-ANALYTICS-P13; PR の宛先は dev ブランチとする
- Ownership boundary: system-dev-planner は intent のみを宣言し、dev-graph が tracker mutation と reconciliation を行う

## Branch and worktree execution

- Branch: assigned after dev-graph registration by C15 as devgraph/SYS-READER-BEHAVIOR-ANALYTICS-P13; system-dev-planner は事前割当を行わない
- Worktree lease: claim SYS-READER-BEHAVIOR-ANALYTICS-P13 before implementation; heartbeat/release は dev-graph worktree lease 契約に従う
...
- テストレベル選定: 回帰: 反映前に全テストが緑であることを前提とし、反映後は計測有無にかかわらず記事が描画されることを実環境で確認する。 N/A: 単体・結合・境界値 の各テストレベルは、本 task の成果物が文書と判定であり実行可能なコードを含まないため適用しない。
...
- Goal: 読者行動計測を dev 経由で本番へ反映し、確定した観測項目と保持期間を system-spec の該当章へ書き戻した状態を成立させる。
- Generic execution prompt: feat-reader-behavior-analytics の goal (読者面が滞在・スクロール到達・要素クリック・ポインタ標本を要素相対比率で束ねて送り、reader_interaction_events へ追記され、管理画面が viewport_bucket ごとの集計分布としてヒートマップを描き、同意が無い読者は reader_key を持たず 90 日で生データが消える状態になっている) と本 task の 前提条件/成果物/write scope/スコープ外 を渡し、実装手段は固定せず P13 の目的を満たす成果物を作らせる
...
- Dependencies: SYS-READER-BEHAVIOR-ANALYTICS-P12
