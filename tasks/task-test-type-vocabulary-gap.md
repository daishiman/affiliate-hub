...
> `REQ-FD02` / `REQ-SEC02` / `REQ-SEC04` も当たり、名乗る要件は **7 件**になった。
...
- **足してよいのは、既に実在して機能している検査に名前が無い場合だけ**
  （検査を書く前に名前だけ足すのは、未宣言の数を下げるための語彙になる）
- **足した名前が要求する内容を壊して測り、赤を確認している**
- **語彙を足す作業と、それで宣言する作業が別のコミットに分かれている**
  （上限が下がった理由が「検査を書いたから」か「名前を作ったから」か、後から読めるように）
- 上限は下げる方向にしか動いていない

> **件数の条件は外した（2026-08-19）。**
> ここには当初「その性質を名乗る要件が 4 件より多いこと」と書き、
> 候補が 5 件見えた時点で「5 件より多い」へ**引き上げた**。これは逆向きだった。
> **候補が条件を満たしそうになったから条件を上げる**のは、
> 「上限は下げる方向にしか動かさない」という決まりの裏返しの形である。
> 数が揃うのを待つ理由は、はじめから無い。名前を足してよいかどうかは
> **検査が実在して機能しているか**で決まり、何件あるかでは決まらない。
> 横断の洗い直し自体は残す（FD 群のためだけの名前になっていないかを見るのに要る）が、
> **合否の条件にはしない。**
