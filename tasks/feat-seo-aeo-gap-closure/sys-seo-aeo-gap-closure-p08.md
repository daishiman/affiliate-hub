---
graph_node_id: "SYS-SEO-AEO-GAP-CLOSURE-P08"
artifact_kind: "task"
artifact_subtypes: []
project_id: "feature-package-feat-seo-aeo-gap-closure"
domain: "backend"
tags: ["p08","feat-seo-aeo-gap-closure"]
priority: null
start_date: null
target_date: null
iteration: null
...
purpose: "P05 で足した導出と履歴を既存の SEO 実装へ馴染ませ、追加した drizzle マイグレーションが既存データに対して前方互換であることを確かめる。"
goal: "P05 で足した導出と履歴を既存の SEO 実装へ馴染ませ、追加した drizzle マイグレーションが既存データに対して前方互換であることを確かめる。"
scope_in: ["Produced artifacts: src/application/seo/ の JSON-LD 組み立て経路の整理; docs/spec/feat-seo-aeo-gap-closure/migration-compatibility.md (追加マイグレーションの前方互換確認と空履歴の扱い)","Consumed artifacts: docs/spec/feat-seo-aeo-gap-closure/data-model.md, src/application/seo/structured-data.ts, drizzle","Write scope/touches: src/application/seo, src/db/schema.ts, drizzle, docs/spec/feat-seo-aeo-gap-closure/migration-compatibility.md, tests"]
...
acceptance: ["Automated commands: `pnpm run typecheck`","Automated commands: `pnpm vitest run --reporter=dot`","Automated commands: `python3 .claude/plugins/system-dev-planner/scripts/validate-system-plan.py --repo-root . --feature-package feature-package/feat-seo-aeo-gap-closure` (本 package の C12 決定論検証を世代非依存に再実行する)","Required evidence: P08 の 成果物 section に記載した produced artifacts のパス"]
architecture_refs: ["arch-system-spec-overview","arch-two-layer-platform"]
parent_feature: "feat-seo-aeo-gap-closure"
feature_package_id: "feature-package/feat-seo-aeo-gap-closure"
phase_ref: "P08"
file_path: "tasks/feat-seo-aeo-gap-closure/sys-seo-aeo-gap-closure-p08.md"
template_id: "task"
...
source_lineage: {"imported_at":"2026-09-04T02:37:00Z","origin_kind":"system-dev-planner","source_digest":"2689733e08bc4e992eabe2e406ecea05433ae3d5312cc7e8e60876b0a3798298","source_path":".dev-graph/published/generations/feature-package-feat-seo-aeo-gap-closure/2689733e08bc4e992eabe2e406ecea05433ae3d5312cc7e8e60876b0a3798298/task-specs/phase-08-refactoring-migration.md","source_plugin":"system-dev-planner","source_version":"0.1.0"}
classification_confidence: 1.0
classification_reason: "feat-seo-aeo-gap-closure の P08 lifecycle 責務への確定写像"
classification_candidates: [{"artifact_kind":"task","candidate_path":"tasks/feat-seo-aeo-gap-closure/sys-seo-aeo-gap-closure-p08.md","confidence":1.0}]
issue_linkage: null
tracker_binding: "beads"
beads_linkage: {"bd_issue_id":"ah-f60p.8","github_mirror":null,"linked_at":"2026-09-04T04:00:00Z","sync_state":"linked"}
github_publication: {"labels":[],"milestone":null,"mode":"local_only","project_aliases":[]}
github_project_linkages: []
pull_request_linkages: []
execution_contexts: []
completion_evidence: {"completed_at":"2026-09-04T08:02:55Z","evidence_refs":["beads:ah-f60p.8"],"policy":"manual","reconciled_at":"2026-09-09T00:00:00Z","source":"reconciliation","status":"done"}
implementation_readiness: {"checked_at":"2026-09-04T02:17:16Z","missing_sections":[],"status":"complete"}
---

# System task overlay: 既存 SEO 実装との統合整理とマイグレーションの前方互換確認
...
- parent_feature: feat-seo-aeo-gap-closure
- phase_ref: P08
- classification: confidence=1.0; reason=feat-seo-aeo-gap-closure の P08 lifecycle 責務への確定写像; candidate=tasks/feat-seo-aeo-gap-closure/sys-seo-aeo-gap-closure-p08.md
...
P05 で足した導出と履歴を既存の SEO 実装へ馴染ませ、追加した drizzle マイグレーションが既存データに対して前方互換であることを確かめる。

## 背景

structured-data.ts は既に 9 個の export を持ち、expression-blocks.ts は 5 種を作る。ここへ HowTo と Speakable を足すと、同じ記事から複数の JSON-LD を組み立てる箇所が増える。組み立ての重複をそのままにすると、次に種類が増えたときに同じ形の変更が複数箇所へ散る。また drizzle/ は 0043 までで、既存の公開記事に対して履歴が空の状態から始まることになる。
...
- Backend: applicable; JSON-LD 組み立て経路の重複を解消する
- API: N/A: API 契約設計は P02 が所有する
- Data: applicable; 追加マイグレーションの前方互換と空履歴からの立ち上がりを確認する
...
- Quality: applicable; 整理の前後で挙動が変わらないことをテストで確かめる
- Documentation: applicable; 整理内容と互換確認の結果を記録する
...
- Produced artifacts: src/application/seo/ の JSON-LD 組み立て経路の整理; docs/spec/feat-seo-aeo-gap-closure/migration-compatibility.md (追加マイグレーションの前方互換確認と空履歴の扱い)
- Consumed artifacts: docs/spec/feat-seo-aeo-gap-closure/data-model.md, src/application/seo/structured-data.ts, drizzle
- Write scope/touches: src/application/seo, src/db/schema.ts, drizzle, docs/spec/feat-seo-aeo-gap-closure/migration-compatibility.md, tests
...
- PR body contract: Closes #issue (beads issue) + dev-graph graph_node_id=SYS-SEO-AEO-GAP-CLOSURE-P08; PR の宛先は dev ブランチとする
- Ownership boundary: system-dev-plannerはintentのみを宣言し、dev-graphがtracker mutationとreconciliationを行う

## Branch and worktree execution

- Branch: assigned after dev-graph registration by C15 as devgraph/SYS-SEO-AEO-GAP-CLOSURE-P08; system-dev-plannerは事前割当を行わない
- Worktree lease: claim SYS-SEO-AEO-GAP-CLOSURE-P08 before implementation; heartbeat/release は dev-graph worktree lease 契約に従う
...
- 層別方針: バックエンド/データ: API 契約 の後方互換と DB 結合 の前方互換 (既存マイグレーション適用済み環境で新テーブルが追加のみであること) を検証する。
...
- Goal: P05 で足した導出と履歴を既存の SEO 実装へ馴染ませ、追加した drizzle マイグレーションが既存データに対して前方互換であることを確かめる。
- Generic execution prompt: feat-seo-aeo-gap-closure の goal (手順記事と読み上げ向けの構造化データが記事から導出され、公開時点検の結果が履歴として残り、公開後の記事も定期に再点検されて陳腐化に気づける状態になっている) と本 task の 前提条件/成果物/write_scope/スコープ外 を渡し、実装手段は固定せず P08 の目的を満たす成果物を作らせる
...
- Dependencies: SYS-SEO-AEO-GAP-CLOSURE-P05
