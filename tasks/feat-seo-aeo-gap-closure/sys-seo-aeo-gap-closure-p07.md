---
graph_node_id: "SYS-SEO-AEO-GAP-CLOSURE-P07"
artifact_kind: "task"
artifact_subtypes: []
project_id: "feature-package-feat-seo-aeo-gap-closure"
domain: "quality"
tags: ["p07","feat-seo-aeo-gap-closure"]
priority: null
start_date: null
target_date: null
iteration: null
...
purpose: "feat-seo-aeo-gap-closure の受入6件 (A1-A6) を、実装と実行結果に対して 1 件ずつ判定し、満たしたと言える根拠をそれぞれに紐づける。"
goal: "feat-seo-aeo-gap-closure の受入6件 (A1-A6) を、実装と実行結果に対して 1 件ずつ判定し、満たしたと言える根拠をそれぞれに紐づける。"
scope_in: ["Produced artifacts: docs/spec/feat-seo-aeo-gap-closure/acceptance.md (A1-A6 の判定・根拠・未達があればその内容)","Consumed artifacts: docs/spec/feat-seo-aeo-gap-closure/test-run.md, docs/spec/feat-seo-aeo-gap-closure/requirements-baseline.md","Write scope/touches: docs/spec/feat-seo-aeo-gap-closure/acceptance.md"]
...
acceptance: ["Automated commands: `pnpm vitest run --reporter=dot` (受入判定の根拠として実行結果を再取得する)","Automated commands: `python3 .claude/plugins/system-dev-planner/scripts/validate-system-plan.py --repo-root . --feature-package feature-package/feat-seo-aeo-gap-closure` (本 package の C12 決定論検証を世代非依存に再実行する)","Required evidence: P07 の 成果物 section に記載した produced artifacts のパス"]
architecture_refs: ["arch-system-spec-overview","arch-two-layer-platform"]
parent_feature: "feat-seo-aeo-gap-closure"
feature_package_id: "feature-package/feat-seo-aeo-gap-closure"
phase_ref: "P07"
file_path: "tasks/feat-seo-aeo-gap-closure/sys-seo-aeo-gap-closure-p07.md"
template_id: "task"
...
source_lineage: {"imported_at":"2026-09-04T02:37:00Z","origin_kind":"system-dev-planner","source_digest":"2689733e08bc4e992eabe2e406ecea05433ae3d5312cc7e8e60876b0a3798298","source_path":".dev-graph/published/generations/feature-package-feat-seo-aeo-gap-closure/2689733e08bc4e992eabe2e406ecea05433ae3d5312cc7e8e60876b0a3798298/task-specs/phase-07-acceptance.md","source_plugin":"system-dev-planner","source_version":"0.1.0"}
classification_confidence: 1.0
classification_reason: "feat-seo-aeo-gap-closure の P07 lifecycle 責務への確定写像"
classification_candidates: [{"artifact_kind":"task","candidate_path":"tasks/feat-seo-aeo-gap-closure/sys-seo-aeo-gap-closure-p07.md","confidence":1.0}]
issue_linkage: null
tracker_binding: "beads"
beads_linkage: {"bd_issue_id":"ah-f60p.7","github_mirror":null,"linked_at":"2026-09-04T04:00:00Z","sync_state":"linked"}
github_publication: {"labels":[],"milestone":null,"mode":"local_only","project_aliases":[]}
github_project_linkages: []
pull_request_linkages: []
execution_contexts: []
completion_evidence: {"completed_at":"2026-09-04T07:47:07Z","evidence_refs":["beads:ah-f60p.7"],"policy":"manual","reconciled_at":"2026-09-09T00:00:00Z","source":"reconciliation","status":"done"}
implementation_readiness: {"checked_at":"2026-09-04T02:17:16Z","missing_sections":[],"status":"complete"}
---

# System task overlay: 受入6件の受け入れ判定
...
- parent_feature: feat-seo-aeo-gap-closure
- phase_ref: P07
- classification: confidence=1.0; reason=feat-seo-aeo-gap-closure の P07 lifecycle 責務への確定写像; candidate=tasks/feat-seo-aeo-gap-closure/sys-seo-aeo-gap-closure-p07.md
...
feat-seo-aeo-gap-closure の受入6件 (A1-A6) を、実装と実行結果に対して 1 件ずつ判定し、満たしたと言える根拠をそれぞれに紐づける。

## 背景

受入は「テストが緑」とは別の問いである。とくに A5 (落ちた理由が読める) はhint が人間に読める文言として画面に出ているかどうかであり、AiSearchCheck の hint フィールドが存在することとは同じではない。
...
- Frontend: applicable; 管理画面一覧の実表示を受入の観点で確認する
...
- Quality: applicable; 受入判定が本 phase の目的である
- Documentation: applicable; 受入判定表が本 phase の成果物である
...
- Produced artifacts: docs/spec/feat-seo-aeo-gap-closure/acceptance.md (A1-A6 の判定・根拠・未達があればその内容)
- Consumed artifacts: docs/spec/feat-seo-aeo-gap-closure/test-run.md, docs/spec/feat-seo-aeo-gap-closure/requirements-baseline.md
- Write scope/touches: docs/spec/feat-seo-aeo-gap-closure/acceptance.md
...
- PR body contract: Closes #issue (beads issue) + dev-graph graph_node_id=SYS-SEO-AEO-GAP-CLOSURE-P07; PR の宛先は dev ブランチとする
- Ownership boundary: system-dev-plannerはintentのみを宣言し、dev-graphがtracker mutationとreconciliationを行う

## Branch and worktree execution

- Branch: assigned after dev-graph registration by C15 as devgraph/SYS-SEO-AEO-GAP-CLOSURE-P07; system-dev-plannerは事前割当を行わない
- Worktree lease: claim SYS-SEO-AEO-GAP-CLOSURE-P07 before implementation; heartbeat/release は dev-graph worktree lease 契約に従う
...
- 層別方針: フロントエンド: 受入 A5 を管理画面一覧の behavior (落ちた記事と hint が読めること) で判定する。
- 保守性制約: pixel位置依存・DOM構造依存のテストを禁止し、可視ラベル・アクセシブル名・返却データの属性など振る舞い検証に限定する。

## Verification and evidence

- Automated commands: `pnpm vitest run --reporter=dot` (受入判定の根拠として実行結果を再取得する)
...
- Goal: feat-seo-aeo-gap-closure の受入6件 (A1-A6) を、実装と実行結果に対して 1 件ずつ判定し、満たしたと言える根拠をそれぞれに紐づける。
- Generic execution prompt: feat-seo-aeo-gap-closure の goal (手順記事と読み上げ向けの構造化データが記事から導出され、公開時点検の結果が履歴として残り、公開後の記事も定期に再点検されて陳腐化に気づける状態になっている) と本 task の 前提条件/成果物/write_scope/スコープ外 を渡し、実装手段は固定せず P07 の目的を満たす成果物を作らせる
...
- Dependencies: SYS-SEO-AEO-GAP-CLOSURE-P06
