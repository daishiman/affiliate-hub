---
graph_node_id: "SYS-SEO-AEO-GAP-CLOSURE-P09"
artifact_kind: "task"
artifact_subtypes: []
project_id: "feature-package-feat-seo-aeo-gap-closure"
domain: "quality"
tags: ["p09","feat-seo-aeo-gap-closure"]
priority: null
start_date: null
target_date: null
iteration: null
...
purpose: "定期再点検が公開済み記事の件数に対して破綻しないこと、点検履歴が保持窓を超えて無限に増えないこと、管理画面一覧が権限境界を越えないことを検査する。"
goal: "定期再点検が公開済み記事の件数に対して破綻しないこと、点検履歴が保持窓を超えて無限に増えないこと、管理画面一覧が権限境界を越えないことを検査する。"
scope_in: ["Produced artifacts: docs/spec/feat-seo-aeo-gap-closure/quality-assurance.md (非機能検査の観点・実測・判定)","Consumed artifacts: docs/spec/feat-seo-aeo-gap-closure/retention-policy.md, docs/spec/feat-seo-aeo-gap-closure/architecture.md, docs/spec/feat-seo-aeo-gap-closure/migration-compatibility.md","Write scope/touches: docs/spec/feat-seo-aeo-gap-closure/quality-assurance.md, tests"]
...
acceptance: ["Automated commands: `pnpm vitest run --reporter=dot`","Automated commands: `pnpm run lint`","Automated commands: `python3 .claude/plugins/system-dev-planner/scripts/validate-system-plan.py --repo-root . --feature-package feature-package/feat-seo-aeo-gap-closure` (本 package の C12 決定論検証を世代非依存に再実行する)","Required evidence: P09 の 成果物 section に記載した produced artifacts のパス"]
architecture_refs: ["arch-system-spec-overview","arch-two-layer-platform"]
parent_feature: "feat-seo-aeo-gap-closure"
feature_package_id: "feature-package/feat-seo-aeo-gap-closure"
phase_ref: "P09"
file_path: "tasks/feat-seo-aeo-gap-closure/sys-seo-aeo-gap-closure-p09.md"
template_id: "task"
...
source_lineage: {"imported_at":"2026-09-04T02:37:00Z","origin_kind":"system-dev-planner","source_digest":"2689733e08bc4e992eabe2e406ecea05433ae3d5312cc7e8e60876b0a3798298","source_path":".dev-graph/published/generations/feature-package-feat-seo-aeo-gap-closure/2689733e08bc4e992eabe2e406ecea05433ae3d5312cc7e8e60876b0a3798298/task-specs/phase-09-quality-assurance.md","source_plugin":"system-dev-planner","source_version":"0.1.0"}
classification_confidence: 1.0
classification_reason: "feat-seo-aeo-gap-closure の P09 lifecycle 責務への確定写像"
classification_candidates: [{"artifact_kind":"task","candidate_path":"tasks/feat-seo-aeo-gap-closure/sys-seo-aeo-gap-closure-p09.md","confidence":1.0}]
issue_linkage: null
tracker_binding: "beads"
beads_linkage: {"bd_issue_id":"ah-f60p.9","github_mirror":null,"linked_at":"2026-09-04T04:00:00Z","sync_state":"linked"}
github_publication: {"labels":[],"milestone":null,"mode":"local_only","project_aliases":[]}
github_project_linkages: []
pull_request_linkages: []
execution_contexts: []
completion_evidence: {"completed_at":"2026-09-04T08:14:18Z","evidence_refs":["beads:ah-f60p.9"],"policy":"manual","reconciled_at":"2026-09-09T00:00:00Z","source":"reconciliation","status":"done"}
...
- parent_feature: feat-seo-aeo-gap-closure
- phase_ref: P09
- classification: confidence=1.0; reason=feat-seo-aeo-gap-closure の P09 lifecycle 責務への確定写像; candidate=tasks/feat-seo-aeo-gap-closure/sys-seo-aeo-gap-closure-p09.md
...
定期再点検が公開済み記事の件数に対して破綻しないこと、点検履歴が保持窓を超えて無限に増えないこと、管理画面一覧が権限境界を越えないことを検査する。

## 背景

scheduled は 1 日 1 回であり、公開記事が増えるほど 1 回の処理量が増える。保持窓は append-with-window で決まっているが、窓が効いていなければ履歴は増え続ける。この 2 つはテストが緑でも起きうるので、非機能として別に見る。
...
- Backend: applicable; scheduled の 1 回あたり処理量と履歴の増え方を検査する
- API: N/A: API 契約設計は P02 が所有する
- Data: applicable; 保持窓が実際に古い分を落としていることを確認する
- Infrastructure: N/A: 既存 cloudflare-workers-opennext-app (Next.js 16 + OpenNext, Workers/D1/R2) のデプロイ単位を変更しない
- Security: applicable; 管理画面一覧の権限境界が既存 admin RBAC を越えないことを確認する
- Quality: applicable; 非機能検査が本 phase の目的である
- Documentation: applicable; 検査結果が本 phase の成果物である
...
- Produced artifacts: docs/spec/feat-seo-aeo-gap-closure/quality-assurance.md (非機能検査の観点・実測・判定)
- Consumed artifacts: docs/spec/feat-seo-aeo-gap-closure/retention-policy.md, docs/spec/feat-seo-aeo-gap-closure/architecture.md, docs/spec/feat-seo-aeo-gap-closure/migration-compatibility.md
- Write scope/touches: docs/spec/feat-seo-aeo-gap-closure/quality-assurance.md, tests
...
- PR body contract: Closes #issue (beads issue) + dev-graph graph_node_id=SYS-SEO-AEO-GAP-CLOSURE-P09; PR の宛先は dev ブランチとする
- Ownership boundary: system-dev-plannerはintentのみを宣言し、dev-graphがtracker mutationとreconciliationを行う

## Branch and worktree execution

- Branch: assigned after dev-graph registration by C15 as devgraph/SYS-SEO-AEO-GAP-CLOSURE-P09; system-dev-plannerは事前割当を行わない
- Worktree lease: claim SYS-SEO-AEO-GAP-CLOSURE-P09 before implementation; heartbeat/release は dev-graph worktree lease 契約に従う
...
- 層別方針: バックエンド/データ: API 契約 の応答時間と DB 結合 の刈り取り時の行数不変条件(1 記事 30 行以下) を非機能検査として検証する。
...
- Goal: 定期再点検が公開済み記事の件数に対して破綻しないこと、点検履歴が保持窓を超えて無限に増えないこと、管理画面一覧が権限境界を越えないことを検査する。
- Generic execution prompt: feat-seo-aeo-gap-closure の goal (手順記事と読み上げ向けの構造化データが記事から導出され、公開時点検の結果が履歴として残り、公開後の記事も定期に再点検されて陳腐化に気づける状態になっている) と本 task の 前提条件/成果物/write_scope/スコープ外 を渡し、実装手段は固定せず P09 の目的を満たす成果物を作らせる
...
- Dependencies: SYS-SEO-AEO-GAP-CLOSURE-P08
