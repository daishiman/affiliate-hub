...
| ~~監査の記録に request ID の列が無い~~ **→ 塞いだ（2026-08-24, P13）** | `audit_logs.request_id`（`src/db/schema.ts:926`）と索引 `audit_logs_workspace_request_idx`（同 937）を追加。移行は `drizzle/0023_aromatic_flatman.sql`。値は `src/application/access-denial.ts:125` で、呼び出し側が持っていなければ `req_` + 採番で必ず入る。**本番 D1 へ 0023 を当てるまでは本番では塞がっていない**（`release-notes.md` §0） | [`release-notes.md`](./release-notes.md) ②、[`runbook.md`](./runbook.md) §6 手順 5 |
