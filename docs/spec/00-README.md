...
| 12-7 | 参考サイトの全体構成を抽象化した部品目録（ページ種別・記事部品列・サイドバー・フッター・配信部品） | `13-参考サイト全体構成解析-抽象ブループリント.md` | 構成の抽象正本。`06` の記事共通構成は**上書きしない**（差分は同書 §7 の CHG として記録する）。固有名・文章・素材・色値は載せない |
...
| `13-参考サイト全体構成解析-抽象ブループリント.md` | 参考サイトの構成を抽象化した部品目録（`review-media-classic`）。ページ種別・記事部品列・サイドバー・フッター・配信部品 | approved（v1.1） | approved | partial | unverified |
