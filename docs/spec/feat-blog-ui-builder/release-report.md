...
| 2 | ~~配色の保存・掲載の増減が操作の記録に届かない~~ **→ D.4 で解消** | 金銭に直結する変更を、いつ誰が何を消したか機械で追えない |
...
### D.3 REQ-P07 の `boundary` 除外を外した — 🔴 → 🟢

D.2 が通ると、さらに次の門（要件ごとの必須テスト種別）が 1 件で落ちた。

```
REQ-P07: boundary は除外されていますが、実際には印が付いています
```

`docs/product/required-test-types.md` の REQ-P07 には
「ウィザードの入力は選択肢と自由記述で、長さ上限を設けていないため端が無い」
という理由で `boundary` の除外が置いてあった。**この理由が古くなっていた。**
配色の上書きと掲載の位置には端がある——上書きは「両軸とも空」が境で、
そこを越えると行が消える。掲載の位置は 0 と負の境で 0 に倒れる。
D.1／D.2 で足したテストは、その端を実際に踏んでいる。

除外の行を外した（`—` にした）。**除外を増やして緑にしたのではなく、
できるようになったので除外をやめた**方向である。理由つき除外は 6 → 5 件、
上限 7 に対して余裕が増えた。`required-test-types-report.md` は
`node scripts/required-test-types.mjs` で作り直している。

## D.4 §4 の 2 —— 記録に届いていなかった書き込みを閉じた

**「つなぎ目の呼び出し」（`node scripts/port-wiring.mjs`）が赤だった。**
...
§E に「本番へ進める前に閉じること」と書いた 🔴 そのものである。閉じた。

### 判定できない 4 件 —— 名前の側を直した

`BlogAppearancePort` の `clearOverride` / `selectTemplate` / `templateOf` / `themeOf` が、
読みとも書きとも判定できずに数えられていた。

| 手続き | 直し方 |
|---|---|
| `clearOverride` | 語彙表 `WRITE_VERBS` へ `clear` を足した（「空にする」も書き込み） |
| `templateOf` / `themeOf` | `NON_WRITE_EXACT` へ名前ごと足した（`Of` の接尾辞を規則にはしない） |
| `selectTemplate` | **`saveTemplate` へ改名した** |

改名だけ扱いが違う。語彙表へ `select` を足せば黙らせられたが、
**そうすると将来の読み取り手続きが黙って書き込み扱いになる**（SQL の `SELECT` は読みの語）。
検査自身が「どちらか決められないなら、それは名前の側の問題」と言っている。

### 届いていない 2 件 —— `audit_log` へ 1 行残すようにした

`createManageBlogAppearanceUseCase`（4 操作）と
`createReviewBlogPlacementsUseCase`（掲載の足し引き）から
`deps.auditLog.append()` を**同じファイルの中で**呼ぶようにした
（別ファイルへ引き上げると検査が辿れなくなる。`src/application/audit.ts` の doc）。

足した語は `blog_appearance.changed` / `blog_placement.changed` / `blog_placement.removed` の 3 つ。
**掲載だけ「足す」と「外す」を分けている。**台帳の行は外すと物理削除されるので、
外した事実が残る場所は `audit_log` しかない。1 語だと「消えている」を差分から読むことになるが、
消えた行の差分は消えているので読めない。

見た目の変更にも記録を足したのは、以前の「法令上の主張を含まないから要らない」という理由が
**誤りではないが足りなかった**ためである。見た目は上書きで消える設定で、
変える前の値はどこにも残らない。枠の並び (`blog_layout.changed`) は残していた。

記録が書けなかったときは保存を取り消さない（押した人の見え方と保存先が食い違う形を
新しく作ることになる）。`auditWriteFailure` で「済んだこと」と「残っていないこと」を両方返す。

検査は **届いていない 0（上限 0）/ 判定できない 0（上限 0）** で緑。
`system-spec/database.md` §4.3 の解消も `set-chapter-note` の正規経路で正本へ入れた。

## E. まだ 🔴 のまま

§4 の 4（公開記事の本文が HTML に出ていない）は手つかずである。
2（配色の保存と掲載の増減が操作の記録に届かない）は D.4 で閉じた。
