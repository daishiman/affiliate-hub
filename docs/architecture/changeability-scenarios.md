...
### (b) 使う提供元を切り替える — **コードを触らない**（2026-08-18 に作り直した）

**この項目は一度「1 ファイル 1 行」と実測して書いたが、その形をやめた。**
以下は経緯ごと残す。数字だけ差し替えると、なぜ変えたのかが読めなくなるため。

**やめる前（2026-08-17 の実測）**

切り替え点を `src/infrastructure/llm/llm-setup.ts` の `ACTIVE_PROVIDER` へ集約し、
`anthropic` → `google_gemini` へ実際に書き換えて測った。結果は 1 ファイル 1 行、
`npx tsc --noEmit` 無警告、`npx vitest run` 489 件通過。数字としては良かった。

**やめた理由**

1 行で足りるのは「アプリ全体で 1 社を使う」ときだけである。しかし記事ごとに
モデルを選びたい（速くて安いモデルで下書き、練るときだけ上位モデル）という
要件が出た時点で、`ACTIVE_PROVIDER` は**選択肢を 1 つに潰す仕掛け**になった。
1 行で切り替わることは、裏返すと**利用者には切り替えられない**ということである。

もう 1 つ、`llm-setup.ts` がモデル名・単価・鍵の参照キーの表を持ち、
目録（`llm-provider-catalog.ts`）も同じことを決めていた。
**同じことを 2 か所が別のやり方で決めていると、片方を直したときに気づけない。**
実際、どちらが効いているのかコードを読んでも判別できなかった。

**いまの形（2026-08-18）**

| 決めること | 置き場所 |
| --- | --- |
| どのモデルを使うか | 依頼が運ぶ（`LlmRequest.model`。記事ごとに選ぶ） |
| 単価 | 目録の設定 1 か所（`LLM_PROVIDER_CATALOG`） |
| 鍵 | 作業場所ごとの預かり所（値に触れるのはアダプタだけ） |

`llm-setup.ts` に残っているのは「呼び出しに要るものが揃っているか」の判定だけで、
提供元の名前はもうどこにも書かれていない。振り分けは
`createRoutingLlm`（`llm-provider-registry.ts`）が `request.model.providerId` で行う。

したがって**提供元の切り替えに要るコード変更は 0 行**である。画面で選べばよい。

**代わりに測るべきもの**: 「切り替えの行数」ではなく
**「提供元を 1 社足すのに触るファイル数」**（= (a)）に一本化した。
切り替えが 0 行になった以上、(b) 単独の数字はもう意味を持たない。

**既定を置かなかったこと**: 未選択のときに何かを選ぶ分岐は**書いていない**。
「とりあえず先頭のモデル」を入れると、高いモデルが黙って選ばれても
誰も気づかない状態に戻る（それが `ACTIVE_PROVIDER` の問題そのものだった）。
未選択は未選択として止める。この決めごとは
`tests/application/draft-content-variant.test.ts`「モデルが選ばれていないとき」が固定する。
