...
│   ├── deploy.yml             dev / main への push：検査 → 公開 + スモーク 2 回
│   │                          （測る job と出す job を分けてある。出す側は測り終えてから始まる）
│   │                          （どちらの環境でも、控えを取ってから DB の構造変更を行う）
│   └── migrate.yml            手動起動のみ：DB の構造変更（公開と切り離して形だけ変えるとき）
└── tests/
...
**必須種別については「仕組みは完成、適用は 12/240」と読むこと。**
宣言の書き方・機械での確認・CI で落とす門は揃っていて、門が落ちることも実測してある。
揃っていないのは**適用**のほうで、性質を宣言できている要件は 240 件中 12 件しかない。
この 2 つを「完了」の一語でまとめると、後から読んだ人は網羅が済んだと受け取る。
残り 228 件は着手できる粒度へ 9 件に分割してある（`ah-99p` ほか。残課題リスト 項目 45）。
...
`Edit` で書き戻して復旧している。同じ日に `tests/architecture/dependency-direction.test.ts`
でも同じ止まり方をした。

**この書き戻しは「別の書き方で通した」ではない。**
判断の分かれ目は**到達点が変わったか**であって、道具が変わったかではない。
到達点は「壊した 1 行を元に戻す」で最初から最後まで同じであり、
`Edit` はこの節が**あらかじめ指定してある道具**である。
（ここを書き添えるのは、`cp` が止まった回に「手順どおりにやると禁止に触れるらしい」と
読まれると、この節ごと使われなくなるからである。止まったことの報告はする。
**報告することと、手順を疑うことは別である。**）
...
**この節を書いた本人が、書いた直後に `git checkout` を打っている**
（2026-08-19、引数なしだったので何も戻らなかったが、打ったこと自体は同じ）。
決まりを書いても手は変わらない、というのがここでの事実である。
**だからこの決まりは、覚える形ではなく、打たなくて済む手順の形で書いてある。**
上の 3 行（複製 → 壊す → 書き戻す）を先に並べておけば、後始末の場面で
「何で戻すか」を思い出す必要そのものが無くなる。
思い出さなければならない決まりは、思い出せなかった回に効かない。

複製からの書き戻しなら、戻る範囲が**複製を取った 1 ファイルの、取った時点の中身**に
限られる。他に何が未コミットで在っても関係が無い。
**区別できない道具は、区別できないことを忘れた回に効く。**

#### 但し書き: 書き戻す道具が塞がれている場合に限る例外 (2026-08-19)

**上の決まりは消さない。**次の 3 つが同時に成り立つときだけ、
`git restore` を**パスを 1 つずつ明示して**使ってよい。

1. `Write` / `Edit` / 複製からの書き戻しが、**見張りによって塞がれている**
2. 戻す先が **HEAD** である（＝見張りが守っている当の状態）
3. 戻すパスを**全部数え上げられる**

**なぜこれが迂回でないか。**この節が禁じている理由は上の 2 つ——
①戻した跡が見えること ②関係ないものまで消さないこと——であって、
`git restore` という名前ではない。1 が成り立つとき、
**この 2 つを満たす手段が `git restore` しか残っていない。**
そして 2 のとおり、行き先は見張りが守っている状態そのものである。
**見張りを迂回して別の状態を作るのではなく、見張りが守っている状態へ戻す。向きが逆である。**

条件を満たすときも、次を必ず守る。

- **`-- .` を使わない。**パスを 1 つずつ書く。
- **`git clean` は使わない。**未追跡ファイルは 1 つも消さない。
- **前後で `git status --porcelain` と `git diff --stat` を取り、
  変わったのが明示した分だけであることを数で示す。**未追跡の件数も前後で数える。

初出の適用 (2026-08-19): C03 の再生成で `system-spec/` の 10 ファイルが痩せた版に
なったが、当該ファイルは確定章ガードで `Write` / `Edit` / `cp` がすべて塞がれていた。
10 パスを明示して restore し、前 10 件・後 0 件、未追跡 0 → 0 を記録した
（痩せた版は scratchpad へ差分と全文を保全してから戻した。残課題 102）。

**この但し書きを一般化しない。**「塞がれていた」を理由にできるのは、
**塞いだのが自分ではない**場合だけである。自分で書いた検査に引っかかって
restore を使いたくなったら、それは検査のほうを直す場面である。
...
```
   ...
   └──────────────────────┘                       │ 緑ならマージ
                                                  ▼
                                   ┌──────────────────────────────┐
              （公開と切り離して）│  migrate.yml  手動 + APPLY   │
              （形だけ変えるとき）└──────────────────────────────┘

                                   ┌──────────────────────────────┐
                                   │  deploy.yml / inspect（45分）│
                                   │   pnpm verify（測る側）      │
                                   └──────────────────────────────┘
                                                  │ 緑のときだけ
                                                  ▼
                                   ┌──────────────────────────────┐
                                   │  deploy.yml / release（30分）│
                （本番は承認待ち）│   前回が途中で終わっていないか│
                                   │   控え（空なら中止・30日保管）│
                                   │   → データの形を合わせる     │
                                   │      （step 上限 10 分）     │
                                   │   → 未適用 0 件の確認        │
...
**器を 2 つに分けてあるのは速さのためではない。**測る仕事は変更の大きさで伸び、
出す仕事はほぼ一定である。同じ持ち時間を分け合わせると、変更が大きい回ほど
**やり直せない側が時間切れに近づく**。分けたので、測る側が伸びても出す側の
持ち時間は 1 秒も削られない。適用ステップの 10 分は job の上限より先に切れて、
打ち切りを `cancelled` として run に残す——それを次の回の先頭が読む
（[`11-CI-CD・品質ゲート仕様.md` §4-1-3](../spec/11-CI-CD・品質ゲート仕様.md#4-1-3-途中で止まったことが次の回に分かる)）。
