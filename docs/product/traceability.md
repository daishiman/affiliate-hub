...
| REQ | 要件 | 実装 | 画面 | 導線 | 状態 | RWD | a11y | test | 結果 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| REQ-TS01 | テストの土台を 1 箇所に集める（ファクトリ・テストダブル・担当者・時刻固定・描画補助・読み上げ検査） | `tests/support/`（`factories.ts` / `doubles.ts` / `actors.ts` / `clock.ts` / `render.tsx` / `a11y.ts`） | — | — | — | — | — | 土台自身は `tests/architecture/` の契約検査で「各テストが自前で組み立てていないこと」を見る | 未着手 |
| REQ-TS02 | 業務の決まりごとの単体テスト（順位に報酬を入れない／公開の条件／ループの外せない約束／止め方と上限／テナント境界） | `tests/domain/`（既存 9 ファイル） | — | — | — | — | — | PASS（`pnpm test tests/domain`） | 実装済 |
| REQ-TS03 | 手順の単体テスト。外側はすべてポートのテストダブルに差し替える | `tests/application/`（既存 8 ファイル） | — | — | — | — | — | PASS（`pnpm test tests/application`）。ただし組み立ては各ファイルが自前で書いており、REQ-TS01 で集約する | 実装済（土台への移行が残る） |
| REQ-TS04 | API の単体テストを**入口 3 種すべて**に対して、道具の一覧から生成する（正常系・入力検証・認証認可・テナント分離・エラー形式・スタブが失敗を返すこと） | `tests/presentation/one-usecase-three-adapters.test.ts`（3 つの入口が同じユースケースを通ることは確認済み）。網羅は未着手 | — | — | — | — | — | 一部 PASS。**道具ごとの入力検証・認可・テナント分離は未作成** | 未着手（拡張） |
| REQ-TS05 | 画面の単体テスト（4 つの状態・孤立ページ禁止・入力検証・キーボード操作・フォーカス・読み上げ） | `tests/ui/patterns-render.test.tsx` / `tool-form.test.tsx`（部品のみ）。ページ単位は未作成 | — | — | — | — | — | 一部 PASS。`src/app` のカバレッジは **0.0%** | 未着手（拡張） |
| REQ-TS06 | 読み上げ検査を機械で行う（axe）。**配色 5 種 × 明暗 2 種すべて**でコントラストを確かめる | コントラストは `tests/ui/theme-contrast.test.ts` で登録表から総当たり済み。axe は未導入（`axe-core` は導入済み、テスト未作成） | — | — | — | — | — | コントラストは PASS。axe は未着手 | 未着手（axe 部分） |
| REQ-TS07 | 結合テスト（生成 → 承認 → 公開 → 計測 → 分析 → 提案 → 承認 → 再生成の 1 周）。実際の D1 とマイグレーションを使う | `tests/integration/`（未作成） | — | — | — | — | — | 未実施 | 未着手 |
| REQ-TS08 | 境界値・異常系（0/1/上限/上限+1、空/最大/最大+1、日付と時差、ページ送りの端）。**とくに統計判定の境界**で、件数が足りないのに「差がある」と言わないこと | 判定の境界は `tests/domain/improvement.test.ts` に一部あり。件数・日付・ページ送りの境界は未作成 | — | — | — | — | — | 一部 PASS | 未着手（拡張） |
...
**未着手が 0 から 15 に増えているのは、要件が 18 件増えたため**（R 節 10 件・S 節 8 件）。
既に実装した 204 件のうち未着手へ戻った行は 1 つも無い。
新しい 18 件は、テストと自動チェックの仕組みそのもので、これから作る。
...
grep -E '^\| REQ-' $T | grep -cE '\| \**実装済'          # → 173
grep -E '^\| REQ-' $T | grep -cE '\| \**スタブ'          # → 34
grep -E '^\| REQ-' $T | grep -cE '\| \**未着手'          # → 15
# 合計（下の「画面を持たないことが正しい行」の表も `| REQ-` で始まるため、
# 単純な行数ではなく結果の欄で分類した 3 つの和を全要件数とする）
grep -E '^\| REQ-' $T | grep -cE '\| \**(実装済|スタブ|未着手)'   # → 204
```
...
画面義務のある要件 = A(10) + B(10) + C(18) + D(12) + E(11) + F(32) + O(5) + P(13) + Q(13) = 124 行。
このうち 9 行は**画面を持たないことが正しい**要件なので、義務のある行は **115**。
...
### 未着手 15 件について（R 節・S 節）

この 15 件は、**この文書を書いた時点でまだ作っていないテストと自動チェック**である。
...
- 「まずはコア機能から」といったスコープ縮小は行っていない。222件すべてを表に載せ、1件も未分類にしていない。
