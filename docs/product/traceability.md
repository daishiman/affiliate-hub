...
| REQ-SEO14 | 分析結果の自動反映。空の題名・要約を**記事の中にある材料**で埋め、記録を残す。記録が残らなかったら書き換えを戻して失敗を返す。止めているときは書き換えないが提示は続ける。導入前から公開されている記事は書き換えない。再現しない系統だけの所見は根拠にしない。画面の作りが原因の所見だけなら書き換えない。前回の反映から 14 日空くまで待つ | `src/application/usecases/seo/manage-seo-auto-apply.ts`、`src/application/seo/aeo-measurement/auto-fix.ts`、`src/domain/seo/aeo-measurement/auto-apply.ts` | `src/app/admin/seo/page.tsx`、`src/presentation/admin/observe/seo-aeo-forms.tsx` | サイドナビ「観測」→「SEO / AI 検索」→「自動反映」 | 止めている／待っている／材料が無い、を別の理由として表示 | 共通のToolFormで対応 | 共通①②（画面の検査は未着手） | `tests/application/seo/manage-seo-auto-apply.test.ts`、`tests/domain/aeo-measurement.test.ts` | 実装済（2026-09-04）。**「提案までとし承認を経ずに書き換えない」は差し替え済みの旧決定である**（運営者が止められる・戻せる・記録が残る、の 3 点で置き換えた） |
