...
| REQ-TS11 | 仕様章（`system-spec/*.md`）のコード塊が**開いてから閉じるまでで完結**し、章の本文や見出しを飲み込んでいないこと。**フェンスの本数が偶数であることでは判定しない**（backend.md は壊れていたときも偶数 2 本だった。行き場の無い閉じフェンス 2 本が互いに対になり、あいだの 169 行を丸ごとコード塊として飲み込んでいた） | `system-spec/` の 8 章と `index.md` / `00-requirements-definition.md`。これらは `spec-state.json` の `qa_log` から生成されるため、生成元が壊れていれば再生成で同じ形が戻る | — | — | — | — | — | **この行は語彙を足すためだけに書いた。まだどのテストもこの名前を名乗っていない**（宣言は次のコミットで行う） | 未宣言 |
