...
| REQ-CI05 | データの形の変更は、**控えが取れたときだけ**走らせる。順番は「控え → 空でないことの確認 → 適用 → 未適用 0 件の確認 → 公開」で、dev / 本番のどちらでも同じ。本番で人が判断するのはこの並びの**手前**（`environment: production` の承認）。手動経路（`migrate.yml` + `APPLY`）も残す | `.github/workflows/deploy.yml` / `.github/workflows/migrate.yml` | — | — | — | — | — | PASS（`tests/architecture/ci-config.test.ts`）。`deploy.yml` は `wrangler d1 export --env "$D1_ENV"` で控えを取り、`[ ! -s "$backup_path" ]` なら `exit 1` して適用へ進まない。控えは成果物として 30 日保管（`if-no-files-found: error`）。適用ステップに `if:` は無く、**環境で抜ける道を持たない**。`migrate.yml` は `workflow_dispatch` のみで `confirm != 'APPLY'` なら最初のステップが失敗する。2026-08-24 の run #17 まで本番だけ自動化から外していたが、その非対称が (a) dev で「一度落ちてから手で流す」を作法にし赤を鈍らせ、(b) 本番で人が踏む手順を機械の手順とずらしていた。**これらを機械が見る**（控えの空判定を消す・適用ステップに `if: env.IS_PRODUCTION != 'true'` を足す・本番の適用を `db:migrate:dev` へ戻す・前倒しの見張りを `fail` にする・保管日数を 29 にする・控えを適用より後ろへ動かす、いずれも赤になることを実測） | **実装済** |
