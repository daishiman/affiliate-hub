...
| REQ-CI05 | **本番の**データの形の変更を自動で走らせない。手動起動＋確認文字列 `APPLY` を必須にし、順番は「形の変更 → 公開」。dev はバックアップ付きで自動適用する | `.github/workflows/migrate.yml` / `.github/workflows/deploy.yml` | — | — | — | — | — | PASS（`tests/architecture/ci-config.test.ts`）。`migrate.yml` は `workflow_dispatch` のみ。`confirm != 'APPLY'` で最初のステップが失敗する。適用前に `wrangler d1 export` で控えを取り、成果物として 30 日保管。**`deploy.yml` のマイグレーションはすべて `IS_PRODUCTION != 'true'` の囲みの中にあり、本番の経路には 1 行も通らない。** 2026-08-24 に dev だけ自動へ寄せた（run #17。手動を dev にも求めていたため、移行を含む変更が必ず一度落ちてから手で流す作法になり、赤が合図として働かなくなっていた）。dev 側も控え → 適用 → 未適用 0 件の確認の順で、控えは 30 日保管する。**これらを機械が見る**（`APPLY` の判定を外す・`deploy.yml` に囲み無しの適用の行を足す・保管日数を 29 にする・控えを適用より後ろへ動かす、いずれも赤になることを実測） | **実装済** |
