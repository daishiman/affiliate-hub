...
> feat-blog-ui-builder A1〜A14 の現行合否は
> `docs/spec/feat-blog-ui-builder/acceptance-report.md` の 2026-08-31 現行判定を正本とする。
> この全体表に残る過去時点の説明を、同featureの最新合否として先回りで引用しない。
...
| REQ-P02 | §9.2 アフィリエイトURL受信箱（貼付・CSV・API・拡張・WebMCP・重複検出・分類・リンク状態・商品候補・4状態管理） | `src/domain/monetization/link-ingestion.ts`（取込元5種 `paste`/`csv`/`api`/`extension`/`webmcp`、状態4種 `received`/`resolved`/`matched`/`rejected`、`normalizeAffiliateUrl` によるURL正規化、`isInternalHost` の内部宛先拒否）、`src/application/usecases/monetization/manage-link-inbox.ts`（`claimNormalizedUrl` の取り合い結果を重複判定の正本にし、勝者本体の保存前でも敗者の重複印を失わない）、`src/presentation/tools/affiliate-tools.ts`（`submit_affiliate_url` ほか）、`src/presentation/admin/earn/inbox-action.ts` / `inbox-forms.tsx` | `src/app/admin/inbox/page.tsx` | サイドナビ「受信箱」+ ホームの「手当てが必要なもの」 | loading / empty（貼り付け欄と使い方を出す）/ error / 操作不可（スタブ表示で「まだ保存されません」を明示） | 対応 | 機械: 共通①（`ToolForm` 共通部品。入力欄にラベルと説明）。目: 共通②③④⑤。**「送信は44px最小」は 2026-08-21 まで根拠が無かった** — `--hit-min` という値が在るだけで、それを画面へ当てて確かめる検査が無く、`tests/ui/layout-density.test.ts` は `.navLink` 1 クラスしか見ていなかった。共通④がその画面側。未: 文中リンクの押しどころ | PASS（`tests/domain/link-ingestion.test.ts`、`tests/integration/d1-link-inbox.test.ts` 12 件。**2 人が同時に同じ URL を貼っても片方に必ず重複の印が付く**ことを実際の D1 で確認。勝者の保存を止める決定的barrier検査で、修正前は重複 0 件となるRED、claim結果を正本にした後はGREENを実測（2026-08-28）） | 実装済（保存先は D1。接続が供給されない環境＝`pnpm dev`・自動テストでは見本データへ回り、そのことを画面に出す。CSV一括と拡張機能の入口はスタブ） |
| REQ-P03 | §9.3 商品インテリジェンス（21属性・情報源・信頼度・有効期限） | `src/domain/product/product.ts`（共通属性を型で固定し、カテゴリー固有の属性は `specifications` で持つ）、`src/domain/product/product-identity.ts`（JAN/ASIN/型番による同一判定）、`src/domain/product/merchant-offer.ts`、`src/domain/shared/provenance.ts`（情報源・信頼度・有効期限）、`src/application/usecases/product/read-product.ts`、`src/presentation/tools/product-tools.ts` | `src/app/admin/products/page.tsx`、`src/app/admin/products/[product]/page.tsx` | サイドナビ「商品」+ 受信箱の商品候補 + 順位表の商品名 | loading / empty / error / 期限切れ（値を出さず「情報が古い」理由を表示） | 対応 | 機械: 共通①。目: 共通②③④⑤。未: 文中リンクの押しどころ | PASS（`tests/presentation/admin-routes.test.ts`） | 実装済（21属性のうちカテゴリー固有分は見本データ。外部からの自動収集はスタブ）。**2026-08-28 に `NAME_SIMILARITY_THRESHOLD`（0.92）を壊して測ったら、0.999 にしても 0.52 にしても 7990 件すべて緑だった**——`tests/domain/boundaries.test.ts`「同じ商品かどうかの境目」が入力を `matchIdentity(…, NAME_SIMILARITY_THRESHOLD)` と `- 0.0001` で組み立てていたので、しきい値をいくつに変えても入力が常にちょうど上と少し下に居続けた。同日、0.92 を手で書き写す形へ直し、床（実装の定数と一致）を 1 件足して両向き赤を再測定した |
...
| REQ-W10 | §16.6 マルチサイト重複対策（10軸差別化・言い換え禁止） | `src/domain/authoring/site-blueprint.ts` `DifferentiationAxes`（10軸）+ `differentiationGap()`（3軸以上）、`src/application/usecases/site/manage-sites.ts` が全ブログ対を判定。言い換え本文は `quality-check.ts` `similarity()` ≥0.85 で停止 | `/admin/sites`（近すぎるブログ対の警告） | PASS（`tests/domain/writing-rules.test.ts`「似たブログを増やさない」3件）。**2026-08-28 に `SAME_AXIS_SIMILARITY`（0.5）を壊して測ったら、0.1 へ**緩める向き**に動かしても 7990 件すべて緑だった**——この定数を見ている検査が 1 件も無く、既存の 4 件はどれも**境目から遠い**ペアしか渡していなかった（言い換えのペアは近さが高く、離れた軸のペアは `similarity()` が 0 を返すので、しきい値を下げてもどちらの側も動かない）。「単なる言い換え記事を量産しない」（§16.6）を止めているのはこの数なので、下げ放題では要件が名指しで禁じたものが通る。同日「軸を「同じ」とみなす近さの物差しが、決めた 0.5 で効いている」を足して塞ぎ、0.1 で赤になることを再測定した | 実装済 |
...
| REQ-W12 | ペルソナ差分の事実境界（fact_fingerprint 不変） | `src/domain/authoring/author-persona.ts` `checkFactBoundary()`、`src/application/usecases/authoring/manage-personas.ts` | `/admin/personas` + `src/presentation/admin/write/fact-boundary-form.tsx` | PASS（`tests/application/manage-personas.test.ts` / `invariants.test.ts`「FACT_BOUNDARY_VIOLATED」） | 実装済 |

## E. 生成基盤（本作業で新設）

| REQ | 要件 | 実装 | 画面 | test | 結果 |
| --- | --- | --- | --- | --- | --- |
| REQ-G01 | プロンプト設計（配置・バージョニング・7ブロック構造） | `src/domain/generation/prompt-blocks.ts`（`PROMPT_BLOCKS` 7件・`promptPath()`・`requireNewVersion()` で版の上書きを禁止） | `/admin/generation`「指示文の 7 つの塊」 | PASS（`tests/domain/generation-plan.test.ts`「指示文の組み立て」）。**件数は書かない**（2026-08-21 時点で 7 件と書いてあったが実際は 8 件。誰も突き合わせていない数は必ずずれるので、括りの名前だけを指す）。**2026-08-28 追記: `SHARED_PROMPT_FILES` から 1 項目抜いても 4 項目とも 8001 件すべて緑だった。** 共通の土台の一覧は src からも 1 か所も参照されておらず、写したきり誰も読んでいなかった。仕様 §07 §1-1 の樹形図を手で書き写して `toEqual` する検査を足した | 実装済 |
...
| REQ-G04 | 出力契約 `generated_variant` の JSON Schema 化 | `src/domain/generation/output-contract.ts`（必須20項目・`generatedVariantJsonSchema()`・`checkOutputShape()`・`verdictMayUse()` で自己申告点数を合否から除外） | `/admin/generation`「受け取りの形」 | PASS（`tests/domain/generation-plan.test.ts`「受け取りの形」）。2026-08-29 に壊して測った: 必須欄は `toHaveLength(20)` が件数を止めるだけで、**名前の当たりは自己参照**（`toEqual([...OUTPUT_REQUIRED_FIELDS])`）だった。`fact_fingerprint` を改名すると型検査 exit 0・検査も緑になる（実測）。同日、期待値を仕様 §07 の `"required": [...]` 20 名の手書き写しへ置き換えて自己参照を消し、改名で赤になることを確認した。`assumptions.who_decides` の enum（`ASSUMPTION_DECIDERS`）は当たりが 1 件も無く、`"supervisor"` を抜いても型検査 exit 0・9747 件緑だった。同日「決めきれなかったことの決め手は、仕様 §07 が並べた 3 者である」を追加して塞ぎ、赤になることを確認（1 failed / 44）。 | 実装済 |
...
| REQ-G07 | 執筆系と検証系の分離（GC-5） | 同 `agent-roster.ts`。`AuthoringAgent \| ReviewAgent` の判別共用体で、検証役に `"generate"` 道具を持たせるとコンパイルが通らない。`freshContext: true` も型リテラル。実行時の崩れは `separationBreaches()` | 画面義務なし（`/admin/generation` に崩れ検知の警告枠） | PASS（同テスト「崩れた一覧を渡すと、崩れとして返る」を含む） | 実装済 |
...
| REQ-E26 | Asset | `authoring/asset.ts`（**由来・利用条件・代替テキスト・保管先が無いと作れない**をテストで固定。2026-08-29 に 4 本の門を同時に素通しにしたところ**保管先だけ赤くならなかった**ので、`tests/domain/entity-invariants.test.ts`「保管先が空だと作れない」を足した） | R2（`storage-r2.ts`） | REQ-S06 | スタブ（解除条件: 画像の登録画面。保管先の実装と型は済んでいる） |
| REQ-E27 | Publication | `distribution/publication.ts` | 見本データ | REQ-S07 | 実装済 |
| REQ-E28 | Metric | `analytics/metrics.ts` | 見本データ | REQ-S08 | 実装済（数字の元は見本データ）。**2026-08-28 に `DEFAULT_METRICS_WINDOW_DAYS`（30）を壊して測ったら、3007 にしても 0 にしても 7990 件すべて緑だった**——`tests/application/read-metrics.test.ts`「問い合わせる期間は、既定で 30 日ぶん」が期待値を `DEFAULT_METRICS_WINDOW_DAYS * 24 * 60 * 60 * 1000` で組み立てていたので、定数と期待値が一緒に動いた。同日 30 を手で書き写す形へ直し、床（実装の定数と一致）を足して両向き赤を再測定した |
...
| REQ-WA02 | 管理側 状態変更8種（`create_affiliate_source_draft` 〜 `publish_approved_content`）+ 確認必須 | 同上。**8 種中 4 種**が動く。確認必須は `requiresHumanApproval` + `invokeTool()` が AI を弾く | REQ-S01〜S08 | スタブ（PASS: 同「面ごとの「呼べる数」が、対応表の判定欄に書いた数と合っている」で 4 を固定） |
...
| REQ-WC01 | `document.modelContext` を正規経路にする（CHG-001） | `src/presentation/ui/webmcp-provider.tsx` `resolveModelContext()`（`navigator` は後ろに置く旧経路） | PASS（`tests/presentation/webmcp-registration.test.ts`「条件 2 つの組合せ 4 通りが、1 行も欠けずに並んでいる」。判定欄は 2026-08-21 まで `—`（検査なし）と書いていたが、検査は在った） | 実装済（2026-08-26 に不具合を 1 件修正。ブラウザへ渡す宣言に、zod が貼る列挙できない隠し属性 `~standard`（中身は関数）が付いたままで、React が「plain object でない」と警告していた。`define-tool.ts` の `toJsonSchema()` でJSON へ写して素に戻す。`tests/presentation/tool-catalog-adapters.test.ts`「宣言が、ブラウザへ渡せる形であること」が全道具に当てて固定） |
...
| REQ | 要件 | 実装 | 結果 |
| --- | --- | --- | --- |
| REQ-M01 | Resources 8種 | `src/presentation/tools/spec-contract.ts` `MCP_RESOURCES`（8種）+ `mcp-adapter.ts` の `resources/list` / `resources/read`。中身は必ず既存のツールから取る（読み出しを二重に書かない）。画面は `/admin/tools` | 実装済（PASS: `tests/presentation/spec-contract.test.ts`「仕様書どおり 8 種ある」「中身はすべて既存のツールから取る」「`resources/list` が一覧を返す」「`resources/read` が中身を返す」「知らない場所は、黙って空にせず理由を返す」） |
...
| REQ-SEC11 | 公開・管理・認証の全配信面で、CSP・HSTS・nosniff・Referrer-Policy・Permissions-Policy・frame 制御を共通の正本から適用する。管理・認証面は frame 埋め込みを拒否し、ヘッダー値を Next 設定へ書き写さない | `src/infrastructure/http/security-headers.ts`（値と経路の正本）+ `next.config.ts`（薄い配信アダプター） | PASS（`tests/infrastructure/security-headers.test.ts`。必須ヘッダー、管理・認証面の `DENY`、公開面の same-origin、Next 設定が共通正本を使うことを固定） | 実装済 |
...
| REQ-QC05 | QC-07 根拠のない主張 | `src/domain/evidence/claim.ts` `isClaimUsable` + `quality-check.ts` `unsourced_number` / `missing_citation` / `stale_price`（`PRICE_STALE_HOURS`=72。§20.3 の価格の鮮度） | 実装済。**2026-08-28 に壊して測った。**`unsourced_number` と「価格の確認記録が無い」枝は赤（後者は消すと 1 件赤）。一方 **`PRICE_STALE_HOURS` を 72 から 9999 にしても 7984 件すべて緑だった**——上限を実質無効化しても止まらず、要件が名乗る 72 時間が飾りになっていた。同日 `quality-check-tables.test.ts` に「価格の鮮度の上限が、実際に効いている（QC-09）」を足して塞いだ（床 2 件 + ちょうど 72h は通る / 73h は止まる / 知らせ文が日数を名乗る）。72→9999 / 72→71 / 不等号を `>=` / 知らせ文の書きぶりの 4 通りで、狙った側だけが赤になることを再測定した。**期待値は要件の 72 を手で書き写している**（定数から組み立てると、値を変えても入力が同じ側に居続けて永久に赤くならない）。なお**表示側にも別の鮮度がある**（`merchant-offer.ts` `PRICE_FRESHNESS_HOURS`=24、REQ-TS08 の欄）。同じ言葉で数が 2 つある |
...
## O2. サムネイルの置き場所（feat-thumbnail-visual-system・2026-09-04 追加）

利用者要望「各画面のサムネイルを表示する。トップ画面にもサムネイルを表示する」への行。

**節 O（`REQ-TH01`〜`05`）とは別物である。**あちらは「画面の配色と明暗」で、
こちらは「記事に添える 1 枚の絵」。名前が似ているので `REQ-VIS` を使う。

「どの絵を使うか」（`src/domain/blogops/thumbnail.ts` の `resolveThumbnail`）は
既に実装があり、`tests/application/blog-ops-usecases.test.ts` が見ている。
**欠けていたのは「投稿された絵をどこへ置くか」**で、`createR2Storage` の
呼び出しが 0 件だった（＝アップロードされた画像の行き先が無かった）。

| REQ | 要件 | 実装 | 画面 | 導線 | 状態 | RWD | a11y | test | 結果 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| REQ-VIS01 | 投稿されたサムネイルの置き場所が 1 つの規則で決まる。形式（png/jpeg/webp）・大きさ（8MB）・派生の幅を門で断り、鍵は原本の中身の指紋を含んで**一度置いたら書き換えない**。配信の口は鍵の形だけを見て、同じバケットの別用途へ手が伸びないようにする | `src/domain/blogops/thumbnail-asset.ts`（`assertThumbnailIsStorable` / `thumbnailContentHash` / `thumbnailObjectKey` / `thumbnailGenerationPrefix` / `isDeliverableThumbnailKey`）。鍵の形は `blog-thumbnails/<サイト>/<記事>/<指紋>/original.jpg` と `.../640.jpg` | （この要件は純粋な規則で画面を持たない。使う側の画面は `REQ-VIS02` 以降） | — | 断る 4 通り（形式違い／空／8MB 超／配信しない幅）はすべて `VALIDATION_FAILED` に `field` を付けて返す | — | — | PASS（`tests/domain/blogops/thumbnail-asset.test.ts`） | 実装済。**派生（320/640/1280）を作るのは投稿する側のブラウザ（Canvas）である。**workerd に画像デコーダが無く、`wrangler.jsonc` に Images バインディングも置いていないため、サーバーでは縮小できない。サーバーが確かめられるのは形式・バイト数・鍵の形だけで、「送られてきた幅が本当にその幅か」は画素では見ていない。**嘘の幅が入っても 1 記事の 1 世代に閉じる**（鍵が中身の指紋を含み、書き換えないため）という設計でその穴を受けている |

| REQ-VIS02 | 上げた 1 枚を R2 へ置き、読者へ配り、差し替えたら古い世代をまとめて消す。**置き場が落ちても運営者の作業を丸ごと失わない**（原本が置けなければ進まないが、派生が落ちても原本は残す） | `src/infrastructure/platform/blog-thumbnail-r2.ts`（`putThumbnailGeneration` / `readBlogThumbnail` / `deleteThumbnailGeneration` / `blogThumbnailHref`）、配信の口 `src/app/api/blog-thumbnails/[...key]/route.ts`、保存先 `blog_article_thumbnail`（`drizzle/0053_blog_article_thumbnail.sql`）、候補への合流 `src/infrastructure/persistence/d1/blog-ops-repository.ts` の `thumbnailCandidatesByArticle` | 読者側の全画面（一覧のカード・記事上部・OGP）。管理画面の登録欄は未着手 | `/api/blog-thumbnails/<鍵>` | 置き場が落ちた＝`UPSTREAM_UNAVAILABLE`（`retryable`）。門を通らない鍵・物が無い＝ともに 404 | 対応（幅は 320/640/1280 の 3 枚。置けた幅だけを記録し、`srcset` に 404 を並べない） | — | PASS（`tests/infrastructure/blog-thumbnail-r2.test.ts` 19 件） | 実装済。**配信の口には認証を置いていない。**隣の `/api/feedback-captures/<id>`（`REQ-FB13`）は 401/404 を厳密に使い分けるが、あちらが渡すのは運営者の画面の写しで、こちらは読者に配る絵である。認証を挟むとログインしていない読者＝ほぼ全員の一覧が絵の無い箱で埋まる。代わりに `isDeliverableThumbnailKey` を **R2 を引く前**の門にして、同じバケットの `feedback-captures/` へ手が伸びないようにしてある（引いてから 404 を返すと、別用途の名前を総当たりで試せる口になる）。**`cache-control` が 1 年・`immutable` なのは鍵が中身の指紋を含むからで、この 2 つは切り離せない** |

| REQ-VIS03 | 運営者が記事 1 本に表紙を 1 枚だけ登録し、いつでも外せる。**失敗しても「参照されているのに絵が無い」状態を 1 つも作らない**（登録は置き場→台帳、取り消しは台帳→置き場の順で、残ってよいのは誰も参照していない絵だけ）。同じ絵を上げ直したときに、いま置いた絵を自分で消さない | `src/application/usecases/blog-ops/manage-article-thumbnail.ts`（`createSetArticleThumbnailUseCase` / `createRemoveArticleThumbnailUseCase`）、口 3 つ `src/application/ports/blog-ops.ts`（`findArticleThumbnail` / `saveArticleThumbnail` / `deleteArticleThumbnail` と `ArticleThumbnailStoragePort`）、実装は D1（`blog-ops-repository.ts`）・sample・test fake の 3 つ、記録は `blog_article_thumbnail.set` / `.removed`（`src/domain/compliance/audit-log.ts`） | 管理画面の登録欄は未着手（この行が持つのはアプリケーション層まで） | — | 断るのは形式違い・8MB 超・説明文が空・記事が無い・権限が無い。置き場が落ちたら `UPSTREAM_UNAVAILABLE` で台帳に何も書かない。**古い世代を消せなかったことは失敗にしない**（差し替えは成功していて読者には新しい絵が出ている。ここで失敗を返すと押し直しのたびに世代が増える） | — | 説明文（`altText`）が空なら断る。絵が出ないときと読み上げで聞く読者には、ここだけが届く | PASS（`tests/application/manage-article-thumbnail.test.ts` 17 件） | 実装済。**`saveArticleThumbnail` は upsert の前に記事の持ち主を確かめる** — 表の主キーが `article_id` なので、確かめないと id を知っているだけで他所の作業場所の表紙を差し替えられる。`derivedWidths` を JSON 文字列 1 列で持つのは、配信する幅を増やす日に migration を要らなくするため。読み出しの `parseDerivedWidths` は壊れていたら例外ではなく `[]` を返す（派生 0 枚は原本 1 枚で成立する既知の状態で、ここで投げると一覧全体の読み取りが落ちる） |

**まだ無いもの**（この行の範囲外。埋めるまで要件表に行を足さない）:
管理画面の登録欄（ブラウザ側 Canvas で派生を作って送る）、
8 画面への `width`/`height` 属性による CLS 対策、
参照が外れた古い世代の掃除を定期実行へ載せること。
...
| REQ-TM06 | 節ごとの滞在時間を構造上のまとまり（導入・順位・比較・根拠・CTA など）で測る | `src/presentation/ui/templates/article-view.tsx`（節が種類を名乗る）、`src/presentation/telemetry/collector.tsx`（`IntersectionObserver` で半分見えたら計時、1 秒未満は捨てる） | 記事画面（`/s/{site}/best/*`・`/reviews/*`・`/compare/*`・`/guides/*`） | 記事を読む操作 | 見えていない節は測らない／同意が無ければ測らない | 対応 | 対応 | PASS（`tests/ui/telemetry-attrs.test.tsx`＝記事の節が持つ識別子と種類の契約、`tests/ui/telemetry-collector.test.tsx`＝半分見えたら計時を始める・1 秒未満は捨てる）。**2026-08-19 まで、この欄が挙げていた `TELEMETRY_SECTION_KINDS` は `tests/` 全体で参照 0 件で、節の名乗りを丸ごと外しても緑だった**（滞在時間が静かに 0 件になる壊れ方） | 実装済（保存先は D1 の `telemetry_events`） |
...
| REQ-TS08 | 境界値・異常系（0/1/上限/上限+1、空/最大/最大+1、日付と時差、ページ送りの端）。**とくに統計判定の境界**で、件数が足りないのに「差がある」と言わないこと | `tests/domain/boundaries.test.ts`（統計判定・有効期限・商品同一性・会計期間・表現ポリシー、64 件）と `tests/domain/boundaries-platform.test.ts`（契約プランの上限・役割・金額・価格の鮮度・比較表の上限・配信の状態遷移、57 件） | — | — | — | — | — | 実測（2026-08-19、`pnpm vitest run`）: `boundaries.test.ts` **64 件** + `boundaries-platform.test.ts` **58 件** = **122 件**。**ここに「121 件」と書いてあったのは古い数字**。必須種別を宣言済（`has-input`）で、`boundaries-platform` の印を外すと `equivalence` が欠けて赤になる。`boundaries` の印だけを外すと緑のままだが、これは相方が `boundary` を別に持っているためで、弱点ではない。**2026-08-28 に「価格の鮮度」を壊して測ったら、ここに数えていた分が空振りしていた。**`PRICE_FRESHNESS_HOURS` を 24 から 9999 にしても 1 にしても **7989 件すべて緑**だった——境目の入力を `hoursAfter(PRICE_FRESHNESS_HOURS)` で作っていたので、**定数をいくつに変えても入力が同じ側に居続けた**（試験名だけが「ちょうど 24 時間」と名乗り続けていた）。同日、名乗る 24 を手で書き写す形へ直し、床（実装の定数と一致）を 1 件足した。24→9999 / 24→1 / 不等号を `>=` の 3 通りで、狙った側だけが赤になることを再測定した。なお**「価格の鮮度」は 2 か所に別の数で在る**（表示側 `PRICE_FRESHNESS_HOURS`=24 と品質検査側 `PRICE_STALE_HOURS`=72）。この欄が数えていたのは前者だけで、後者は誰も見ていなかった（REQ-QC05 の欄に別記） | 完了 |
...
| REQ-TS12 | ログイン不要の「静止した写し」（`pnpm run preview:static` / `preview:blog` / `preview:site`）が、**本物の CSS を読まずには書き出せない**こと。写しは見た目を見て判断してもらうために渡すので、実物からずれると**ずれた見た目のほうで判断が決まる**。しかもずれは開いた人には見えない（トークンが読めていなければ、素の見た目の 1 枚が「これが実物です」という顔で出るだけ）。あわせて、焼いた写しを**アプリが配る場所（`public/`）へは置かない** — そこへ置くと入口の門を通さずにアプリ自身が配ってしまい、「別に作った静止画」ではなく門に開けた穴になる（門そのものは REQ-S10 / `tests/architecture/open-doors.test.ts` の側で測る） | `scripts/lib/static-preview.mjs`（`src/app/globals.css` の変換、module CSS 探索、`buildDocument`、`docs/` 配下への安全な書き出しを担う共通 runner）、`scripts/write-static-preview.tsx` / `scripts/write-blog-preview.tsx` / `scripts/write-site-preview.tsx`（body・metadata・出力先だけを渡す adapter）、`scripts/lib/css-module-hook.cjs`（`.module.css` の名前をそのまま通す）。見本帳の 22 節は `src/app/admin/ui-catalog/density-samples.tsx` へ出してあり、焼く側と画面側が**同じ部品を描く**（写しを作らない） | — | — | — | — | — | `tests/architecture/static-preview-writer.test.ts`（16 件）。**2026-08-28 実測**: 共通 runner 契約を先に置いた RED は 15 件中 2 件が失敗（runner 未実装 / 3 writer が CSS 取得・書き出しを重複）。実装後は 16/16 が緑で、`docs/../public` を含む `docs/` 外の出力も CSS を読む前に拒否する。3 つの preview コマンドはすべて成功し、生成 HTML 3 件の SHA-256 はリファクタ前後で完全一致。**2026-08-19 の 12 件は writer 1 本時点の旧記録**である。必須種別は `has-input`（→ `equivalence`, `boundary`）と `has-shared-visual-form`（→ `visual`）。 | 完了 |
...
| REQ-TS20 | **`qa_log[].source.sha256` が何の指紋なのかを、数で固定する。**欄の名前は `source.sha256`、隣には `source.path` が並ぶ。読んだ人は素直に「出典文書の指紋」と受け取る。実際に入っているのは **`sha256(answer)`** — **その entry が自分で書いた文章の指紋**である。自分で書いた文字列から作る値なので、**書き換えれば指紋も一緒に動く**。引用が原典からずれていっても、この値は永遠に一致し続ける。**この欄は、名前が約束していることを一度も果たしていない。**文章では直せない — 契約の側に「出典文書の指紋」を持つ欄が無いからである。だから `REQ-TS13` / `REQ-TS14` と同じ扱いにする: **塞げていないことを数で固定し、塞がった日にここを赤くする（② の形）。**当てどころは 5 つ: ①母集団の床（書面由来 ≥ 23。0 件なら以下の主張は全て空振り。REQ-TS17 の形） ②**欄を持つのは書面由来だけ・書面由来は全て欄を持つ**を両方向で固定（**件数では固定しない** — 「欄なし」の数は対話で聞いた論点が増えるだけで動き、増えるのを止める理由が無い） ③`source.sha256 === sha256(answer)` が 23 件すべてで成立（不一致 0 件） ④`source.sha256` が引用先ファイルの指紋と **1 件も一致しない**（引用先 23 件の実在を同じ床で確かめてから測る。開けなかっただけを「一致しない」と読ませない） ⑤**9 つの文書に対して 23 通りの指紋がある。**③④ は「たまたま全部ずれている」で説明が付くが、**1 つの文書が 11 通りの指紋を持つことは、文書の指紋では原理的に起こらない**（`docs/spec/01-要求仕様書-v1.0.md` だけで 11 通り）。**⑤ だけが、この値が指しているものが違うことの証明になっている。** | `system-spec/spec-state.json` の `qa_log[].source`（`kind` / `path` / `section` / `sha256`）、および読む側 `.claude/plugins/system-spec-harness/scripts/validate-coverage-matrix.py` / `.../scripts/foundation_provenance.py` | — | — | — | — | — | `tests/architecture/qa-source-digest-meaning.test.ts`（8 件、2026-08-28 実測で全緑）。**起票時（2026-08-19）の 4 つの数のうち 2 つが動いていたので、閉じる前に取り直した**: 一致 23（不変）/ 不一致 0（不変）/ **欄なし 6 → 19**（`qa_log` が 29 → 42 件へ増え、増えたぶんは全て `user-dialogue`）/ **この値を読んで判定するコード 0 件 → 2 件**。後者が動いたので**陽性対照を足した** — 読む側が居なくなると ③④⑤ は全部緑のまま欄だけが腐る。**陽性対照は 1 度トートロジーで書いて、締め直した**: 最初は `'hashlib.sha256(...)'` の前方一致で、注釈でも通る形だった。次に「digest を作る行と比べる行が同一行」に締めたら `validate-coverage-matrix.py:202` が赤くなった — そこは `actual = hashlib.sha256(...)` の**単純代入**で、比較は 205 行目にある。`foundation_provenance.py:89` は 1 行の `elif ... != digest` に収まる。**同じ意味・違う形を、書き方の癖で落としていた。**作った行から **5 行の窓**で比較を探す形へ替えた。**窓を無制限（ファイルのどこかに `==` があればよい）にしないのは、無関係な比較で緑になるからである** — 緩めるときは緩めた幅を有限にする | 未解決（穴として固定中。**塞ぐには契約側に「出典文書の指紋」の欄が要る。**足す先は確定 23 entry の `source` なので、R4-reopen 経由でしか触れない。塞がった日にこの行の ④⑤ が赤くなる。**そのとき消さず「0 件」へ反転させて残すこと**） |
| REQ-TS21 | **`reopen` で退避した欄が再確定で戻らない穴を、症状ではなく原因の側で固定する。**`reopen` は確定セルの欄を `reopen_log[].discarded` へ退避してセルを「未収集」へ戻すが、**再確定（`confirm`）は退避を戻さない。**戻す窓口は欄ごとに個別に作られている（`restore-qa-refs` は `qa_refs` 専用、`set-serves` は `serves_goals` 専用）。**だから欄が増えるたびに、同じ穴がもう 1 つ空く。**実測された 3 度は (1) `required_info` / `required_info_checks`、(2) `qa_refs`、(3) 再び `required_info_checks`——3 度目のとき確定 8 セル全ての `required_info_checks` が 1 件から 0 件になっていた（`ah-nuu`）。**症状だけを見ても足りない。**「戻っていない欄」は直せば消え、2026-08-28 の実測では実際に 0 件である。**だが原因は 2 つとも構造の側に残っている**: **(A)** 退避する欄が `state_transition_matrix.py` の `reopen` 分岐に**リテラルの一覧**で書かれており、確定セルに新しい欄が生えたとき載せ忘れると**その欄は reopen で黙って消える**（これが 4 度目の入口）/ **(B)** 戻す窓口が欄ごとに個別なので、**退避されるが戻す窓口が無い欄**が作れてしまい、その欄は「一度 reopen したら二度と戻せない」。**(A) は上限 0、(B) は上限 1 で固定し、どちらも上げる向きには動かさない。****(B) がいま 1 件なのは `serves_intents`** — 退避リストに載っているのに値を書ける op が 1 つも無い。いま実データのどのセルも持っていないので無害だが、**誰かが書いた瞬間に「二度と戻せない欄」になる。**これは (2) の `qa_refs` が辿った道そのもので、そのときは `split-qa-bundle` が `scope_notes.bundled=true` を要求するため解除済みの 6 件が全部拒否され、戻す道が実質無かった。**先回りで退避リストへ載せたことが、逆に「戻せない欄」を 1 つ作っている。**消すのではなく 1 件であることを固定し、窓口ができた日に赤くする | `.claude/plugins/system-spec-harness/skills/run-system-spec-elicit/scripts/state_transition_matrix.py` の `apply_cell_op` の `reopen` 分岐（退避リスト）と、値を書く 6 op（`confirm` / `restore-qa-refs` / `extend-qa-refs` / `split-qa-bundle` / `set-serves` / `set-required-info` / `record-required-info-check`）、および `system-spec/spec-state.json` の `matrix` と `reopen_log[].discarded`。**このファイルはキット配布物で、次回のキット更新で上書きされる**（`REQ-TS18` と同じ事情） | — | — | — | — | — | `tests/architecture/reopen-discard-restore-gap.test.ts`（7 件、2026-08-28 実測で全緑）。**退避リストを検査へ書き写していない** — 書き写すと実装から欄が消えた日に検査だけが古い一覧を持ち続け「載っている」と言い続ける。実装の `for key in (...)` を読む 1 経路にしてある。**同じ理由で、窓口の表（手で書いた `WRITER_OPS`）が実物から離れていないことを別の `it` で見る** — `action == "…"` を実装から集め、表にあって実装に無い op が 0 件であること。**床を 3 つ置いた**（REQ-TS17 の形）: 確定セル 8 件 / `reopen_log` ≥ 69 かつ**退避を実際に持つ entry** ≥ 50（退避を持たない entry が混ざるので全体の数だけでは床にならない）/ 退避リストを実装から 5 欄以上読めていること（読めていないと差分は「全部載っていない」か「全部載っている」へ倒れて意味を失う）。**壊して測った**（2026-08-28）: 退避リストから `required_info_checks` を 1 行外す → **(A) だけが赤で、消えた欄を名指しした**（`expected [ 'required_info_checks' ] to deeply equal []`）。他の 6 件は緑のまま。戻して `git diff` が空であることを確認済み | 未解決（**症状は 0 件、原因は 2 つとも残っている。**塞ぐには「退避した欄を再確定時にまとめて戻す」単一の窓口が要る。それは `confirm` の契約を変えることであり、確定 8 セルすべてに影響する。いまは 4 度目が起きた瞬間に赤くなる状態で止めてある） |
...
| REQ | 要件 | 実装 | 画面 | 導線 | 状態 | RWD | a11y | test | 結果 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| REQ-CI01 | `pnpm verify` が CI とまったく同じ検査を再現し、依存解決の正本も pnpm 1 つに固定する（機械の上でしか試せない状態や、別の lockfile だけが更新される状態を作らない） | `scripts/verify.mjs`、`package.json` の `verify` / `packageManager: pnpm@10.19.0`、`pnpm-lock.yaml` | — | — | — | — | — | PASS（`tests/architecture/ci-config.test.ts` / `tests/architecture/package-manager-source.test.ts`）。**2026-08-28 依存正本の実測**: 追加した検査は未追跡 `package-lock.json` の存在を名指しして RED。それと参照 0 の `list-sweep.json` だけを除き、`packageManager` が `pnpm@`で始まること、`pnpm-lock.yaml` が存在すること、root に `package-lock.json` / `yarn.lock` / `bun.lock` / `bun.lockb` が無いことで GREEN。`pnpm install --force` で欠落 binding を復旧し、続けて `pnpm install --frozen-lockfile` も成功。`pnpm-lock.yaml` の SHA-256 は前後で同一（`0d62d1cf828f535b664b0c6f327543a86de9dd2112f7ea6a935caf119ca90ffe`）。**2026-08-19 に、この欄は事実と違っていた**——「検査ステップは `pnpm run verify` の 1 行のみ」と書いてあったが、実際には「マイグレーション未生成の検出」という独自のステップが `ci.yml` にあり、`pnpm run verify` には含まれていなかった。文章ではなく実装を直し、`scripts/migration-generated.mjs` として `verify` の `CHECKS`（1 段）へ移した。いまは `ci.yml` の `run:` が 3 行（`pnpm install` / `verify --tier 1` / `verify`）で、それ以外の検査ステップと複数行シェルが無いことを機械が見る。**2026-08-21 追記**: `tests/architecture/ci-config.test.ts` は `.github/workflows/` 5 本すべての `run:` を読み、検査の道具の直叩きを禁じる。検査は `verify` か正本に載る `scripts/*.mjs` を通し、読み取り先と行数に床を置いて空振りを防いでいる。 | **実装済** |
...
| REQ-CI04 | 日常的に書き込む経路は検査・公開・明示的な D1 変更の 3 責務に分け、読み取り中心の枝順・深い門・AI 評価・Actions 使用量を合わせた 7 ワークフローの責務を重ねない。詳細は [`11-CI-CD・品質ゲート仕様.md` §4](../spec/11-CI-CD・品質ゲート仕様.md#4-ワークフローは壊すものの重さで分ける) | `.github/workflows/*.yml` | — | — | — | — | — | PASS（`tests/architecture/ci-config.test.ts`が名前・総数・引き金を固定） | **実装済** |
| REQ-CI05 | 通常公開は「控え → 未適用の前方移行 → 台帳・形の確認 → 公開」を `deploy.yml` だけで完了する。`migrate.yml` + `APPLY` は公開と分けた明示適用・前方修復に限定し、同一環境の D1 操作は同時実行しない。詳細は [`11-CI-CD・品質ゲート仕様.md` §4-1](../spec/11-CI-CD・品質ゲート仕様.md#4-1-データの形は控えが取れたときだけ変える) | `.github/workflows/deploy.yml` / `.github/workflows/migrate.yml` | — | — | — | — | — | PASS（`tests/architecture/ci-config.test.ts`） | **実装済** |
...
| REQ-CI14 | GitHub Actions の**口座全体**の月間使用量を見張り、枠の 70% 超で警告・90% 超で誤り扱いにする。**この見張り自体ではマージも公開も止めない** | `quality-gates.config.mjs`（`ACTIONS_USAGE` の正本）、`scripts/actions-usage.mjs`（現行 user/org usage API としきい値の判定）、`.github/workflows/actions-usage.yml`（週 1 回＋手動） | — | — | — | — | — | PASS（`tests/architecture/actions-usage.test.ts` **35 件**）。**公開のあいだ、これは見張りではない**——標準ランナーは公開リポジトリでは無料・無制限で分母が無く、使用率という数が作れない。だから公開中は `::notice::`「対象が無い」だけを出し、**0% を報告しない**（0% は「余裕がある」ではなく「測っていない」の意味になる）。**非公開になった日に黙って通らない**——非公開かつトークン未登録なら `::warning::` が出る。武装していない見張りが緑で通ると、見ていないまま「見ている」と思われる。しきい値の踏み方は `ACTIONS_USAGE_MINUTES` で外から与えて**実測した**（1,400 分=70.0% は緑、1,402 分=70.1% で `::warning::`、1,802 分=90.1% で `::error::`、いずれも終了コード 0）。**壊して測った**: ①境目を `>=` に緩める → 2 件赤 ②公開時に 0% を出す形へ戻す → 1 件赤 ③非公開＋トークン無しを緑にする → 2 件赤 ④yml にしきい値 `2000` を書き写す → 1 件赤 ⑤ワークフローの yml を消す → `ci-config.test.ts` が 5 件赤。さらに、テストの中に偽のトークンを**直に書いた**版では `tests/architecture/test-honesty.test.ts` の「鍵らしい形の文字列が 1 つも無い」が **1 件赤**になった（実測。値は組み立てて渡す形に直した）。現行GitHub APIのuser/org fixture、旧schema拒否、正本importも検査済み。**見ていないもの**: 実際の非公開口座のAPI応答（tokenを登録して実行するまでは未確認） | **実装済**（非公開になるまで待機中） |
| REQ-CI15 | 適用後・公開前に、全 migration を適用した一時ローカル D1 と対象環境の `sqlite_master` を同じ方法で比べ、不足・余り・同名異所属/異定義・測定不能のいずれも落とす。事故経緯、判定範囲、修復手順は [`11-CI-CD・品質ゲート仕様.md` §4-1-2](../spec/11-CI-CD・品質ゲート仕様.md#4-1-2-形のずれ) を正本とする | `scripts/check-schema-drift.mjs`、`.github/workflows/deploy.yml`（適用後・公開前）、`.github/workflows/migrate.yml`（適用後） | — | — | — | — | — | PASS（`tests/architecture/schema-drift.test.ts` 17 件。同名異所属・異定義、外部失敗、JSON 解析、有限再試行、timeout の fail-closed を含む。CI-AC-11 / CI-AC-21） | **実装済** |
| REQ-CI16 | 公開物（Worker）の大きさを、**配る前に**測って残りの余白を必ず示す。Worker の入口が引き込むコードは `handler.mjs` と二重に載るため、入口の引き込み量にも上限を張る | `scripts/worker-size.mjs`（`deploy:dev` / `deploy:prod` の build と deploy の間）、`src/infrastructure/platform/distribution-scheduler.ts`（cron が総目録を引かない） | — | — | — | — | — | **2026-08-31 に事故から起こした要件**。Cloudflare Workers の上限は **1 Worker あたり 3 MiB（gzip 後）**で、8/30 の公開は gzip 3065 KiB で超過して落ちた。原因は急な増加ではなく、**残り 6.5 KiB という状態が何回もの緑の公開をまたいで誰にも見えていなかった**こと。内訳は `worker-entry.js` → `distribution-scheduler.ts` → `createDeps()` が 226 ファイル・1018 KiB を二重に載せていたぶんで、cron が使う口は 5 つだけだった。直に組み直して **gzip 3065.49 → 2937.80 KiB**（余白 6.5 → 134 KiB）。PASS（`tests/architecture/worker-entry-weight.test.ts` 3 件：入口から手が届く量の上限 130 ファイル / 1050 KiB、総目録を引いていないこと、0 件で緑にならない床）。**壊して測った**: `createDeps` の import を書き戻すと 2 件赤（195 ファイル／総目録の名指し）。`scripts/worker-size.mjs` も上限を 2000 KiB に下げて**赤を実測**、読み取り不能・環境未指定もそれぞれ終了コード 1。**落とす条件は上限そのもので、余白が細いだけでは止めない**（止めても直す手立てがその場に無く、人は上限のほうを緩める）。余白 200 KiB を割ると警告が出る＝**いまは鳴っている状態**。**見ていないもの**: `handler.mjs` 側（画面・API・ルート）が何で太るかの内訳 | **実装済**（余白は警告域） |

**REQ-CI16 の現行判定（2026-09-07）**: 上の行に残した 3 MiB / 10 MiB と
gzip 余白の記録は、2026-08-30〜31 の事故と当時の回帰試験を示す履歴である。
Cloudflare は 2026-09-04 に圧縮後のプラン別上限を廃止し、全プラン共通の
**非圧縮 64 MiB**へ変更した。`worker-size.mjs` は非圧縮値を合否へ使い、gzip は
参考表示だけにする。2026-09-07 の dev dry-run は非圧縮 16,766 KiB / 65,536 KiB。
Worker入口の構造上限は別に維持し、GSC検索語の取得・継続保存という実際のcron責務を
追加した実測 141ファイル / 1,237 KiBに対して、145ファイル / 1,300 KiBとした。
...
| REQ-FB03 | 押すと出る書き込み欄。画面名が自動で入る。種類は 3 つ。「改善したいこと」は必須、「どうなってほしいですか」は任意 | `feedback-button.tsx`（ボタンと書き込み欄は**同じ 1 ファイル**。開閉の状態が 2 つに割れないため）、`src/domain/feedback/report.ts` | 同上（重ねて出す） | ボタン → 書き込み欄 → 送信 → 一覧へ | 未入力 / 入力中 / 送信中 / 失敗 / 送信済み | 対応 | 対応（Esc で閉じる、フォーカスを閉じ込める、必須は色以外でも示す。**この欄は 2026-08-18 まで嘘だった**——Esc もフォーカスの閉じ込めも実装に無く、2026-08-19 に書いてあるほうへ実装を合わせた） | PASS（`tests/ui/feedback-button.test.tsx`、`tests/domain/feedback.test.ts`：必須の空欄・本文 4000 文字の境界・「どうなってほしいか」200 文字の境界・種類は 3 つだけ・Esc で閉じる・開くとフォーカスが中へ入る・Tab が両端で折り返す）。**赤を実測済み**（2026-08-19）: Esc / 折り返し / 開いたときの移動をそれぞれ外すと落ちる。端も別に当てた（4000 と 200 を 1 ずつ動かすと落ちる）。**2026-08-28 に `STORED_DIAGNOSTICS_LIMIT`（診断行を 8 件までに切る）を壊して測ったら、807 にしても 0 にしても 7990 件すべて緑だった**——この定数を見ている検査が 1 件も無かった（`W03` 型）。上限を実質無効化しても、逆に全部捨てても止まらない。同日 `tests/domain/feedback.test.ts` に「診断行は新しいほうから 8 件だけ残す（古いぶんは落ちる）」を足して塞ぎ、807 / 0 の両向きで赤を再測定した。**同日、この行が「端も別に当てた（4000 と 200 を 1 ずつ動かすと落ちる）」と書いていたぶんも測り直した。**上限を 4000→400007 / 200→20007 と**緩める向き**へ動かすと**どちらも緑のまま通った**——`MAX + 1` で超える側の入力を作っていたので、上限をいくつ上げても入力が一緒に伸びて常に 1 文字だけ超えた位置に居続けた。欄が書いていた「1 ずつ動かすと落ちる」は**入力の側を 1 動かした話**で、**上限そのものを動かした話ではなかった**（`TM11` 型：欄が実態より強い）。4000 と 200 を手で書き写す形へ直し、床を足して緩める向きでも赤になることを再測定した（期待値は 8 を手で書き写している。`slice(-STORED_DIAGNOSTICS_LIMIT)` で作ると定数を変えても同じ側に居続ける） | 実装済 |
...
| REQ-FB05 | 写しへ書き込める（手書き・四角・矢印・文字・黒塗り／赤・茶・青・黒、黒塗りは色を選べない）。**元に戻す・撮り直す・画像を外して文章だけで送る**ができる | 同上 + `src/domain/feedback/capture-policy.ts` | 同上 | 道具を選ぶ → 描く → 戻す／撮り直す／外す | 道具ごとの選択状態、履歴の有無で「元に戻す」の可否が変わる | 対応（指でも操作できる） | 対応（道具の選択を色だけで示さない。道具・色・逃げ道に加え、**描く操作そのものもキーボードだけで完了できる**——矢印キーで位置、Enter で始点と確定、Esc で取り消し、Shift で 1 画素刻み。位置と段は `aria-live` で読み上げる。**「対応」に戻した根拠はこの 1 文ではなく、`tests/ui/capture-canvas.test.tsx` の describe「キーボードだけで印を置ける」9 件が緑であること**。2026-08-19 に、9 件のうち先に書いた 7 件で実装前の赤 6 件を実測し（残り 1 件は退行の見張りなので実装前から緑）、あとから足した 2 件は壊して赤を実測した。**2026-08-26 追記: 座標を扱わない道を足した。**黒塗りを選ぶと画面を縦横 3 つずつに区切った 9 つのボタンが出て、「上段の左」のように**位置の数を 1 度も動かさずに**1 つ確定できる。矢印キーの道は**置き換えていない**——要素になっていないもの（画像の中の文字、表の一部）は座標でしか指せないため、両方残してある。**要素の名前（「3 番目の見出し」）で選ぶ道は採れなかった**: 写しは `getDisplayMedia` の映像 1 枚で、本人が別のウィンドウを選べる以上、いま開いている DOM の位置が写った画像と一致する保証が無い。ずれた位置を要素の名前で呼んで塗ると**隠したはずのものが隠れていない写し**が送られる（`tasks/task-capture-element-selection.md` の実行の記録に測定を残した）） | PASS（`tests/ui/capture-canvas.test.tsx`：道具・元に戻す・画像を外す・道具と色は素の `<button>`・選択は `aria-pressed` が運ぶ・道具の並びと色の並びはそれぞれ名前を持つ・正の `tabindex` を使わない）。加えて**キーボードだけで印を 1 つ確定できる**（矢印＋Enter で黒塗り／四角／文字、Esc で取り消し、確定した印が写しの申告に入る、位置の目印は外へ出す 1 枚へ焼き込まない）。**赤を実測済み**（2026-08-19）: `aria-pressed` を外す／まとまりの名前を外す／外へ出す 1 枚に目印を焼き込む／目印を常に描く、いずれでも落ちる。区画の道も同じ file の describe「座標を扱わずに、区画で黒塗りを置ける」7 件で固定してある（2026-08-26。**赤を実測済み**: 区画の塗りを画素へ描かなくすると 3 件落ちる）。現在 42 件緑 | 実装済 |
...
| REQ | 要件 | 実装 | 画面 | 導線 | 状態 | RWD | a11y | test | 結果 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| REQ-SEO01 | 画面と機械向け出力を同じ読み取りモデルから出す。構造化データ（BlogPosting / FAQPage / BreadcrumbList）は公開記事の読み取りモデルから pure 関数で生成し、本文と食い違わない | `src/application/seo/structured-data.ts` | — | — | — | — | — | `tests/application/seo/structured-data.test.ts` | 実装済（2026-08-24）。**2026-08-28 に壊して測った。**`headline` と `description` を固定文字列へ書き換えると赤（各 1 件）。一方 **`buildPerson` の `description: person.bio` を固定文字列にしても 10/10 緑だった**——「本文と食い違わない」は人物の説明文には届いていなかった。同日に「人物の説明文は読み取りモデルの bio をそのまま出す（著者・監修者とも）」を足して塞ぎ、同じ壊し方で赤になることを再測定した |
...
| REQ-SEO03 | AI 検索適合の自動監査。結論・要点・FAQ・出典・鮮度の各ブロックの有無を機械判定し、公開前に不足を利用者へ示す（推定効果値を受入条件に置かない） | `src/application/seo/ai-search-audit.ts`, `src/application/seo/expression-blocks.ts`, `src/application/usecases/site/publish-article.ts`（`createAuditArticleDraftUseCase`）, `src/presentation/admin/publish/publish-article-action.ts`（`intent=check`） | — | — | — | — | — | `tests/application/seo/ai-search-audit.test.ts`, `tests/application/seo/expression-blocks.test.ts`, `tests/application/publish-article.test.ts`（出す前の点検）, `tests/presentation/admin-actions.test.ts` | 実装済（公開前に点検できる。判定は `expressionBlocksOf()` の射影＝読者に出るものと同一。点検は公開を止めない＝要件の「示す」どおり） |
...
| REQ-SEO05 | SEO / AI 指針の参照レジストリ。各指針は確認日（checkedAt）を必ず持ち、90 日を超えたら review_due として見直しを促す。本文変更は最後に仕様を再評価した指紋（reEvaluatedSha256）との差で判定し、同じ本文の再取得だけでは警告を消さない。画面で確認した本文版だけを再評価完了にでき、表示後に最新版が変わった競合は断る | `src/domain/seo/guideline-reference.ts`、`src/application/usecases/seo/manage-guideline-references.ts`、`src/infrastructure/persistence/d1/guideline-reference-repository.ts`、`drizzle/0038_flimsy_hobgoblin.sql` | `src/app/admin/settings/seo/page.tsx`、`src/presentation/admin/maintain/guideline-reference-form.tsx` | サイドナビ「設定」→「SEO/AI 検索の指針」→「仕様を評価し直す対象」 | 原典未取得 / 期限切れ / 本文変更を理由つきで表示。本文変更だけ再評価完了操作を表示し、競合時は最新版の再確認を案内 | 共通のDataTable/ToolFormで対応 | 機械: 共通①。目: 共通②④⑤。hidden指紋はServer Action・usecase・D1条件付き更新で再検証 | `tests/domain/seo/guideline-reference.test.ts`、`tests/application/manage-guideline-references.test.ts`、`tests/infrastructure/d1-guideline-reference-repository.test.ts`、`tests/presentation/guideline-reference-actions.test.ts`、`tests/ui/guideline-reference-page.test.tsx`、`tests/architecture/guideline-reevaluation-migration.test.ts` | 実装済（2026-08-28。A取得→B変更→B再取得でも警告継続、B再評価完了後だけ消え、C変更で再発。対象114件PASS） |
| REQ-SEO06 | 手順のある記事は HowTo、読み上げられうる冒頭は Speakable として機械にも渡す。どちらも本文の読み取りモデルから pure 関数で導き、散文に無い手順や結論を作らない（手順が無い記事には HowTo を出さない） | `src/application/seo/structured-data.ts`（`buildHowTo` / `buildSpeakable`） | 公開記事ページの `<script type="application/ld+json">` | 記事を公開 → 機械向け出力に同梱 | 手順 0 件は HowTo を出さない。冒頭の結論が無い記事は Speakable の対象要素を持たない | 公開面共通 | Speakable の指す要素は本文中に実在する id を持つ（読み上げ先が無い指定を出さない） | `tests/application/seo/structured-data.test.ts`、`tests/ui/article-speakable-anchor.test.tsx` | feat-seo-aeo-gap-closure P05〜P06 で実装中。P06 で「壊して測る」（`buildHowTo` の step 生成を先頭 1 段落だけに縮めると赤）を実施する |
| REQ-SEO07 | AI 検索適合の点検を記録として残す。公開時に 1 件追記し、記事ごと直近 30 件だけを保持する。最後の点検から 7 日を超えた記事を 1 起動あたり 50 件まで再点検し、**最新の点検が落ちている記事**を管理画面に理由つきで一覧する | `src/db/schema.ts`（`ai_search_audit_history`）、`drizzle/0044_ai_search_audit_history.sql`、`src/infrastructure/persistence/d1/ai-search-audit-history-repository.ts`、`src/application/usecases/seo/list-failing-audits.ts` | `src/app/admin/content/published/page.tsx` | サイドナビ「公開済み記事」→「AI 検索の点検で落ちている記事」→ 各行の「直す」 | 0 件は「落ちている記事はありません」を出し、列名だけの空表を出さない。上限で切ったときは続きがあることを言う。先週落ちて今週直った記事は出さない | 共通の DataTable で対応 | 落ちた項目は内部名ではなく直し方の文言（hint）で読める | `tests/integration/d1-ai-search-audit-history.test.ts`、`tests/application/list-failing-audits.test.ts`、`tests/ui/published-articles-failing-audits.test.tsx` | feat-seo-aeo-gap-closure P05 でテストと保存先・usecase・画面を実装。定期再点検の起動側は P07 以降 |

### AEO / SEO の計測ループ (feat-seo-aeo-measurement-loop・2026-09-04 追加)

利用者要望「AEO・SEO の対策ができるように。それを分析・解析して、ブログへ反映できる仕組み」への行。
データ源は **3 つとも自作**（①自サイトの静的解析 ②Search Console ③AI 検索での被引用）。
**鍵はいずれも運営者が Cloudflare の画面から登録し、リポジトリには置かない。**

観測 → 所見 → 反映の 3 段を別の要件に分けてある。**混ぜると「読めたか」と
「直してよいか」が同じ場所で決まり、外が落ちた日に記事が書き換わる。**
系統②③は再現性が無い（同じ問い合わせでも返事が変わる）ため、
**自動反映の根拠にできるのは系統①だけ**という線を要件として置く。

| REQ | 要件 | 実装 | 画面 | 導線 | 状態 | RWD | a11y | test | 結果 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| REQ-SEO08 | 系統①: 自サイトの静的解析。公開ページの HTML を自分で読み、title / meta description / 見出しの段 / 内部リンク / 画像の alt / 構造化データなどの事実 を観測する（良し悪しの判定はしない）。読み取りは本番と同じ `HTMLRewriter` で行う。問い合わせごとに中身が変わるページ（`/search`）と合言葉が埋まった URL は鍵を作らず断る。読めなかったときは失敗として返し、5xx は再試行可・4xx は不可に分ける 2026-09-07追補: OGP値・JSON-LDの型/項目/日時・実リンク先を抽出し、規則判定はREQ-SEO11へ渡す。本文10秒/1,000,000 bytes・内部リンク先500件の上限超過を失敗にし、llms.txtは設定と別のtext応答として観測する。 | `src/infrastructure/seo/aeo-measurement/static-audit-collector.ts`、`src/domain/seo/aeo-measurement/page-key.ts`、`src/domain/seo/aeo-measurement/page-observation.ts`、`src/application/seo/static-audit-inventory.ts` | — | — | — | — | — | `tests/integration/workerd-static-audit-collector.test.ts`（本物の workerd を Miniflare で立てて 20 件）、`tests/domain/aeo-measurement.test.ts`、`tests/application/seo/static-audit-inventory.test.ts`、`tests/domain/static-audit-page-key.test.ts` | 実装済（2026-09-04）。**Node 側に偽の `HTMLRewriter` を置かない。**属性の順・改行・コメント内の紛らわしい `<title>` で静かに間違えるのを避けるために選んだ読み取りなので、検査でも本物を通す 2026-09-07に公開集合・page=N・実HTML/llms抽出と応答上限を追加。 2026-09-08最終訂正: 公開ブログはworkspaceの未retire設計図で判定しnetwork状態は使わない。公開済み未削除の固定文書はblueprint.pages外も含める。初期20件は当時の検証数であり、現在の対象テストは右記参照。 |
| REQ-SEO09 | 系統②: Google Search Console の実績。サービスアカウントの資格情報は**サーバー環境変数からのみ**読み、リポジトリ・管理画面・ログ・エラーの `details` のいずれにも出さない。要求する権限は読み取り（`webmasters.readonly`）だけ。表示回数と押された回数を日付×ページで取り、欄の欠けた行だけを捨てる。未登録は失敗ではなく「まだ使っていない」 | `src/infrastructure/seo/aeo-measurement/search-console-client.ts` | — | — | — | — | — | `tests/infrastructure/search-console-client.test.ts`（18 件。RSA 鍵はテストの中で作って捨てる） | 実装済（2026-09-04）。JWT を base64url で開いて `scope` と有効期間を直接見る。**署名を差し替えると PEM の読み取りが一度も動かない**ため、本物の鍵で 1 度は通す |
| REQ-SEO10 | 系統③: AI 検索での被引用チェック。鍵は環境変数からのみ読み、要求本文にも失敗の `details` にも現れない。「引用された」の判定は返事の文ではなく **URL の一致** で行い、追跡用のクエリや `#見出し` が付いていても同じページとして拾う。**引用されなかったことは失敗ではない**（`cited: false` は正常な観測）。2026-09-07追補: 1問い合わせ最大3検索の予約枠を受け、提供元usageの実検索数を返す。本文citationのURLだけを引用ありとし、検索結果への掲載だけでは引用としない。HTTP 200のツールエラー・上限到達・未完了・検索未実行・使用数不明を引用なしへ変えない | `src/infrastructure/seo/aeo-measurement/ai-citation-client.ts`、`src/domain/seo/aeo-measurement/citation-budget.ts` | — | — | — | — | — | `tests/infrastructure/ai-citation-client.test.ts`、`tests/domain/aeo-measurement.test.ts` | 実装済（2026-09-04）。2026-09-07に検索回数の精算情報・本文citation・失敗/欠測の回帰試験を追加。月次枠はREQ-SEO12に接続 |
| REQ-SEO11 | 観測から所見への判定。突合の鍵の作り方、静的解析の規則、自動反映してよいかの判定、効果判定の保留（28 日）、系統が止まっていることの検出、所見の並び順を、外への問い合わせも保存も伴わない純粋な判断として置く。**合格印は並べず、欠けているものだけを所見にする** 2026-09-07追補: 同じブログの全成功集合でtitle/description重複、他ページからの被リンク0、canonical行き先の集合内存在を判定する。llms配信設定、OGP、対応する5種JSON-LDの項目と可視timeの日付照合を行う。任意外部canonicalの疎通や全schema検証を称しない。 | `src/domain/seo/aeo-measurement/`（`page-key` / `page-observation` / `finding` / `auto-apply` / `staleness` / `citation-budget` / `measurement-source`）、`src/domain/seo/aeo-measurement/site-audit.ts` | — | — | — | — | — | `tests/domain/aeo-measurement.test.ts`、`tests/domain/static-audit-page-key.test.ts` | 実装済（2026-09-04）。「自動反映の根拠にできる系統は 1 つだけ」を**表そのものを見る検査**で固定してある（系統を足した日に黙って根拠が増えない） 2026-09-07に単体/横断規則とllms設定照合を追加。部分巡回は横断判定の入力にしない。 |
| REQ-SEO12 | 三系統の収集。`site.manage` を持たない役では断る。1 ページ読めなくても他を続け、落ちた件数を見送りとして返す。1 ページも読めなければ失敗を記録してから失敗を返す。資格情報の無い系統は失敗ではなく見送り。取り下げた記事と他サイトの記事は見に行かない。公開記事が 0 件でも失敗にしない。2026-09-07追補: 全ブログ共有のUTC月次検索枠を外部送信前に原子予約し、使用済み・未確認・予約を残量から引く。未設定/0は通信せず、古い設定・並行予約・二重確定・月替りを保護。ブログ間配分と失敗を含む試行順で巡回する 2026-09-07静的監査追補: workspace×公開ブログの世代/対象/lease/checkpointを保持し、失敗後も後続を試す。全対象とllms確認の成功時のみ、旧static所見置換・観測stamp・source成功・完了を同D1 batchで確定する。対象上限1,000と全ブログ共有のHTML+llms外部取得25回を区別する。記事読取1,001件は下限値の非エラーlimitedとして取得を見送り、正確な全件数や全体完了に変えない。単体128 KiB超をstageのresource失敗件数から実行結果・失敗記録へ返し、成功件数から除く。合計8 MiB超は最終HTML batch直後にも判定し、その実行の見送り理由と失敗記録へ返す。limitedで旧所見を保持し、同hashでは14日以降の次回実行、hash変更時は待たず新世代にする。 | `src/application/usecases/seo/collect-seo-measurements.ts`、`src/application/ports/seo-measurement.ts`、`src/infrastructure/persistence/d1/seo-citation-budget-repository.ts`、`src/application/ports/seo-static-audit.ts`、`src/infrastructure/persistence/d1/seo-static-audit-repository.ts`、`src/infrastructure/platform/seo-measurement-scheduler.ts` | `src/app/admin/seo/page.tsx` | サイドナビ「観測」→「SEO / AI 検索」 | 見送り（鍵未登録・上限到達）と失敗を別の言葉で出す | 共通のDataTableで対応 | 共通①②（画面の検査は未着手＝下の「残り」） | `tests/application/seo/collect-seo-measurements.test.ts`、`tests/integration/d1-seo-citation-budget.test.ts`、`tests/integration/d1-seo-static-audit.test.ts`、`tests/infrastructure/seo-measurement-scheduler.test.ts` | 収集の旧実装（2026-09-04）に、2026-09-07の月次検索枠・原子予約/精算・試行巡回を追加。月次設定UIの証拠はREQ-SEO15へ接続 2026-09-07に静的監査の全体完了writer・継続・公開境界・競合/失敗時不変の実D1検証を追加。夜間は引き続き観測のみ。 |
| REQ-SEO13 | 収集した所見の身元。系統①の所見だけが記事の身元（`siteSlug` / `articleSlug`）を持ち、**系統②③は身元を持たない形で保存する**（再現しないものを自動反映の候補に上げないため）。同じページの複数日ぶんは実績として全部積み、ページは 1 件に畳む。鍵を作れない URL の行は捨てる | 同上 | 同上 | 同上 | — | — | — | `tests/application/seo/collect-seo-measurements.test.ts` | 実装済（2026-09-04） |
| REQ-SEO14 | 分析結果の自動反映。空の題名・要約を**記事の中にある材料**で埋め、記録を残す。記録が残らなかったら書き換えを戻して失敗を返す。止めているときは書き換えないが提示は続ける。対象は作成日時を問わず全ての記事（作成日時が不明な記事も含む）。書き込みは読み出した版と現在の版が一致する場合だけ行い、違えば書かずに止めて保留にする。再現しない系統だけの所見は根拠にしない。画面の作りが原因の所見だけなら書き換えない。前回の反映から 14 日空くまで待つ | `src/application/usecases/seo/manage-seo-auto-apply.ts`、`src/application/seo/aeo-measurement/auto-fix.ts`、`src/domain/seo/aeo-measurement/auto-apply.ts` | `src/app/admin/seo/page.tsx`、`src/presentation/admin/observe/seo-aeo-forms.tsx` | サイドナビ「観測」→「SEO / AI 検索」→「自動反映」 | 止めている／待っている／材料が無い、を別の理由として表示 | 共通のToolFormで対応 | 共通①②（画面の検査は未着手） | `tests/application/seo/manage-seo-auto-apply.test.ts`、`tests/domain/aeo-measurement.test.ts` | 実装済（2026-09-04）。**「提案までとし承認を経ずに書き換えない」は差し替え済みの旧決定である**（運営者が止められる・戻せる・記録が残る、の 3 点で置き換えた）。**「導入前から公開されている記事は書き換えない」も 2026-09-10 に差し替え済みの旧決定である**（接地根拠 `qa-seo-apply-target-scope-20260910` / `qa-seo-apply-approval-mode-20260910`。範囲で守らないぶん、同一確定単位での保存と版の一致検査が必須条件になる。新規記事の外部公開と予約投稿は承認免除の対象外で従来どおり承認を要する） |
| REQ-SEO15 | 反映の取り消しと見え方。変更前の姿をそのまま書き戻し、二度目の取り消しは断る。無い記録は取り消せない。変更前の記録が壊れていたら手で戻す必要があることを伝える。見るだけの役（`analytics.read`）は見え方を読めるが書き換えられない。鍵を入れていない系統は「止まっている」ではなく「まだ使っていない」。反映の効果は 28 日経つまで判定を保留する。未反映が溜まり **かつ** 古いままのときだけ止まっている疑いを出す。2026-09-07月次枠追補: 既存SEO画面の補助区画に全ブログ共通・UTC月と残量/確認済み/実行中確保/使用数未確認を表示し、人かつsite.manage権限で上限を保存する。未設定と0、枠到達と処理中確保を区別し、上限変更で使用数を戻さない 2026-09-07静的監査追補: 既存補助区画へ選択ブログの確認済み/対象・未確認・失敗・再確認待ち・所見を表示する。14日超または確認後更新を再確認待ちにし、途中の所見は前回全体完了分と説明する。全体完了日時は保存/読取のみで今回の直接表示には含めない。 2026-09-08補足: 件数超過と合計容量超過を分け、対象1,000件超や容量超過後の14日再試行を表示する。 | 同上、`src/domain/seo/aeo-measurement/staleness.ts`、`src/presentation/admin/observe/seo-aeo-state.ts`、`src/presentation/admin/observe/seo-aeo-action.ts`、`src/presentation/admin/observe/seo-dashboard.tsx` | 同上 | 同上→「反映の記録」 | 取り消し済み／取り消せない／手で戻す、を別々に出す | 共通のDataTableで対応 | 月次枠の表示と保存は上記UI回帰試験で確認（2026-09-07追補） | `tests/application/seo/manage-seo-auto-apply.test.ts`、`tests/ui/seo-dashboard.test.tsx`、`tests/ui/seo-aeo-action.test.ts` | 実装済（2026-09-04）。並べ替えは usecase 側で済ませ、画面は並べ替えない。2026-09-07の月次枠表示・保存を追補。改稿/取消の現行契約はSEO実装要件の2026-09-06承認契約を参照 2026-09-07に静的監査の未観測/途中/失敗/完了とブログ別内訳のUI回帰を追加。現在の静的契約はSEO実装要件「公開ページ全体の静的監査」を参照。 |
...
| REQ-BLOG03 | ブログ運用の言葉が持つ決まり（サイト網の親子・URL 名・記事型ごとの必須部品・鮮度・読者評価）。境目（URL 名 60 文字・鮮度 180/365 日・評価 1〜5 点）は画面ではなく言葉の側で決まり、画面と Server Action のどちらから呼んでも同じ答えになる | `src/domain/blogops/`（`validateParent`・`validateSiteSlug`・`buildNetworkTree`・`missingBlocks`・`freshnessOf`・`summarizeRatings`） | `/admin/site-network`、`/admin/blog/*` | 管理トップ →「サイト網」/「ブログ運用」 | 親を失った節点は捨てず孤児として並べる／0 件の平均は `0` ではなく `null` | — | — | `tests/domain/blog-ops.test.ts` | 実装済（2026-08-26。境目の値はテスト側に手で書いた表で固定。壊して測る実測は未収載） |
| REQ-BLOG04 | ブログ運用の CRUD は**画面が守れない決まり**をユースケース側で断る。権限の無い人・別の作業場所の人には出さない。重複した URL 名・空の題名・理由の無い削除・部品が欠けたままの公開は保存しない。記録（監査）が書けなかったときに「保存できた」と言わない | `src/application/usecases/blog-ops/`（`manage-site-network`・`manage-blog-layout`・`manage-blog-articles`・`manage-blog-pages`・`evaluate-blog-articles`）、`src/application/ports/blog-ops.ts` | `/admin/site-network`、`/admin/blog/{layout,articles,pages,tags,delivery,evaluate}` | 管理トップ → 各画面。保存は Server Action（REST の入口は置かない。`docs/spec/feat-blog-ops-crud/api-contract.md`） | 権限なし／別テナント／0 件（何をすればよいかを言葉で返す）／保存を断る | — | — | `tests/application/blog-ops-usecases.test.ts` | 実装済（2026-08-26。テスト緑。壊して測る実測は未収載） |
| REQ-BLOG05 | 記事の本文は**節の中の断片**（段落・小見出し・箇条書き・引用・注意書き・商品カード・比較表・画像・区切り線）として書ける。保存の形は今までどおり拡張 Markdown の 1 本の文字列で、記法を 1 つも使っていない本文は段落だけとして読める（既存記事の移行が要らない）。書き出して読み直すと元の断片へ戻る（往復が変わらない） | `src/domain/blogops/prose-node.ts`（10 種・`emptyProseNode`・`isEmptyProseNode`・`/` メニューの表）、`src/domain/blogops/prose-format.ts`（`serializeProse`・`parseProse`）、`src/presentation/prose/prose-editor.tsx`（`/` で挿す編集欄）、`src/presentation/prose/prose-body.tsx`（管理画面と公開面で同じ描き方）、`src/presentation/prose/prose-table-frame.tsx`（比較表の枠は読む側と書く側で 1 つ）、`src/presentation/prose/prose-outline.tsx`（目次と節の錨の名前） | `/admin/blog/articles/[article]`、記事の公開面 | 管理トップ →「ブログ運用」→ 記事 → 本文の欄 | 記号で始まる段落を別の断片として読み直さない／閉じ忘れた囲みは本文を飲み込まず段落として残す／知らない種類の囲みは捨てず見える形で残す／空の断片は保存しない | — | `/` の候補は同じことができるボタンを併置（読み上げ操作でも足せる）。断片ごとの操作ボタンは種類名を含む `aria-label` を持つ。アイコンのみで意味を伝えない | `tests/domain/blogops/prose-format.test.ts` | 実装済（2026-08-26。往復不変条件 23 件で緑。E2E は記事の作成・公開・削除まで（`tests/e2e/blog-ops-crud.spec.ts` 16 件、2 画面幅）で、`/` メニューの操作そのものは未収載） |
| REQ-BLOG06 | 手元の CLI（Claude Code / Codex）から記事を書ける。門（`Authorization: Bearer <MCP_TOKEN>`）と身元（`X-Integration-Key`）は別のヘッダーで、`MCP_TOKEN` 未登録のうちは 503 で閉じている。AI が呼べるのは**下書きを積むところまで**で、公開（`set_blog_article_status`）と削除（`delete_blog_article`）は`requiresHumanApproval` によりどの入口からも実行できない。報酬額のポートは記事づくりの入力にならない | `src/presentation/tools/blog-ops-tools.ts`（7 道具）、`src/presentation/tools/catalog.ts`（一覧の単一正本）、`src/presentation/http/tool-scope.ts`（`isToolAllowedForScope`）、`src/application/usecases/blog-ops/manage-blog-articles.ts`（`guardEditorial`）、`docs/product/local-ai-authoring.md`（手順書） | `POST /api/tools/[tool]`、`/api/mcp`、`/admin/tools` | `docs/product/setup-tasks.md` S-12 →「連携の鍵」を `/admin/settings/integration-access` で発行 | `MCP_TOKEN` 未設定は 503（fail-closed）／承認の要る 2 つは `bearer`・`same-origin` の**どちらからも**断る／商業区分のポートは組み立てで触らない | — | 値をコマンドライン引数に置かせない（`curl --config -` で標準入力から渡す） | `tests/presentation/blog-ops-tools.test.ts` | 実装済（2026-08-26。10 件で緑。`deps.conversions` を足して赤くなることを実測。**E2E の対象外**——画面ではなく手元の CLI から叩く口のため、ブラウザ操作では通らない） |
**2026-08-26 にこの 3 行を足した理由。** `tests/domain/blog-ops.test.ts` と
`tests/application/blog-ops-usecases.test.ts` は先に入っていたが、ヘッダの `@req` に
受入 ID（`A1, A3, …`）だけを書いていた。`scripts/traceability.mjs` が拾うのは
`REQ-[A-Z]+\d+` の形だけなので、**この 2 本は「由来不明のテスト」として数えられていた**
（由来不明 4 件 / 上限 2 件で赤）。上限を上げて緑にすることは禁じられているので、
要件表の側に行を足して `@req REQ-BLOG03` / `@req REQ-BLOG04` で結び直した。

### ブログ運用 CRUD の派生実装要件 (feat-blog-ops-crud・2026-08-27 追加)

受入正本は `features/feat-blog-ops-crud.md#frontmatter.acceptance` の A1–A14。
以下はその実装への一意な投影で、既存の `REQ-BLOG01`–`REQ-BLOG06`
(ブログの型・配色・ドメイン/ユースケース/本文断片/CLI) とは別 namespace である。

| REQ | 要件 | 実装 | 画面 | 導線 | 状態 | RWD | a11y | test | 結果 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| REQ-BOPS01 | サイト網の節点を作成・一覧・更新・論理削除・復元でき、参照中の子節点がある削除は断る | `src/domain/blogops/`, `src/application/usecases/blog-ops/manage-site-network.ts` | `/admin/site-network` 一式 | 管理トップ → サイト網 | 別 workspace の既知 ID は操作不可 | — | 管理画面共通ゲート | `tests/domain/blog-ops.test.ts`, `tests/application/blog-ops-usecases.test.ts`, `tests/integration/d1-blog-ops-tenancy.test.ts`, `tests/e2e/public-site-lifecycle.spec.ts` | 実装済・完了判定は in_progress |
| REQ-BOPS02 | ハブトップ帯の表示可否・並び・件数を保存し、公開トップに反映する | `src/application/usecases/blog-ops/manage-blog-layout.ts`, `src/infrastructure/persistence/d1/blog-ops-repository.ts`, `src/presentation/site/public-site-projection.ts` | `/admin/site-network/[node]`, `/s/[site]` | 節点設定 → 公開面 | 無効帯は公開面に出さない | 公開面共通 | 公開面共通ゲート | `tests/integration/d1-blog-ops-tenancy.test.ts`, `tests/ui/public-site-projection.test.ts` | 実装済・完了判定は in_progress |
| REQ-BOPS03 | ヘッダー・サイドバー・フッターの枠を設定し、無効な枠は描画しない | `src/application/usecases/blog-ops/manage-blog-layout.ts`, `src/presentation/admin/publish/blog-layout-form.tsx`, `src/presentation/site/public-site-projection.ts` | `/admin/blog/layout`, `/s/[site]` | ブログ運用 → レイアウト | 未設定/無効枠は非表示 | 公開面共通 | `tests/ui/blog-ops-a11y-floor.test.tsx` | `tests/application/blog-ops-usecases.test.ts`, `tests/ui/public-site-projection.test.ts` | 実装済・完了判定は in_progress |
| REQ-BOPS04 | 記事を T1–T4 で作成し、題名規則・必須ブロック列違反を保存前に断る | `src/domain/blogops/blog-article.ts`, `src/application/usecases/blog-ops/manage-blog-articles.ts` | `/admin/blog/articles/new` | ブログ運用 → 記事 → 新規 | 不明型・必須欠落は保存しない | 編集画面共通 | `tests/ui/blog-ops-a11y-floor.test.tsx` | `tests/domain/blog-ops.test.ts`, `tests/application/blog-ops-usecases.test.ts` | 実装済・完了判定は in_progress |
| REQ-BOPS05 | `articles` を唯一の編集正本として記事を一覧・更新・論理削除・復元し、状態遷移を監査に残す | `src/infrastructure/persistence/d1/blog-ops-repository.ts`, `drizzle/0030_unify_blog_article_ssot.sql` | `/admin/blog/articles` 一式 | 記事一覧 → 詳細 | 公開 snapshot と編集正本は分離 | 編集画面共通 | `tests/ui/blog-ops-a11y-floor.test.tsx` | `tests/architecture/blog-ops-content-separation.test.ts`, `tests/integration/d1-blog-ops-tenancy.test.ts`, `tests/ui/blog-ops-restore.test.tsx` | 実装済・完了判定は in_progress |
| REQ-BOPS06 | 固定ページ 8 種を管理し、公開状態を公開面へ反映する。削除済み本文は保存で暗黙復活させず明示復元する | `src/application/usecases/blog-ops/manage-blog-pages.ts`, `drizzle/0031_publish_fixed_pages.sql` | `/admin/blog/pages`, `/s/[site]` | ブログ運用 → 固定ページ → 削除済み一覧 | 非公開/削除済みは公開しない。別workspace・二重復元は拒否 | 公開面共通 | `tests/ui/blog-ops-a11y-floor.test.tsx` | `tests/application/blog-ops-usecases.test.ts`, `tests/infrastructure/sample-blog-ops-tenancy.test.ts`, `tests/integration/d1-blog-ops-tenancy.test.ts`, `tests/ui/public-site-projection.test.ts`, `tests/e2e/public-site-lifecycle.spec.ts` | 実装済・完了判定は in_progress |
| REQ-BOPS07 | ブランドタグを管理し記事へ複数割り当て、非ブランドを cloud から除外する | `src/domain/blogops/`, `src/application/usecases/blog-ops/` | `/admin/blog/tags` | ブログ運用 → タグ | 非ブランドは cloud 非表示 | 管理/公開共通 | `tests/ui/blog-ops-a11y-floor.test.tsx` | `tests/domain/blog-tag-cloud.test.ts`, `tests/ui/blog-tag-form.test.tsx` | 実装済・完了判定は in_progress |
| REQ-BOPS08 | 配信部品 9 種の設定と観測履歴を分離し、欠落状態を一覧可能にする | `src/domain/blogops/`, `src/application/usecases/blog-ops/` | `/admin/blog/delivery` | ブログ運用 → 配信 | 未設定/失敗/欠落を区別 | 管理画面共通 | `tests/ui/blog-ops-a11y-floor.test.tsx` | `tests/domain/blog-delivery-snapshot.test.ts`, `tests/application/blog-delivery-check.test.ts` | 実装済・完了判定は in_progress |
| REQ-BOPS09 | 閲覧者評価を受け付け、管理側の非表示状態を公開集計に反映する | `src/application/usecases/blog-ops/evaluate-blog-articles.ts` | 公開記事, `/admin/blog/evaluate/[article]` | 公開記事 → 評価 / 管理側一覧 | 非表示評価は公開集計から除外 | 公開面共通 | `tests/ui/blog-ops-a11y-floor.test.tsx` | `tests/domain/blog-ops.test.ts`, `tests/application/blog-ops-usecases.test.ts` | 実装済・完了判定は in_progress |
| REQ-BOPS10 | 記事・サイトの適合・配信健全性・鮮度を表示し、並べ替え・絞り込みできる | `src/domain/blogops/operational-health.ts` | `/admin/blog/evaluate` | ブログ運用 → 評価 | 端値と空集合を明示 | 一覧共通 | `tests/ui/blog-ops-a11y-floor.test.tsx` | `tests/domain/blogops/operational-health.test.ts` | 実装済・完了判定は in_progress |
| REQ-BOPS11 | 公開面をrequest-scoped site readerからサイト網・レイアウト・記事型・固定ページ・配信設定で構成する | `src/infrastructure/persistence/d1/blog-ops-repository.ts`, `src/presentation/site/public-site-projection.ts`, `src/presentation/site/page-frame.tsx` | `/s/[site]` 一式 | サイト URL → `openSite`（identityを1回解決）→ 公開射影 | inactive/削除済み/曖昧slug/別workspaceはreaderを開かず404 | 公開面共通 | `tests/ui/blog-ops-a11y-floor.test.tsx` | `tests/ui/public-site-projection.test.ts`, `tests/integration/d1-blog-ops-tenancy.test.ts`, `tests/e2e/public-site-lifecycle.spec.ts` | 実装済・完了判定は in_progress |
| REQ-BOPS12 | D1 が無い管理面は見本データへ暗黙 fallback せず `StorageNotice` を出す | `src/app/admin/`, `src/presentation/ui/patterns/stub-notice.tsx` | 全管理画面 | 保存操作 → D1 未構成の明示 | D1 無しは fail-closed | 管理画面共通 | 文言と role を保持 | `tests/presentation/storage-notice.test.ts`, `tests/infrastructure/sample-blog-ops-tenancy.test.ts` | 実装済・完了判定は in_progress |
