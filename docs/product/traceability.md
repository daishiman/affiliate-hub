...
| REQ-P08 | §9.8 Distribution Hub（Connector契約・予約・投稿・失敗リトライ・Publication状態8+異常5） | `src/domain/distribution/channel.ts`（外部直接媒体を自社サイトから分離しprovider identityを保持）、`src/domain/distribution/publication.ts`（状態遷移・本文版・最大5試行・取得token付きlease・provider key・不変record時刻）、`src/domain/distribution/delivery-audit.ts`（system監査の完全payload）、`src/domain/compliance/external-publication-gate.ts`（予約時と送信時の共通評価）、`src/application/usecases/distribution/manage-distribution.ts`（実認証したDIDへの接続登録・create-if-absent・見本記事の外部予約／手動配信からの外部変更拒否・監査回復）、`src/application/usecases/distribution/execute-due-publications.ts`（本文版CAS・provider通信前の同一DID分散lease・workspace横断due・監査outbox分離）、`src/infrastructure/channels/bluesky.ts`（DID照合・公式XRPC `putRecord`・AT URI完全照合）、`src/infrastructure/persistence/d1/distribution-repository.ts`（DID/秘密参照/TID一意境界・Publication全列CASと現在本文版の同一UPDATE照合）、`src/infrastructure/persistence/d1/publication-delivery-audit-outbox.ts`（状態CAS+outboxのD1 batchと監査だけの冪等flush）、`src/infrastructure/platform/distribution-scheduler.ts`、`worker-entry.js` | `src/app/admin/distribution/page.tsx`、配信詳細・カレンダー、`register_channel_connection` / `schedule_publication` | サイドナビ「配信」+ owner/workspace admin本人の承認が必要な接続登録・予約ツール | 接続期限・失効・認証未登録・DID差替え・未実装providerを理由つきで停止。秘密値・参照名・JWT・provider本文は表示しない | 対応 | 機械: 共通①。実機: `docs/product/bluesky-distribution.md` | LOCAL PASS（DID/handle実認証、秘密非開示、URI完全照合、同一rkey、本文版TOCTOU、旧版欠落fail-closed、見本記事の外部予約／外部変更拒否、retry上限、即時配信の不変record時刻、取得token付きDID分散lease、provider key一意境界、接続登録監査の冪等性、配信状態と監査outboxの原子化、監査障害後の外部無再送復旧）／DEV PARTIAL PASS（remote 0039・deploy・HTTP/認証境界は確認済み） | 条件付き実装。devにBluesky SecretとDID固定済み接続が無いため、実投稿・Publication/attempt/audit照合はNOT RUN。productionは未適用 |
...
| REQ-S06 | §22.6 Site Builder（11要素） | `src/application/usecases/site/build-site.ts`、`src/application/usecases/site/manage-sites.ts`、`src/application/usecases/site/manage-site-documents.ts`（**2026-08-26 追加**。固定ページ＝運営者情報・各方針・規約・特定商取引法に基づく表記。`legal_page` 表は置き場だけあって書く口が無く、読者に出ていたのは見本の文だった。判定は `tests/application/manage-site-documents.test.ts` と `tests/integration/d1-published-article.test.ts`）、`src/infrastructure/persistence/d1/site-document-repository.ts`、`src/presentation/admin/site-wizard-form.tsx` | `src/app/admin/sites/page.tsx`、`.../new/page.tsx`、`.../[site]/page.tsx`、`.../[site]/documents/page.tsx` | サイドナビ「ブログ」 | loading / empty / error / 未入力のステップは次へ進めない理由を表示 | 対応 | 機械: 共通①。目: 共通②③④⑤。未: 文中リンクの押しどころ | PASS（`tests/application/build-site.test.ts`） | 実装済 |
...
| REQ-B07 | `/s/{site}/tools/{tool}`（診断・計算） | `src/app/s/[site]/tools/[tool]/page.tsx` + `reader-tool-section.tsx` + `src/infrastructure/persistence/d1/reader-tool-repository.ts` | トップの「試せること」・カテゴリ | D1 の定義・計算式を実行時に形検査して読む。定義が無いときだけ同住所の記事へ落ち、保存先障害は「いま表示できません」と返す。D1 が無い環境では作り付け 1 件が控えとして動く | 対応 | 対応 | PASS（`tests/domain/reader-tool-formula.test.ts` / `tests/infrastructure/d1-reader-tool-repository.test.ts` / `tests/ui/tool-page-identity.test.tsx`） | 読み取り・計算は実装済（運営者が道具定義を書き込む入口は未実装） |
...
| REQ-B09 | `/s/{site}/shortlist`（気になる商品） | `src/app/s/[site]/shortlist/page.tsx` + `src/infrastructure/persistence/d1/reader-shortlist-repository.ts` | 共通ヘッダ + 記事内の保存操作 | D1 が結び付いた環境ではブラウザごとの合言葉で永続保存し、未使用環境だけ処理中メモリの控えへ落ちる。空 / 保存 / 取り外し / 保存先障害を分けて返す | 対応 | 対応 | PASS（`tests/domain/site-routes.test.ts` / `tests/infrastructure/d1-reader-shortlist-repository.test.ts` / `tests/application/reader-interaction.test.ts`） | 実装済（D1 未使用環境のみ控えあり） |
...
| REQ-B18 | `/s/{site}/contact` | `src/app/s/[site]/contact/page.tsx` + `contact-form.tsx` + `d1/contact-repository.ts` + `/admin/contact` | フッタ | 実データ（D1 に保存し、`/admin/contact` で運営者が読む。本文保存と回数上限はD1の同じINSERT文で確定。**メール通知はまだ無い**。Turnstile鍵が無い環境はfail-closedで断る） | 対応 | 対応 | LOCAL PASS（`tests/ui/contact-form.test.tsx`でspin-v2 widget契約、`tests/infrastructure/turnstile.test.ts`でsecret/response/remoteip、Cloudflare dummy Siteverifyでsuccessを手動実測）／DEV PARTIAL PASS（remote 0039・現コード公開、実widgetのsitekey/action/script、実secretで不正token拒否、保存先0件を確認）／PRODUCTION NOT RUN | 条件付き実装（devの実widget token→Siteverify成功→D1保存を通常ブラウザで1件確認待ち。未確認時もfail-closed） |
...
| REQ-E12 | AffiliateLink | `monetization/affiliate-link.ts`（**2026-08-26 追加**: `disableAffiliateLink` — 二度押しを断り、止めた日時が後ろへずれないようにする）／`application/usecases/monetization/manage-affiliate-links.ts`（一覧と停止）／`app/admin/affiliate/links/page.tsx`（画面）／`presentation/admin/delete-form-action.ts` の `disableAffiliateLinkAction` | `affiliate_links`（`disabled_at`）+ 見本データ | REQ-P09 | 実装済。**止める口が無かったため、表記が古くなったときの直し方が 1 つも無かった。** 写しは上書きしない決まり（`docs/product/design-decisions.md` §2）なので、直し方は「止める → 登録し直す」の 2 手で、その 1 手目が欠けていた。判定は `tests/application/manage-affiliate-links.test.ts` |
...
| REQ-E32 | AuditLog | `compliance/audit-log.ts` + `infrastructure/persistence/d1/audit-log-repository.ts` | `audit_logs`（追記専用ポート。更新・削除の口なし）+ ローカル見本実装 | REQ-S09 | 実装済（D1 の追記・対象別読出し・workspace 分離・秘密情報の伏せ字を結合テストで確認。見本実装はローカル表示用であり正本ではない） |
...
| REQ-R03 | Brand Manager | 同 `brand_manager`（ブランド配下の運営一式。会員管理と報酬管理は持たない）。membershipの`scopedBrandIds`がある場合はブランド・企画・生成・feedbackを対象brandに限定。`SiteBlueprint`はbrandIdを持たないため推測で結び付けず、サイト作成・管理・管理read・問い合わせ管理をfail-closedで拒否する | 同上 + `tests/domain/permissions.test.ts` / `tests/application/manage-sites.test.ts` | 実装済（サイトを限定brandへ割り当てる要件を採用する場合は明示的な関連モデルを追加する） |
...
| REQ-SEO03 | AI 検索適合の自動監査。結論・要点・FAQ・出典・鮮度の各ブロックの有無を機械判定し、公開前に不足を利用者へ示す（推定効果値を受入条件に置かない） | `src/application/seo/ai-search-audit.ts` | `src/application/usecases/site/publish-article.ts` | `src/presentation/ui/templates/article-view.tsx` | — | — | — | `tests/application/seo/ai-search-audit.test.ts`, `tests/ui/article-faq.test.tsx` | 実装済。2026-08-26 に「よくある質問」を端から端へつないだ（入力欄→`buildArticle`→読者の画面→`FAQPage`）。それまで `EXPRESSION_BLOCK_KINDS` の `faq` は domain にあるだけで、点検項目にも読者の画面にも無かった（点検が 5 項目とも緑でも FAQ は 1 件も無い、という状態を作れていた） |
...
| REQ | 要件 | 実装 | 画面 | 導線 | 状態 | RWD | a11y | test | 結果 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| REQ-BLOG01 | ブログの型（テンプレート）は記事の並び方だけを決める。差し替えても記事のブロックは 1 つも落ちない（並びに無い種類は末尾へ元の順で残す）。スロットの差し替え先が無いときは黙って空にせず fallback を出す | `src/domain/authoring/blog-template.ts`（`BLOG_TEMPLATES` 6 種・`orderBlocksForTemplate`・`fillSlots`・`findBlogTemplate`） | 据え置き（2026-08-26 に方針決着。`docs/product/design-decisions.md` 3 節。配色の正本は `SiteBlueprint.theme` 側で、表を配線すると置き場が 2 つになる。`tests/architecture/tenant-scoped-schema.test.ts` の `UNWIRED` が「使い始めたら赤くする」見張り） | — | 知らない型 ID は `null`（既定へ落とす判断は呼び出し側） | — | — | `tests/domain/authoring/blog-template.test.ts` | 実装済（2026-08-24 にテストを追加。画面への接続は ah-6lf.4） |
| REQ-BLOG02 | 配色は 2 層（ブログ既定 `blog_theme` とページ単位の上書き `page_theme_override`）。上書きを外す（`null`）とブログ既定へ戻る。片方の欄だけの上書きは、その欄だけが効く | `src/domain/authoring/blog-template.ts`（`resolvePageTheme`・`BlogTheme`・`PageThemeOverride`）。値は `tokens.css` の `light-dark()` を選ぶ属性名であって色そのものではない | 据え置き（2026-08-26 に方針決着。`docs/product/design-decisions.md` 3 節。配色の正本は `SiteBlueprint.theme` 側で、表を配線すると置き場が 2 つになる。`tests/architecture/tenant-scoped-schema.test.ts` の `UNWIRED` が「使い始めたら赤くする」見張り） | — | 上書きなし（`null`）/ 一部上書き / 全部上書き | — | — | `tests/domain/authoring/blog-template.test.ts` | 実装済（同上） |
