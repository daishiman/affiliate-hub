...
- 未宣言の上限 `TEST_TYPES_MAX_UNDECLARED` を実測に置く（置いた当初は 158。現在 14）
...
| `TS06` 読み上げ検査とコントラスト | `a11y` だけを求める性質が語彙に無い | 変わらず。判定欄が「PASS」だけだったので実測（配色 10 種 × 明暗 2 種 = 20 通り + 空振り防止 2 件）へ置き換えた |
