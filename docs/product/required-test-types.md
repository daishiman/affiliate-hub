...
| `REQ-CI12` | 超過時の挙動を測るには時間を外から渡せる作りが要る | `scripts/verify.mjs` は `Date.now()` で自分で測っており、いまも受け取らない |
| `REQ-E16` | `ProductVariant` に作る関数が無く、断る場所が 1 つも無い | `src/domain/product/product.ts` に `createProduct` はあるが variant 側は型だけ |
| `REQ-FD04` | 要件そのものを見る検査が無い（残課題 88 / `ah-abs`） | 道具と画面の対応表をどこに置くかを先に決めないと書けない |
...
`REQ-E16`（`src/domain/product/` に `createProductVariant` は無い）、
