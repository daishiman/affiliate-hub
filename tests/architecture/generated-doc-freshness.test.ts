...
/**
 * 許してある古さ。**いまは 0 で、上げない。**
 *
 * この数は**悪いことをしたときだけ増える**。テストを足して生成し直さない、
 * という一手でしか増えない。だから上限を張る先として正しい。
 * 画面が増えれば無害に増える母集団（生成物の総数など）に張ってはいけない。
 *
 * --- ここに至るまでに測ったこと（同じ道を二度通らないために残す）---
 *
 * 1. 最初この値を **1** と書いて赤くなった。**検査が自分自身を数えたからである。**
 *    「古さを見張るファイル」も母集団の一員なので、置いた瞬間に古さが 1 増える。
 *    予想ではなく実測で決めるべき数だった。
 * 2. 次に **2** で通した（この枝が足したテスト 2 件ぶん）。生成物を直せない
 *    あいだの一時的な許しで、`node scripts/traceability.mjs` を走らせたら
 *    0 へ下げる、という反転先を書いてあった。
 * 3. 生成し直して **0** へ下げ、緑を確かめた。**上の 1・2 はもう成り立たない。**
 *
 * 0 にして赤くなったら、それは**別の古さ**である。下げ幅を戻して緑にせず、
 * 何が古いのかを追うこと。
 */
const KNOWN_STALE_MAX = 0;
