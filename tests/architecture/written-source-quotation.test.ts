 ...
 * --- なぜ ② の形なのか ---
 *
 * **原因は残っている。**確定セルが引く entry のうち 3 件は、まだ引用ではなく要約で
...
/**
 * まだ引用になっていない entry の**上限**。2026-08-21 実測 **3**
 * （`python3 3.11.4` / 単位は qa_log entry / 分母は `source.kind=written-requirements` かつ
 * `source.path` を持つ 23 件 / 基点は同日の修復前 14 件）。
 * **下げる方向にしか動かさない。**
 *
 * **9 → 6 と 6 → 3 は、同じ「減った」でも中身が正反対である。**
 *
 * 9 → 6: **entry は 1 件も直していない。**引用ブロック記号 (`> `) を体裁として
 * 正規化したことで、元から逐語だった 3 件 (`qa-foundation-u1` / `u3` / `u4`) が
 * 「逐語でない」から外れた。**数え方が間違っていたのであって、中身は変えていない。**
 *
 * 6 → 3: **こちらは本当に直した。**`qa-ops-web` / `qa-infra-web-redirect` /
 * `qa-frontend-web-analytics` の引用は、文書が後から改訂されたのに取り残された
 * **古い引用**だった（例: ファネル行は文書側に `→支払` と `consent区分` が
 * 増えていたのに、引用は増える前の短い版のまま）。`requote-written-source` で
 * 文書側から引き直し、`reseal-written-source` で封を取り直した。
 * 実測: 3 件合計 886 → 1003 文字、**縮んだ行は 0**、失われた錨は 0。
 *
 * 残る 3 件 (`qa-backend-web-analytics` / `qa-database-web-analytics` /
 * `qa-uiux-web-spec-intake`) は**錨が無い**——表の行でも番号付き箇条でもない
 * 自由文なので、writer は「文書の何処を引いたのか決められない」と言って書かずに
 * 止まる。**これは writer の不足ではなく、当てずっぽうで書き換えないための拒否である。**
 * ここを 0 にするには人が引用元の行を決める必要がある。
 */
const NOT_VERBATIM_MAX = 3;
/**
