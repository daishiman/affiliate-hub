...
const CHAPTERS: readonly Chapter[] = [
  ...
  },
  {
    ...
    */
    /*
      2026-09-09: origin/dev を合流した。dev 側で確定したセルと質疑が正本へ入り、
      正規投影は 775 → 798 行（実増 23 行）になった。同じ便で
      「compile が保てなかった行」節（古い版の行と、合流時に紛れ込んだ
  ...
  },
  {
    ...
    //
    // 2026-09-09 (origin/dev 合流): 正規投影は 633 → 746 行 (実増 113 行)。
    // dev が育てた確定セル・質疑録・章の注記が正本へ入ったものである。同じ便で
    // 「compile が保てなかった行」節を `--acknowledge-prior-residue` で落とした
    // (中身は古い版の行と、合流時に紛れ込んだ衝突マーカー文字列だけで、
    // 新しい版はいずれも再生成後の本文に実在する)。
    // **床 172 も余裕 22 行も動かさない。**直前の天井 683 へ実増 113 だけを
    // 加えて 796 とする。
    ceiling: 796,
    headings: 21,
    principles: 2,
  ...
  },
  {
    ...
    /*
      ...
      **その 34 行を位置ごと移す**（426 + 34 = 460）。
      ...
      2026-09-09 (origin/dev 合流): 正規投影は 446 → 470 行 (実増 24 行)。
      **床 179 も余裕も動かさない。**直前の天井 460 へ実増 24 だけを加えて 484 とする。
    */
    ceiling: 484,
    headings: 23,
    principles: 2,
  ...
  },
  {
    ...
    floorMeasuredOn: "2026-08-19",
    /*
      2026-09-09: この章に初めて天井を明示する。**余裕が尽きたので判断した記録**である
      (ui-ux / backend / frontend / database / infrastructure と同じ扱い)。

      これまでは既定の「床 + 150」= 323 で足りていた。origin/dev の合流で
      正規投影が 310 → 337 行になり (実増 27 行)、既定の余裕 150 を初めて使い切った。
      増えたのは dev 側で確定したセルと質疑録で、章へ直接書いた散文ではない。
      **床 173 は動かさない。**既定の天井 323 へ実増 27 だけを加えて 350 とする。
    */
    ceiling: 350,
    headings: 21,
    principles: 2,
  ...
  },
  {
    ...
    //
    // 2026-09-09 (origin/dev 合流・6 度目): **上の宿題は果たされている。**
    // 切り出しは dev 側の便で実施済みで、`#### 本章での適用` は
    // `applied/ui-ux.md` (329 行) へ移り、章に残るのは参照 5 行だけになった。
    // 5 度目に「生成器を変えないと章が縮まない」と見立てた障害も消えている
    // (`--on-handwritten preserve --acknowledge-prior-residue` で 8 章とも
    // 一時ディレクトリの再生成物と 1 バイト一致することを今日確かめた)。
    //
    // そのうえで実測 842 行、基準 751 に対する実増分は 91 行。節ごとに測ると
    // 主因はもう 4 段展開ではない:
    //   意思決定が本章に効く形 +24 / 近道の選定基準 +16 (新規) /
    //   一段目は「作業の対象物」5 つに畳んだ +14 (新規) / 保守規則と手順の正本 +11 (新規)
    // いずれも dev 側で確定したセルと質疑録が正本へ入ったもので、章へ直接
    // 書いた散文ではない。**切り出せる塊はもう章に無い** — 逃がし先へ逃がした後に
    // 残った、正本そのものの成長である。だから今回の置き直しは緩和にあたらない。
    // **床 223 も余裕 22 行も動かさない。**直前の天井 826 へ実増 91 だけを
    // 加えて 917 とする。
    //
    // **次に当たったときの宿題:** 切り出しという逃げ道は使い切った。次は
    // 「章 1 枚に 8 カテゴリ分の質疑を集め続ける形」そのもの — 章の分割 — を疑うこと。
    ceiling: 917,
    headings: 28,
    principles: 2,
    answers: [2, 49],
  ...
  describe("痩せた章を止められること", () => {
    ...
    describe("auth.md（この章にしかない当てどころ）", () => {
      ...
      it("対象外の 5 行を消すと、収集状態の表が床を割る", () => {
        /*
          2026-09-09: 当てどころを `approval-platform-web-only` から
          「Web 以外を対象外にした帰結として」へ移した。**表の中身が変わったのでは
          なく、5 行の根拠の書き方が変わった。**dev 側で対象外セルの根拠が
          `approval_ref`（承認 ID への参照）から `reason`（理由の実文）へ置き換わり、
          章に承認 ID の語が 1 つも出なくなったため、旧い当てどころは 1 行も
          拾わなくなっていた（filter が素通りし、7 行のまま落ちていた）。

          新しい語も**5 行が同じ 1 つの決定に由来する**ことを保っている
          （5 行とも同一文言で、Web 以外を外した帰結だと書いてある）。
          `| 対象外 |` で拾えば通りはするが、それは「対象外という語がある行」を
          消すだけで、どの決定から来た行なのかを問わなくなる。
        */
        const cut = full
          .split("\n")
          .filter((l) => !l.includes("Web 以外を対象外にした帰結として"))
          .join("\n");
        expect(measure(cut).tableRows("カテゴリ別収集状態")).toBeLessThan(7);
      });
