...
describe("テストが実際に何かを確かめていること", () => {
  ...
  it("中身が空のテストが無い", () => {
    // **母集団の床**（残課題 78 ㉗）。0 件は「違反が無い」でも
    // 「テストファイルを 1 つも歩けていない」でも出る。ここで見分ける。
    expect(ALL.length, "テストファイルを歩けていません。歩く側を先に疑うこと").toBeGreaterThan(150);
...
describe("秘密情報がリポジトリに入っていないこと", () => {
  ...
  const SECRET_SHAPES: readonly { readonly what: string; readonly pattern: RegExp }[] = [
    ...
    /*
      ...
      **見出しの行だけでは鍵ではない。** PEM を読み書きする側は、何を読むのかを
      説明するために `-----BEGIN PRIVATE KEY-----` と書く（そう書かないと、
      読んだ人はどの形式の話か分からない）。見出しだけで赤くすると、
      避け方は「語をずらして書く」になり、**次に PEM を扱う人が同じ空振りを踏む**。

      本物には必ず中身が続く。だから見出しの後ろに base64 の並びを要求する。
      40 文字は「説明の文が偶然そう見えることは無く、本物なら必ず超える」位置。
      改行が `\n` の 2 文字で貼られた鍵も同じように見つける。
    */
    {
      what: "秘密鍵ファイルの中身",
      pattern: new RegExp("-----BEGIN [A-Z ]*PRIVATE" + " KEY-----(?:\\s|\\\\n)*[A-Za-z0-9+/]{40,}"),
    },
  ];
