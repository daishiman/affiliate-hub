...
/**
 * ⚠ **この検査はまだ壊して赤を見ていない（2026-08-19 時点）。**
 *
 *   何が未実施か: 下の 12 件は緑だが、**赤になるところを見ていない。**
 *     「見つける側が効いていること」の 8 件（`looksLikeDate` の両方向）だけは
 *     確かめてあるが、**実体（`system-spec/*.md`）を壊して測る**ほうが残っている。
 *   なぜ後回しにしたか: 壊す先が `system-spec/*.md` で、この時点で
 *     C05（完全性評価）が同じファイルを読んでいた。読まれている最中に入力を動かすと、
 *     返ってきた判定がどの状態に対するものか言えなくなる。
 *     **測定は、測ったときの条件とセットでないと意味を持たない。**
 *   いつ解けるか: C05 の完了応答が届いた直後。
 *     壊し方は**要件の文がそのまま禁じている行為**にする——
 *     確かめずに版番号を書く（例: `cloudflare-d1` の版を `2026-08-16` から `4.20` へ）。
 *     判定式（`looksLikeDate`）を偽に変える壊し方は使わない。
 *   **解いたら、この囲みごと消すこと。**残すと、赤を見た検査と見ていない検査が
 *     同じ顔で並ぶ。緑 12 件だけを見た人は「固定済み」と読む。
 *
 * ── ここから下は測定済み ─────────────────────────────────
 *
