...
describe("最新ドキュメント出典の欄が欄名どおりの値を持っているか", () => {
   ...
   */
  it("章 md と fetched-references の食い違いは 0 件（主対象だけでなく全 17 行）", () => {
    expect(driftBetween(rows, refs)).toEqual([]);
    expect(rows.length).toBe(17); // 母数。突合する相手が消えたら赤くする。
  });
  ...
  it("確認が取得より前になっている行は、いずれも本文以外を鮮度根拠にしている", () => {
    ...
    /*
     * 2026-09-03 の `vitest` 再取得で、registry 照会とページ取得が同じ時刻に
     * 記録された。先行する行はいまは 1 件も無い。**空で固定してよいのは、
     * 上の「本文根拠なら取得以降」がこの母集団を別途見ているからである。**
     */
