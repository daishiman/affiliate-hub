...
describe("上流指針の条項が要件文へ引かれていない (塞げていないことの固定)", () => {
   ...
   * - **緩すぎる**: 当時 15 件のうち 3 件しか見ていない。残りは節名を持ち込んでも黙る。
   *
   * いま測っているのは目的そのもの——**どの証跡のどの階層にも、章・節・条項・
   * 見出しを指す名前の欄が無いこと**——を全件に対して、である。
  ...
  // 2026-09-04: 15 → 22。検索と AI からの見え方のために出典を 7 本足した
  // （web-search-tool / d1-use-indexes / google-search-grounding /
  //  search-console-api / llms-txt / sqlite-fts5 / webmcp）。
  it("取得証跡は 22 件そろっている（母数が縮んで 0 件で緑になる形を止める）", () => {
    expect(EVIDENCE_IDS.length).toBe(22);
  });
