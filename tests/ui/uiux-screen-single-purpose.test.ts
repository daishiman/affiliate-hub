...
describe("A1 §2 全business mutationを単一のprimary taskへ所属させる", () => {
  ...
  it("同じactionの複数route・複数form用途を畳まず、意味entry 45件を床固定する", () => {
    ...
    expect(discovered).toHaveLength(44);
    expect(ADMIN_SCREEN_RUNTIME_ENTRIES).toHaveLength(45);
    expect(new Set(ADMIN_SCREEN_RUNTIME_ENTRIES.map((entry) => entry.id)).size).toBe(45);
  });
