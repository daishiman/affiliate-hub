 ...
 *
 * **排他の枝の境目で並びを切る。同時に描かれない枝は、同時に画面へ出ない。**
 * 枝をまたいだ窓は画面のどこにも存在しない形で、写しようがない。
 * 2026-08-26 に `prose-body.tsx` の `SectionHeading>ul>li` がこれで出た —
 * 見出しの枝と箇条書きの枝は排他で、並んで描かれることは無い。
 *
 * 切る境目は 3 つ。`switch` の `case`、JSX 三項の `) : (`、そして early return の
 * `return (`。2026-08-30 に後ろ 2 つを足した。理屈は `case` とまったく同じで、
 * 足す前は `p>ul>li` が「空なら 1 行 / あれば一覧」の 2 画面から出ていた——
 * どちらも排他分岐で、`p` と `ul` が並ぶ瞬間はどちらの画面にも無い。
 * `case` にだけ認めて他に認めないのは、規則ではなく書き方の当たり外れになる。
 *
 * 切りすぎれば窓が消えて §2 が何も見つけなくなるが、それは §1 の 2 つの床
 * （`windowsWithRawTag` > 50、`windowsWithRawPair` > 30）が受け止める。
 * 実測はこの変更で 184 → 161（`windowsWithRawPair`、床は 30）。
 * 境目を増やして母集団を痩せさせた人はそこで赤くなる。
 */
function tagSequence(source: string): readonly (readonly string[])[] {
  return source.split(/\bcase\s|\)\s*:\s*\(|\breturn\s*\(/).map((branch) =>
    /*
...
describe("A6 §1 検査そのものが効いている", () => {
  ...
  it("写しを探す母集団が空でない", () => {
     ...
     *
     * 2026-08-22 の実測は 56（`MIN_RAW_TAGS=2`）で、3 にすると 26 まで落ちた。
     * 床 30 はその差から置いてある。**次に閾値を上げた人はここで赤くなる。**
     * 画面が増えて 2026-08-30 現在は 161。床は動かしていない——床は
     * 「今どれだけ有るか」ではなく「規則を緩めたら割る線」なので、
     * 母集団が増えたからといって上げると、緩めた人を捕まえる意味が薄れる。
