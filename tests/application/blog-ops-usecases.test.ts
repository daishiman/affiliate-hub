 ...
 */
function fakeRepository(seed: Partial<Store> = {}): {
  ...
  const port: BlogOpsRepositoryPort = {
    ...
    findNetworkNode: async (ws, id) => ok((of(ws).network.find((n) => n.id === id) ?? null) as SiteNetworkNode | null),
      ...
      ok(
        Object.fromEntries(
          of(ws)
            ...
            .map((detail) => [
              detail.article.id,
              detail.blocks.map((block) => block.kind),
            ]),
        ),
      ),
...
function node(over: Partial<SaveSiteNetworkInput> & { siteSlug: string }): SaveSiteNetworkInput {
  return {
...
function article(over: Partial<BlogArticle> & { id: string }): BlogArticle {
  return {
    id: over.id,
