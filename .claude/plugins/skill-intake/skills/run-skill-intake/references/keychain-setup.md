...
- [ ] `bash ${CLAUDE_PLUGIN_ROOT:-plugins/skill-intake}/scripts/post-keychain-add.sh` が `OK: トークン取得成功 (長さ=N, prefix=ntn_...)` を出力
...
4. `bash ${CLAUDE_PLUGIN_ROOT:-plugins/skill-intake}/scripts/post-keychain-add.sh` で取得確認
