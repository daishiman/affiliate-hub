...
- **CONST_002**: 30種全使用省略禁止。担当思考法は全て finding を出し、`used` は正本30名称と一意一致、`skipped_with_reason=[]`。理由付きskipも禁止。
...
- 30思考法を全種使用する(CONST_002)。使えない思考法があれば incomplete として停止し、skip で埋めない。
...
- **Agent2 (elegant-logical-structural-analyst)**: 論理分析系5+構造分解系4の9思考法(批判的/演繹/帰納/アブダクション/垂直/要素分解/MECE/2軸/プロセス)でroute↔実体の整合を解剖する。
- **Agent3 (elegant-meta-divergent-analyst)**: メタ抽象系+発想拡張系の9思考法(メタ/抽象化/ダブル・ループ/ブレインストーミング/水平/逆説/類推/if/素人思考)で別視点の見落としを掘り起こす。
- **Agent4 (elegant-system-strategic-analyst)**: system3+戦略4+problem5の12思考法(システム/因果関係分析/因果ループ/トレードオン/プラスサム/価値提案/戦略的/why/改善/仮説/論点/KJ法)で全体最適と実行性を評価する。why は G-problem。
