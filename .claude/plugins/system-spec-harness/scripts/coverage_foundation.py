"""上位概念 (requirements_foundation) と decisions[] の C9 anti-drift 検証ヘルパー。
...
def validate_foundation(data: dict) -> list[str]:
    ...
    for g in goals:
        if isinstance(g, dict) and g.get("id"):
            # 重複 goal id は serves_goals のトレース先を一意に定められなくする
            if g["id"] in goal_ids:
                findings.append(
                    f"requirements_foundation: goal id {g['id']!r} が重複"
                    " (serves_goals のトレース先が一意に定まらない)"
                )
                continue
