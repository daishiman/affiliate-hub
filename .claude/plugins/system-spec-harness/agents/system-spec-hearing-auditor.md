...
- `Bash`: Python 標準ライブラリによる次の read-only 検査に限定する。(1) canonical state の位置 `<project-root>/system-spec/spec-state.json` から project root を決める、(2) `(project_root / source.path).resolve()` が `project_root.resolve()` 配下の実在 file であることを `Path.relative_to` で確認する、(3) `hashlib.sha256(answer.encode("utf-8")).hexdigest()` を `source.sha256` と照合する。redirect、file open の write mode、ネットワークは禁止 (effect=none)。
