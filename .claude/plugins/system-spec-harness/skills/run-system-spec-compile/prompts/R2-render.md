...
- card全文の無目的な転載はしない。章のgoalとカテゴリに対応する深度項目を実体レンダリングし、適用理由を評価可能にする。card本文は汎用原則の逐語転記のため、末尾の「本章での適用」節には `qa_log[].design_applications` の具体原則・`applied|not_applicable`・章固有理由・trade-off を確定qa_ref・対応セル・serves_goalsへ束縛して描画する。compiler 自身が定型の「適用済み」文を捏造してはならず、旧 state で記録が無ければ未記録を fail-visible にする。
...
- [ ] 設計知識card群の直後に「本章での適用」節があり、`qa_log[].design_applications` の具体原則・採否・章固有理由・trade-off を本章の確定 qa_ref・対応セル・serves_goals へ紐付けている (欠落や確定セルなしは未確定を明示)
