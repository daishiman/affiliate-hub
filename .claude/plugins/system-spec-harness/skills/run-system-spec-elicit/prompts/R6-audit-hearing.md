...
- **監査 5 軸**:
  ...
  2. **誘導質問 (leading question)**: 評価母集団は、現在の `state=確定` セルが `qa_refs[]` (無ければ `qa_ref`) で参照する質疑と canonical foundation 質疑から成る**現行の確定根拠**とする。`reopen_log` だけが参照する旧質疑など現行根拠でない履歴は参考情報として分離し、現行 verdict の検出数へ混ぜない。各質問について (a) 断定・前提埋め込み型 (「〜ですよね」「当然〜」)、(b) 望ましい答えを暗示する片側 Yes/No、(c) 独立に選べる複数論点を 1 問に束ね、1 つの回答を強いて中立回答を妨げる、を評価する。**質問文に複数の名詞・確認項目があることだけでは誘導と判定しない**。また「可否を問い、可なら成立条件を聞く」のように、否定回答をそのまま受理でき、肯定時にだけ追加情報が必要な**条件付き後続問だけでは片側 Yes/No と判定しない**。望ましい側を示す形容・推奨・否定側の排除など、回答の自由を狭める別の機序がある場合だけ検出する。`asks_for` は質問が外部的に狙うセル、`scope_notes.bundled` は writer が保存した論点分割の判定として併読するが、どちらも質問文の中立性を無条件に上書きする免罪符にはしない。`bundled=true` や複数の `asks_for` は重点確認し、`bundled=false` かつ単一 `asks_for` は 1 つの機能・判断を成立させる確認項目かを確かめる。検出時は「項目が複数」ではなく、回答のどの独立性が失われるかという**中立回答を妨げる具体的な機序**を `id` ごとに示す。
  ...
  5. **上位概念の遡及性 (foundation challenger)**: `requirements_foundation.confirmed=true` のとき、U1-U9 の各値が AI の誘導・推測でなく canonical id `qa-foundation-u1`〜`qa-foundation-u9` の利用者一次入力へ遡れるか (challenger 視点)。対話入力は `source.kind=user-dialogue`、書面要件は質問の path/section・回答の原文・`source.kind=written-requirements` と原文 SHA-256 を持つ**Uごとの 1論点 source-index**を根拠とする。書面 entry は `spec-state.json` から解決した安全な相対 `source.path` の指定 `source.section` を Read し、`answer` がその section 内に逐語で実在し、`source.sha256 == sha256(answer UTF-8 bytes)` であることを照合する。path 逸脱・section 不在・逐語原文不在は根拠なしとする。**根拠の 3 段 (必須 / 代替 / 補助) は次のとおりで、順に試す。**(i) **必須**: canonical `qa-foundation-u1`〜`u9` の `answer` に当該 U 欄の値が逐語で在ること。(ii) **代替 (必須と同等に受理する)**: canonical `answer` に逐語が無くても、`requirements_foundation.provenance.field_sources[]` に当該 `field` を名指す entry が在り、`sealed_with=seal-foundation-sources` の封を持ち、その `path` の `section` を Read して `quote` が逐語で実在し、`sha256 == sha256(path のファイル全体の UTF-8 bytes)` であるなら根拠として受理する (**`provenance.field_sources[].sha256` は原典ファイル全体の指紋であって `quote` の指紋ではない** — `seal-foundation-sources` writer が path を読んで打刻する。引用の改竄は逐語実在の照合が、原典の差し替えはこの指紋が、それぞれ別に受け持つ。片方だけでは根拠にしない)。**封の無い field_source は代替にしない** (封は writer が実ファイルへ照合して打つものであり、自己申告の path 記載と区別する唯一の印である)。**U7 `scope` だけは代替の形が異なる**: `provenance.scope` の `source_qa_id` が `qa-foundation-u7` を指し、`sections.{in,out}.heading` が canonical `answer` の見出しに一致し、`items[].covers` が当該見出し区間の箇条書き番号 1..N を**漏れなく・重複なく**指すなら、`scope` の各項目は根拠を持つとする。原文の複数行を 1 項目へまとめた結合表記は、この `covers` 申告がある限り逐語不一致を理由に根拠なしとしない (`validate_foundation_scope_coverage` が scope と申告の双方向一致を writer 側で強制しており、消失は差集合として必ず出る)。**`covers` 申告の無い結合表記は根拠なしとする** — まとめが合法なのは何をまとめたかが行番号で辿れるときだけである。これは C05 `decision_guidance` が同じ `field_sources` を trace 根拠として読む範囲と一致させるための規定で、緩和ではない — 封と逐語照合と sha256 一致という 3 条件は canonical entry へ課すものと同じである。(iii) **補助 (単独では根拠にしない)**: 承認ログの文書名だけ、複数論点を束ねた質問、AI 要約、または AI が生成した entry 自身の digest。補助しか無い U 欄は根拠なしとする。判定観点 = (a) U1-U9 の値がこの根拠を持たず AI が代弁・創作していないか、(b) `confirmed=true` なのに承認 `approval_ref` が `approval_log[].id` に実在するか (無ければユーザー未承認の勝手確定)、(c) U1/U2/U3 が値でなく N/A で埋められていないか (値必須の違反)。利用者一次入力へ遡れない U 項目・dangling な `approval_ref` を検出する。canonical foundation 質疑と、その代替として当該 U の `field_sources` が成立していればこの軸は充足であり、それ以外の孤立 `qa_log` は**評価母集団外**である。特に `対象外` セルの `reason` と approval の結び付きは C07 の担当であり、そこだけに関わる `qa_log` を foundation 欠落へ数えない。
...
- [ ] 現行の確定根拠となる全 qa_log 質問が誘導性評価の対象になり、`asks_for` / `scope_notes.bundled` / `reopen_log` を併読した (現行根拠でない履歴は参考情報として分離した)
