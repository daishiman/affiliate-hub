...
```json
{
  ...
  "reopen_log": [{"category": "database", "platform": "web", "reason": "...", "from": "確定", "discarded": {"qa_ref": "qa-001", "approval_ref": "approval-001", "serves_goals": ["G1"]}}],
...
- **唯一の例外は「超過値を消さない床」(loop_count)**: 上の 0 代入には 1 つだけ例外が在る。`run_chunk` は代入前に `floor = 既存値 if 既存値 > max_loops else 0` を求め、`loop_count = max(floor, 処理した turn 数)` を書く。**既存値が上限以下なら `floor` は 0 なので、上の一文がそのまま成り立つ。**床が立つのは既存値が上限を超えているときだけであり、`run_chunk` 自身は上限を超える値を書かない (`processed >= max_loops` で `break` する) ので、**この例外は writer が作った state では発火しない。**発火しうるのは writer 非経由で書かれた state だけで、そこでの床の役目は「迂回の唯一の痕跡である超過値を、通し直しで黙って丸めてしまわない」ことである。超過値を消す正規の経路は `reopen` → `init --state` → `chunk` の通し直しであり、そこでは値が writer 由来へ置き換わる (丸めではない)。この条項とコードの一致は `tests/test_spec_transition_version_sections.py::test_the_contract_documents_the_overrun_floor` が機械で突き合わせている。
