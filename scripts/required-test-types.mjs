...
/**
 * 判定して報告書を更新する。**CLI から起動されたときだけ走る。**
 *
 * ここで境目を作っているのは、`readRegistry` のような読み取りを
 * 単体テストから import できるようにするためである。
 * 境目が無いと、テストが 1 行 import しただけで報告書が書き換わり、
 * `process.exit(1)` がテスト実行そのものを落とす。
 */
function main() {
...
if (process.argv[1]?.endsWith("required-test-types.mjs")) main();
