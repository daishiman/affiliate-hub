 ...
 */
/*
 * **`content.created` を消した（2026-08-21）。**
 *
 * 記事を作る操作は、この時点でどこにも無い（`packages.save` を呼ぶ入口が 0 か所、
 * `variants.save` は承認の 1 か所だけ）。それでも語だけが残っていて、
 * **見本の記録に 1 行だけ「記事を作った」が並んでいた**。画面は動いて見えるが、
 * その行を作る操作は存在しない。見本を消した時点で 0 件になる種類の穴である。
 *
 * 消すほうを選んだ理由は、**要件がこの記録を求めていない**ため。
 * 必須記録対象は仕様 §7 の 6 つ（公開 / 削除 / リンク差し替え / 権限変更 /
 * 成果データ修正 / エクスポート）と、この docstring の 3 つで、作成はどちらにも無い。
 * 出す場所を先に作っても、呼ばれないコードが増えるだけになる。
 *
 * 作る操作が入った日に語を足し直すのは 1 行である。逆に、語を残したまま
 * 待つほうは代償が大きい——次に数える人が同じ計測をやり直すことになる。
 */
