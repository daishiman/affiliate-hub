...
export type AuditAction =
  ...
  | "blog_rating.shown"
  /**
   * ブログの住所（独自ドメイン）を登録した／外部の状態を写した／
   * 正規の住所を切り替えた／取り下げた。
   *
   * **写し取り (`synced`) を別語にしている。** 登録も切り替えも取り下げも
   * 人の操作だが、写し取りだけは定期実行と外部の返答で起きる。1 語にまとめると、
   * 「誰も触っていないのに住所が落ちた」を追うときに、人の操作の行を
   * 何百件もかき分けることになる。逆に写し取りを記録しないと、
   * 外部が `failed` を運んできて配信が止まった瞬間がどこにも残らない。
   *
   * 取り下げは理由必須（`REASON_REQUIRED`）。読者が来ている住所を止める
   * 判断は、行の差分（`active` → `revoked`）からは動機が読めない。
   */
  ...
  | "blog_domain.revoked"
  /**
   * 日次集計を、日付を指定して人が作り直した。
   *
   * **定期実行の分は残さない。** あれは毎日 2 日ぶんを黙って置き換える動きで、
   * 記録に入れると 1 日 2 行が永久に積まれ、承認や公開の行がその中に埋もれる。
   * ここに残すのは、人が「この日が壊れている」と判断して呼んだ回だけである。
   *
   * 集計は足し込みではなく**置き換え**なので、走った瞬間に前の値は残らない。
   * 数字が変わったことに気づいた人が、実装を直したから変わったのか、誰かが
   * 作り直したから変わったのかを区別できる唯一の手がかりがこの行になる。
   */
  | "metrics_rollup.rebuilt"
  /**
   * SEO 診断を回した／指摘から下書きを作った／「直さない」と決めた。
   *
   * 診断そのもの (`ran`) を残すのは、**指摘が消えた理由**を後から言うため。
   * 記事を直したから消えたのか、診断器を差し替えて観点ごと無くなったのかは、
   * 指摘の行だけを見ても区別できない。回した回と対象がここにしか残らない。
   *
   * 「直さない」は理由必須。これは診断より強い判断（次の診断で復活しない）で、
   * その強さに見合う説明が要る。
   */
  | "seo_assessment.ran"
  | "seo_finding.drafted"
  | "seo_finding.dismissed"
  /**
   * AEO の構えを保存した／記事から引用単位を抽出し直した。
   *
   * 抽出は**置き換え**なので、前にあった問いが黙って消える。消えたことは
   * 表の差分に残らない（行ごと入れ替わる）ため、いつ何本になったかを
   * `after` に残す。回答エンジンへ出していた Q&A が消えた日を辿れるようにする。
   */
  | "aeo_profile.changed"
...
const REASON_REQUIRED: ReadonlySet<AuditAction> = new Set<AuditAction>([
  ...
  "blog_rating.shown",
  /*
   * 読者が現に来ている住所を止める操作。差分は `active` → `revoked` としか
   * 言わないので、なぜ止めたか（移転・誤登録・契約終了）は理由の欄にしか残らない。
   */
  "blog_domain.revoked",
  /*
   * 診断より強い判断。次の診断で復活しないので、根拠が残っていないと
   * 「なぜこの指摘だけ出てこないのか」を後から誰も説明できない。
   */
  "seo_finding.dismissed",
]);
