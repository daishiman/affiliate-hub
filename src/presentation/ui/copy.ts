...
export const UI_COPY = {
   ...
   * 出し先へ配る。
   *
   * 自動で投稿できない先（note）は、貼り付け用の下書きが唯一の道になる。
   * その道の途中で人が迷う場所——手順・本文・広告表記——の言い回しをここに置く。
   */
  distribution: {
    draftTitle: "貼り付け用の下書き",
    draftSteps: "この順番で進めます",
    draftBodyTitle: "貼り付ける本文",
    copyBody: "本文をコピー",
    copySteps: "手順をコピー",
    /** コピーが使えない環境の人が、必ず本文へ辿り着けるようにするための一文。 */
    draftManualHint:
      "コピーが使えないときは、下の本文をそのまま選んでコピーしてください。同じものです。",
    draftDisclosureWarning:
      "本文の先頭にある広告表記は消さないでください。消すと、法令で決められた表示ができていない記事になります。",
    draftExportedNote:
      "この画面を開いた時点で「書き出した」ことが記録に残ります。誰がいつ持ち出したかを後から辿れます。",
  },
  ...
  article: {
    ...
    /**
     * 記事の要点。結論の直後に置く箇条書き。
     * 1 件も無い記事では欄ごと出さない（見出しだけの空欄を作らない）。
     */
    keyPointsTitle: "この記事の要点",
    /** 目次。節が 3 つ以上のときだけ出す（2 つ以下では逆に読みにくい）。 */
    tocTitle: "目次",
    /**
     * よくある質問。1 件も無い記事では欄ごと出さない。
     * 見出しの言葉は構造化データ（`FAQPage`）と揃える必要は無いが、
     * 読者が「ここに問いの答えがある」と分かる言い方にする。
     */
    faqTitle: "よくある質問",
  ...
  /**
   * 無いものを開いたとき。**無いブログ名も、無い記事・商品・書き手も同じ画面**。
   *
   ...
   *
   * 「ブログ」でも「記事」でもなく **「ページ」** と言うのは、Next.js の
   * `not-found.tsx` がアドレスの `[site]` も `[product]` も受け取らないため。
   * 受け取れない以上、どちらが無かったのかをここで書き分けると必ずどちらかで嘘になる。
   * 無い名前を推測して出すより、**戻り先を確実に示す**ほうが読者は先へ進める。
   *
   * 戻り先を公開中のブログ一覧（`/`）にしているのは、同じ理由でブログのトップへ
   * 戻す URL を組み立てられないため。読者は 1 回余分に辿るが、行き止まりにはならない。
   */
  pageMissing: {
    title: "このページは見つかりませんでした",
    /** 見出しの繰り返しを避ける。同じ文を 2 回読み上げても情報は増えない。 */
    detailTitle: "指定されたページはありません",
  ...
  feedback: {
    ...
    disclosureBody:
      "画面の名前・アドレス・画面の大きさ・エラーの記録・うまくいかなかった通信・直前に押したものを一緒に送ります。通信先のアドレスは「？」より後ろを落としてあります。入力欄に打った文字は送りません。お名前とメールアドレスは、作業する側へ渡す文には入りません。",
    /** 何がいくつ控えられているか。**数だけ出す**（中身は開発者向けの文字列なので読めない）。 */
    disclosureCounts: "いま控えているもの",
     ...
     * 区画で隠す道の言い回し。
     *
     * **「3 番目の見出し」とは言わない。**要素の名前で呼ぶには、撮った画像の
     * どこに何の要素があるかを知っている必要があるが、写しは画面共有で撮った
     * 映像の 1 枚で、いま開いている DOM と一致する保証が無い
     * （利用者は別のウィンドウを渡せる）。要素の名前を騙って塗ると、
     * **隠したつもりで隠れていない**写しが送られる。
     * 画像そのものを区切った言い方なら、写っているものが何であれ嘘にならない。
     */
    captureRegionTitle: "区画で隠す",
    captureRegionHint:
      "位置を決めずに隠せます。画面を縦横 3 つずつに区切ってあり、押すとその区画が黒く塗られます。もう一度押すと外れます。",
    captureRegionTop: "上段",
    captureRegionMiddle: "中段",
    captureRegionBottom: "下段",
    captureRegionLeft: "左",
    captureRegionCenter: "中央",
    captureRegionRight: "右",
    /** 押した結果を読み上げへ渡す。画素は見えないので、言葉にしないと分からない。 */
    captureRegionAdded: "を黒塗りにしました",
    captureRegionRemoved: "の黒塗りを外しました",
    toolPen: "手書き",
    toolRect: "四角",
    toolArrow: "矢印",
    toolText: "文字",
