...
export type AffiliatePreviewActionState =
  | { readonly status: "done"; readonly preview: AffiliatePreview }
  | { readonly status: "failed"; readonly message: string };

/**
 * 貼り付け時の確認は保存と分ける。この action は D1 も audit も書き換えない。
 */
export async function previewAffiliateUrlAction(
  rawUrl: string,
): Promise<AffiliatePreviewActionState> {
  const actor = await signedInActor();
  if (actor === null) {
    const denied = notSignedInFailure("成果リンクのプレビュー");
    return { status: "failed", message: denied.message };
  }
  const result = await (await linkInboxUseCases()).preview.execute(actor, { rawUrl });
  if (!result.ok) {
    const failure = failureFromDomainError(result.error);
    return { status: "failed", message: failure.message };
  }
  return { status: "done", preview: result.value.preview };
}
