...
export async function generateMetadata({ params, searchParams }: Props): Promise<Metadata> {
  ...
  return articleIndexMetadata(site, "comparison", articlePageNumber(query?.page));
}

export default async function Page({ params, searchParams }: Props) {
  const [{ site }, query] = await Promise.all([params, searchParams]);
  return <ArticleIndexPage siteSlug={site} type="comparison" page={articlePageNumber(query?.page)} />;
}
