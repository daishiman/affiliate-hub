 ...
 */
export default async function SiteAudiencePersonasPage({
  params,
}: {
  ...
  /*
    読者像はワークスペース単位で持っている。だからブログを先に確かめる。
    順番が逆だと、存在しないブログの住所でも一覧が出て、
    「そのブログの読者像」として読まれる。
  */
  ...
  return (
    <AdminShell
      routeId="sites/[site]/audience/personas"
      ...
      actions={
        <>
          <TextLink href={`${sitePath}/audience/personas/new`}>読者像を作る</TextLink>
          <TextLink href={`${sitePath}/authors`}>書き手へ戻る</TextLink>
        </>
      }
    >
      ...
      {!audiences.ok ? (
        ...
        */
        <ErrorView
          ...
          action={<TextLink href={`${sitePath}/authors`}>書き手へ戻る</TextLink>}
        />
        ...
        <Section title="読者像">
          <EmptyView
            ...
            action={<TextLink href={`${sitePath}/audience/personas/new`}>読者像を作る</TextLink>}
          />
        </Section>
      ) : (
        <>
