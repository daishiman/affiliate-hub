 ...
 * 読者像（誰に向けて書くか）は同じブログの `audience/personas` にある。
 *
 ...
 */
export default async function SiteAuthorsPage({
  params,
}: {
  ...
  /*
    ワークスペースの書き手を読む**前に**ブログを確かめる。
    書き手はワークスペース単位で持っているので、順番が逆だと
    存在しないブログの住所でも一覧が出る。
  */
  ...
  return (
    <AdminShell
      ...
      actions={
        <>
          <TextLink href={`${sitePath}/authors/new`}>書き手を作る</TextLink>
          <TextLink href={sitePath}>このブログへ戻る</TextLink>
        </>
      }
    >
        ...
        <>
          ...
          */}
          ...
          {authors.value.total === 0 ? (
            <Section title="書き手">
              <EmptyView
                ...
                action={<TextLink href={`${sitePath}/authors/new`}>書き手を作る</TextLink>}
              />
            </Section>
          ) : (
          ...
          >
            <ListView
              rows={[
                { key: "audiences", label: "読者像を決める", href: `${sitePath}/audience/personas` },
              ]}
            />
          </Section>
        </>
