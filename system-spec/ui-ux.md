...
確定した内容を、章にしか居場所が無いまま消えないよう正本へ移したものである。
上の質疑録と混ぜて読まないために区切ってある。

`qa-uiux-web-site-scoped-authoring-ia` で利用者は「記事・読者像・書き方の決め事を
ブログ単位の管理へ移す」「横断画面には何を残すか」を決めた。**何をどう畳んだかの
具体形は、実装しなければ決まらなかった。**その決まった形をここに残す。

#### 一段目は「作業の対象物」5 つに畳んだ

畳んだ結果は ブログ / 記事 / 読者 / 商品 / 配信 の 5 つである。正本は
`src/presentation/ui/admin-route-metadata.ts` の `ADMIN_NAV_GROUP_LABELS` 1 箇所で、
サイドバーもホームのボードもパンくずもここからの射影であり、第 2 の表を持たない。

**動詞 (素材/書く/出す/稼ぐ/見る/整える) で切る案を捨てた理由**を残す。動詞で切ると
1 つの対象物についての作業が複数の見出しに散った。「商品」は *素材* (登録する) と
*稼ぐ* (提携と成果) の両方にあり、運営者はどちらを開くか決められず両方開いていた。
対象物で切ると「これについて作業する」と言えるものが 1 か所に集まる。

設定・AI の利用と費用・道具・画面部品の見本は 5 つに入れず `nav.group = null` で
分類の外へ置いた。対象物と言えないものを無理に混ぜると、ラベルから中身が
言い当てられなくなる。

#### 近道の選定基準 — 旧入口をそのまま近道にする

所属替えで 5 画面が `/admin/sites/[site]/...` へ移った。移った先は「ブログ群 →
1 本のブログ → その画面」なので、そのままでは 1 クリックが 2 クリックになる。

**サイドバーに新しい近道帯を足さなかった。**既にある一段目の入口を消さずに残し、
行き先を転送の殻にしてある。押すとそのときのブログの配下へ着く。

サイドバーに出るのは `route.nav !== null && route.label !== null` を満たすものだけで、
子 (`child()`) は `nav: null` が固定である。したがって近道になるのは
`/admin/personas` (書き手)、`/admin/writing` (書き方の決めごと)、`/admin/content` (記事)
の 3 本で、`/admin/personas/audiences` (読者像) は畳む前からサイドバーに出ていない。

**別の解決規則を書かないための形である。**近道用にブログ解決をもう一度呼ぶ設計にすると、
サイドバーを描いた時刻と押した時刻の間に選択ブログが変わったとき、ラベルの下の URL と
実際に開くブログが食い違う。href を旧 URL のままにすれば、解決は押した瞬間に 1 回だけ起きる。

#### ラベルは 2 つの対象物を束ねない

「書き手と読者像」というラベルを「書き手」に分けた。書き手は読者ではないので
`reader` には置けず、記事を書く人なので `article` に置いた。2 つの対象物を 1 つの
ラベルに束ねたままだと、開くまで何の画面か分からない。

#### 保守規則と手順の正本

規則: `docs/spec/feat-site-scoped-authoring-ia/ia-rules.md`
手順: `docs/spec/feat-site-scoped-authoring-ia/operations.md`

各規則は対応する検査を 1 つずつ持つ。検査を書けない規則は規則ではなく心構えとして
扱い、この章にも規則としては載せない。
