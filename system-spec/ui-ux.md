...
- 原則: 1 画面 1 主要タスクとし、情報の優先/補助/省略を task 頻度×失敗コストで決める (`ref-system-design-knowledge:information-design`)
  ...
  - トレードオフ:
    - 画面分割により遷移回数は増える。週 1 回利用・PC 中心という本人回答の文脈では、1 画面の判断負荷低減が遷移コスト増より優先される
    - 文章量削減は初見時の説明不足リスクと引き換えになるため、補助情報は省略でなく折りたたみ・ツールチップへ退避する
- 原則: 同一の視覚要素・操作パターンはコンポーネント化して再利用し、ハードコーディングの重複を排除する (`user-dialogue:2026-08-21#コンポーネント共通化`)
  ...
  - トレードオフ:
    - 共通化は初期コストが掛かり、画面固有の微調整には variant/prop 設計が要る。過剰な抽象化を避け、3 回目の重複が出た時点で共通化する基準を設ける
