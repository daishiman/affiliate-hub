...
- 原則: 宛先の違う出力物を、1 つの画面契約の中に混ぜない (`ref-system-design-knowledge:information-design`)
  - 採否: `applied`
  - 章固有の根拠: この画面が同時に相手をするのは 3 種類の宛先であり、それぞれ求めるものが違う。読者は画面を見る。検索エンジンのクローラは HTML と構造化データ (JSON-LD) を読む。AI は llms-full.txt を取りに来るか、WebMCP を呼ぶ。3 者を 1 つの要件として書くと『読者に見せないが検索には見せる』ような、宛先ごとに内容が食い違う実装が正当化されてしまう。よって要件は宛先ごとに分けて書き、共通の制約として『同じ記事について 3 者へ矛盾する内容を出さない』を課す。構造化データは画面に出ている内容の再記述に限り、画面に無い主張を構造化データにだけ書かない
  - トレードオフ:
...
- 原則: 画像を置く場所は、寸法を先に宣言してから中身を流し込む (`ref-system-design-knowledge:usability-accessibility`)
  - 採否: `applied`
  - 章固有の根拠: サムネイルが出る面が読者向けだけで 6 つ (トップ・カテゴリ・タグ・検索結果・関連記事・アーカイブ) に増え、さらに運営画面が加わる。面ごとに寸法を書くと、書き漏らした面だけ読み込み中に版面が飛ぶ。よって寸法は面ごとではなく、面をまたいで使える少数の型 (読者向けの大・読者向けの小・運営向け) として 1 か所で定め、各面はその型を選ぶだけにする。画像の実体が届く前から高さが確定するため、どの面でも並びが動かない。読者向けと運営向けを別の型にするのは、前者が『内容を想像させる』ため後者が『行を見分ける』ためという用途の違いによる
  - トレードオフ:
...
- 原則: 外へ開く操作は、開く前に閉じている状態を既定にする (`ref-system-design-knowledge:api-design-patterns`)
  - 採否: `applied`
  - 章固有の根拠: WebMCP はページを訪れた AI エージェントへ操作を差し出す入口であり、何を呼ばせるかをこちらが定義する。llms-full.txt が既に全文を出している以上、記事の取得を秘匿する理由は無いため、公開するのは読み取りの操作に限る — 記事の検索、記事の取得、カテゴリの一覧。書き込み・更新・削除の操作は公開せず、実装として持たない。持たなければ設定の誤りで開いてしまう事故が起きない。また WebMCP から返す内容は llms-full.txt と同じく公開済みの記事に限り、下書き・非公開の記事へ到達する経路を作らない
  - トレードオフ:
