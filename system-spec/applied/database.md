...
> 以下の 2 件は正本 `spec-state.json` の `qa_ref` / `qa_refs` / `required_info[].grounded_by` のいずれからも導けない (`##### 接地根拠 qa-neutral-auto-scope-v6 (対応セル: web)`, `##### 接地根拠 qa-request-thumbnail-coverage-v6 (対応セル: web)`)。compile が消さずに引き継いでいるだけで、**章が正本の投影である性質はここだけ破れている**。正本へ接続するか、不要と確かめて消すこと。
...
> 正本から導出できず、節・小節の引き継ぎでも守れなかった 4 行。版の更新のように**正しく消える行**も混ざる。正本へ接続するか、不要と確かめて消すこと。この節は compile のたびに作り直す。

- `##### 確定内容 qa-seo-approved-diff-20260906 (対応セル: web)`
- `- 確定要件: つづけて`
- `##### 接地根拠 qa-database-web-article-image-verbatim (対応セル: web)`
- `- 本文: 「確定内容 (質疑録)」の `qa-database-web-article-image-verbatim` を参照`
