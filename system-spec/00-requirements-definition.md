...
- **6 件すべて `status: confirmed`**（分母 = 正本 `spec-state.json` の `decisions[]` 全件）。うち 5 件は `decision-auth-method` に遅れて 2026-08-19〜20 に確定した。
- **`decision-auth-method` の行だけ書式が違う**のは、その 1 行が生成器の出力そのままだからである。`cost={'category': 'free', 'amount': 0, ...}` は **Python の dict を文字列にした形**が残ったもので、真似すべき書式ではない。残り 5 行は手で書いた。**書式を揃えるために 1 行目の dict 形へ寄せないこと**（読めない形が 6 行に増えるだけになる）。
