 ...
 */
export const TENANT_ISOLATION_MIN_REACHED = 66;
 ...
 *
 * 2026-08-18 の実測（対象 111 件）:
 *
 *   持ち主 ok → 他社 TENANT_MISMATCH    10 件  ← 確かめられた
 *   持ち主 ok → 他社 NOT_FOUND          10 件  ← 確かめられた
 *   持ち主 VALIDATION_FAILED             10 件  ← 見本の値が enum に合っていない
 *   持ち主 NOT_FOUND                      5 件  ← 見本の識別子が存在しない
 *   持ち主 CONFLICT                       5 件  ← 状態が合わない
 *   持ち主 UPSTREAM_UNAVAILABLE           3 件  ← 記録の書き先が無い
 *   持ち主 NOT_IMPLEMENTED                3 件  ← スタブ
 *
 * **確かめられたのは 20 件だけ**である。上の 66 と比べると分かるが、
 * `ok` の数（66）と証明の数（20）は 3 倍違う。**安く動くほうを下限にしない。**
 * 対象を増やすのは 1 行でできるが、証明を増やすには見本を作る作業が要る。
 *
 * 上の 15 件（VALIDATION_FAILED 10 + NOT_FOUND 5）は
 * `tests/presentation/tool-inputs.ts` の見本を直せば試せる。残課題 77。
 * 残り 30 件は入力の問題ではないので、直しても増えない。
 *
 * **下げて緑にすることは禁止。上げる方向にしか動かさない。**
 */
export const TENANT_ISOLATION_MIN_SEPARATION_PROVEN = 20;
