 ...
 * たいていは記録を足す側が正しい。
 *
 * 2026-08-26 に 5 件目（記事に星をつける）を登録した。**理由は 1 つ目のまま**
 * （読者の操作である）で、種類は増えていない。読者の投票と、運営者がそれを
 * 伏せる操作は別の入口になっていて、伏せる側は記録へ届いている。
 *
 * 2026-09-05 に 6 件目（読者の行動を束で受け取る `createRecordReaderInteractions`）を
 * 登録した。**理由は 2 つ目のまま**（自動の計測である）で、種類は増えていない。
 * 既に登録してある読者の計測（`createRecordTelemetry`）と同型で、
 * 1 人が 1 記事を読むだけで数十件届く。ここを記録へ入れると、
 * **承認や公開の行がその中に埋もれて読めなくなる** ——
 * この上限が守りたかったものが、この上限を守ることで壊れる形になる。
 *
 * **記録を足す側が正しくないと言える根拠は、代わりの記録があること。**
 * 観測層で人が判断して起こす操作は集計のやり直し（`metrics_rollup.rebuilt`）で、
 * そちらは記録へ届いている。生の観測の正本は `reader_interaction_event` にあり
 * 90 日で消えるので、「誰が承認したか」を問われる場面では最初から使わない。
 *
 * **上げてよいのは、理由の種類が 2 つのままで、記録へ届いていない入口
 * (`PORT_WIRING_MAX_UNRECORDED`) が 0 のときだけ**である。今回もその両方を満たす
 * （届いていない 0 / 上限 0 は据え置き）。この 2 条件のどちらかが崩れたときは、
 * この数を動かさずに記録を足すこと。
 */
export const PORT_WIRING_MAX_WRITE_EXCLUSIONS = 6;
...
// `submitReaderRatingAction()` で、名前も連絡先も要らない
// (要求すると点の分布が「登録した人の分布」に変わる)。管理側の入口は 1 つも増えていない。
// 2026-08-26: 手元で画面を確かめるための入口 `src/app/api/dev-signin/route.ts` を 1 本足した。
// **これは読者の道ではない。** この一覧に読者以外が並ぶのは上の説明どおり例外なので、
// 何がこの 1 件を無害にしているかをここに書く。旗が 2 つ同時に立たないと存在しない:
// (1) `DEV_SIGNIN_ENABLED === "1"` — `.dev.vars` にしか書けず、`wrangler.jsonc` に
//     書かれていないことを `tests/infrastructure/dev-signin.test.ts` が固定する。
// (2) `NODE_ENV !== "production"` — 積んだ Worker は必ず production である。
// 積んだ環境では 404 を返すので、公開面には口そのものが無い。
// 通行証は本番と同じ [[session-issuer]] が出すため、担当者の登録が無いアドレスは
// この口を通っても入れない (門を素通しにしたのではなく、Google との往復だけを飛ばした)。
// 2026-08-27: 保存済み固定ページ8種の公開入口を1本追加した。
...
// 管理側の入口は 1 つも増えていない。実測どおり 1 件だけ床を更新する。
// 2026-09-06: 記事の挿絵を取り出す口 `src/app/api/article-images/[image]/route.ts`
// を 1 本足した。**これは読者の道である**（上の 2 件のような内部の道ではない）。
// 匿名取得は有効サイトの未取り下げ公開記事が参照する画像だけに限定する。
// 非公開画像は同じ workspace の content.read と所属記事の存在を確認する。
// この条件付き公開も匿名公開の宣言数に含め、上限から除外しない。
//   (1) **台帳にあるだけでは公開しない。** 公開参照または閲覧認可の確認を
//       R2 の読み取りより前に終える。置き場の鍵は `article_image` 表から得るため、
//       別用途の物（画面の写し・書き出しファイル）の鍵を要求へ直接渡せない。
//   (2) **書き込みは同じ道に無い。** 送る口 (`/api/article-images`) は別ファイルで、
//       そちらは「ログイン」かつ `content.write` を要求する。
 ...
 *
 * ==========================================================================
 * これは「速さの基準」ではなく「止まったことの検知」である
 * ==========================================================================
 *
 * 画面の検査は `import()` で本物のページ部品を取り込む。**費用のほとんどは
 * 描画ではなく取り込みで、しかもその取り込みは他のテストファイルと同じ
 * 機械を取り合う。** 2026-08-26 の全走行（279 ファイル / 7115 件）の実測:
 *
 * - `route-branch-reached.test.ts` は 104 件を 93 秒で走り切った（1 件あたり約 0.9 秒）
 * - **その中の 1 件だけが 20 秒を超えて赤になった**
 *
 * 落ちた画面が重かったのではない。**同じ画面が、空いている機械では 1 秒で
 * 終わる。** つまり 20 秒という数字は、コードではなく「そのとき何が同時に
 * 走っていたか」を測っていた。これを 3 回の全走行で 3 回とも、**毎回違う
 * テストが**踏んだ（`route-branch-reached` / `page-degraded` / `page-empty`）。
 *
 * 赤が機械の混み具合で決まると、**次に本物の赤が出た日に「また例のやつだ」
 * と読まれる。** 検査が飾りになるのはこの経路である。
 *
 * ==========================================================================
 * 数字を上げてよい理由（上げてはいけない数字との違い）
 * ==========================================================================
 *
 * このリポジトリには**上げてはならない数字**がある。違反件数の上限
 * （`FORM2_MAX_WITHOUT_FLOOR` など）とカバレッジの下限がそれで、
 * あれらは**主張そのもの**なので、緩めれば守っているものが減る。
 *
 * **待ち時間は主張ではない。** ここを広げても「断りに置き換わっていない」
 * 「空の案内が描かれる」といった判定は 1 つも緩まない。広げて失われるのは
 * 「異常に遅い」を掴む力だけで、**それはもともと掴めていなかった**
 * ——0.9 秒と 20 秒が同じ緑だったのだから。
 *
 * ==========================================================================
 * 残っている穴（書いておく）
 * ==========================================================================
 *
 * **描画の費用を見張るものは、いまどこにも無い。** 画面が本当に 10 倍
 * 遅くなった日、この検査は緑のまま通る。時計で測る判定は取り合いの山を
 * 拾ってしまうので、ここに足すのは誤り。費用を見るなら別の測り方
 *（取り込むモジュールの数など、機械の混み具合に依らない量）が要る。
 */
export const SCREEN_RENDER_BUDGET_MS = 90_000;

/**
 * **1 つのフックで全画面をまとめて描くときの待ち時間。**
 *
 * 上のものと役割が違うので別の名前にしてある。上は「1 枚ぶん」、
 * こちらは「85 枚ぶんをまとめて」で、**同じ数字にすると片方が必ず嘘になる。**
 * まとめて描くのは費用を減らすためではなく、対象の一覧を手で持たずに
 * 済ませるため（手で持つと新しい画面だけ検査から抜ける）。
 *
 * 上と同じく、これは止まったことの検知であって速さの基準ではない。
 */
export const SCREEN_SWEEP_BUDGET_MS = 240_000;
