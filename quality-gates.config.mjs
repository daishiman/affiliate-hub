...
/**
 * テナント分離の検査で、**中身まで確かめられた道具の下限**。
 *
 * ここだけは上限ではなく**下限**である。理由がある。
 *
 ...
 * 実測すると、中身まで見られたのは 61 → **66 件**（増えたのは 5 件だけ）。
 * 残る 45 件は入力の見本が足りず、他社かどうかを見る前に断られている。
 *
 * **件数だけを見ると守りが強くなったように見える。**だから
 * 「実際に中身まで見た件数」を別に数えて下限に置く。
 * 見本の入力を整えれば増える。減ったら、どこかの道具が呼べなくなっている。
 *
 * **下げて緑にすることは禁止。上げる方向にしか動かさない。**
 */
export const TENANT_ISOLATION_MIN_INSPECTED = 66;
